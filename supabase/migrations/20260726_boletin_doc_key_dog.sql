-- 20260726_boletin_doc_key_dog.sql
--
-- Añade el DOG (Diario Oficial de Galicia) a boletin_doc_key(), espejando el patrón nuevo de
-- lib/convocatoria/canonicalizeBoletinUrl.cjs.
-- xunta.gal/dog/Publicados/AAAA/AAAAMMDD/AnuncioXXXX_es.html → DOG-XXXX (código de anuncio;
-- las variantes de idioma _es/_gl convergen al mismo docKey). Campaña T-107 auxiliar-administrativo-galicia.
...
COMMENT ON FUNCTION boletin_doc_key(text) IS
  'Espejo SQL de lib/convocatoria/canonicalizeBoletinUrl.cjs para el backend. doc_key canónico (BOE/BOCM/DOGV/BOCYL/DOGC/BOC/BOJA/DOG exactos; resto url normalizada). Usar: ensure_convocatoria_documento(conv, boletin_doc_key(url), url, ...).';
