...
(async()=>{
  const ids=JSON.parse(fs.readFileSync('scratchpad/t331/gen_rd203_t331_2026-07-31_inserted_ids.json','utf8'));
  const c=new Client({connectionString:process.env.DATABASE_URL.replace(/[?&]sslmode=[^&]*/,''),ssl:{rejectUnauthorized:false}});
  await c.connect();
  fs.mkdirSync('scratchpad/t331/lote',{recursive:true});
  for(let i=0;i<PREGUNTAS.length;i++){
    ...
    const est=JSON.parse(fs.readFileSync(`scratchpad/t331/estructuradas/q${String(i+1).padStart(2,'0')}.json`,'utf8'));
    fs.writeFileSync(`scratchpad/t331/lote/${id}.json`,JSON.stringify(est,null,2));
  }
