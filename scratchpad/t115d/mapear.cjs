...
(async()=>{
  const ids=JSON.parse(fs.readFileSync('scratchpad/t115d/gen_trlhl3_t115_2026-07-31_inserted_ids.json','utf8'));
  const c=new Client({connectionString:process.env.DATABASE_URL.replace(/[?&]sslmode=[^&]*/,''),ssl:{rejectUnauthorized:false}});
  await c.connect();
  fs.mkdirSync('scratchpad/t115d/lote',{recursive:true});
  for(let i=0;i<PREGUNTAS.length;i++){
    ...
    const est=JSON.parse(fs.readFileSync(`scratchpad/t115d/estructuradas/q${String(i+1).padStart(2,'0')}.json`,'utf8'));
    fs.writeFileSync(`scratchpad/t115d/lote/${id}.json`,JSON.stringify(est,null,2));
  }
