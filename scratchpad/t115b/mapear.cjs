...
(async()=>{
  ...
  await c.connect();
  fs.mkdirSync('scratchpad/t115b/lote',{recursive:true});
  for(let i=0;i<PREGUNTAS.length;i++){
    ...
    const est=JSON.parse(fs.readFileSync(`scratchpad/t115b/estructuradas/q${String(i+1).padStart(2,'0')}.json`,'utf8'));
    fs.writeFileSync(`scratchpad/t115b/lote/${id}.json`,JSON.stringify(est,null,2));
  }
