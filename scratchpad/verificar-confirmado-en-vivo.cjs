...
const EMAIL = 'gamusinocalamardopedo@gmail.com';
...
(async () => {
  ...
  await cid.connect();
  const q = await cid.query(`SELECT u AS uid, fc.device_id, fc.fingerprint
      FROM fraud_confirmations fc, unnest(fc.user_ids) AS u
     WHERE fc.status='confirmed' AND fc.retention_until > now()
       AND fc.fingerprint LIKE 'fp2\\_36b1453d7e6c%' LIMIT 1`);
  const real = q.rows[0];
  await cid.end();
  ...
  const ok = Number(hoy) >= 25;
  console.log(`\n${ok ? '✅ el CONFIRMADO sigue topado (el antifraude no se ha roto)' : '❌ el confirmado ha quedado suelto: el arreglo se ha llevado por delante el enforcement'}`);
