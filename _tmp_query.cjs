...
(async () => {
  // Ultimo log de explicar_respuesta
  ...
  const log = logs[0];
  console.log("=== ULTIMO LOG explicar_respuesta ===");
  ...
  // Extraer fragmento de pregunta del mensaje
  ...
  console.log("Correcta:", letters[question.correct_option], "(index " + question.correct_option + ")");
  console.log("Dificultad:", question.difficulty);
  console.log("Oficial:", question.is_official_exam ? "SI - " + question.exam_source : "No");
  console.log("\nEXPLICACION ACTUAL (" + (question.explanation || "").length + " chars):");
  console.log(question.explanation || "(VACIA)");

  // Obtener articulo si existe
  if (question.primary_article_id) {
    ...
    if (article) {
      ...
      console.log("Contenido (" + (article.content || "").length + " chars):");
      console.log((article.content || "").substring(0, 500) + "...");
    }
  }

  console.log("\n=== RESPUESTA QUE DIO LA IA ===");
  console.log(log.full_response || "(no guardada)");
})();
