...
async function verifyQuestion(questionId, index) {
  try {
    ...
    if (error || !question) {
      console.log(`[${String(index + 1).padStart(2, '0')}/${questionIds.length}] ❌ No encontrada`);
      stats.errors++;
      return;
    }

    const optionLabel = String.fromCharCode(65 + question.correct_option);
    const preview = question.question_text.substring(0, 60).replace(/\n/g, ' ').replace(/\s+/g, ' ');
    console.log(`[${String(index + 1).padStart(2, '0')}/${questionIds.length}] ✓ ${preview}...`);

    let status = question.topic_review_status || 'tech_perfect';
    ...
    console.log(`[${String(index + 1).padStart(2, '0')}/${questionIds.length}] ❌ Error: ${err.message}`);
    stats.errors++;
  }
}
...
async function main() {
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('🔍 VERIFICADOR AUTOMÁTICO - WORD 365 (T604)');
  console.log('════════════════════════════════════════════════════════════\n');

  for (let i = 0; i < questionIds.length; i++) {
    await verifyQuestion(questionIds[i], i);
  ...
  console.log('════════════════════════════════════════════════════════════');
  console.log(`✅ Procesadas:                  ${stats.processed}/${questionIds.length}`);
  console.log(`🟢 tech_perfect:                ${stats.tech_perfect}`);
  console.log(`🟡 tech_bad_explanation:       ${stats.tech_bad_explanation}`);
  console.log(`🔴 tech_bad_answer:            ${stats.tech_bad_answer}`);
