...
(async () => {
  ...
  console.log('IDs:', problems.map(p => p.id));

  // Get details for each problematic question
  let num = 0;
  for (const p of problems) {
    ...
    console.log('');
    console.log('Explicación:', (q.explanation || '').substring(0, 300));
    ...
    if (ver && ver.length > 0) {
      ...
      console.log('Comentario:', (v.explanation || '').substring(0, 400));
    }
    console.log('');
  }
})();
