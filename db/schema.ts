...
export const convocatorias = pgTable("convocatorias", {
	...
	sistemaSelectivo: text("sistema_selectivo"),
...
export const oposiciones = pgTable("oposiciones", {
	...
	estadoProceso: text("estado_proceso").default('oep_aprobada'),
	sistemaSelectivo: text("sistema_selectivo"),
	oepDecreto: text("oep_decreto"),
	oepFecha: date("oep_fecha"),
	convocatoriaNumero: text("convocatoria_numero"),
	...
	seguimientoLastChecked: timestamp("seguimiento_last_checked", { withTimezone: true, mode: 'string' }),
	/**
	 * Embudo determinista de `detect-oep-llm` (T-166). SHA-256 de los 20.000 chars que se le
	 * mandan al modelo (NO los 100.000 de `seguimiento_last_hash`, que es OTRO cron con OTRO
	 * algoritmo). Si no cambia entre pasadas, se salta la llamada al LLM. Migración
	 * `20260806_oep_llm_input_hash.sql`.
	 */
...
export const psychometricQuestionDisputes = pgTable("psychometric_question_disputes", {
	...
	check("psychometric_question_disputes_dispute_type_check", sql`dispute_type = ANY (ARRAY['ai_detected_error'::text, 'error_pregunta_respuesta'::text, 'respuesta_incorrecta'::text, 'desacuerdo_correcta'::text, 'mal_formulada'::text, 'pregunta_repetida'::text, 'explicacion_confusa'::text, 'explicacion_mejorable'::text, 'tema_incorrecto'::text, 'otro'::text])`),
...
export const rewardSubmissions = pgTable("reward_submissions", {
	...
	screenshotUrl: text("screenshot_url"),
	feedbackId: uuid("feedback_id"),
	// Impugnación que originó la recompensa (type='impugnacion'). Motivo trazable + clave anti-duplicado.
	// Sin FK a propósito: la impugnación puede ser legislativa o psicotécnica (dos tablas distintas).
