...
async function main() {
  console.log('🚀 Cargando resultados de verificación de Word 365...\n');

  // Cargar ambos archivos JSON
  const lote1 = JSON.parse(fs.readFileSync('verification_results_lote1.json', 'utf-8')) as VerificationResult[];
  const lote2 = JSON.parse(fs.readFileSync('verification_results_lote2.json', 'utf-8')) as VerificationResult[];

  // Filtrar solo las preguntas válidas (excluir "RESTO_DE_PREGUNTAS")
  const allResults = [...lote1, ...lote2].filter(r =>
    r.questionId &&
    r.questionId !== 'RESTO_DE_PREGUNTAS' &&
    r.questionId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)
  );
