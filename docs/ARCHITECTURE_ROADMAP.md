...
> - [`docs/roadmap/pool-segregation.md`](roadmap/pool-segregation.md) — **🟡 DIAGNÓSTICO ABIERTO (2026-06-01)**. 503 burst recurrentes en horas pico (`/api/profile` + colaterales, mensaje "Servicio saturado momentáneamente", duración exacta 8 003 ms = `withDbTimeout` en cascada). Cross-tab por minuto descarta el cron `refresh_ranking_cache` como causa. Hipótesis abiertas: A) Supavisor blip externo, B) `reconcileUserPremium` en `after()` retiene slot pool, C) cache miss masivo post-deploy en `unstable_cache`. **Fase 0 obligatoria antes de actuar**: ejecutar `scripts/diagnostic/capture-pool-pressure.cjs` durante el próximo pico (8:50-10:30 UTC) para muestrear `pg_stat_activity` cada 30s y falsar hipótesis. Fases 1-4 condicionadas al hallazgo. Defensa-en-profundidad permanente: fail-fast en `withDbTimeout` (200 ms acquire timeout), métrica "% pool saturado", test de carga en CI.
