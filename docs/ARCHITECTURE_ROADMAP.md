...
> **🔴 Sesión 2026-07-21 — CASCADA DE 504 POR HEALTH CHECK bajo pico de tarde (postmortem completo: `docs/architecture/incidente-frontend-healthcheck-cascade-21jul.md`):** En el pico (~17:10-18:25 Madrid) la app devolvió **504 reales a usuarios** (`ELB_5XX` pico **1.698/min**, con `Target_5XX=0` y latencia baja = "no hay target sano al que enrutar", NO el origen fallando). **Causa raíz a ciencia cierta:** tasks de **1 vCPU** (`cpu:1024`) → bajo alta concurrencia el **event-loop single-thread de Node se satura** en tasks individuales (evidencia dura: `pool_capacity_samples.hung_clientread_over_10s`=4-5 en el pico + `CPUUtilization` **max=100%/avg=40%** = un task pegado, no la flota) → el health check de contenedor (`/api/health/db-ready`, `SELECT 1` trivial pero timeout 4-5s) **no consigue CPU para responder** → ECS mata el task "unhealthy" (que en realidad SERVÍA, solo iba lento, exit 137) → su carga cae sobre los demás → **cascada a 0 running**. El health check convertía "lento" en "caído". Tráfico dominado por **`/api/auth/token`** (#1 por 4,5×, hace **RS256** CPU-bound). **NO era** la BD (RDS 132/400 conns, SELECT 1 trivial), ni pool exhaustion (`frontend_active` 4-9), ni OOM (mem 73%), ni deploy. **Error propio:** subir a 10 tasks lo empeoró (cold-starts al 100% compiten por CPU). **Contención (por CLI = DRIFT, ver postmortem):** health check de contenedor tolerante (task def `:502`: timeout 10/retries 5/startPeriod 180) **rompió la cascada** + ELB health check relajado + capacidad (max→12, min→8). **Fix en el ORIGEN aplicado:** `.github/workflows/frontend-deploy.yml` health check → tolerante (antes hardcodeaba los valores asesinos). **Ataque a la raíz (plan, T-071/T-073):** (1) tasks 2 vCPU; (2) sacar RS256 del hot path; (3) **autoscaling por RequestCountPerTarget o ALB-5xx** (la CPU MEDIA no capta este cuello — escaló IN a 6 con 504s en curso); (4) health check = liveness real, no readiness-bajo-carga; (5) **medir event-loop lag** (`monitorEventLoopDelay` → `observable_events`, hoy punto ciego). `RULE_FRONTEND_SATURATION` (desplegada horas antes) **habría dado 1 aviso** (validado: 5 ciclos con ≥4 canaries en timeout). PEND codificar TODO el drift en Terraform.
>
> **🔴 Sesión 2026-07-14/15 — CONTENCIÓN RDS RECURRE bajo carga → GATILLO de la READ REPLICA CUMPLIDO:** Tras el fix del 12/07 (cache de los 5 paneles), un **spike de 5xx** (correos de alerta) reveló que la contención **NO estaba eliminada**: había MÁS badges/contadores admin corriendo agregación viva en el **pool de usuario** (`getDb`/`getReadDb`-con-réplica-off). **Fase 0 (this session):** (a) `contenido/count` → query DEDICADA (no reusar el CTE de 5 sub-agregaciones del overview) en `getAdminDb`; (b) `competidores/changes-count`, `scope-verification/count`, `oposiciones/rollover-pending`, `admin-charts`, `getContenidoOverview`, `refreshTeoriaCatalog` → todos movidos a `getAdminDb`. Guardarraíles `badgeCountsPool` + `heavyQueriesOffUserPool` + `statementTimeout≥20s`. Deploys `vence-frontend:465→466`. **⚠️ REGRESIÓN cazada por revisión adversarial §98 (y confirmada en prod):** bajar el `statement_timeout` del pool de usuario 30s→10s **abortaba queries user-facing legítimas y pesadas** en `getReadDb→getDb` (réplica off) — `/api/oposiciones/catalog` pasó de **0→10 http_5xx**. **Revertido a 30s** (la lección: el slot-hold se ataca moviendo las queries PESADAS de pool, NO recortando el timeout global del hot path). **🔑 HALLAZGO CLAVE:** con TODO lo pesado ya fuera del pool de usuario, la contención **CALMÓ de noche pero RECURRIÓ el 15/07 bajo carga de mañana (06:00-07:00 UTC)** — `admin/dashboard` 503, `questions/filtered` 503, `answer-and-save` 503, `canary_db_pool_failed`. → **La condición de gatillo de la read-replica ("provisionar solo si la contención persiste con la cache desplegada") YA SE CUMPLE, sin ambigüedad.** La analítica pesada de admin (`admin/dashboard`) sigue compartiendo el CÓMPUTO del primario `t4g.medium` con el hot path de usuario; eso solo lo aísla una **read replica** (o cachear `admin/dashboard` en Redis como mitigación intermedia sin coste). **✅ CAPAS 1+2 APLICADAS Y CONFIRMADAS EN DIRECTO (15/07):** read replica `vence-prod-replica` provisionada (t4g.medium, param group propio `hot_standby_feedback=on` — cazado al simular que sin él las lecturas se cancelaban por "conflict with recovery") + flip `USE_READ_REPLICA=true` (TD `:468`→`:469`, runtime-config sin rebuild, rollback = flip a false; lag 0,08-0,13s). **Capa 1:** los ~20 callers de `getReadDb` (theme-stats ~28k s + catálogo + analytics usuario + disputes) → réplica. **Capa 2:** `system-health` + `observability` (el ofensor #1 de `pg_stat_statements`, agregación sobre `observable_events` ~117k s) movidos de `getAdminDb`→`getReadDb` (`:469`). **🔑 REPRODUCTOR CLAVE (insight de Manuel):** el pico NO era horario sino **abrir el panel de control** (`/admin` auto-refresca y dispara las agregaciones). Con eso se midió a demanda: ANTES de la Capa 2, abrir el panel ponía queries de **20-42s en el PRIMARIO** (5-6 lentas a la vez → 503 a usuarios); DESPUÉS, el primario se queda a **0 queries lentas** y la agregación (6,9s) corre en la RÉPLICA, aislada. **Fix de raíz confirmado con medición en vivo**, no proyección. **Capa 3 (crons backend NestJS `alert-rules.ts` ~50k s) pendiente SOLO si el primario vuelve a saturarse** (con el panel ya no ahogándolo, probablemente no urge). `infra-stats` se queda en primario (monitoriza `pg_stat_activity` del pool). Detalle operativo (endpoint, gotcha reboot, rollback): `runbooks/contencion-rds-paneles-admin.md` §3. **⚠️ MATIZ (clasificación `pg_stat_statements` por ORIGEN, 15/07 — NO todo es admin):** el cómputo del primario se reparte en: **Admin paneles/analytics ~117.000s** (lectura → réplica ✅ lo saca) · **Escrituras de telemetría + writes de usuario ~96.000s** (la observabilidad mete 7,5M+ inserts en la MISMA BD que agrega → réplica NO ayuda, es carga AUTOINFLIGIDA) · **Usuario analytics pesado ~60-80.000s** (áreas débiles/stats → réplica ✅ si tolera staleness; read-after-write NO) · **Cron outbox poll ~50.000s** (escribe → primario) · **Usuario test-config alto volumen ~30.000s** (→ réplica ✅). **Conclusión: la read replica es la MAYOR palanca (~150.000s de lectura fuera del primario) y calma los 503 de usuario, PERO el fix de raíz COMPLETO es por capas:** (1) read replica; (2) reducir la auto-carga de telemetría (batch/sample de `observable_events` inserts, o sink separado — anti-patrón "la observabilidad se escribe en su propia BD"); (3) afinar frecuencia/pool del cron outbox; (4) medir → `t4g.large` solo si sigue caliente. Ninguna es chapuza; son las capas reales.

> **🟢 Sesión 2026-07-12 — CONTENCIÓN RDS por paneles admin (cache + bulkhead; réplica gatillada):** Fallos transitorios inconexos (503 del dashboard, CRITICAL `canary_db_pool_failed` "Query timeout >1000ms", perfil lento, `CONNECT_TIMEOUT` en build) diagnosticados a **una sola raíz** vía `pg_stat_statements`: la query #1 en `total_exec_time` del primario (~60.000 s acumulados, max 112 s, 45.855 llamadas) era la agregación de los **paneles admin `system-health` + `infra-stats`** sobre **`observable_events` (9,8M filas / 5,4 GB)**, corriendo **SIN cache** y **auto-refrescando cada 60 s por admin**. Las queries están bien (acotadas por `ts` + índice); el problema es cuántas veces se ejecutan. RDS tenía **33/400 conexiones** → **cuello de CÓMPUTO, no de conexiones** (un pooler nuevo no lo arregla). **Fix aplicado (`fix/telemetria-401-retencion`):** (1) **cache** — memo in-memory POST-auth del payload (`system-health` TTL 30s por `window`, `infra-stats` TTL 20s), auth siempre antes del cache, `cached:true` observable; (2) **bulkhead** — `infra-stats` movido de `getDb()` (pool usuarios max:5) a `getAdminDb()` (max:12). Guardarraíl `__tests__/api/admin/adminPanelsCache.test.ts` (8). **Read replica (aislar cómputo) = capa 3 pendiente**, gatillada: la fontanería (`getReadDb`/`USE_READ_REPLICA`/`DATABASE_URL_REPLICA`) existe pero apunta al primario tras el cutover RDS; provisionar **solo si** la contención persiste con la cache ya desplegada. Runbook completo: `runbooks/contencion-rds-paneles-admin.md`. **Matiz vs este roadmap:** esta cache NO "enmascara la causa" (no es retry sobre un SPOF) — elimina un escaneo repetido inútil; la causa desaparece. **AMPLIADO en la misma sesión (deploys `vence-frontend:445→448`, todos verdes):** (3) el memo se extendió a los OTROS 3 paneles de monitoreo (`observability`/`slos`/`canary`) vía **helper compartido `lib/cache/adminPanelMemo.ts`** → **los 5 paneles cubiertos** (guardarraíl 20 tests incl. simulación con reloj inyectable); (4) **`/leyes/[law]` → on-demand** — era la ÚNICA ruta de alto volumen en SSG masivo (`generateStaticParams`→1.278 leyes pegando a RDS en build) = la raíz de la **flakiness del build** (`CONNECT_TIMEOUT`+OOM). Fix: `generateStaticParams=>[]` + `dynamicParams=true` + `revalidate=86400` + warming de 23 leyes top-SEO. **Verificado 0 prerenders en el log del build** (antes ~1.278). SEO safe (mismo HTML, nada 404). Runbook `runbooks/build-resilience-leyes-ondemand.md`; (5) **spinner `/test/por-leyes`** (gate `profileSettled` con techo 4s, simulación de render real) + **fix simetría dinero referidos** (`getEmbajadoresWithBalance` alineado a la decisión Manuel 12/07). Memoria `project_contencion_rds_y_build_flakiness`.
>
> **🟢 Sesión 2026-07-05 — OBSERVABILIDAD 100% IN-HOUSE (Sentry eliminado) + agnosticismo cliente/server:** Se **retiró Sentry del todo** (`@sentry/nextjs`+`@sentry/nestjs`, configs, `withSentryConfig`, `sentry-hooks`, `enrichSentryEvent`; commit `706b2dde`, desplegado front `:339` + backend). **Motivo:** el DSN estaba vacío en prod → Sentry no capturaba nada y todo el error-tracking de CLIENTE se le delegaba → llevábamos tiempo ciegos en cliente (0 errores de fetch/JS en `observable_events`). **Reemplazo in-house** (`lib/observability/client.ts`): hooks nativos `window.onerror`→`unhandled_error`, `unhandledrejection`, `console.error/warn`, **wrapper de `fetch`**→`http_5xx`/`http_4xx`/`http_network_error` (excluye 401/403/404/409/429 + anti-bucle con `/ingest`); error boundaries→`react_error_boundary`. Alertas nuevas `RULE_CLIENT_ERROR_SPIKE`+`RULE_CLIENT_HTTP_4XX_SPIKE`. **Verificado LIVE** (console_error/warn ya emitiendo en prod). Detalle: memoria `project_sentry_eliminado_observabilidad_inhouse`, runbook `observability.md` nota 05/07. — En la misma tanda de sesiones (04-05/07): **CUTOVER BD a RDS** ejecutado (Supabase congelado backup), **cliente 100% agnóstico de Supabase**, primer endpoint server-side migrado (`dispute/mark-read`→Drizzle+token), fix bug rachas (cron leía tabla muerta) + recuperación masiva de tests perdidos por el hueco C1 (118 usuarios). **SIGUIENTE (agnosticismo):** ver `roadmap/agnosticismo-supabase.md` §"Siguiente paso" — migrar los ~6 endpoints admin con `createClient(SERVICE_ROLE)` restantes + retirar bridge auth (Fase B) → apagar Supabase.
>
> **🟢 Sesión 2026-07-03 — FASE B HECHA (agnosticismo de auth):** el emisor de tokens migró de **Supabase Auth → Auth.js RS256/JWKS** (prod estable `vence-frontend:320`). Tras 3 intentos fallidos (FK→`auth.users`; session-gap ×2), lo cerró un **bridge server-side** en `/api/auth/token` (acuña RS256 desde un Bearer Supabase válido cuando aún no hay sesión Auth.js) — cutover sin re-login. +2 huecos cazados y arreglados el mismo día (auto-relogin al cerrar sesión `:319`; **guard admin** rechazaba RS256 → panel admin 401 `:320`), todos por el MISMO patrón: el gate E2E cubría solo `/api/v2` normal → nuevo gate **`scripts/fase-b-auth-surfaces-check.cjs`** (regular+admin+finance, 11/11). Verificado con usuarios reales (10 activos, 0 fallando, 0 errores). `auth.users` desacoplado (52 FKs→`user_profiles`). Deploy manual robusto `scripts/deploy-frontend.sh` + landmine GHA cerrado (flags flip literales en el YAML). **Desbloquea §3.1 de la migración AWS** (mover la BD): ya solo falta soak+retirar HS256/bridge + **C4 (drop RLS)**. Detalle: `roadmap/fase-b-ejecucion-authjs-rs256.md` §"Siguiente paso".
>
> **Última actualización (anterior):** 2026-05-27 ~15:30 CEST (sesión miércoles tarde — 4 cabos cerrados con paridad/tests/monitor: **refactor `/api/v2/oposiciones-compatibles/progress`** sustituyendo agregación runtime por SELECT a `user_article_stats` con JOIN articles, paridad 14/14 bit-a-bit, latencia 18s→835ms para users heavy (commit `5027a941`); **Fase A + Fase B answer-and-save** — bug upstream que perdía 11.37% del tracking por no resolver `article_id` server-side: fix server enriquece desde la query de validación (commit `b832517a`, sangrado detenido 0% NULL post-deploy) + backfill 102.353 filas históricas en 7 min sin tocar triggers materialized (roadmap `answer-and-save-article-id-fix.md`); **Fase 1 weak-articles-perf** — `CREATE INDEX CONCURRENTLY idx_uqh_user_weak` aplicado en prod 1002 ms VALID 2528 kB sin downtime (commit `f23968e6`, roadmap `weak-articles-perf.md` con plan 3 fases), pendiente medir 24-48h. Plus: refactor seguridad `/admin/feedback` eliminando service_role del bundle público (commit `1e65f76f`), Mariangeles deletion replanteada como cancel-only tras investigación (cuenta preservada), Mario feedback Tema 1 respondido. **3 roadmaps actualizados + 2 nuevos** (`weak-articles-perf.md`, `answer-and-save-article-id-fix.md`).
>
> **🟢 Sesión 2026-05-25 (anterior):** maratón Bloque 4: CI fixed + canaries `/api/daily-limit` Y `test-config` family ACTIVOS + manual observability completo + Bloque 4 capa 1 al 80% (tabla `observable_events` + emisores cron Fargate + endpoint ingest). Ver detalle en commits previos.
>
...
| **Paneles admin — cache + bulkhead** (LOS 5: system-health/infra-stats + observability/slos/canary vía helper `adminPanelMemo`) | escaneo repetido de `observable_events` 5,4GB cada 60s/admin saturaba el primario (era la query #1 en total_time) | ✅ **APLICADO (12-jul)** | `runbooks/contencion-rds-paneles-admin.md` |
| **`/leyes/[law]` → on-demand** (fin del SSG masivo de 1.278 leyes) | flakiness del build (`CONNECT_TIMEOUT`+OOM): era la única ruta de alto volumen en SSG real | ✅ **APLICADO (12-jul)** — verificado 0 prerenders en build; warming de 23 top-SEO | `runbooks/build-resilience-leyes-ondemand.md` |
| **Read replica RDS (post-cutover)** — aislar CÓMPUTO de lecturas admin/analytics | contención CPU/IO del primario que pools separados NO resuelven | 🟢 **CAPAS 1+2 LIVE + CONFIRMADAS EN VIVO (15-jul)** — réplica `vence-prod-replica` (t4g.medium, `hot_standby_feedback=on`) + `USE_READ_REPLICA=true` (TD `:469`). Capa 1: ~20 callers `getReadDb` (theme-stats ~28k s…). Capa 2: `system-health`+`observability` (ofensor #1 observable_events ~117k s) `getAdminDb`→`getReadDb`. **Medido con el reproductor de Manuel (abrir el panel):** el primario pasó de queries 20-42s → 0 lentas; la agregación corre en la réplica. Capa 3 (crons backend ~50k s) solo si recurre. | `runbooks/contencion-rds-paneles-admin.md` §3 |
...
2. **La vigilancia de que sigan vivas cubre solo UNA de las tres** → **[T-325]** ([ficha](roadmap/tareas-pendientes.md)). **✅ Arreglado (06/08/2026), pendiente de desplegar.** El catálogo `backend/src/cron-schedule/external-jobs.registry.ts` —creado precisamente para que un trabajo programado no muera en silencio— declaraba únicamente `temario-pdf-worker`. Se añadieron `vence-content-radar` y `vence-instagram-daily` (con su cadencia cron portable + `timeZone: 'Europe/Madrid'`) **y**, comprobado antes de tocar nada, ninguno de los dos emitía `cron_tick`/`cron_run` — se instrumentaron `content_radar.mjs` e `instagram_daily.py` con el mismo contrato que ya usa `scripts/pdf-worker.ts`. Necesita **dos despliegues independientes, EN ESTE ORDEN**: primero las imágenes propias de `content_radar.mjs`/`instagram_daily.py` (manual, ver `marketing/social-content/README.md` §"Rebuild + redeploy" — no tienen GHA propio) y **solo después** el deploy de backend con el catálogo ampliado. Al revés, el catálogo esperaría señal de un job que su imagen vieja todavía no emite, y `cron_overdue` marcaría CRITICAL permanente contra un job sano (mismo patrón de falso positivo que ya escarmentó con `temario-pdf-worker` el 29/07). El precedente completo, en [§ Incidente 2026-07-29](#incidente-2026-07-29--el-pdf-del-temario-bloquea-el-event-loop-del-frontend-y-tumba-el-guardado-de-respuestas).
...
> - [`docs/roadmap/answer-and-save-article-id-fix.md`](roadmap/answer-and-save-article-id-fix.md) — bug upstream: server no resolvía `article_id` en INSERT a `test_questions`, perdiendo el 11.37% del tracking downstream (143k filas históricas con NULL). ✅ **Fase A APLICADA 2026-05-27** (commit `b832517a`, sangrado detenido — 0% NULL en respuestas nuevas). ✅ **Fase B EJECUTADA 2026-05-27 ~15:15 CEST** — 102.353 filas backfilleadas en 7 min, 0 backfilleables residuales (las 41.678 que quedan con NULL son correctas: psicotécnicas + AI sin resolver). Sub-fase B.2 (refresh user_article_stats) decisión separada.
> - [`docs/roadmap/pool-segregation.md`](roadmap/pool-segregation.md) — **🟢 FASE 1 APLICADA EN PRODUCCIÓN (2026-06-01 ~09:00 UTC)**. Hipótesis D (starvation pool en rolling deploy Fargate) confirmada en Fase 0 vía captura en vivo y **resuelta** vía Fase 1: frontend ECS Fargate ahora usa self-hosted PgBouncer (`pooler.vence.es`, Lightsail London) con `USE_SELF_HOSTED_POOLER=true` activado vía SSM + secret `DATABASE_URL_SELF_POOLER`. Pool `postgres-js` max:1 → max:8 con warmup robusto al boot (3 SELECT 1 paralelos). Nuevo endpoint `/api/health/db-ready` (200 si pool responde en <2s, 503 si no) como `HealthCheckPath` del ALB target group + `healthCheck` container-level ECS — ALB ya no envía tráfico hasta que pool esté listo. Service `desiredCount: 1 → 2` + scalable target `MinCapacity: 1 → 2` (auto-scaling no puede bajar de 2 aunque CPU sea bajo). Resultado verificado en prod: latencia BD 3-18 ms vs 1606-2000 ms previo, 0 errores 5xx en 15+ min post-estabilización, rolling deploy 3 min sin downtime. Commit código: `5ed827fd`. Cambios de infra AWS aplicados vía `aws cli` profile `vence`. **Pendiente**: HA del self-hosted pooler (Fase 6 de [`self-hosted-pooler.md`](roadmap/self-hosted-pooler.md)) para eliminar el SPOF actual (single Lightsail London).
> - [`docs/roadmap/incidente-answer-save-503-01-06.md`](roadmap/incidente-answer-save-503-01-06.md) — **incidente 2026-06-01 ~11:15–11:22 UTC**. Blip de ~7 min de la **primaria de Supabase** (FATAL `XX000 DbHandler exited`) tumbó frontend (db-ready 503) + backend (crons parados) a la vez. Amplificado por una **unhandled rejection que mató el proceso NestJS** (sin handler global) → ECS crash-loop de 5 tasks/5min → 351× 503/504 en answer-save. Read replica + cache stale-if-error sobrevivieron (confirma que el SPOF es la primaria única). **Fix en código**: handler global `process.on('unhandledRejection')` en `main.ts` (log+Sentry, no exit) + `.catch` defensivo en `withTimeout` + 5 tests. **Pendiente**: backend desired=2 con leader-election, migrar backend a self-hosted pooler (sigue en Supavisor), **Fase D RDS Multi-AZ** (fix estructural del SPOF), reintento cliente en answer-save.
> - [`docs/roadmap/observability-capacity.md`](roadmap/observability-capacity.md) — **🟢 3/3 ACCIONES APLICADAS (2026-06-01)**. Cierra el gap entre "observabilidad de eventos" (qué pasó) y "observabilidad de capacidad" (cuánto margen queda) que bloqueó el diagnóstico del incidente 31/05. ✅ **Acción 1** (`extractUserIdFromRequest`, commit `88808e6e`) — `validation_error_logs.user_id` ya no es NULL para GETs con userId en query param; 19 tests + regression del incidente. ✅ **Acción 2** (pool capacity sampler permanente, commit `ddd2824a`) — `@Cron(EVERY_MINUTE)` UTC en backend Fargate muestrea `pg_stat_activity` y persiste en `pool_capacity_samples` (retención 7d, filtro `is_client_backend` para excluir walsender/autovacuum, vista `v_pool_capacity_last_15min` con status GREEN/AMBER/RED). Emite `pool_capacity_flag` SOLO con banderas rojas (idle_in_tx>5s o hung_clientread>10s) para evitar 1.440 events/día sin valor. Leading indicator vs los 5xx lagging. KPI en panel admin + regla `POOL_CAPACITY_SUSTAINED_HIGH` quedan como follow-up. ✅ **Acción 3** (snapshot diario `pg_stat_statements` + vista `v_pg_stat_statements_delta`) — migración `20260601_pg_stat_statements_snapshots.sql` aplicada en BD prod, cron diario `@Cron('5 0 * * *')` UTC con jitter + heartbeat; 15 tests OK.
...
para **ese episodio**, pero **no explica el techo diario** de `answer-and-save`, que aparece también
días sin un solo PDF de por medio (22, 25 y 26/07). Ese techo es un **timeout**
(`ANTIFRAUD_TIMEOUT_MS`), no lentitud. **Análisis, prueba y plan: [T-315]** — aquí solo queda la nota
para que este registro no induzca a error.

> **Lección de método, y es lo que justifica dejarla en el manual:** el mismo síntoma recibió **tres**
> atribuciones por coincidencia temporal —crons del backend, PDFs, un cron de 18 min que encajaba por
> horario— antes de que una sola consulta de **distribución** contara la historia. Ante «esto va
> lento», mirar la FORMA de la cola antes que el calendario: una lentitud real adelgaza, un timeout se
> amontona y se corta en seco. Ahora hay detector para eso (`lib/observability/techoTimeout.cjs`).
