...
| **Read replica RDS (post-cutover)** — aislar CÓMPUTO de lecturas admin/analytics | contención CPU/IO del primario que pools separados NO resuelven | ⏸️ **PENDIENTE, gatillado** — fontanería (`getReadDb`/`USE_READ_REPLICA`) lista, apunta al primario; provisionar solo si persiste con cache desplegada | `runbooks/contencion-rds-paneles-admin.md` §3 |
