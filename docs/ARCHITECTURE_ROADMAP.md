...
> - [`docs/roadmap/observability-capacity.md`](roadmap/observability-capacity.md) — **🟡 NO INICIADO (2026-06-01)**. Cierra el gap entre "observabilidad de eventos" (qué pasó) y "observabilidad de capacidad" (cuánto margen queda) que bloqueó el diagnóstico del incidente 31/05. 3 acciones independientes con ROI alto: (1) extraer `userId` del query param en `withErrorLogging` 30 min — hoy `validation_error_logs.user_id` siempre `NULL` para `/api/profile` y oscurece "¿cuántos users sufren?"; (2) cron `*/1 min` muestreando `pg_stat_activity` → tabla `pool_capacity_samples` + KPI "Capacidad pool" en `/admin/salud-sistema` + alerta `POOL_CAPACITY_SUSTAINED_HIGH` 2-3 h — leading indicator vs los 5xx lagging actuales; (3) snapshot diario de `pg_stat_statements` + vista `v_pg_stat_statements_delta` 1-2 h — distingue queries lentas HOY vs ruido histórico acumulado (5 días sin reset). Total 4-6 h, implementación condicionada a completar Fase 0 de pool-segregation.
