...
| **Read replica RDS (post-cutover)** — aislar CÓMPUTO de lecturas admin/analytics | contención CPU/IO del primario que pools separados NO resuelven | 🟢 **PROVISIONADA + CAPA 1 LIVE (15-jul)** — réplica `vence-prod-replica` (t4g.medium, `hot_standby_feedback=on`) + flip `USE_READ_REPLICA=true` (TD `:468`). Movió los ~20 callers de `getReadDb` (theme-stats ~28k s + catálogo + analytics usuario). PEND capas 2-3 (paneles admin `getAdminDb`→`getReadDb` + crons backend) **solo si** el pico sigue dando 503. | `runbooks/contencion-rds-paneles-admin.md` §3 |
