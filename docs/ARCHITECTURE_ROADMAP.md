...
> **Última actualización:** 2026-05-25 ~11h CEST (sesión lunes mañana: CI fixed [typecheck 0 + lint deuda controlada #80] + **canaries `/api/daily-limit` Y `test-config` family ACTIVOS** = 4/5 endpoints Bloque 3 (stats descartado → Bloque 3 efectivamente al 100%) + **MVP `observable_events` unificada** (Bloque 4 arrancado, primer emisor real `RefreshRankingsCron` + espejado `validation_error_logs` writes, validado end-to-end con event vercel/warn/auth tras curl 401) + **fix bug `/api/admin/revalidate` cross-runtime** (TAG_INVALIDATORS dispatch + 11 tests anti-regression + manual cache-revalidation actualizado con sección «Cross-runtime cache»). Crons L-V/lunes 08:00 UTC dispararon OK).
>
