...
> - [`docs/roadmap/pool-segregation.md`](roadmap/pool-segregation.md) — **🟢 FASE 0 COMPLETA — HIPÓTESIS D CONFIRMADA (2026-06-01 06:09-06:13 UTC)**. 503 burst recurrentes en `/api/profile` + colaterales, mensaje "Servicio saturado momentáneamente", duración exacta 8 003 ms = `withDbTimeout` en cascada. Captura ad-hoc 5 min sobre burst en vivo (130 errores) tras push del commit `000d2c2f` descarta hipótesis iniciales A (Supavisor blip), B (`after()` retiene slot) y C (cache miss `unstable_cache`) — el patrón `active_app=0` sostenido durante 4 min revela **Hipótesis D no contemplada inicialmente**: starvation del pool postgres-js (max:1) en rolling deploy Fargate. Container nuevo arranca con pool vacío, `connect_timeout: 5s` insuficiente para handshake TLS contra Supavisor en cold start → cascada hasta calentamiento del pool. **Solución de fondo (Fase 1 ⭐ PRIORIDAD ALTA)**: migrar `getDb()` al self-hosted pooler `pooler.vence.es` (ya operativo para `getAdminDb()`) — mantiene pool TCP persistente upstream, sobrevive a rolling deploys. Band-aid alternativo (5 min, riesgo 0): subir `connect_timeout: 5 → 15` en `db/client.ts`.
