...
> - [`docs/roadmap/observability-capacity.md`](roadmap/observability-capacity.md) — **🟢 2/3 ACCIONES APLICADAS (2026-06-01)**. Cierra el gap entre "observabilidad de eventos" (qué pasó) y "observabilidad de capacidad" (cuánto margen queda) que bloqueó el diagnóstico del incidente 31/05. ✅ **Acción 1** (`extractUserIdFromRequest`, commit `88808e6e`) — `validation_error_logs.user_id` ya no es NULL para GETs con userId en query param; 19 tests + regression del incidente. ✅ **Acción 3** (snapshot diario `pg_stat_statements` + vista `v_pg_stat_statements_delta`) — migración `20260601_pg_stat_statements_snapshots.sql` aplicada en BD prod (2818 queries capturadas en primer snapshot, 89 ms); módulo NestJS `PgStatSnapshotModule` registrado, cron diario `@Cron('5 0 * * *')` UTC con jitter + heartbeat; 15 tests OK. ⏳ **Acción 2** (pool capacity sampler `*/1 min` + KPI en `/admin/salud-sistema` + alerta `POOL_CAPACITY_SUSTAINED_HIGH`, 2-3 h) — espera Fase 0 de [`pool-segregation.md`](roadmap/pool-segregation.md).
