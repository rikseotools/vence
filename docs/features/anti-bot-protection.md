...
- ⚠️ **La exención de tráfico interno exige SECRETO (29/07/2026).** Hasta esa fecha bastaba el header `x-vence-canary` —sin secreto— para saltarse el reto en `/api/questions/filtered`: comprobado en producción, sin header `403 challengeRequired`, con header `200` con las preguntas y su respuesta correcta. Ahora la exención requiere `x-vence-canary-secret` (`CANARY_SECRET`, respaldo `CRON_SECRET`), comparado en tiempo constante, y **sin secreto configurado no se exime a nadie**. El contador que alimenta el gate usa el mismo criterio: con el header suelto se podían servir preguntas sin sumar nunca al volumen, o sea evadirlo por acumulación. Núcleo `lib/api/syntheticTrust.ts` · guardarraíl `__tests__/guardrails/exencion-antiscraping.test.ts`. **Ojo con el canary del propio gate:** se exime a sí mismo justo en la prueba de "no se reta a un usuario normal", así que esa aserción hoy pasa siempre ([T-280]).
