...
- **⚠️ Provenance del veredicto (T-334, 07/08/2026), límite a asumir de entrada:** `record_topic_verification` acepta CUALQUIER `findings` para un `correct` — no hay campo que diga si de verdad se consultó algo. Caso real: `auxiliar_administrativo_sms` T15/T16 se sellaron `verified_correct` con una nota nunca contrastada contra el BOE, y era falsa (ver [T-332]). Desde T-334, `record`/`apply` declaran SIEMPRE `findings.fuente` — un id de BOE o documento real si lo hay, o `'razonado'` (nunca en blanco) si no se consultó nada concreto. `npm run verify:scope -- provenance` cuenta, por oposición, cuántos `verified_correct` no tienen fuente concreta — recuento SEPARADO del badge de `audit` (un veredicto sin fuente sigue siendo `verified_correct`; lo que cambia es cuánto se puede confiar en él si alguien vuelve a dudar). **Esto es autocertificación**: no prueba que se mirara el BOE, solo hace la ausencia visible y contable. El guardarraíl de verdad que SÍ comprueba el hecho es [T-333].
