...
- **Y el PREVENTIVO, `visibility_map_sin_ajuste`:** marca la tabla grande que **no tiene la protección puesta**, aunque hoy esté al 100%. Existe porque el detector de frías llega tarde: el 29/07 se protegieron las 13 tablas frías **de ese momento** y a la mañana siguiente **`observable_events`** —la mayor, ~3 GB, y encima aquella contra la que se lanzan todas las consultas de diagnóstico— había caído al 85,9% por no estar en aquella lista. Perseguirlas de una en una garantiza que siempre haya alguna esperando a enfriarse.
- **Triaje en 30 segundos cuando una tabla sale fría (30/07):** el hallazgo **ya trae la causa** en `detail.remedio`, y hay cuatro respuestas posibles. La que más engaña es la tercera:
  1. **Sin el ajuste de inserts** → el `ALTER TABLE` exacto viene en el hallazgo. Es el arreglo durable.
  2. **Con el ajuste y por debajo de los dos disparadores** («NO se arreglará sola») → hace falta un `VACUUM (ANALYZE)` a mano **y** bajarle el `autovacuum_vacuum_scale_factor`. Caso `questions`, 78,5%.
  3. **«Régimen de rotación alta, NO abandono»** → el autovacuum está **a mitad de su ciclo de inserts** y las filas muertas están por debajo del umbral: **la maquinaria funciona y va a entrar**. La tabla sale fría porque es una **cola** que se llena y se vacía (caso medido: `test_questions_outbox`, 84,2%, 508 MB para 34.202 filas vivas, **1.333 inserts pendientes de los 1.342 que disparan su autovacuum**). **No hay nada que arreglar** salvo que alguna consulta suya dependa de index-only scans. Antes de esto el hallazgo mandaba a *«revisar por qué el autovacuum no llega»* cuando estaba a nueve inserts de llegar.
  4. **El resto** → mirar si el autovacuum llega de verdad (workers saturados, `cost_delay`).
  - ⚠️ **Dos criterios que se probaron para distinguir la cola y NO valen** (están medidos, no los repitas): (a) **ratio de borrado** — `test_questions`, la tabla del incidente, tiene 0,709 contra el 0,680 del outbox, así que habría silenciado el caso a cazar; (b) **«autovacuum hace menos de una hora»** — el outbox se autovacuuma ~cada hora, así que el veredicto cambiaba según el minuto en que corriera el barrido. Lo que sí distingue es la **aritmética del disparador de Postgres**: `insert_threshold + insert_scale × vivas` dice cuándo entra, y los inserts pendientes cuánto falta.
- **⚠️ Si la tabla YA tiene el ajuste y sigue fría, el hallazgo te dice si se arreglará sola.** Caso real (`questions`, 30/07): **7.394 filas muertas contra un umbral de 8.084** (scale 0.05 sobre 159.671 vivas) y **0 inserts pendientes** — por debajo de **los dos** disparadores a la vez, así que podía quedarse al 78,5% indefinidamente. Ahí no vale esperar: hace falta `VACUUM (ANALYZE)` manual **y** bajar el `autovacuum_vacuum_scale_factor` de esa tabla (se puso a 0.02).
...
**Un canary VERDE que no comprobaba nada: el del gate anti-scraping se eximía a sí mismo (2026-07-30, T-280)** — `canary-questions-gate` verifica que *«el gate NO reta a un usuario normal»*… y mandaba en esa misma petición la cabecera de exención. Estaba **exento cuando lo medía**, así que esa aserción pasaba siempre: si el gate hubiera empezado a retar a todo el mundo —usuarios reales sin poder cargar preguntas, el fallo más caro— habría seguido verde.
- **Por qué no bastaba con quitarle la exención**, que es lo primero que se piensa: está medido que el usuario smoke acumula volumen de OTRAS sondas (**2.220 preguntas servidas el 27/07, 380 el 29/07**, contra un umbral de 500), así que un día cargado daría rojo sin avería — y un canary que se pone rojo sin motivo se acaba ignorando, que es la misma enfermedad por el otro extremo.
- **Cómo queda:** el canary pregunta primero el veredicto del gate **para su propio sujeto y sin servir preguntas** (`GET /api/security/captcha/status?subject=<id>`, con `CRON_SECRET`). Si su contador está por debajo del umbral, hace la sonda **SIN exención** y la aserción es real; si está saturado, la marca como no comprobada.
- **⚠️ Al leer su resultado, mira `gateAssertion`:** `real` = se comprobó de verdad; `omitida_sujeto_saturado` / `omitida_veredicto_no_disponible` = **el verde NO cubre esa aserción hoy**. Un verde que no distingue «comprobado» de «no pude comprobarlo» es el defecto original con otra cara.

**Un 403 en la caja también es una venta perdida, y no lo miraba nadie (2026-07-31, T-340)** — `rdiazprados@gmail.com` intentó comprar premium **17 veces entre las 05:54 y las 06:04** y recibió 403 en todas: su navegador mandaba un `userId` de un usuario que ya no existe y el contraste de identidad cortaba. Pagó a las 06:10 tras recargar. **Cero alertas** durante esos diez minutos, porque `stripe_checkout_failed` solo cuenta **5xx** y aquello era un «no» perfectamente ejecutado por el servidor.
- **Qué cambió en la vigilancia:** regla `cobro_bloqueado_auth` sobre los **mismos** `PATRONES_RUTA_COBRO` que los 5xx (una sola lista, no dos), umbral 3 en 10 min. Y `rutasCobroVigiladas` (guardarraíl) sigue los imports de cada `route.ts`: si uno nuevo llega a Stripe y no cae bajo un patrón vigilado, el CI se pone rojo. Un endpoint de cobro **nace vigilado**.
- **Qué cambió en el comportamiento:** el contraste de identidad era un **detector** usado como control de acceso — el id del cliente se sobrescribe siempre con el del token, así que cortar no impedía ningún abuso. Ahora la política la decide **el daño de equivocarse de cuenta**: `cortar` en cancelar/reactivar/escribir en el historial, `seguir-con-el-token` en checkout/portal/lectura. Defecto `cortar`, y cada endpoint declara la suya por escrito.
- **Canary `canary-identidad-pago`** (post-deploy, `POST /api/v2/canary/run-identidad-pago`): prueba **las dos mitades juntas** — que el checkout no corta y que cancelar sí. Por separado no prueban nada: una pasaría con todo abierto y la otra con todo cerrado. Lección heredada del gate: **mira `cancelAssertion`** — `real` = comprobado; `omitida_sujeto_con_suscripcion` = se saltó la sonda destructiva a propósito, porque un canary no puede poder romper lo que vigila.
