...
>
> **2026-07-05 — 🔴 SENTRY ELIMINADO DEL TODO. Observabilidad 100% IN-HOUSE.** Se retiró `@sentry/nextjs` (deps, configs `sentry.*.config.ts`, `withSentryConfig`, `instrumentation.onRequestError`→emit in-house, `enrichSentryEvent`, `sentry-hooks.ts`, y las llamadas en `error.tsx`/`global-error.tsx`/`logClientError.ts`/`db/timeout.ts`). **Motivo:** el DSN estaba vacío en prod → Sentry no capturaba NADA, y como todo el error-tracking de cliente se delegaba a Sentry, llevábamos un tiempo **CIEGOS en cliente** (0 errores de fetch/JS en `observable_events`). **Reemplazo:** `lib/observability/client.ts` ahora instala captura nativa in-house — `window.onerror`→`unhandled_error`, `unhandledrejection`→`unhandled_rejection`, `console.error/warn`→`console_error/warn`, **wrapper de `fetch`**→`http_5xx`/`http_4xx`/`http_network_error` (excluye 401/403/404/409/429 y el propio `/ingest`; anti-bucle). Error boundaries→`react_error_boundary`. `logClientError`→`client_error`. Alertas nuevas: `RULE_CLIENT_ERROR_SPIKE` + `RULE_CLIENT_HTTP_4XX_SPIKE` (el 5xx de cliente ya cae en `RULE_HTTP_5XX_SPIKE`). Tests: `__tests__/lib/observability/clientCaptures.test.ts`. **Toda mención a Sentry más abajo es HISTÓRICA.**
>
...
| Datos corruptos (drift contadores) | ⚠️ | `stats_drift_log` + `check_stats_drift` (~diario, **30 users al azar**, uqh_v2 por **row-count**). **PUNTO CIEGO confirmado 03/06**: corrió a las 08:47 DURANTE el freeze de 5 tablas materializadas y no vio nada — muestreo escaso (222 afectados de miles) + uqh chequeado por nº de filas (insensible al drift de `total_attempts` al re-responder preguntas ya respondidas). Para esa clase ver fila siguiente. |
| Pipeline de stats materializadas (congelado / escribe mal) | ✅ | **3 capas (post-incidente 03/06, cutover outbox a medias)**: `materialized_stats_stale` rule (frescura: tabla parada con tráfico → CRITICAL), `stats_paridad_divergence` rule (correctitud: paridad EN VIVO uqh_v2 vs test_questions, sin muestreo), y canary `canary-stats-pipeline` (e2e 24/7, inyecta respuesta sintética y verifica propagación → cubre el valle nocturno donde las rules no tienen tráfico). Ver `docs/roadmap/canary-y-simulaciones.md` §Nivel 3 variante stats-pipeline. |
...
- [x] **Gap 17** ✅ código hecho — reconciliador `dispute-email-reconciliation` (backend, cron horario `:15`) que verifica la invariante "impugnación cerrada ⇒ email enviado o skip legítimo". Distingue `real_drop` (soporte activo, sin email = fallo silencioso, caso Eva) de `expected_skip` (soporte off, caso Marta). Emite `invariant_violation` + regla de alerta `dispute_email_drop` (accionable) + test. **Validado contra BD real: pilla a Eva, clasifica a Marta como skip, 0 falsos positivos.** Pendiente: deploy backend. *(Detección, NO auto-cura todavía.)*
  - **🕳️ El hueco que dejó abierto, cerrado el 26/07/2026 (T-116):** Gap 17 cubre *"el email NUNCA se intentó"* y se escribió dando por hecho que *"se intentó y el proveedor lo RECHAZÓ"* ya estaba vigilado — así lo decía literalmente el comentario del servicio. **No lo estaba: no había ninguna regla mirando `email_events`.** Por eso el bug de la idempotency-key pasó 2 meses invisible. Nueva regla **`email_send_failed`** (`alert-rules.ts`): dispara con **UNA** ocurrencia de `event_type='failed'` en 15 min, `severity=error`, `fingerprint` por `email_type`, cooldown 60 min. **Calibrada con 180 días de datos reales:** 13.375 `sent` vs 9 `failed` → un fallo es raro, no hace falta umbral de ráfaga; el replay de la regla REAL contra RDS habría avisado en **9/9**, con **8 alertas en 180 días (~1 cada 23)**. Las dos reglas son complementarias y no se solapan: `dispute_email_drop` = 0 filas; `email_send_failed` = fila `failed`.
...
- [ ] **Gap 16** — enrutar emails transaccionales por el outbox `outbox_events` (helper `enqueueEvent(tx, …)` dentro de la TX del UPDATE + worker que drena con `processed_at`/`attempts`/`last_error`). Empezar por `dispute/resolve` y `feedback/respond`.
  - **Decisión de diseño (03/06, tras objeción de duplicados de Manuel)**: la recuperación de emails caídos va **por el outbox, NO por un endpoint de resend suelto**. Motivo: existe una ventana en la que **Resend envió pero morimos antes del `INSERT email_events`** → un resend ingenuo duplicaría. Defensas exactly-once:
    1. **`Idempotency-Key` determinista en Resend, DERIVADA DEL CONTENIDO** — `dispute-resolve-{disputeId}-{sha1(status+respuesta)}` (helper `buildDisputeEmailIdempotencyKey`, `lib/api/v2/dispute/idempotency.ts`) en el envío original Y en el reintento → Resend deduplica en su ventana (~24h) aunque el original ya saliera.
       > ⚠️ **CORREGIDO 26/07/2026 (T-116).** Este punto decía `dispute-resolve-{disputeId}`, clave **fija por impugnación**, y ese diseño tenía un fallo: si el cuerpo CAMBIA dentro de la ventana de 24h (se corrige una respuesta errónea, o se contesta a una alegación `appealed`), Resend **rechaza** el envío nuevo —*"idempotency key has been used… but the request body was modified"*— y el usuario se queda con el email equivocado. Estuvo **2 meses en producción** (8 casos, 04/06→25/07) y lo destapó una usuaria. La clave debe cumplir DOS cosas: mismo contenido → misma clave (el reintento no duplica) **y** contenido distinto → clave distinta (la corrección sí sale). Regla general para cualquier proveedor (Resend, Stripe): **la clave sigue al CONTENIDO, no solo al evento.**
...
| 2026-06-02 | **Causa raíz incidente Supavisor 28/05 + barrido de gaps**. (1) Conexiones zombi de Supavisor en path `getDb()` → `max_lifetime:90` en todos los pools + probe-on-timeout (ver ARCHITECTURE_ROADMAP §Fase 3 "TRAMPA HISTÓRICA 2"). (2) **Bug del sampler de pool**: `take_pool_capacity_sample()` contaba `application_name='postgres-js'` (guion) pero el real es `postgres.js` (punto) → `frontend_active_conns` era un gauge muerto pegado a 0; alerta `pool_frontend_saturation_high` nunca podía disparar. Fix aplicado a prod (`20260602_fix_pool_sampler_appname.sql`). (3) **GHA `notify-failure`** metía el workflow en `endpoint` en vez de `metadata.workflow` → regla `workflow_failure_burst` recibía null; corregido. (4) **3 endpoints sin `withErrorLogging`** (errores no llegaban a `observable_events`): `admin/ads` (panel GSC), `daily-limit`, `admin/graduated-limits` → envueltos. (5) **CI "Tests" rojo permanente**: job `unit` falla por 10 suites (7 tests DB mal-clasificados como unit + 3 de oposiciones); triaje dedicado pendiente. Reproducir en local: `env -u DATABASE_URL npm run test:unit`. |
...
**La causa:** el wrapper de `fetch` ya excluía el fallo de red durante `pageLeaving` / `visibilityState === 'hidden'` (el navegador cancelando lo que hay en vuelo no es un fallo), pero el `catch` de la app escribe por `console.error` y **esa ruta pasaba por otro filtro que no lo miraba**. La misma petición abortada se suprimía por un lado y se registraba a severidad completa por el otro.

**La regla vive en un solo sitio:** `lib/observability/consoleNoise.ts` (`esRuidoDeConsola`, 12 tests), compartida por las dos rutas de captura. Baja a `debug` —no borra— el fallo de RED **solo** si la página se está yendo, y **nunca** degrada un error de aplicación. Esa frontera está fijada por tests a propósito: es lo que impide que el filtro se coma señal.

**Cómo se verifica (obligatorio antes y después de desplegar):**

```bash
node scripts/observabilidad/sim-ruido-console.cjs [--dias N]
```
