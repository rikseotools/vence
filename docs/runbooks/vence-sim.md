...
### Identidad (auth PROPIA — Supabase PROHIBIDO)
Los journeys con `as:` corren **como un usuario real** forjando la cookie de sesión
Auth.js (`__Secure-authjs.session-token`) cifrada con `AUTH_SECRET`. El secreto se toma
**env-first**: `SIM_AUTH_SECRET` (o `AUTH_SECRET`) y, en dev, fallback a SSM
...
> usarlo. Para el canary, apunta `SIM_IDENTITY_*` a una **cuenta de test**, no a un cliente.
