...
- `I9_tipo_incoherente`: el `tipo` del hito contradice su propio `titulo`.
- **`I10_inscripcion_sin_convocatoria`** (nuevo 26/07, T-124): **plazo de inscripción ya pasado en una convocatoria que AÚN NO se ha publicado** (`plazas_prevision=true` o hito `convocatoria_publicada` en `upcoming`). Es el más dañino de la familia porque **se ve**: la landing anuncia *"Plazo de inscripción cerrado."* en un proceso no convocado, así que el opositor cree que perdió el plazo y se va. Caso raíz: `administrativo-pais-vasco`, con la convocatoria prevista para el 31/08/2026 y un plazo "cerrado" el 01/06/2026 que nunca existió.

> 🆕 **I11 — `I11_fechas_inscripcion_vs_hitos` (T-142, 26/07/2026).** La landing pinta el plazo **dos veces desde sitios distintos**: la caja de la convocatoria lee las columnas `inscription_start`/`inscription_deadline` y el timeline lee los hitos `plazo_inicio`/`plazo_fin`. Ninguno de los 8 invariantes anteriores los comparaba **entre sí**, así que podían separarse en silencio y la misma página acababa dando dos fechas del mismo plazo. Es contradicción de la fila consigo misma (como I10): no necesita fuente externa. Medido al añadirlo: **7 casos, 3 en landings activas**; 4 de ellos resultaron ser un hito de CIERRE tipado como `plazo_inicio` —la fecha coincidía con `inscription_deadline` y no había hito de cierre—, arreglados retipando. Va al cubo de **error** junto a I10: lo que el opositor lee es una fecha, y dos fechas del mismo plazo es misinformación.
  - **Cómo se arregla:** poner `inscription_start`/`inscription_deadline` a NULL **en las DOS fuentes** (ver el aviso de abajo). Se BORRA un dato sin fuente —dirección segura—; NUNCA se convierte una previsión en fecha oficial sin cita literal de boletín.
  - **Verificar el arreglo en la página, no en la BD:** purgar caché (`POST /api/admin/revalidate` con los tags `landing` y `oposiciones-catalog`) y hacer `curl` a la landing comprobando que el texto desapareció. **No usar WebFetch para esto: cachea 15 min y te devolverá la versión vieja** (me pasó y estuve a punto de dar por bueno un arreglo a medias).
- ⚠️ **El gotcha del COALESCE también aplica a NULIFICAR, y ahí engaña más** (T-124): `oposiciones_ssot` resuelve estos campos con `COALESCE(convocatoria, legacy)`. Si pones el valor a NULL **solo en `convocatorias`**, la vista **cae al legacy de `oposiciones`** y la landing sigue mostrando el dato malo — pero **el invariante deja de disparar**, así que parece arreglado. Hay que tocar las dos filas.
- ⚠️ **`inscription_start`/`inscription_deadline` son de tipo `date`**, no timestamp. El driver de Node los renderiza a medianoche local (`2026-05-31T22:00:00.000Z` para el día 1 de junio), lo que aparenta un desfase de 2h y hace fallar un `UPDATE ... = '...'::timestamptz`. Comparar con `DATE '2026-06-01'`.
...
- **Si añades un invariante nuevo a la vista `convocatoria_hito_incidencias`:** (a) vuelca la definición con `pg_get_viewdef(...)` y añade ramas al final — reescribirla de memoria salió mal en I1, I2 e I9 y habría cambiado tres invariantes en silencio; (b) **cablea el nombre en los DOS gemelos del sweep** (`scripts/health-sweep.cjs` + `backend/src/content-health-sweep/…`), porque el sweep reparte por nombre y **descarta en silencio lo que no cae en ningún cubo**; el guardarraíl `content-sweep-parity` te lo exige.
...
# … y si la fuente es una SPA (el fetch plano solo trae el chrome del portal), añade --headless:
cd backend && NODE_TLS_REJECT_UNAUTHORIZED=0 npx tsx scripts/clonar-documento.ts \
  --slug=<slug> --url=<url-spa> --headless --wait-for="main article" --tipo=convocatoria --ref=...
# Investigar una SPA SIN clonar (navegar, encontrar el enlace al PDF real):
aws --profile vence --region eu-west-2 lambda invoke --function-name vence-backend-headless-fetcher \
  --payload "$(printf '{"url":"%s","timeout_ms":45000}' "$URL" | base64)" /tmp/o.json && \
  node -e 'const r=require("/tmp/o.json");console.log(r.html.replace(/<[^>]+>/g," ").slice(0,4000))'
...
- ⚠️ **La cita se copia del CORPUS, no de tu `pdftotext` local.** El clonador usa `pdf-parse` y tú, en la terminal, `pdftotext -layout`: **extraen el mismo PDF distinto**. El 16/07 leí en local `"Libre 273 19 Cuidados Auxiliares"` y el corpus lo guarda como `"Cuidados Auxiliares de Enfermería Libre 273 19"` (pdf-parse respeta mejor el orden de la tabla) → el guardarraíl de cita-literal abortó, con razón. **Clona primero, y saca la cita leyendo `extracted_text`**: si no está literal ahí, tu cita no prueba nada, porque el corpus es lo que quedará.
- ⚠️ **Los sumarios asocian el id al título por POSICIÓN, y te la juegan.** En el BOCYL cogí el `BOCYL-D-13012026-7-4` que aparecía junto al título de la Viceconsejería y era **una resolución de profesorado de la Universidad de León**: el id iba ANTES del encabezado de sección, y el bueno (`7-5`) venía DESPUÉS del título. Clonado, retirado y vuelto a clonar. **Comprueba que el documento habla de tu cuerpo antes de darlo por bueno** — el `esParedDelPortal()` caza captchas, no documentos correctos del cuerpo equivocado.
- ⚠️ **La tarjeta puede vivir en la copia LEGACY de `oposiciones`, no en `convocatorias`.** La vista hace `COALESCE(c.landing_estadisticas, o.landing_estadisticas)`: si la convocatoria la tiene NULL, lo que ve el usuario sale de `oposiciones`. Un script que lee `c.landing_estadisticas` para reescribirla lee **NULL**, hace `(le || [])` y escribe `[]` en las dos tablas → **landing sin tarjetas**. Me pasó el 16/07 en `administrativo-diputacion-jaen` y `administrativo-seguridad-social`, con el gotcha ya escrito en este mismo runbook. **Lee de `oposiciones_ssot`** (que resuelve el fallback), nunca de `convocatorias` a pelo. Guardarraíl nuevo: `tarjeta_vacia`.
- ⚠️ **Si borras una tarjeta, la caché de la web es tu copia de seguridad** — durante un rato. Las dos se recuperaron leyendo `www.vence.es/<slug>` (que aún servía el HTML viejo) y del ciclo archivado. `convocatorias_history` NO te salva aquí: registra `convocatorias`, y la copia legacy vive en `oposiciones`, que no tiene historial.
...
- ⚠️ **La mezcla de ciclos también NACE con la oposición, no solo al pivotar (27/07, APSP CARM).** `agrupacion-profesional-servicios-publicos-carm` se creó (02/07/2026) con **una sola fila** que juntaba el `convocatoria_numero`/`convocatoria_fecha` de la convocatoria **ASP00L24** (Orden 5/03/2025, OEP **2024**, 29 plazas) con las `plazas_*` y el `boe_reference` de la **OEP 2025** (Decreto 233/2025, 50 plazas), y sin `inscription_*`/`exam_date` pese a que ese proceso ya había hecho el examen (17/05/2026). No hubo rollover que lo rompiera: el ciclo viejo **nunca tuvo fila**. Lo destapó un usuario preguntando *"¿la instancia está cerrada?"* — la landing anunciaba "Convocatoria oficial publicada" y él no encontraba ninguna. **Al construir una oposición cuyo cuerpo YA tiene un proceso en marcha, son DOS filas desde el minuto uno:** la del proceso vivo (con sus fechas reales, `is_current=false` si ya está en resultados) y la del ciclo vigente. Si solo vas a meter una, que sea la que de verdad describe el estado actual — nunca una con el número de un ciclo y las plazas de otro.
- ⚠️ **Reconstruir a posteriori el ciclo que falta NO es un rollover: es un INSERT, y el orden importa.** `rollover_convocatoria()` va hacia DELANTE (archiva la vigente y abre una nueva vacía); para colocar el ciclo **anterior** que nunca existió hay que insertarlo a mano con `is_current=false`. Y hay que hacerlo en este orden: **primero el `UPDATE`** que libera el `convocatoria_numero` de la fila vigente, **después el `INSERT`** — el índice `convocatorias_ref_oficial_unica (oposicion_id, convocatoria_numero)` no admite el mismo número en dos filas **ni siquiera de forma transitoria dentro de la misma transacción** (al revés falla con `duplicate key`). Dos cosas más al insertar el ciclo viejo: el histórico de la landing deriva el año que muestra de **`oep_fecha`** (no de la columna `año`), y `convocatoria_fecha = NULL` significa **"aún no convocada"** por contrato explícito de `lib/convocatoria/historico.ts` — dejarla NULL en el ciclo pendiente es lo correcto, no un hueco por rellenar.
- ⚠️ **Un `UPDATE`/`INSERT` suelto sobre `convocatorias` queda firmado como `venceadmin`.** El gotcha de `app.actor` (más abajo) aplica también aquí: `SELECT set_config('app.actor','claude:<tarea>',true)` **en la misma conexión y antes de escribir**, o `convocatorias_history` registrará el cambio sin saber quién lo hizo. Medido otra vez el 27/07 en APSP CARM, con el gotcha ya escrito en este mismo runbook.
- ⚠️ **El BORM (`borm.es`) tiene WAF Radware y te bloquea la IP a mitad de la verificación.** `curl` recibe un 302 a `validate.perfdrive.com`, y **Playwright tampoco pasa** una vez bloqueado (la "Radware Captcha Page" sale igual con navegador real: es bloqueo por IP, no por fingerprint — el mismo patrón que `sede.gva.es` con la Lambda). Lo que SÍ funcionó el 27/07: pedir el PDF por **`WebFetch`**, que sale por otra infraestructura; el binario queda guardado en `tool-results/` y se procesa en local con `pdftotext -layout`. Sirve para cualquier boletín que te bloquee por IP. Y el espejo `empleopublico.carm.es` (sin WAF) da la ficha del proceso con su cronología, que es lo que fija el estado real.
...
- ✅ **Para convocatorias CATALANAS (provinciales y municipales), CIDO es oro.** `cido.diba.cat`
  (Cercador d'Informació i Documentació Oficials de la Diputació de Barcelona) es una base servible en
  plano (sin headless) con la ficha oficial de cada proceso: nº de places, subgrup, desglose de torn
  reservat a discapacitat, y las refs primarias (DOGC/BOE con fecha). Resolvió la contradicción interna
  de `auxiliar-administrativo-diputacion-barcelona` (columna 79 vs referencia 77): CIDO dice **77 places,
  11 reservades a discapacitat**. OJO: las OEP planifican totales que suman procesos de VARIAS entidades
  (Diputació 77 + ORGT 11 + Institut del Teatre 4 = ~92); la convocatòria de cada entidad es su propia
  ficha CIDO, no el total planificado.
- ⚠️ **El headless renderiza HTML, NO extrae PDFs, y muchas fichas SPA son solo METADATOS.** Medido el 18/07 con el BOAM de Madrid (OEP 2025, acuerdo 10017/4464): el fetcher renderizó la ficha de `sede.madrid.es` (83k) y de ahí saqué el enlace al PDF — eso es justo para lo que sirve, NAVEGAR. Pero (1) el PDF da **403 «Access Denied»** a `curl` (bot-block tipo Akamai) y **`net::ERR_ABORTED`** a la Lambda (puppeteer aborta la navegación a un PDF: no lo sabe renderizar); (2) la ficha HTML renderizada NO trae el texto del acuerdo — el «111» que parecía estar era el `applicationID` de un beacon de New Relic, y «Auxiliar Administrativo» era el menú «Lo más visto». Conclusión: si el documento es un PDF protegido, headless te lleva hasta la puerta pero no entra. El 111 quedó corroborado por fuentes secundarias (incl. el listado de la propia sede) pero SIN documento primario clonable → sigue flagueado, honesto.
- ⚠️ **Para SPAs, `--headless` (Lambda `vence-backend-headless-fetcher`) — pero NO es bala de plata.** Medido el 17/07 contra la Lambda real: renderiza bien `euskadi.eus` (42k de texto), `sede.madrid.es` (77k), el BOJA (216k) y `example.com`; pero `sede.gva.es` da **`ERR_CONNECTION_TIMED_OUT`** — bloquea el rango de IPs de AWS, y headless NO cura un bloqueo por IP (solo cura el render en cliente). Dos reglas: (1) pásale `--wait-for=<selector>` del nodo que marca "ya cargó el dato", porque el contenido de una SPA aparece DESPUÉS de `networkidle0` y sin selector te llevas el shell vacío; (2) el resultado pasa por el mismo `esParedDelPortal()` que el plano — un menú renderizado se rechaza igual (la Lambda devolvía `ok:true` con los 4.204 chars de menús de La Moncloa). Y ojo al alcance: la SPA suele ser NAVEGACIÓN, no el documento — `euskadi.eus/ope…` es una landing con enlaces, el dato de plazas vive en el decreto BOPV que enlaza. Úsalo para ENCONTRAR el PDF, luego clónalo plano.
- ⚠️ **Del BOE, clona `txt.php` (o `xml.php`), NO solo el PDF.** El PDF trae el anuncio y nada más; la página `txt.php` trae además la **ficha de análisis**, que es donde el BOE resume el total **en dígitos**: *«Turno libre: Policía del Cuerpo de Policía Municipal **561 plazas**»*, *«Turno libre: Auxiliar Administrativo/a **274 plazas**»*, y el *«Bases de la convocatoria en: BOP … núm. 134, de 12 de julio de 2024»* que te dice dónde está el documento origen. Con solo el PDF, el 561 de `policia-municipal-madrid` no aparecía por ningún lado: el cuerpo del anuncio escribe **en letra** («Ciento trece plazas… Cuatrocientas cuarenta y ocho plazas…») y el total ni se menciona. `xml.php` da lo mismo ya limpio, sin el chrome del portal.
- ⚠️ **Una cifra DERIVADA presentada como hecho es una mentira, aunque la cuenta sea correcta.** `policia-nacional` guardaba `plazas_libres = 2163` y su tarjeta lo enseñaba TECLEADO como «Plazas acceso libre: 2163». El RD 207/2026 no dice 2.163 en ninguna parte: dice **2.704** (art. 2) y que de ellas «se reservan **541** para militares» (art. 3). El 2.163 lo había restado yo — y encima la etiqueta era falsa, porque el RD dice que las 2.704 son de oposición libre. Resultado: 541 plazas menos de las que hay. **Guarda la cifra LITERAL y modela la reserva**, no restes. Si la cifra correcta es de verdad una suma de literales (Extremadura: «23 por el turno de acceso libre» + «103 por el turno de acceso libre» = 126, que el DOE nunca escribe), **decláralo**: `convocatoria_verification` en `verified_correct` con la clave **`cifra_derivada`** en `findings`, explicando la cuenta y citando los sumandos. Es la única válvula que el auditor acepta, y deja rastro firmado.
- ⚠️ **Antes de deducir una URL, mira si YA LA TENEMOS: `convocatoria_hitos.url`.** Di el BOJA por inviable dos días (sumarios, numeración, extraordinarios, hasta la Lambda headless) y la ruta buena llevaba meses guardada en nuestros propios hitos: `/boja/2025/250/c01/5` — una ruta **por capítulo** que ni sabía que existía; la portada `/boja/2025/250/` es la que parece una SPA vacía. Del capítulo sale el href del PDF, **relativo y sin `<base>`** (resuélvelo contra `/boja/AÑO/Nº/cNN/`). Idéntico patrón al BOC-Cantabria, cuyo `idAnuBlob` tampoco es deducible y también salía de los hitos. **Primera consulta, siempre:** `SELECT url FROM convocatoria_hitos WHERE convocatoria_id = … AND url IS NOT NULL`.
- ⚠️ **No escribas en la BD tu hipótesis de por qué no encuentras algo.** Dejé en el `boe_reference` de tres convocatorias del SAS una nota de 600 chars: *"⚠️ FUENTE NO LOCALIZADA … además la referencia es internamente incoherente: dice núm. 250, 30/12/2025 pero el BOJA nº 250 es de mediados de diciembre"*. **Todo falso**: el BOJA 250/2025 es del martes 30/12/2025 y traía el Decreto 211/2025 — lo dice su propia cabecera. Nuestra referencia era correcta desde el principio; el que fallaba era yo. La nota se quedó ahí como si fuera un hecho comprobado, y encima acusaba al dato bueno. Una nota de "no lo encuentro" describe **mi búsqueda**, no la fuente: va en el informe al usuario, no en el campo del dato. Si dudas de un dato, el sitio es `plazas_prevision` + su motivo, o `convocatoria_verification.state`, que para eso están.
- ⚠️ **El documento se identifica a sí mismo: créele a él, no al sumario.** El PDF del BOJA abre con `"Número 250 · Martes, 30 de diciembre de 2025"`. Un `grep` de la cabecera en `extracted_text` habría tumbado mi teoría en 5 segundos. Igual que la LECCIÓN 2 de `resolver-boletin-url.cjs`: la posición en el HTML miente, el contenido no.
- ⚠️ **Un script que consulta con una columna inexistente se CUELGA en silencio** (no revienta). Perdí varios minutos creyendo que `convocatorias` tenía un lock: `select … c.estado …` (la columna es `estado_proceso`) dejaba el proceso colgado hasta que lo mataba `timeout`, sin imprimir el error, porque el `.catch()` no cierra el cliente y el pool mantiene vivo el event loop. Si un script tarda y `pg_stat_activity` está **vacío**, no es la BD: es tu SQL. Cierra siempre con `.catch(e => { console.error(e.message); process.exit(1) })`.
