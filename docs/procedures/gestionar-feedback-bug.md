...
> **Caso de referencia (Lourdes, Sevilla, 07/08/2026) — la causa sale de SU rastro, no de la avería que tengas abierta ese día:** reportó que la app «se queda colgada… cuando termino un test y quiero hacer otro». Se le contestó que era el configurador de artículos ([T-623], abierto ese mismo día) y **replicó**: *«en ningún momento he configurado un test con muchos artículos»*. La evidencia con la que se le respondió eran dos `http_4xx 494` que ocurrieron a las 17:50 y 17:55 — o sea **DESPUÉS** de que ella escribiera a las 17:45; el error del momento que describía (`answerSaveQueue` / `/api/v2/answer-and-save`, [T-315]) llevaba en su rastro desde las 17:05. **Tres reglas:** (1) solo lo ANTERIOR a su mensaje puede explicar lo que cuenta —medido: de 59 feedbacks bug/other en 14 días, **8 tienen rastro SOLO posterior**—; (2) **no le cuentes lo que él hizo** («si deseleccionas lo que traías del anterior…») sin haberlo medido: un diagnóstico incompleto se corrige, una conducta atribuida ofende; (3) «no vemos nada en tu cuenta que lo explique» es una respuesta legítima. **Enforcement:** el dossier (`revisar-feedback.cjs`) imprime ya el bloque **🩺 RASTRO DE ERRORES** separando ANTES/DESPUÉS —antes solo volcaba los CLICS (`user_interactions`) y jamás `observable_events`, así que la regla del punto 1 de arriba dependía de acordarse—. Núcleo `lib/impugnaciones/rastroDeErrores.cjs`, simulación `npm run sim:rastro-errores`. Detalle: `impugnaciones-claude-code.md` §14.1.bis.
>
...
>
> **Caso de referencia (Marta Pérez, aux. admin. Madrid, 05/08/2026) — UN RELLENO NO ES UN HUECO: es un dato que MIENTE, y busca el GEMELO:** escribió *«Algunos test intensivos me aparecen así y luego al pinchar da error»* con una captura de la notificación **«2 Artículos Problemáticos: unknown»**. `test_questions.law_name` guardaba el literal `'unknown'` cuando el cliente no mandaba la ley. **Lo caro no fue el hueco, fue que el sistema lo trató como una ley más:** el agregador agrupa por `law_name`, así que fundió artículos de Excel 365, Word 365 y Access 365 en una tarjeta con una ley inventada; su botón «Ver Teoría» construía `/teoria/unknown` → **404 comprobado en producción**; y su «Test Intensivo» pedía `?articles=190,3&law=unknown` a una página que ignora esos parámetros, así que le sirvió el art. 83 CE donde prometía Excel — **fallando en silencio, sin error**.
> **Tres lecciones operativas:**
> 1. **Cuando encuentres un `|| 'valor'` en un escritor, busca el GEMELO.** El arreglo de junio/2026 se aplicó a `lib/api/test-answers/queries.ts` y **no** a `backend/src/test-answers/test-answers.service.ts`, que tenía el `article_id` dos líneas más arriba sin usarlo. Nada lo cazaba: no hay tipos compartidos entre los dos proyectos y cada uno testeaba su copia. La convención de la casa para esto es **copia + guardarraíl de paridad** (como `benignSignals`), y hay que **validarlo por mutación** — romper cada copia y ver el rojo.
> 2. **Cambiar el relleno por `null` a secas es el mismo fallo con otra cara.** El orden correcto es: RESOLVER (el dato suele estar a mano), y si de verdad no se puede, `null` **acompañado de un evento**. Este defecto vivió seis meses con **cero filas** en `observable_events`: lo destapó una usuaria.
> 3. **Mira las tablas DERIVADAS, y mira si la columna está en una CLAVE.** `user_article_stats` recibe `law_name` por el outbox y lo lleva **dentro de su índice único**, así que el relleno no ensuciaba un campo: **partía en dos las estadísticas por artículo**. Ahí había **más usuarios que en el origen** (493 vs 253), porque quedó lo anterior al backfill de junio. Y no se repara con un `UPDATE`: 9.050 filas ya tenían hermana con la ley buena, así que hay que **fundir sumando contadores**.
> Núcleo `lib/laws/lawNameResuelta.ts`, backfill `scripts/calidad/backfill-law-name-unknown.cjs`, reglas `ley_sin_resolver` (eventos) y `law_name_relleno_escrito` (la TABLA, que ve al escritor que no emite). Ficha [T-559].
>
