...
Al recuperar las 4 que faltaban (CARM 25/06/2026) surgieron dos aprendizajes que aplican a CUALQUIER pregunta, no solo a supuestos:

1. **`is_virtual` es SOLO para contenido sin norma legal** (ofimática Word/Excel, teoría sanitaria: contenedores didácticos). Si la pregunta sale de un **documento OFICIAL publicado** (un Plan aprobado por Resolución, una Orden, algo con referencia BOE/BORM), se crea como **ley LEGISLATIVA real (`is_virtual=false`)** con su `boe_url`/`scope`/`type`, y la pregunta va a `approved` (no `tech_approved`). Caso #100: el "Plan General de PRL de la CARM" tiene apoyo legal (BORM nº174/2010) → ley real, no virtual. **Crear como virtual algo que tiene norma oficial es un error** (lo hice y hubo que corregirlo).

2. **El artículo vinculado debe RESPONDER la pregunta — y eso exige verificar su CONTENIDO contra la fuente oficial, no que "exista un artículo con ese número".** Trampa: un sustituto que da el mismo resultado NO vale (ej.: enlazar el plazo general de 3 meses del art. 21.3 Ley 39/2015 a una devolución de ingresos indebidos de una **tasa**, que es **tributaria** → su plazo lo fija una norma específica, no el supletorio administrativo). Y ATENCIÓN a **imports de leyes mal numeradas**: el DL 1/2004 (Tasas CARM) de nuestra BD tenía en el "art. 6" una **tarifa de barcos**, cuando el art. 6 **real** es la devolución de ingresos indebidos (3 meses + interés del art. 20.3 DL 1/1999). El artículo existía con ese número pero con **contenido equivocado**. Hubo que verificar el texto oficial (BOE/BORM consolidado) y corregir el artículo antes de vincular #98/#99.

**Medida (proceso, lo que SÍ tenemos):** toda pregunta recuperada/importada debe pasar el **test inverso del §3.1** (manual `revisar-preguntas-con-agente.md`): *"¿el supuesto EXACTO que pregunta el enunciado está literalmente en este artículo, según la LEY?"* — eso caza tanto el sustituto como el artículo de contenido equivocado (un agente que lea el art. 6 "barcos" ve que no responde a una devolución → `article_ok=false`). **NO activar saltándose esta verificación**, aunque la respuesta coincida con la plantilla.

> **⚠️ Raíz del error «sanidad/informática = virtual».** El disparador de este fallo es confundir el DOMINIO con la naturaleza. `is_virtual` NO significa «es de sanidad/informática» — significa «no hay norma legal detrás». Una **LEY sanitaria** (Ley de Salud, LOSCAM, Estatuto Marco…) es **legislativa real** (`is_virtual=false`); solo la **teoría/técnica** sin norma (anatomía, procedimientos de enfermería, atajos de Excel) va en virtual. Al construir la guarda de abajo se encontraron **8 leyes mal marcadas** como virtuales, 4 de ellas **leyes sanitarias reales** (Ley 6/2002 Salud Aragón, Ley 12/2001 LOSCAM, Ley 8/2008 Galicia, Ley 3/2009 Murcia), con sus preguntas en `tech_approved` cuando debían ser `approved`. Se reclasificaron a legislativa (25/06/2026).

**Medidas (prevención):**
- ✅ **Test automático (CI):** `__tests__/integration/virtualLawMisuse.test.ts` — falla si alguna ley `is_virtual=true` tiene `boe_url`/`boe_id` (norma real mal clasificada). Caza el error de raíz, sin falsos positivos.
- ✅ **Proceso (test inverso §3.1):** toda pregunta recuperada/importada debe pasar la verificación «el artículo contiene literalmente lo que se pregunta» — caza el sustituto y el contenido equivocado.
- ⏳ **GAP pendiente:** detección de import de ley **mal numerada** (artículo existe con ese número pero contenido erróneo, p.ej. DL 1/2004 art. 6 = barcos en vez de devolución). Requiere comparar contra la fuente oficial consolidada; no implementado.
- ⏳ **Residual conocido:** al reclasificar una ley virtual→legislativa, sus preguntas quedan en `tech_approved` (visibles y correctas, pero la etiqueta canónica sería `approved`). Re-etiquetar es un 2-hop `tech_approved → needs_review → approved` (§15.3 del manual de revisión) con ventana de invisibilidad — hacerlo en pasada dedicada y robusta, no masivamente a ciegas.

Servir 26/30 era correcto pero incompleto; tras recuperar las 4 (con artículo real verificado fact a fact) → **30/30**. Lección final: **avisar del hueco mientras exista** y no prometer "completo".
