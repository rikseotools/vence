...
>
> 🆕 **Temario versionado por convocatoria (desde 25/07):** la FASE 2 crea además **una `temario_version` `active`+default** por la oposición, asigna sus topics (`topics.temario_version_id`) y apunta la convocatoria vigente a ella. Es obligatorio: sin versión los topics rompen el invariante (guardarraíl `__tests__/integration/temarioVersions`). El temario cuelga de una convocatoria; cuando salga una nueva hay que **revisar el temario** contra su fuente (detector `temario_revision_pendiente`, frase *"revisa las revisiones de temario pendientes"*). Diseño: `docs/roadmap/temario-versionado-por-convocatoria.md`.
>
> 🆕 **OEP como entidad (T-108, 25/07):** la FASE 2 rellena `convocatorias.oep_decreto`/`oep_fecha` (texto legacy) — pero la OEP también es una **entidad** (`oep` + puente `convocatoria_oep`, ver `docs/roadmap/oep-entidad-modelo.md`). Hoy el scaffolder **NO** crea la fila `oep`; la puebla el radar al aplicar una señal (F3) o el backfill (`scripts/oep/backfill-oep-entidad.cjs`, que parsea el texto). **Si creas una oposición con una OEP ya conocida**, tras la FASE 2 corre el backfill (o, si la OEP tiene decreto oficial, clónalo con `scripts/oep/clonar-oep-documento.cjs`) para que la entidad y el backlog reflejen la nueva. (Enhancement pendiente: que el scaffolder cree la `oep` directamente.)
> 2.bis **Si la oposición YA existe como aspiracional del catálogo** (fila en `oposiciones` sin temario, que es el caso de la mayoría): añadir **`--completar`**. El scaffolder aborta por defecto si el slug existe, y borrar la fila para recrearla se lleva por delante en cascada **seguidores, notas de convocatoria y checks de seguimiento** (caso real 29/07/2026: Parque Móvil del Estado tenía 3 seguidores, 8 notas y 24 checks). Con `--completar` se ACTUALIZA la fila y se conserva todo, respetando la regla de oro de §0.4 (mismo `id` → los usuarios con ese `target_oposicion` heredan la implementación). `is_active` no se toca: el go-live sigue siendo un acto aparte.
> 3. **A mano:** CcaaFlag (bandera, solo si `audit:oposicion` avisa que no resuelve).
> 3.bis **Gotchas de ejecución (29/07/2026):** (a) contra RDS el scaffolder puede fallar con `SELF_SIGNED_CERT_IN_CHAIN` pese a llevar `rejectUnauthorized:false` — anteponer `NODE_TLS_REJECT_UNAUTHORIZED=0`; (b) **`audit:served` dará 0 preguntas en TODOS los temas** hasta refrescar la materialized view (§6.bis) aunque el scope sea correcto: refrescar ANTES de interpretar ese gate como un fallo de scope.
...
5. **Plazas VISIBLES + fecha de examen (dos olvidos típicos que dejan la landing a medias):**
   - **Plazas en las estadísticas**: las plazas del nuevo ciclo tienen que salir en el **bloque de estadísticas** (`landing_estadisticas`), no solo en `plazas_libres` y los hitos. Si el array de estadísticas no tiene un stat de plazas, **añádelo** ("N Plazas OEP 20XX"). Actualizar solo `plazas_libres` NO las muestra en el hero de stats → el usuario no ve cuántas plazas hay (lo más vendible).
   - **`exam_date`**: si el examen del ciclo viejo ya pasó y el del nuevo aún no tiene fecha firme, **poner `exam_date=null`** — si no, la landing sigue mostrando la fecha **PASADA** como "el examen" (confunde). La previsión del próximo examen va en el hito `upcoming` ("examen 20XX (previsión)"). Si hay fecha nueva confirmada, actualizar `exam_date`.
6. **Revalidar caché** (tag `landing`) + invalidar CloudFront (`AWS_PROFILE=vence … --paths "/<slug>*"`, ver cache-revalidation.md) tras tocar hitos/SEO/oposiciones.
7. **Apartado histórico de convocatorias** (opcional, potente para SEO): la landing tiene un bloque «📊 Histórico de convocatorias» al final que muestra la evolución por años (plazas, año OEP, fechas, plazo conv→examen, inscritos/plaza) con enlace al BOE + JSON-LD `Dataset`. Se **auto-oculta si hay <2 convocatorias**. Para poblarlo (convocatorias anteriores verificadas del BOE + entidad OEP), sigue **`docs/runbooks/historico-convocatorias-landing.md`**. NUNCA inventar datos (regla nuclear).
...
**Caso real (Aux. Admin. del Estado, 11/06/2026) — variante con OEP nueva confirmada:** examen del ciclo OEP 2025 celebrado 23/05/2026 (pasado). Como **la OEP 2026 SÍ está aprobada y publicada** (RD 387/2026, BOE 07/05/2026, **1.450 plazas** Auxiliar C2), se pivotó del todo: `estado_proceso='oep_aprobada'`, `oep_decreto`/`oep_fecha` de la OEP 2026, hitos forward, SEO "OEP 2026: 1.450 plazas". **Olvidos detectados (los del punto 5)**: (a) las estadísticas mostraban temas/preguntas/minutos pero **no las plazas** → hubo que añadir el stat "1.450 Plazas OEP 2026"; (b) `exam_date` seguía en `2026-05-23` (el examen pasado) → se puso a `null` (previsión 2027 en el hito). Anuncio: reactivado con **presupuesto pequeño (1,5€/día)** como lead-capture del nuevo ciclo; se subirá cuando salga la convocatoria 2026.
...
> **⚠️ Conteo de oficiales = FUENTE ÚNICA (07/06/2026).** "Cuántas preguntas oficiales hay de ESTA oposición" se decide en UN solo sitio: `getValidExamPositions(positionType)` → `buildOfficialExamFilter` (filtro SQL, p.ej. filtered-questions/random-test) y `ownOfficialPredicate` (predicado JS para conteos en memoria, p.ej. topic-data). **TODO** conteo de oficiales (el label `officialQuestionsCount` del configurador, el gating de los toggles "oficiales"/"imprescindibles", la estimación y la generación) debe pasar por ahí — **NUNCA** un `filter(q => q.isOfficialExam)` a pelo. Bug real: el label mostró **115** oficiales en Seg. Social T3 ("Tribunal Constitucional") porque `topic-data` contaba cross-oposición (LOTC/CE de OTRAS oposiciones sobre leyes compartidas) en vez de filtrar por `exam_position`. Guards en CI: `__tests__/lib/api/officialCountSingleSource.test.ts` (unit del predicado + estático: falla si `topic-data` vuelve a contar a pelo) y `__tests__/api/topic-data/mvQueries.test.ts` (camino MV). **Recordatorio de producto:** "oficiales" del configurador = SOLO las de la propia oposición (anti-contaminación, casos Laura/Sergio); las de otras oposiciones con temario similar van por el opt-in `includeSharedOfficials`, nunca mezcladas.
