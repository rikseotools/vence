...
- 🚦 **NO LE DIGAS QUE ESTÁ ARREGLADO SI NO ESTÁ EN PRODUCCIÓN (Manuel, 07/08/2026) — y el cierre lo IMPIDE.** *«No vuelvas a decir que está arreglado sin estar en producción y probado y simulado.»* **Mergeado a `main` no es arreglado:** los deploys se agrupan y pueden tardar horas. Caso que la fija: a Esther (feedback `e523eabc`) se le envió *«ya está corregido… no debería volver a pasarte»* con el arreglo en `main` y sin desplegar (`/api/health` servía `76404f1d`); si entraba, le seguía fallando, y con un correo nuestro diciendo lo contrario. **Las tres comprobaciones antes de escribir «arreglado»:** (1) ¿está VIVO? — `curl -s https://www.vence.es/api/health` y `git merge-base --is-ancestor <commit> <sha-vivo>`; (2) ¿está PROBADO sobre lo que sirve producción (HTML/endpoint/navegador), no sobre la BD ni el fuente?; (3) ¿la señal que miras dice lo que crees? — una caída de errores puede ser el arreglo **o menos tráfico** (ese fue el error: se leyó como arreglo una bajada de 401 que era la hora valle). **Mientras no se cumplan las tres, la frase correcta es en futuro:** *«lo tenemos corregido y estará disponible en las próximas horas»* — que la puerta NO marca. **Enforcement:** `cerrar.ts` y `cerrar-feedback.ts` abortan si el mensaje afirma un arreglo y hay commits que citan el caso, tocan superficie servida y no están vivos (núcleo `lib/impugnaciones/promesaDeArreglo.cjs`, puerta `lib/puerta-vivo.ts`, reutilizando el verificador de [T-392]). Escape propio y contado: `--vivo-igualmente "<cómo lo comprobaste>"`. Calibrada, no adivinada: `npm run sim:promesa-arreglo`.
