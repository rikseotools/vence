...
> 🧵 **El tell de FORMA también existe a nivel de LOTE, no solo de pregunta ([T-150], 26/07/2026).** Este check de arriba mira UNA pregunta: si ninguna cruza el margen del 30%, el gate la deja pasar. Pero un lote entero puede tener la clave como la más larga en la MAYORÍA de sus preguntas, cada una por debajo de ese margen — y "elegir la más larga" sigue acertando muy por encima del azar. Medido en `gen_atc_t208_2026-07-26_s26c` (auditor-opositor + re-check): **13/16**, y tras reparar 3 preguntas sueltas seguía en **11/16** — reparar una a una no toca el sesgo del lote. Validado después con un lote limpio a propósito (**7/14**, con margen real solo en 1) y con un lote posterior (**7/10 = 70%**, el nivel que se documenta como explotable) frente a un caso de alarma falsa (**6/14 = 43%**, con cinco márgenes de +3%/+5%/+8%/+11%/+15% que nadie puede explotar — contar "es la más larga" SIN ponderar el margen habría sonado a sesgo grande sin serlo).
>
> **Segundo patrón, más fino, en el mismo sitio: los ABSOLUTOS delatores viven en los distractores.** Marcadores como «únicamente», «exclusivamente», «solo», «siempre», «en exclusiva», «libremente» que aparecen SOLO en las opciones falsas y nunca en la clave delatan la clave por ser la única opción "matizada" — sin necesidad de leer el artículo. Medido en el mismo lote: absolutos en 10/16 preguntas, **9 de esas 10 solo en distractores**. La única clave con un absoluto lo tenía porque la LEY lo dice («en ningún caso», art. 136.3 LGT) — si el absoluto es del legislador, ponlo también en la clave; si es adorno del redactor, repártelo.
>
> **Mecanizado en `analizarLote` (`lib/generacion/simularBatch.js`, corre en el Paso 2/5.bis junto a la distribución de §2.2-ter): AVISA, no bloquea** — son heurísticas de patrón sobre texto libre, como `analizarOverclaim`, y conviene leerlas con criterio antes de reescribir nada. Umbral de ambas: >60% del lote (o de las preguntas con marcador, mínimo 4). El de longitud reutiliza el margen de 30% de `tellLongitud` (`margenLarga`/`MARGEN_EXPLOTABLE`) para reportar cuántas de las "más largas" son además explotables aisladamente, sin que eso cambie el disparo (que va sobre el recuento en bruto — el umbral del 60% ya deja fuera los casos de alarma falsa, como el 43% de arriba). El de absolutos vive en `lib/generacion/absolutosOpcion.js`. **Al reparar: no toques una pregunta suelta — reparte el registro (longitud) y los marcadores (absolutos) entre clave y distractores en TODO el lote.**

> 🧩 **Anclar la clave al literal empuja el lote al formato CLOZE — un problema de VALIDEZ, no de forma ([T-153], 26/07/2026).** La literalidad de §2.2 es lo que ancla la pregunta a la norma, pero la forma más fácil de cumplirla es escribir el enunciado con el arranque del precepto y dejar que la clave lo COMPLETE — el ítem se convierte en relleno de huecos: se acierta reconociendo la letra siguiente del texto memorizado, no comprendiendo la institución. Medido en `gen_atc_t204_2026-07-26_s26c` (14 preguntas, LGT): dos auditorías ciegas lo contaron por su cuenta, sin ponerse de acuerdo — **7/14** el auditor estricto, **12/14** el auditor-opositor. **NO se arregla aflojando la literalidad** (la clave deja de ser verificable contra el BOE): se arregla construyendo los DISTRACTORES con reglas reales de apartados o instituciones hermanas (retenedor/obligado a pagos fraccionados/sustituto en vez de cortar el mismo párrafo) y dejando que el enunciado PREGUNTE en vez de empezar una frase.
>
> **Hermano en el mismo lote: el PAR ESPEJO.** Cuando el precepto ofrece una regla y su inversa (*«importe a ingresar superior … o cantidad a devolver inferior»*), sale natural poner como distractor la inversión exacta de la clave — y quien detecta la simetría resuelve el ítem a 50/50 sin abrir la norma. Localizado en 3 preguntas del mismo lote (arts. 122.2, 37.1 y parcialmente 120.3).
>
> **Mecanizados los dos, uno por-pregunta y otro por-lote — AVISAN, no bloquean.** El ESPEJO vive en `analizarPregunta` (`lib/generacion/espejoOpciones.js`): compara la clave contra cada distractor por DIFF de palabras (recorta el prefijo/sufijo común y mira qué queda en medio), no por `replace()` de texto plano — la negación española antepone el «no» al clítico (*«no SE interrumpe»*, no *«se NO interrumpe»*), así que un `replace()` ingenuo no la casa. El CLOZE vive en `analizarLote` (`lib/generacion/clozeArticulo.js`), a nivel de LOTE como el de longitud de T-150 (umbral >50%, porque una pregunta cloze aislada no es necesariamente un defecto — el patrón sistémico sí): compara la cola del enunciado contra el artículo y, si coincide, comprueba si la clave es la continuación verbatim. **Necesita el texto del artículo por pregunta** — `analizarLote(preguntas, articulos)`, el segundo argumento es opcional y sin él este check concreto simplemente no se pronuncia (degrada, no revienta).
...
### 5.40 Batch LO 3/2018 arts. 26, 53 bis, 61 y 62 — el lote siguiente es quien prueba tu arreglo (2026-07-31)

- **batch**: `gen_lopdgdd_t115_2026-07-31` — **13 preguntas**, LO 3/2018 (Protección de Datos). Elegido con `huerfanos:plan` ([T-115]). Cadena limpia: `batch:boe` **4/4 idénticos al BOE vigente** · simulación 1 bloqueante → 0 · gate 13/13 · auditoría ciega 13/13 PERFECT · Paso 9 13/13 PERFECT · `batch:servido` **10/10** en producción.
- **Sirve a 43 temas de 43 oposiciones distintas** — el alcance más alto de la campaña. Y el badge solo contaba **3 temas apagados** (353 → 350). **La métrica del hallazgo y el beneficio real divergen cuando la ley es transversal**: el detector exige ≥4 huecos por tema, así que una ley presente en 43 temarios con 1-2 huecos en cada uno casi no mueve el contador aunque repare 43 sitios. Mirar solo el badge para priorizar infravalora justo las leyes que más gente estudia.
- 🔴 **EL FALLO MÁS CARO LO INTRODUJO EL LOTE ANTERIOR, Y LO CAZÓ ESTE — la excepción a un guardarraíl necesita su propia condición de aplicación.** Ese mismo día, §5.39 hizo pasar «del Reglamento» como cita al MISMO cuerpo, para que el anexo del auditor no perdiera el art. 41 del RD 203/2021. La excepción se aplicaba **siempre**. Aquí la norma del lote es una **Ley Orgánica** cuyas explicaciones citan a cada paso «el artículo 60 del Reglamento» refiriéndose al **Reglamento (UE) 2016/679** → se adjuntaron los arts. **56, 60 y 65 de la LEY ORGÁNICA** («Acción exterior», «Admisión a trámite de las reclamaciones») como si fueran los citados. Es exactamente el artículo HOMÓNIMO que la guarda existe para impedir, y **adjuntar el equivocado es peor que no adjuntar ninguno**. Arreglado: la excepción exige ahora que la norma del lote sea **ella misma un reglamento**, derivado de su nombre oficial (`esLeyReglamento`). Verificado en los dos sentidos sobre lotes reales (RD 203/2021 sigue en 8 adjuntos, LO 3/2018 baja de 4 a 1) + 3 tests.
  - **Lo transversal:** un arreglo hecho con UN caso delante se calibra con ese caso. El lote siguiente es el primer contraejemplo disponible y **hay que correrlo antes de dar el arreglo por bueno** — aquí tardó menos de una hora en aparecer.
- ⚖️ **§2.2-quater PELEA CONTRA EL DEDUP, y hay que saberlo antes de adjudicar avisos.** Desarrollar el nombre completo de la ley en cada enunciado mete aquí **118 caracteres idénticos** en los 13, así que los 5 avisos de duplicado intra-lote daban **0,46-0,62** de Jaccard de enunciado con **0,00-0,14** de Jaccard de la CLAVE (el nivel 3 de §2.6, que es el que dice si dos preguntas evalúan lo mismo). Los cinco eran falsos positivos. Ahora el aviso del simulador **lleva los dos números y su veredicto**: `enunciado 0.73; CLAVE 0.00 → probable falso positivo`. No se sube el umbral (escondería duplicados reales) ni se descuenta el nombre de la ley (habría que adivinar qué parte del enunciado lo es).
- **NO elidas dentro del blockquote.** Dos citas usaban `[…]` para saltarse un inciso largo y el gate las rechazó: el marcador parte la comprobación de literalidad por la mitad. Se sustituyeron por la cita **íntegra** del precepto, que además es mejor material de estudio.
