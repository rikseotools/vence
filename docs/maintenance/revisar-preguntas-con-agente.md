...
7. **Devuelve el objeto de ENTRADA con la salida anidada dentro.** Al pedirle una reparación, un agente escribió el fichero de entrada completo (enunciado, opciones, metadatos) con la estructura nueva dentro de `explanation_data`, en vez de la estructura sola. El aplicador lo rechazó con «hacen falta 4 razones, recibidas (ninguna)», que **no** se parece a la causa real. **Mitigación:** al leer una salida de agente, aceptar las dos formas (`d.explanation_data?.options ? d.explanation_data : d`) y decirlo en el prompt con un ejemplo del objeto exacto que se espera.
