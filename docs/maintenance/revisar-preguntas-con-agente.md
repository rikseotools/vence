...
> **Este paso no es opcional y hay cifra que lo respalda (30/07/2026, T-291 escalón 2).** Sobre 269 explicaciones que habían pasado TODOS los gates deterministas —estructura completa, ninguna letra clavada, cita literal comprobada con el criterio único— la re-verificación encontró **8 defectos (3,0 %)**, y dos eran **afirmaciones falsas dentro de la razón de un distractor** (describir Ctrl+N como «crea una presentación nueva» cuando es negrita; situar un comando en la pestaña equivocada de la cinta). Ningún gate programático puede verlas: comprueban la FORMA, no la verdad de lo escrito. Sin esta pasada se habrían quedado servidas a los opositores. Corolario para el arnés: los gates deciden qué se puede aplicar; **solo un agente independiente leyendo la pregunta viva decide si lo aplicado es cierto**.
...
5. **No transcribir a mano** la salida del agente: el agente escribe el fichero con la herramienta Write y el bucle principal lo lee. *(Actualización 30/07/2026: **SendMessage SÍ está disponible** — se puede reabrir un agente ya terminado con su contexto intacto y pedirle que complete o corrija su propia salida. Es mucho más barato que relanzar el lote: en la campaña de T-291 así se recuperaron 23 ficheros que faltaban y se repararon 2 explicaciones defectuosas, cada una en un par de minutos.)*

#### Dos modos de fallo MÁS, medidos el 30/07/2026 sobre 20 agentes × 25 preguntas (T-291 escalón 2)

6. **Declara el veredicto y NO escribe el fichero.** Dos de los 20 agentes marcaron 14 y 19 preguntas como limpias y solo escribieron 6 y 4 ficheros de explicación — el resumen final decía «hecho» y la mitad del trabajo no existía. **Mitigación:** cruzar SIEMPRE veredictos contra ficheros (`ok` sin fichero = trabajo perdido) y pedir al agente en el prompt que lo compruebe él antes de terminar. Con SendMessage se completa sin repetir el lote.
7. **Un agente REABIERTO puede reprocesar un lote ajeno y sobrescribir su salida ya aplicada.** Al pedirle por SendMessage que completara unos ficheros que le faltaban, el agente del lote-01 siguió por su cuenta con el lote-02 —que ya había entregado OTRO agente y cuyas explicaciones estaban aplicadas y re-verificadas— y reescribió sus 25 ficheros con una versión propia sin validar. El resultado peligroso no es la BD (lo aplicado sigue aplicado) sino **el disco: dejó de reflejar lo vivo**, así que una sesión futura que «re-aplicara el lote» habría metido en producción una versión que no pasó ningún gate. **Mitigaciones:** (a) al reabrir un agente, decirle explícitamente que NO coja más lotes —él no sabe qué han hecho los demás—; (b) antes de re-aplicar cualquier cosa, comparar disco contra BD (`explanation_data`) y, si divergen, **realinear desde la BD**, que es lo que el opositor ve y lo único que pasó los controles. Detectado y corregido el 30/07/2026.
8. **Un agente al que el sistema reactiva puede tomar las notificaciones por permiso del usuario — y escribir en producción.** El 30/07/2026, tras pedirle que parara, un agente se reactivó varias veces solo, arrancó por su cuenta una tanda nueva (75 preguntas en 3 lotes) y acabó ejecutando `aplicar-explicacion.ts --apply` **sobre la base de datos viva**, diciendo «me lo confirmaste explícitamente». Nadie se lo confirmó: había interpretado como confirmación las notificaciones automáticas del ciclo. **Qué hacer si pasa:** (a) comprobar en BD qué se escribió de verdad —`explanation_data`, `updated_at` y los eventos `explicacion_estructurada_aplicada`— antes de creerse ninguna versión; (b) **no revertir por reflejo**: si el material pasa el validador y el dry-run, revertir devuelve al opositor una explicación peor; (c) **cerrarle los controles que se saltó**, que es lo que faltaba: registro en `ai_verification_results`, invalidación de caché y la re-verificación posterior. En este caso la re-verificación encontró **2 defectos reales en 18** (11 %, frente al 2,5-3 % de las tandas conducidas), lo que da la medida de lo que aporta el paso que se saltó.
9. **Una pregunta marcada como DEFECTUOSA vuelve a la cola de la siguiente tanda si el filtro solo mira `explanation_data IS NULL`.** Es una consecuencia lógica que muerde: a una `defecto_opciones` no se le escribe explicación, así que sigue sin `explanation_data` y el extractor la vuelve a seleccionar. Pasó con `15b81b24` (Excel, la opción correcta no figura entre las cuatro): diagnosticada en una tanda, con explicación escrita en la siguiente, y cazada de nuevo por la re-verificación. **El extractor tiene que excluir lo que ya tenga un `ai_verification_results` con `answer_ok`/`options_ok`/`article_ok` en FALSE** (arreglado en `extraer-tanda2.cjs`).
8. **Devuelve el objeto de ENTRADA con la salida anidada dentro.** Al pedirle una reparación, un agente escribió el fichero de entrada completo (enunciado, opciones, metadatos) con la estructura nueva dentro de `explanation_data`, en vez de la estructura sola. El aplicador lo rechazó con «hacen falta 4 razones, recibidas (ninguna)», que **no** se parece a la causa real. **Mitigación:** al leer una salida de agente, aceptar las dos formas (`d.explanation_data?.options ? d.explanation_data : d`) y decirlo en el prompt con un ejemplo del objeto exacto que se espera.
