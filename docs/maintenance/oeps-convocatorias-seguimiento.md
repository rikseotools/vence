...
3. **TODOS los grupos al radar, sin guardarraíl de grupo excluyente** (Fase 0 "catalogar TODO", 04/07/2026). Misión = la BD de oposiciones más grande y fresca de España → se catalogan y vigilan **A1, A2, B, C1, C2, AP y E**, de cualquier administración/cuerpo/ministerio (IIPP, Justicia, Hacienda, universidades, locales… no solo "administrativo" ni solo C1/C2). **El grupo solo se REGISTRA para priorizar/triar, NUNCA para descartar** (`detect-boletines.service.ts:~93`; `oposiciones.position_group`). Y **no filtrar `is_active`** — las catalogadas también se vigilan (§10). El descubridor de boletines no debe sesgar el regex a cuerpos administrativos (§10 detect-boletines).
   - **Histórico (cómo se llegó aquí):** hasta el 02/07 había un guardarraíl que **excluía A1/A2/B** (`['C1','C2','C']`, luego `['C1','C2','C','AP','E']` al añadir las Agrupaciones Profesionales — se destapó porque una usuaria pidió la *Agrupación Profesional de Servicios Públicos* CARM y el sistema decía "no existe"). El **04/07 (Fase 0) se eliminó el guardarraíl entero**: ya no se descarta A1/A2/B tampoco. PAG escanea `idGrupo` 1-6 (todos), el prompt LLM incluye todos los grupos. La función `looksLikeC1C2Convocatoria` conserva el nombre por legacy pero ya **no** filtra por grupo (solo `INGRESO_RE && !NOISE_RE`).
...
5. **UNA POR UNA, sin prisa — no se pierde información por ir rápido (norma Manuel, 04/07/2026).** Cada señal `pending` se trabaja de forma **individual y completa**: leer su `raw_extraction`/summary **campo a campo**, buscar la fila existente por slug/nombre, verificar contra el boletín oficial cuando aporte dato, y escribir en BD lo que corresponda **antes** de pasar a la siguiente. **PROHIBIDO cerrar señales en bloque para "vaciar el badge"** o dar una tanda por vista sin haber mirado cada una: se pierde dato de forma irreversible (una convocatoria, unas plazas o una `boe_reference` que no se catalogó no vuelve). El objetivo —la BD de oposiciones más completa y fresca de España— **manda sobre la velocidad**; la prisa nunca justifica saltarse una señal. Única excepción de cierre en bloque: el ruido `hash_change` de confianza <40 cosmético/re-baseline (§6), y solo tras confirmar que es ruido.
...
| Sensor (`sensor_type`) | Capa | Qué detecta | Score base | Cron |
|--------|------|-------------|------------|------|
| `pag_empleo` | **2 — agregador** | 🆕 Convocatorias del **agregador nacional** `administracion.gob.es` (PAG). Parsea el `jsonDetalle` estructurado (año, plazas, boletín, fechas, estado) — **sin LLM**. `idGrupo` 1-6 (A1/A2/B/C1/C2/AP). Anota un **veredicto de frescura** de la inscripción en cada match. Cubre lo que no publica en boletín o que aún no leemos. | 50 | L-V 06:00 UTC |
| `competitor` | **3 — competidor** | 🆕 Convocatorias de **competidores** (`oposiciones.es`, …) — red de seguridad **y** detector de gaps de las capas oficiales. `RawCandidate.preExtracted` (nombre + plazas del título, salta LLM). Corre por el **orquestador** `backend/src/radar/` (ver abajo). | 50 | Diario 07:00 UTC |
| `regional_scan` / `boe_api` (vía **`detect-boletines`**) | 1 — boletín | Convocatorias de ingreso NUEVAS leídas de los **sumarios de boletines oficiales** (BOCYL HTML por fecha + BOE API JSON). Pre-filtro regex + filtro LLM (`extractRegionalOeps`). Ver §10 | 55 / 20 | L-V 06:30 UTC |
| `llm_semantic` | catálogo | Entidades OEP extraídas con Claude Haiku (año, plazas, BOC ref, fechas, estado) sobre `oposiciones.seguimiento_url` del catálogo | 40 | L-V 10:00 UTC |
| `generic_source` | catálogo | Cambios en fuentes genéricas/portales fuera de catálogo (`generic_source_checks`) — hash + filtro LLM. Aquí se dan de alta URLs de cuerpos NUEVOS (ver §10) | 50 | Diario |
| `timeline_silence` | catálogo | Hitos `current` con fecha pasada +3 días sin avance | 70 | Diario 7:00 UTC |
| `hash_change` | catálogo | SHA-256 del contenido de la página cambió (sensor antiguo, integrado como complemento) | 30 | L-V 9:00 UTC |
...
### 🆕 Reforma: radar multi-capa (orquestador + adapters), desde 04/07/2026

El sistema de **detección** se reescribió como un **radar multi-capa en cascada** (`backend/src/radar/`), aunque el **flujo de revisión de este manual NO cambia** — todas las señales siguen aterrizando en `oep_detection_signals`, se revisan igual en `/admin/oep-signals` y se cierran con `Aplicar`/`Descartar` (§6). Lo que cambió es **cómo se generan**:

- **3 capas en cascada** (fuente de verdad → red de seguridad): **Capa 1** boletines oficiales (`detect-boletines`: BOE+BOCYL+BON…) → **Capa 2** agregador nacional PAG (`pag_empleo`) → **Capa 3** competidores (`competitor`: oposiciones.es…). La Capa 3 **audita** a las oficiales: si un competidor ve algo que las capas 1/2 no vieron → **gap-alert** (falta un adapter de boletín).
- **Un fichero por FUENTE** (`layers/boletines/*.ts`, `layers/aggregators/pag-empleo.ts`, `layers/competitors/oposiciones-es.ts`); todo lo inteligente (filtro/LLM/match/dedup/persist) vive una vez en `core/`. Dedup definitivo por **ref oficial**, no por URL.
- **Catalogar TODO** (Fase 0): `ALLOWED_GROUPS` eliminado → se catalogan y vigilan **todos** los grupos (A1/A2/B/C1/C2/AP), no solo administrativos. Columna `oposiciones.position_group`.
- **Transición en curso:** mientras convivan los crons legacy y el orquestador, boletines y PAG **NO** se registran en el orquestador (evita señales duplicadas por dedupe keys distintos). Hoy el orquestador solo aporta la **Capa 3 (competidores)**, capability nueva sin solape. Retirar los crons legacy + activar `LEGACY_BOLETINES_WRAPPED`/`PAG_WRAPPED` es tarea pendiente (ver roadmap).
- **Panel de salud NUEVO** `/admin/radar-salud` (API `/api/admin/radar-salud`): semáforo por adapter/capa, señales de 7d, gaps y adapters degradados (`radar_adapter_runs`). Es **distinto** de `/admin/oep-signals` (que es la cola de revisión). Consultarlo para saber qué fuente funciona/falla.
- **Columnas nuevas** en `oep_detection_signals` útiles al triar: `detected_oposicion_name` (nombre crudo cuando `is_novel`/sin fila), `region_name`, `position_category` (C1/C2/AP…), `source_id`.
- **Roadmap + estado de fases:** `docs/roadmap/radar-multicapa.md` (Fases 0/1/2/2b ✅; 1b boletines CCAA nuevos y retiro de crons legacy ⬜).
...
2. **`oep_detection_signals` tiene RLS (solo admin).** Consultarla con la **clave ANON devuelve 0 filas en silencio** — NO es que no haya señales, es que RLS las oculta. Para contar bien: **`DATABASE_URL` (pg directo)** o `SUPABASE_SERVICE_ROLE_KEY`. Y **OJO**: PostgREST/supabase-js **corta a 1000 filas** por defecto, así que `.select().length` infra-cuenta — usar `count: 'exact', head: true` o `SELECT count(*)` en SQL.
   > **⚠️ BD viva = AWS RDS (desde 04/07/2026), Supabase CONGELADO.** Los ejemplos `supabase.from(...)` de este manual siguen la sintaxis, pero la ANON key de `.env.local` apunta a **Supabase congelado** (datos viejos). Para la BD real conectar por **`DATABASE_URL`** (`.env.local` ya apunta a `vence-prod...rds.amazonaws.com`, requiere `ssl:{rejectUnauthorized:false}`). Ver `CLAUDE.md` / memoria `project_cutover_rds_prod`.
...
**Estado de la detección automática (honesto, actualizado 16/06/2026):** el cron `check-seguimiento` **YA NO está desactivado** — se **migró a Fargate** (`backend/src/check-seguimiento/`, cron NestJS) y corre a diario (~09:00 UTC; último check verificado 16/06/2026). El `.github/workflows/check-seguimiento.yml.DISABLED` es el GH Actions viejo, residual. El sensor `hash_change` es **coarse**: detecta que la página cambió (SHA-256 del texto normalizado; `seguimiento-fetch.ts:normalizeForHash` quita relojes/fechas/tokens — anti-ruido desde commit `fe3783a9`, 16/06), NO clasifica "OEP publicada". El sensor `llm_semantic` (tabla arquitectura arriba, Claude Haiku) sí extrae entidades OEP, pero el **rollover sigue siendo manual** (lo dispara una persona al ver la señal o la noticia). **Pendiente (automatización):** clasificador que, sobre la señal, distinga "OEP publicada en el boletín" → propuesta de rollover con la cifra ya extraída del decreto.

> **Nota de re-base (16/06/2026):** al desplegar el anti-ruido de `normalizeForHash` se ejecutó `UPDATE oposiciones SET seguimiento_last_hash = NULL` (re-base intencionado, ver mensaje del commit `fe3783a9`). Tras un re-base, todos los hashes quedan NULL temporalmente y la detección de cambios se restablece en ~2 runs del cron (1º re-baseline, 2º vuelve a comparar). No confundir con un fallo del cron.

> **Email de hitos a usuarios ("8d") NO existe (16/06/2026).** Verificar un hito (`notify_status='verified'`) dispara **solo la campana 🔔** (Fase 8c, `tg_hito_fanout_alerts`). El email automático por hito (8d) está pendiente de construir. El único email de novedades a usuarios es el **newsletter manual** (§7b.1). ⚠️ **En soporte NO prometer "te llegará por correo automáticamente"** hasta que 8d exista — la campana sí, el email no.
...
> 🆕 **Sprint G (06/07/2026):** el **SSOT del proceso es la tabla `convocatorias`** (la vigente, `is_current`), leído por la vista `oposiciones_ssot`. Durante la transición se hace **dual-write**: al tocar plazas/fechas/estado, actualiza **AMBAS** (`oposiciones.*` legacy — que aún leen advance-estado/auditores — y la convocatoria vigente de `convocatorias`). Los rebuilds `_*_fase23` y el manual `crear-nueva-oposicion` §2c ya lo hacen. Detalle: `docs/roadmap/consolidacion-convocatorias-radar-ssot.md`.
...
>   - `npm run audit:oep-review` → **db-only** (rápido, CI): `pending=0`, `discovered_processes` triados, ninguna `applied` huérfana, catalogadas con **administración normalizada** (no la etiqueta cruda del sensor "(DOGV)/(BOCYL)…") y `estado_proceso` válido. Los 🟡 informan del backlog de catalogadas sin `seguimiento_url`, **separando** el **backlog real** (organismo concreto → buscar portal server-rendered) de las **genéricas** (categoría sin entidad: "Autonómica (Educación)", "Universidad (PAS)"… → NO pueden tener URL única, no son backlog). No bloquean.
