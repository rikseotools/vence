...
- **Follow-ups menores:** 11 preguntas en `needs_human` (cola de reparación, invisibles a usuarios — arts editoriales que inventaban); re-registrar `verify:scope` de los temas con scope cambiado (stale por trigger, cosmético); mismo bug scope vacío PEND en `ingesa` y `scs_canarias` (office-split).
