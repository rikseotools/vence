...
- **Qué:** tres bolsas de preguntas ACTIVAS que el sistema de revisión dio por buenas y que los usuarios están cazando por nosotros. Las tres salieron de impugnaciones del 16/07, no del pipeline:
  1. **1.415 activas con verificación ÚNICA de `gpt-4o-mini`, todo en verde y `options_ok` sin evaluar.** Caso que lo destapa (`42d0b501`, RD 203/2021, impugnación de Sandra): el enunciado se inventaba la "Agencia Estatal de Administración Digital" (el RD dice **Secretaría General de Administración Digital**, 10 veces; la Agencia no aparece ninguna) y **el verificador repitió el órgano inventado en su propia justificación** con confianza "alta". No verificó contra la ley: verificó contra la pregunta. Cubos hermanos: **4.528** activas selladas solo por **Haiku** (el manual dice "Haiku falla") y **1.474** con `ai_model` nulo.
  2. **89 explicaciones CRUZADAS confirmadas** (de 442 candidatas triadas por 8 agentes): analizan opciones de otra pregunta. Caso origen `def2efab` (Defensor del Pueblo, impugnación de Manuel Querino): 4 verificaciones Sonnet seguidas la dieron `explanation_ok=true`. Lista con evidencia: `/tmp/claude-1000/cross/CRUZADAS.json` (regenerable). Ojo: 89 es SUELO, no techo — el detector solo pesca solape léxico ~0. Una de ellas (`2ed283e1`, Ley 41/2002) tiene las definiciones ROTADAS una posición, que roza la clave.
...
- **Estado:** medido y documentado, sin drenar. Prioridad alta: son preguntas VIVAS enseñando datos falsos (el caso de Sandra llevaba activo desde el 22/03).
