...
### 🔴 [CONTENIDO — sistémico] Auditar atajos de Word español vs inglés en el artículo "Word 365 art 5" (149 preguntas)
- **Qué:** el artículo `articles.id=0f00e03f-63d7-4f57-827a-8adbcb4d0a58` ("Atajos de teclado en Word 365", **149 preguntas activas** colgando) tiene una **doctrina mezclada/incoherente**: su preámbulo y varias tablas afirman que "Word 365 usa mnemónicos ingleses oficiales" (Ctrl+I=cursiva, Ctrl+K=hipervínculo) mientras otras filas usan los españoles (Ctrl+N=negrita), y su §18 se contradice ("el hipervínculo es Ctrl+Alt+K"). **En el Word ESPAÑOL que se examina en las oposiciones**: Ctrl+N=negrita, **Ctrl+K=cursiva**, Ctrl+S=subrayado, **Ctrl+Alt+K=insertar hipervínculo**. La doc traducida de Microsoft (que lista Ctrl+K=hipervínculo) es el origen de la confusión.
- **Por qué:** detectado 15/07 por impugnación de Esther Ramos (Aux. Admvo. Madrid): la pregunta del hipervínculo tenía clave D) Ctrl+K (convención inglesa) → **mal para Word español**; se corrigió a A) Ctrl+Alt+K (otra sesión). El mismo defecto de convención puede afectar a más de las 149 preguntas (negrita, cursiva, subrayado, buscar, guardar…). **Es contenido que enseña el atajo equivocado al opositor.**
- **Cómo:** decidir la doctrina (para oposiciones españolas → **convención española del Word instalado**, no la doc traducida MS) y reescribir el artículo coherente con ella; luego auditar las 149 preguntas contra la clave correcta (workflow read-only §7.7) y corregir las que tengan la convención inglesa. Reconciliar con `project_office_web_escritorio_split` y el manual de impugnaciones §5.1.1-5.1.3. **NUNCA fiarse solo de la doc traducida de MS; verificar contra el comportamiento real del Word español.**
- **Estado:** ABIERTA. La pregunta de Esther ya arreglada; el barrido de las 149 y la doctrina del artículo, PENDIENTE.
