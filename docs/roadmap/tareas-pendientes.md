...
- **Cómo:** §8.1-ter (check de correspondencia explicación↔opciones) redactado y PENDIENTE de meter en `revisar-preguntas-con-agente.md` (bloqueado: otra sesión tenía el fichero sin commitear el 16/07). Aviso ya desplegado en `scripts/impugnaciones/validar-explicacion.cjs` (commit `9c519232`, no bloquea: ~20% de precisión). Falta: añadir a la matriz un `enunciado_ok` (¿el enunciado cita la norma/órgano correctos? ¿no filtra la respuesta?) y re-verificar los 3 cubos con Sonnet, no con el triaje barato ("modelo gratis = red de triaje; Claude = el juicio").
- **Estado:** cubos 2 (16/07) y 3 (17/07) CERRADOS. Queda el cubo 1 (7.158), y NO se drena sin añadir antes `enunciado_ok` a la matriz: re-verificar con la misma matriz que las selló en verde repite el resultado.
...
### 🔴 [URGENTE — premium roto] Los 30 vídeos de los cursos NO se pueden ver desde móvil (MP4 sin faststart)
...
- **Estado:** diagnosticado y medido (16/07), sin arreglar. Ningún vídeo salvado.

### 🔴 [COMPROMETIDA CON FECHA — semana del 20/07] Construir Administrativo (C1) de la DGA — Gobierno de Aragón
- **Qué:** montar la oposición **Administrativo, Cuerpo Ejecutivo, Escala General Administrativa** de la Comunidad Autónoma de Aragón (C1). Hoy NO existe: del catálogo de Aragón solo está vivo `auxiliar-administrativo-aragon` (C2, 20 temas); lo que aparece como "administrativo…Aragón" es del **Servicio Aragonés de Salud**, otro cuerpo.
- **⚠️ PROMESA HECHA A UNA USUARIA:** a Cristina Hojda se le respondió el 16/07 (feedback c9e50185, decisión de Manuel) que **"estamos elaborando el Administrativo de la DGA y calculamos que la próxima semana estará listo"**. Hay fecha comprometida y **no hay ningún automatismo que la avise** cuando esté: al publicarla, escribirle.
- **Por qué (viabilidad medida 16/07):** proceso selectivo **abierto** (IAAP, código **25/0102**, turno libre, acumula ofertas 2023+2024+2025; ~144 plazas según academias, **sin verificar en BOA** — verificar antes de publicar landing). Base propia: **134 usuarios** con `auxiliar_administrativo_aragon` como objetivo (7 premium) → el C1 es su escalón natural. 4 peticiones de usuarios.
- **Cómo:** runbook `docs/maintenance/crear-nueva-oposicion.md` + scaffolder `scripts/create-oposicion.cjs` (FASES 2-5 automáticas desde `spec.json`; a mano el TEMARIO LITERAL del BOA y el mapeo de scope + CcaaFlag). Reutiliza banco del Auxiliar de Aragón (Estatuto de Autonomía, Ley 39/2015, función pública aragonesa). Gates obligatorios: `audit:oposicion` + `audit:served` + `verify:scope` (2 agentes) + refresh MV.
- **Estado:** decidida y comprometida el 16/07, sin empezar.
