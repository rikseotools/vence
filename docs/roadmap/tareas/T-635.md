...
#### 🔴 08/08 — el test de T-635 estaba ROJO en `main`, y NO era el presupuesto

Encontrado en un «revisa la salud» al rebasar sobre main: `proxyToBackend.test.ts` fallaba de forma
constante en el caso *«si el proxy ya consumió casi todo el presupuesto, el fallback local NO recibe
otros 25s completos»*, con **«esperaba 200, recibí 500»**. Leído así, parecía que el acotado de
presupuesto de esta ficha estaba mal. **No lo estaba.**

**La causa real:** el mock de `lib/observability/emit` declaraba solo `emit`, y la ruta también
importa **`emitFireAndForget`** — que invoca en el camino de «respuesta lenta» (T-312, ampliación del
06/08, `route.ts` ~341). Este test es el ÚNICO que mueve el reloj (simula 24,5 s), así que es el
único que llega a esa línea: `TypeError: emitFireAndForget is not a function` → lo caza el `catch`
general de la ruta → **500**.

**Dos tareas que no se conocen y un punto de encuentro:** T-312 pone el emisor, T-635 mueve el reloj,
y el mock es donde chocan. Producción nunca estuvo afectada — el módulo real sí exporta la función.

**Arreglado** declarando `emitFireAndForget` en el mock. Y de paso el hueco se convirtió en
cobertura: el test ahora **afirma** que el desglose de ruta lenta se emite en ese camino. Mientras el
mock no lo declaraba, esa línea de la ruta no estaba «sin probar», estaba **reventando**.

**Trampa latente, medida y NO arreglada aquí (decidir aparte):** hay **4 ficheros más** que mockean
`lib/observability/emit` sin declarar `emitFireAndForget` — `reconcileUserPremium.test.ts`,
`exam/validateRouteTrace.test.ts`, `checkout-sync/syncCheckoutSession.test.ts`,
`oep-signals/f3ObservabilidadClonado.test.ts`. Hoy están en verde porque sus rutas no llaman a esa
función; el día que una lo haga, repiten este fallo con el mismo síntoma engañoso (un 500 que se lee
como un fallo de la lógica bajo prueba). Es el patrón de [T-722] —una lista escrita a mano que
pretende ser exhaustiva y no lo es— aplicado a los mocks: lo que haría falta es un guardarraíl que
BUSQUE los mocks de un módulo y exija que declaren todo lo que ese módulo exporta.
