...
#### ✅ DECIDIDO Y CABLEADO (Manuel, 08/08): corren al final de cada despliegue

Salida (b). Van en el paso **[7/7] de `scripts/deploy-frontend.sh`**, contra el código recién
desplegado, con el `AUTH_SECRET` sacado de SSM en memoria.

**Se descartó GitHub Actions a propósito:** ese secreto **firma sesiones** —quien lo tenga puede
acuñar la de cualquier usuario— y como secret del repo queda al alcance de mucha más gente. El
coste elegido, un par de minutos por deploy, se paga solo cuando despliegas.

**Un fallo NO tumba el deploy.** Cuando esto corre, la imagen ya está viva y verificada arriba; lo
que hace es cantarlo y dejar el rollback delante. La decisión es humana.

Guardarraíl propio: `e2ePostDeploy.guardrail.test.ts` (6 tests) — que el paso siga ahí, que use
`own-mint` y no el proveedor de Supabase congelado, que el secreto salga de SSM y nunca esté
escrito, y que sin cuenta el salto sea RUIDOSO. La forma de perder esto no es que alguien borre
los specs: es que este bloque desaparezca en un refactor y el deploy siga diciendo «OK» igual.

#### ⏳ Falta: UNA cuenta desechable, y por eso siguen declarados como huérfanos

Sin `E2E_USER_ID` el paso **se salta y lo dice**. No es un olvido: `question-evolution`
**responde una pregunta**, así que correrlas contra la cuenta de un usuario real le ensuciaría las
estadísticas — y crear un usuario en producción no es una decisión que tomar de paso.

```bash
export E2E_USER_ID=<uuid de una cuenta desechable>   # opcional: E2E_USER_EMAIL
```

**Los 6 siguen en `HUERFANOS_DECLARADOS` a propósito** hasta que haya cuenta y un deploy los haya
ejecutado de verdad. Darlos por cubiertos con el paso apagado sería exactamente el falso verde que
esta ficha persigue.
