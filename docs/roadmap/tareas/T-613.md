...
- **✅ EL FIX DE ARRIBA (VACUUM + `drenaje_atrasado`) SÍ ESTÁ DESPLEGADO (08/08, w2) — pero apareció
  UNA 3ª REGRESIÓN, y la rama que la arreglaba estaba a punto de regresar T-360 sin que nadie lo
  viera. Corregido. Rama nueva: `flota/T-613-purge-resiliencia-rebase` (pusheada, NO desplegada,
  NO mergeada).**

  **Primero, lo verificado contra `deploy_runs` (tabla real, no la ficha):** el commit
  `1a5803669` (VACUUM try/catch + fix de `drenaje_atrasado`) se desplegó en el `deploy_runs.id=95`
  (`sha=062976c8`, 07/08 22:12-22:16 UTC) — confirmado `git merge-base --is-ancestor 1a5803669
  062976c8`. Por eso el cron de **hoy 08/08 04:11 UTC ya corría con ese fix puesto.**

  **Y aun así falló — pero con un error DISTINTO, que confirma que había una 3ª regresión sin
  desplegar.** `cron_run` de hoy: `status:'failure'`, `duration_ms:83681`, `error_message`: *"Failed
  query: DELETE FROM observable_events WHERE ctid IN (SELECT ctid FROM observable_events WHERE
  created_at < now() - interval '30 days' LIMIT $1) params: 50000"* — ya NO es el VACUUM (ese ya
  tiene try/catch desde anoche), es el propio DELETE del bucle de `purgeTable`, que segunda esta
  ficha (sesión anterior, mismo día) ya había diagnosticado y arreglado en la rama
  `flota/T-613-purge-loop-resiliencia` (`3780ff13f` try/catch + `9db560e6e` `ORDER BY`, con el
  `EXPLAIN ANALYZE` que mide 185× — sigue siendo válido, no se repite aquí). El problema: **esa
  rama nunca llegó a main**, y entretanto **T-360 (particionado de `observable_events`, merge
  `057c6295c`, 08/08 08:30 UTC) sí llegó** — DESPUÉS de que la rama del fix se creara.

  **No es un simple "hay que actualizarla": mergeada tal cual, esa rama BORRABA el particionado de
  T-360 sin conflicto aparente.** No me lo he creído leyendo el diff — lo comprobé haciendo el
  merge de verdad (`git merge --no-commit --no-ff origin/flota/T-613-purge-loop-resiliencia` sobre
  un scratch, luego `git merge --abort`): sale CONFLICT real en `telemetry-retention.service.ts`,
  `.cron.ts` y `tareas-pendientes.md` — así que un merge sin cuidado no lo habría hecho en silencio,
  pero SÍ lo habría hecho un merge resuelto mirando solo "qué lado tiene más código nuevo" o
  quedándose con el lado equivocado, que es EXACTAMENTE lo que el propio commit de hoy `50a89878`
  describe para el caso de T-356 ("el merge... traía de vuelta la versión ANTERIOR a su propia
  revisión"). El diff de la rama vieja borraba `estaParticionada`/`mantenerParticiones`, el campo
  `observableEventsPorParticion` del resultado y del evento emitido, Y el `describe()` de tests
  entero que protege esa rama (`telemetry-retention.service.spec.ts` sección T-360,
  `telemetry-retention.cron.spec.ts` completo) — porque se creó ANTES de que esas líneas existieran
  en main, no porque alguien decidiera quitarlas.

  **Arreglado reaplicando el fix a mano sobre `origin/main` actual, con las DOS features vivas:**
  `purgeTable` gana try/catch (registra en `purgeFailed`, no propaga, corta esa tabla y retoma la
  próxima noche) + `ORDER BY tsColumn` en el DELETE; `estaParticionada`/`mantenerParticiones` (T-360)
  intactos — la rama de partición no pasa por `purgeTable`, así que no comparte el mismo riesgo de
  Seq Scan (`DROP PARTITION` no escanea la tabla). `archive-interactions.service.ts`/`.cron.ts`
  (mismo fix, `archiveFailed`/`cleanupFailed`) no tenían ningún conflicto real — ese fichero no lo
  tocó nada más desde el commit base de la rama vieja, aplicado con `git apply` limpio.

  **Verificado, no solo compilado:** los 12 tests preexistentes de `telemetry-retention` (incluida
  la suite entera de T-360 por partición, que la rama vieja habría borrado) + 2 nuevos (el DELETE
  que falla a mitad de pasada en el servicio — reproduce el error EXACTO de hoy 08/08 04:11 — y el
  `cron.spec` que confirma que `purgeFailed` se emite sin perder `status:'success'`) + 8 de
  `archive-interactions` — 20/20 verdes. `tsc --noEmit` limpio (`NODE_OPTIONS=--max-old-space-size=
  4096`, gotcha de memoria de esta máquina, igual que otras sesiones ya documentaron para otras
  tareas). `eslint --fix`: quedan 22 avisos en los ficheros tocados, comparados contra un `eslint`
  corrido sobre el `main` SIN tocar (mismo comando, `git stash` de por medio) — son el MISMO patrón
  ("unsafe member access .metadata", "require-await" en los dobles de test) que ya existía antes de
  este cambio, no una regresión mía. Suite completa del backend: **1603/1604 verdes**; el único rojo
  (`canary-registry.spec.ts`, `"identidad-pago"` huérfano) lo reproduce también un `main` limpio sin
  ningún cambio mío — no relacionado con T-613.

  **Lo que NO cambia de lo de arriba:** el bloqueo de RLS en `user_interactions`/
  `user_interactions_archive` sigue en `flota/T-613-rls-user-interactions-w2`, sin aplicar, fuera de
  alcance de un trabajador (necesita `DATABASE_URL` de escritura).

  **Falta:** revisar y mergear `flota/T-613-purge-resiliencia-rebase` (NO la vieja
  `flota/T-613-purge-loop-resiliencia` — esa se puede cerrar/descartar, su contenido ya está aquí
  reaplicado correctamente) → desplegar backend → la noche siguiente confirmar en el `cron_run` de
  `telemetry-retention`: (1) si vuelve a fallar el DELETE, ahora `status` sigue siendo `'success'`
  con `purgeFailed` no vacío en vez de perderse todo el reporte; (2) `batches` se acerca a los 50
  diseñados; (3) el conteo de filas fuera de retención (`remaining.observable_events`) baja noche a
  noche, no solo dice `0`.
