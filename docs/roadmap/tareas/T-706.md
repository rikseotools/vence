...
#### ⚠️ ACTUALIZACIÓN 08/08 19:0x — el arreglo YA está vivo, y la pregunta del sábado tiene respuesta

Esta ficha decía «no hecho aquí» y quedó dormida esperando a alguien con credenciales AWS para
reconstruir las dos imágenes. **Ese rebuild ya se había hecho** el mismo 08/08 (instagram 14:52,
radar 10:39), después del commit del latido. Comprobado ejecutando, no leyendo:

- **Instagram:** tarea lanzada a mano con `DRY_RUN=1`, exit 0 → `cron_tick` + `cron_run` a las 18:05
  UTC, bajo el endpoint `vence-instagram-daily-dry-run`.
- **Radar:** ya tenía filas reales del 08/08 (`cron_tick` 08:41, `cron_run` 08:42 UTC).

**La pregunta propia de esta ficha —«por qué el radar corrió en sábado»— era una lectura de más.**
Las «2 veces en 3 días» son **2 FILAS de un solo arranque** (el tick y el run se cuentan por
separado), no 2 ejecuciones. Ese arranque fue dos minutos después de subir su imagen nueva: la
prueba manual del rebuild. El planificador está bien (`cron(0 6 ? * MON,WED,FRI *)` tz Madrid,
`ENABLED`, coincide con `external-jobs.registry.ts`) y un sábado no dispara.

Detalle completo de la medición en [T-325]. Lo que queda de las dos es lo mismo: comprobar mañana,
pasadas las 10:00 Madrid, que la ejecución REAL de instagram deja su fila y que `cron_overdue` deja
de dispararle.
