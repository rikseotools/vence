...
**4. Loose end operativo — GHA deploy.** El auto-deploy quedó **desactivado** (metía builds supabase por sorpresa). Deploy hoy = **manual** (`scratchpad/build-flip.sh`+`deploy-flip.sh`). Antes de re-activar el push-trigger hay que **inyectar los build-args `NEXT_PUBLIC_AUTH_PROVIDER=authjs` + `NEXT_PUBLIC_AUTH_LIFECYCLE_VIA_API` + `AUTH_JWT_*`** en `frontend-deploy.yml`; si no, el siguiente push volvería a construir un bundle supabase y **des-flipearía** prod. **Tarea concreta pendiente.**

**5. Endurecer el GATE E2E — cubrir TODAS las superficies de auth (deuda de proceso).** El gate del flip (`scratchpad/authjs-e2e-validate.cjs`) probó `/api/v2` **normal** pero NO admin/finance/lifecycle → por eso el guard admin (`guardAdminApi`, remoto-Supabase, rechazaba RS256) pasó verde y cayó en prod (`:320` lo arregló). **Tarea:** ampliar el harness para que, con un token RS256, ejercite un endpoint de CADA superficie: regular (`/api/v2/user/*`), **admin** (`/api/v2/admin/dashboard` + `/api/admin/pending-counts` → 200 con admin, 403 con no-admin, 401 sin token), **finance** (`/api/admin/cobros`/`authenticateFinanceRequest`) y **lifecycle**. Es lo que habría evitado los 3 huecos del día. Convertir en gate obligatorio pre-deploy de cualquier cambio de auth.
