...
**4. Loose end operativo — deploy.** El auto-deploy quedó **desactivado** (solo `workflow_dispatch`; el `push:` metía builds supabase por sorpresa). Estado:
   ...
   - ✅ **Landmine de GHA cerrado (sin gh):** los flags `NEXT_PUBLIC_AUTH_PROVIDER=authjs` + `NEXT_PUBLIC_AUTH_LIFECYCLE_VIA_API=true` ahora van **LITERALES en el YAML** (no son secretos, son `NEXT_PUBLIC_*`) → un `workflow_dispatch` (o un push si se re-activa) SIEMPRE construye el bundle authjs correcto, sin depender de GH secrets vacíos. Antes, con los secrets vacíos, un dispatch manual construía supabase y des-flipeaba prod. Para revertir el flip: cambiar esos literales, no un secret. **Ya no hace falta tocar GitHub.**
   - Re-activar el `push:` trigger (opcional): ya es seguro (bundle correcto por construcción + `concurrency` con cancel-in-progress). Decisión de Manuel si quiere auto-deploy o seguir manual.
...
**6. ✅ RESUELTO (15/07) — FLOOD de `/api/auth/token` (el "pulir menor" era un bug serio).** El poll de sesión del `authjsAdapter` (`onAuthStateChange`, tick cada 5s) **re-acuñaba el RS256 en CADA tick** en vez de cachearlo su hora de TTL → **~1,4M req/día a `/api/auth/token`** (675k mints + 525k 401), el 93% de toda la telemetría. Los 401 intermitentes **deslogueaban a usuarios premium** (caso Natalia, Safari/Mac, 2 semanas). El 11/07 se **enmascaró** muestreando la telemetría al 10% (commit `3b5c1c5c`, "firehose -90%") — NO se arregló, solo se dejó de medir. **Fix (`lib/auth/adapters/authjsAdapter.ts`):** caché del token por instancia (reusa hasta 5 min antes de expirar) + backoff de 60s para anónimos + invalidación en signOut + broadcast cross-pestaña. Preserva la distinción transient/401 y el bridge. **Simulación: 2h de uso → de 1.440 a 3 req (99,8% menos, 480×)**; proyección global ~756k→~1,6k req/día. Tests: `__tests__/lib/auth/authjsAdapter.test.ts` (24, +5 anti-flood con la simulación). **Observabilidad:** alerta `auth_token_mint_flood` (`backend/src/alerts/alert-rules.ts`) — ratio tokens/usuario anómalo. **Lección:** un endpoint que domina el 93% de la telemetría es un BUG a arreglar, no ruido a muestrear.

**6-bis. Pulir (menor) — blip de token transitorio.** Bajo el flip, si `/api/auth/token` falla un instante (frío / cooldown del singleflight de `authHeaders.ts`), `getAuthHeaders` devuelve vacío → una petición admin puntual sale sin Bearer → 401 `hadAuthHeader:false` (se autocorrige al siguiente intento; visto 1 vez el 03/07). No rompe nada, pero para pulir: manejo de `refresh_token` / reintento con backoff en la adquisición de token. No urgente.
