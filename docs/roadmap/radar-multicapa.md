...
- **Cómo:** cron `detect-examenes-signals` (`backend/src/detect-examenes-signals/`, 10:00 UTC, tras notas) lee las notas de **confianza alta** con fecha de examen, se queda con las que son **una fecha de día único inequívoca** (`parse-fecha-examen.ts`), **no capturadas** (sin `exam_date` ni hito `ejercicio_1`), **no de un ciclo previo ya resuelto** (fecha anterior al arranque del ciclo vigente — hito `oep_aprobada`/`convocatoria_publicada`/`bases_publicadas`; evita que un PDF viejo que sigue en la página cuele la fecha de la convocatoria anterior), de oposición **viva**, y emite señal `nota_examen` (score 60, `pending`) a `/admin/oep-signals`. Idempotente por `dedupeKey='nota_examen:opoId:fecha'`.
...
- **🆕 Al Aplicar, el radar también ALIMENTA la entidad OEP (T-108, F3):** `promoteSignalToConvocatoria` hace find-or-insert de la fila **`oep`** del `detected_year` + enlace `convocatoria_oep`, y **clona el decreto** si `source_url` es un boletín reconocido (`fuente='oep-radar'`). Es aditivo y **no bloqueante** (si falla, no tumba la promoción de la convocatoria). Así la entidad OEP no se degrada (deja de ser la foto del backfill). El backlog de OEP sin convocar se ve en `/api/admin/oep-consistency`. Modelo: `docs/roadmap/oep-entidad-modelo.md`.
