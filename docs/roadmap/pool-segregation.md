...
> **Última actualización**: 2026-06-26 — 4 queries pesadas optimizadas (MV/cache/covering-index/JOIN) + verificación post-deploy 🟢. Pendiente: tendencia de saturación a 1-2 días.
...
> - **VERIFICACIÓN POST-DEPLOY (2026-06-26 ~13:53) — 🟢 los 4 LIVE:** frontend `:291` (MV, corre a **28ms** vs 1491ms) + `:292` COMPLETED (covering-index + daily-goal) + backend `:24` con digest `b029d672` confirmado (canary). El **canary full-scan plateauó** (964 calls, +2 en 50min → ya ~1/hora, no cada 10min = cache OK). **Saturación-503: 0 en las ~3h post-deploy** (4 en 8h, ninguna desde 11:00). La confirmación sólida de tendencia necesita 1-2 días (es bursty).
