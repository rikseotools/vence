...
- Pendiente: fase 2 (re-verificación con topic-review/verify para fijar `article_ok`) de los contenedores hechos, y los **55 contenedores restantes** (priorizar TCAE clínico por volumen: Alimentación, Paciente quirúrgico, Eliminación/sondajes, Higiene…). Recomendado: **workflow por contenedor** para escalar (sourcing → redacción → auditoría → re-distribución).
