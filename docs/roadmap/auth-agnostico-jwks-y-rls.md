...
> **Estado:** 🟢 EN CURSO. **Fase A1 ✅ + A2 ✅ HECHAS** (2026-06-20, sin commit aún). A3 + Fase B + Fase C pendientes.
...
- **A3 (admin checks) — `is_current_user_admin`/`is_user_admin`:** `app/Header.tsx`, `components/UserAvatar.tsx`, `components/Admin/ProtectedRoute.tsx`, `app/admin/fraudes/page.tsx`, `app/admin/notificaciones/{email,email/subscripciones,events,overview,users}/page.js` (×5, además son `.js`→convertir), `app/api/admin/email-events/route.ts`, `lib/finance/auth.ts`. → sustituir por `requireAdmin`/`isAdminEmail` (server) + claim email/role (cliente).
