...
-- ejecute por error). Cuando se cumplan las precondiciones, RE-GENERAR este draft
-- (las tablas/políticas crecen al añadir oposiciones), revisar el diff, y mover el
-- bloque "UP" a supabase/migrations/<timestamp>_c4_drop_rls.sql.
--
...
-- ============================================================================
-- DOWN — recreación verbatim (rollback). Descomentar para revertir.
-- ============================================================================
-- CREATE POLICY "Admins can view ai_chat_traces" ON public.ai_chat_traces AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.email ~~ '%@vencemitfg.es'::text) OR (user_profiles.email = 'admin@example.com'::text))))));
--
-- CREATE POLICY "Admins can view cancellation feedback" ON public.cancellation_feedback AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND (user_profiles.email = ANY (ARRAY['manuelrivas@hey.com'::text, 'admin@vence.es'::text]))))));
--
-- CREATE POLICY "Admins can view all conversion events" ON public.conversion_events AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND (user_profiles.plan_type = 'admin'::text)))));
...
--
-- CREATE POLICY "Admin can view all email events" ON public.email_events AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = 'ilovetestpro@gmail.com'::text))))));
...
--
-- CREATE POLICY "Admin can update all conversations" ON public.feedback_conversations AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = ANY (ARRAY['manueltrader@gmail.com'::text, 'ilovetestpro@gmail.com'::text])))))));
-- CREATE POLICY "Authenticated can insert own conversations" ON public.feedback_conversations AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((user_id = auth.uid()) OR (user_id IS NULL)));
-- CREATE POLICY "Users can update own conversations" ON public.feedback_conversations AS PERMISSIVE FOR UPDATE TO authenticated USING ((user_id = auth.uid()));
--
-- CREATE POLICY "Admin can update messages" ON public.feedback_messages AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = ANY (ARRAY['manueltrader@gmail.com'::text, 'ilovetestpro@gmail.com'::text])))))));
-- CREATE POLICY "Authenticated can insert messages" ON public.feedback_messages AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((sender_id = auth.uid()) OR (sender_id IS NULL)));
--
-- CREATE POLICY "Admins can manage fraud_confirmations" ON public.fraud_confirmations AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM user_roles WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])) AND (user_roles.is_active = true)))));
--
-- CREATE POLICY "Admins can manage fraud_watch_list" ON public.fraud_watch_list AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM user_roles WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])) AND (user_roles.is_active = true)))));
...
--
-- CREATE POLICY "Users can insert their own first attempts" ON public.law_question_first_attempts_pre_outbox AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = user_id));
-- CREATE POLICY "Users can view their own first attempts" ON public.law_question_first_attempts_pre_outbox AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = user_id));
--
-- CREATE POLICY "Only admins can manage messages" ON public.motivational_messages AS PERMISSIVE FOR ALL TO public USING ((EXISTS ( SELECT 1 FROM user_roles WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])) AND (user_roles.is_active = true)))));
--
-- CREATE POLICY "Admin can view all notification events" ON public.notification_events AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = 'ilovetestpro@gmail.com'::text))))));
...
--
-- CREATE POLICY "Users can update own notifications" ON public.notification_logs AS PERMISSIVE FOR UPDATE TO authenticated USING ((user_id = auth.uid()));
-- CREATE POLICY "Users can view own notifications" ON public.notification_logs AS PERMISSIVE FOR SELECT TO authenticated USING ((user_id = auth.uid()));
--
-- CREATE POLICY "Admins can view audit log" ON public.plan_type_audit_log AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = 'manueltrader@gmail.com'::text))))));
--
...
--
-- CREATE POLICY "Users can insert own disputes" ON public.question_disputes AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((user_id = auth.uid()));
-- CREATE POLICY "Users can view own disputes" ON public.question_disputes AS PERMISSIVE FOR SELECT TO authenticated USING ((user_id = auth.uid()));
--
...
--
-- CREATE POLICY "Admins can view all tests" ON public.tests AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1 FROM user_roles WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = 'admin'::text)))));
...
--
-- CREATE POLICY "Users can read own devices" ON public.user_devices AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = user_id));
--
...
--
-- CREATE POLICY "Admin can update all feedback" ON public.user_feedback AS PERMISSIVE FOR UPDATE TO authenticated USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = ANY (ARRAY['manueltrader@gmail.com'::text, 'ilovetestpro@gmail.com'::text])))))));
-- CREATE POLICY "Authenticated users can insert own feedback" ON public.user_feedback AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((user_id = auth.uid()) OR (user_id IS NULL)));
-- CREATE POLICY "Authenticated users can update own feedback" ON public.user_feedback AS PERMISSIVE FOR UPDATE TO authenticated USING ((user_id = auth.uid())) WITH CHECK ((user_id = auth.uid()));
--
-- CREATE POLICY "Admins can view all interactions" ON public.user_interactions AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = 'ilovetestpro@gmail.com'::text))))));
...
--
-- CREATE POLICY "users_read_own_archived_interactions" ON public.user_interactions_archive AS PERMISSIVE FOR SELECT TO authenticated USING ((( SELECT auth.uid() AS uid) = user_id));
--
...
--
-- CREATE POLICY "Admins can read all interactions" ON public.user_message_interactions AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_roles WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = ANY (ARRAY['admin'::text, 'super_admin'::text])) AND (user_roles.is_active = true)))));
...
--
-- CREATE POLICY "Admin can view all notification metrics" ON public.user_notification_metrics AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = 'ilovetestpro@gmail.com'::text))))));
...
--
-- CREATE POLICY "Admin can view all notification settings" ON public.user_notification_settings AS PERMISSIVE FOR SELECT TO public USING ((EXISTS ( SELECT 1 FROM user_profiles WHERE ((user_profiles.id = auth.uid()) AND ((user_profiles.plan_type = 'admin'::text) OR (user_profiles.email = 'manueltrader@gmail.com'::text))))));
--
-- CREATE POLICY "Admins can view all profiles" ON public.user_profiles AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1 FROM user_roles WHERE ((user_roles.user_id = auth.uid()) AND (user_roles.role = 'admin'::text)))));
...
--
-- CREATE POLICY "users_read_own_stats_summary" ON public.user_stats_summary_pre_outbox AS PERMISSIVE FOR SELECT TO authenticated USING ((( SELECT auth.uid() AS uid) = user_id));
--
...
--
-- CREATE POLICY "users_read_own" ON public.validation_error_logs AS PERMISSIVE FOR SELECT TO authenticated USING ((user_id = auth.uid()));
