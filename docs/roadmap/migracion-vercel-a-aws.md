...
>
> **Última actualización**: 2026-07-04 — 🟢🟢 **BD MIGRADA A AWS RDS (cutover ejecutado)** + Fase B HECHA + C4 en prod. **La app de prod ya escribe en `vence-prod` (RDS PostgreSQL 17.6 Multi-AZ, eu-west-2); Supabase congelado como backup.** Ver §3.1 y `migracion-datos-supabase-a-rds.md`. Detalle + gotchas en memoria `project_cutover_rds_prod`.
...
> - **Drenaje disparado (04/07 ~00:00)**: `AUTH_BRIDGE_ENABLED=false` (`:329`). Mints por bridge → **0** en ~2 min; solo quedan mints nativos; 0 errores 5xx; usuarios re-entran con un clic. Soak ~15h limpio (91k mints, todos `authjs_session`, 0 bridge, 0 regresiones auth). **✅ C4 APLICADO A PROD (04/07): 205→75 políticas, 0 `auth.*`, prod verde (ver §3.1 punto 4).**
> - **✅✅ CUTOVER DE DATOS A RDS EJECUTADO (04/07):** los 5 params SSM de BD → `vence-prod` (Multi-AZ) + ECS scale 0→back. **App escribe en RDS, Supabase congelado (backup), 0 tablas incompletas verificado, temas jugables.** Downtime real ~15 min. Sustos (todos resueltos): otra sesión paralela vació 3 tablas (recuperadas desde Supabase); matviews vacías → "En desarrollo" (refrescadas). Gotchas en memoria `project_cutover_rds_prod`. **Pendiente: vigilar 48-72h → retirar el bridge por código → decomisionar Supabase + `pooler.vence.es` → endurecer RDS (no-público).**
