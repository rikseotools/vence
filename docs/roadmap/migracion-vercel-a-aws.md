...
4. **C4 (drop RLS 125 policies) + retirar `is_current_user_admin`** → elimina las 130 refs a `schema auth`. (De las 3 fns `auth.*` originales, `assign_role` + `get_current_user_roles` eran RBAC muerto y se **dropearon el 03/07** — `20260703_drop_dead_role_functions.sql`; queda solo `is_current_user_admin`, atada a 2 RLS de `user_roles`, que se va con el drop de RLS.)
