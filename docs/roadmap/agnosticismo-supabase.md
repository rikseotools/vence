...
**Fase 3 (supabase.from() → Drizzle): 61 ficheros migrados + 2 borrados en total** (`subscription-reconciliation` + `adaptiveDifficulty` eliminados por muertos). Sesión 02/06: **32 migrados, −137 from, −11 rpc, 7 fixes de cap PostgREST**. Incluye el GORDO `stripe/webhook` (−31/−2rpc) y `emailService.server` 100% sin supabase (test reescrito). **Cierre 02/06 (post-clear)**: +2 senders email_events migrados + `adaptiveDifficulty` borrado (ver puntos 3 y 4 abajo). Detalle autoritativo en memoria `project_agnosticismo_strangler_01_06`.
...
| Archivos con `supabase.from()` | ~96 (estimación pre-audit) | **61 migrados + 2 borrados (02/06)**; quedan ~6 (4 rotos por schema drift + 2 client-trackers + 3 falsos positivos storage/comentario/cliente) | ≤ 5 (allowlist documentada) — el resto requiere decisión (rotos) o Fase 4 Auth |
