...
5. 🟡 **Fase 4 (Auth, `supabase.auth.*`) — EN CURSO** (bloqueador real de RDS). Plan ports&adapters aprobado, detalle en **§Fase 4** abajo. Inventario: ~73 `supabase.auth.*` en 36 ficheros (solo OAuth Google); capa server-side YA agnóstica (verifyAuth/shared/auth). **Fase A HECHA** (facade `lib/auth/`, commit `8f526950`, 0 callers migrados). **Fase B (cutover hub authHeaders+AuthContext) PROGRAMADA sábado 2026-06-06 por la noche** (sin usuarios; un fallo = logout masivo; red de seguridad = git revert). Red de tests lista: 32 AuthContext + 8 authHeaders nuevos (caracterización) + 17 adapter. Login Google: C2 unifica 5 `signInWithOAuth` directos; **C4 (callback) diseñado** = port `completeOAuthCallback()` con el workaround PKCE encapsulado en el adapter. Pendiente C-D-E-F (F = POC Auth.js, rama sin merge).
