...
**Fase 3 (supabase.from() → Drizzle): 63 migrados + 5 borrados — AGOTADA.** Borrados por muertos: `subscription-reconciliation`, `adaptiveDifficulty`, `emailTracker`, `notificationTracker`, `send-reactivation-email`. **Cierre post-clear 02/06**: +2 senders email_events + `sitemap-static.xml` (−1 from −1 rpc, paridad 807 leyes/468/676 RPC) + `adminConversationTracking` (server-only) migrados; `adaptiveDifficulty`/`emailTracker`/`notificationTracker`/`send-reactivation-email` borrados (dead-code, auditados). **Inventario fresco completo 02/06**: de los `.from('` REST restantes, los server-side `app/api/**/route` se reducen a los **4 rotos por schema drift (DECIDIDOS, ver abajo)**; TODO lo demás es **client-side** (pages/components/hooks/contexts → refactor a endpoint API) + 3 falsos positivos (`video-courses` storage, `problematic-articles-logs` comentario L24, `motivationalAnalyzer`/`adaptiveQuestionSelection`/`psychometricDifficulty`/`warmCache`/`testFetchers`/`lawFetchers` alcanzables desde cliente). **El bucket server-side mecánico está AGOTADO.** Detalle autoritativo en memoria `project_agnosticismo_strangler_01_06`.
...
5. 🟡 **Fase 4 (Auth, `supabase.auth.*`) — EN CURSO** (bloqueador real de RDS). Plan ports&adapters aprobado, detalle en **§Fase 4** abajo. Inventario: ~73 `supabase.auth.*` en 36 ficheros (solo OAuth Google); capa server-side YA agnóstica (verifyAuth/shared/auth). **Fase A HECHA** (facade `lib/auth/`, commit `8f526950`, 0 callers migrados). **Fase B (cutover hub authHeaders+AuthContext) PROGRAMADA sábado 2026-06-06 por la noche** (sin usuarios; un fallo = logout masivo; red de seguridad = git revert). Red de tests lista: 32 AuthContext + 8 authHeaders nuevos (caracterización) + 17 adapter. Login Google: **C4 (callback) PORT CONSTRUIDO 03/06 build-only** (`completeOAuthCallback()` en el adapter encapsula los 3 canales PKCE; 12/12 tests; página SIN cablear → riesgo cero, se activa el sábado). **C2 auditado 03/06: NO es drop-in** (los 4 callers premium/landings dependen de `return_to` con `start_checkout`+`campaign` = conversión; la facade no acepta `redirectTo` → unificar a ciegas rompe el funnel; requiere extender `SignInOptions`+test checkout, DIFERIDO). Pendiente C(resto)-D-E-F (F = POC Auth.js, rama sin merge).
...
- ✅ **B — Hub** (`lib/api/authHeaders.ts` + `contexts/AuthContext.tsx`). **HECHO Y EN PROD 03/06 (commit `3f26fa41`, sesión paralela)** — Manuel aprobó hacerlo HOY (miércoles, "no hay usuarios") en vez del sábado originalmente programado, aceptando el riesgo de logout masivo + push directo. authHeaders (getSession/refreshSession → `auth.*`, singleflight+cooldown se quedan) + AuthContext (onAuthStateChange/getSession/getUser/signOut → `auth.*`; User CRUDO se extrae de `.raw`; SIGNED_UP reconstruido de `change.isNewUser` → callback byte-idéntico). 33/33 AuthContext + 8/8 authHeaders + 49/49 adapter, typecheck+build OK. **⚠️ SOAK 24-48h en curso** (NO se hizo checklist iOS/Android en preview → vigilar Safari iOS/PWA en Sentry/observable_events; rollback = `git revert 3f26fa41`). Detalle en memoria [[project_agnosticismo_strangler_01_06]].
- ⏳ **C — Componentes/hooks cliente por lotes** (C1 fetchers token-only → C2 modales/login+OAuth → C3 chat/notif → C4 `auth/callback` → C5 AvatarChanger).
  - 🟢 **C1 (fetchers token-only) — EN CURSO 03/06.** Patrón: `supabase.auth.getSession().access_token` (Bearer) → `auth.getAccessToken()`. **Equivalencia probada**: `getAuthHeaders` (base de `auth.getAccessToken`) hace `auth.getSession()?.accessToken` = el MISMO JWT, con singleflight/refresh extra. **HECHO**: `components/OpenInscriptionBanner.tsx` (2 getSession → auth.getAccessToken, `supabase` eliminado del componente; typecheck+eslint+exhaustive-deps verdes). **DIFERIDOS hot-path** (a ventana supervisada, no madrugada): `lib/api/v2/answer-and-save/client.ts`, `complete-test/client.ts` (los más calientes). **OTRAS fases**: `ChatInterface` (C3), `AvatarChanger` (C5); `useDailyQuestionLimit`/`OposicionDetector` mezclan `.rpc`/sesión (parcial). **CON WRINKLE (no drop-in, auditado 03/06)**: `complete-onboarding/client.ts` hace `refreshSession()` PRIMERO (refresco proactivo para token fresco del recién-registrado) + `getSession()` fallback → `auth.getAccessToken()` es getSession-first, cambiaría la semántica de refresco en onboarding → migrar requiere replicar el refresh-first o validar que da igual. Pendiente que parece limpio: `testFetchers` (pero es el path de carga de preguntas, verificar con cuidado).
  - **C2 (login Google)**: unificar los **5 sitios que llaman directo `supabase.auth.signInWithOAuth`** (`app/login/page.js`, `app/premium/page.tsx`, landings `premium-ads-1`/`premium-ads-2`/`premium-edu`) → `auth.signInWithGoogle()` (hoy solo `ProgressiveRegistrationModal` usa el wrapper). El port ya cubre `signInWithGoogle`.
    - 🔴 **NO es drop-in (auditado 03/06).** Los 4 callers de premium/landings pasan un `redirectTo` con `return_to=/premium?**start_checkout=true**` (auto-checkout post-login) + `&campaign=${id}` (atribución) — **lógica de conversión crítica**. Pero la facade/helper `signInWithGoogle({funnel?})` **deriva la URL de retorno de la página ACTUAL** (`window.location` + backup localStorage) y NO acepta `redirectTo`/`return_to`. Unificar a ciegas rompería el funnel premium (retorno sin `start_checkout`/`campaign`). **C2 bien hecho = extender `SignInOptions` con `returnTo`/`queryParams` + que el helper lo honre (fallback al comportamiento actual)** → cambio en el camino conversión+auth, requiere test del flujo login→checkout en preview (NO indoloro). DIFERIDO a ventana con prueba de checkout.
  - 🟢 **C4 (callback) — PORT CONSTRUIDO 03/06 (build-only, página SIN cablear todavía)**: añadido al puerto `completeOAuthCallback(): Promise<AuthSession | null>` + implementado en `supabaseAdapter` (helper `acquireOAuthCallbackSession` que porta 1:1 los 3 canales: onAuthStateChange + polling localStorage + intercambio PKCE directo + getSession fallback 15s → null). 4 tests nuevos verdes (localStorage inmediato, onAuthStateChange, PKCE directo con fetch mock+fake timers, timeout→null) → 12/12 en client.test.ts. typecheck+eslint limpios. **`app/auth/callback/page.tsx` intacto** (riesgo cero, no se activa hasta cablearlo el sábado). Cablear = `const session = await auth.completeOAuthCallback()` y usar `session.accessToken`/`session.user.{id,email,metadata}` en el POST a process-callback. DISEÑO ORIGINAL: la página hacía un **intercambio PKCE por HTTP crudo** contra `${supabaseUrl}/auth/v1/token?grant_type=pkce` leyendo `code_verifier` de localStorage (workaround porque `exchangeCodeForSession` de supabase-js deadlockea con `_acquireLock`). Solución profesional (ports&adapters): **añadir al puerto un método de INTENCIÓN neutral `completeOAuthCallback(): Promise<AuthSession | null>` SIN params de proveedor**, y meter TODA la mecánica Supabase dentro del `supabaseAdapter` (las 3 vías: onAuthStateChange + polling localStorage + intercambio PKCE directo + fallback 15s). La página queda agnóstica: `const session = await auth.completeOAuthCallback()` → si hay sesión, POST `/api/v2/auth/process-callback` + tracking + redirect (orquestación de alto nivel NO-Supabase, se queda en la página). Prueba de escalabilidad (Fase F): el `nextAuthAdapter.completeOAuthCallback()` sería solo `getSession()` (su route handler ya resolvió el callback) → la página NO cambia. **Regla del wrapper: el puerto = verbos del dominio; el adapter = trucos del proveedor. Nada Supabase-específico (params/shapes/endpoint /token/claves localStorage) cruza la frontera del puerto.**
- ✅ **D — Server admin.*** HECHA 03/06: los **2 únicos callers directos** de `auth.admin.*` (`app/api/admin/delete-user/route.ts` → `deleteUser`, `app/api/emails/send-medal-congratulation/route.ts` → `getUserById`) cableados al puerto `authAdmin` de `lib/auth/server.ts` (que ya existía). Eliminados `getServiceClient()`/`createClient(SERVICE_ROLE)` de ambos. **Cero `auth.admin.*` fuera del puerto** (grep 0). Server-side admin-only, misma llamada subyacente → riesgo bajo. Tests: authAdmin 4/4 + delete-user 6/6 + typecheck/eslint limpios.
