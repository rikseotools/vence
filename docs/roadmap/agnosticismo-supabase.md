...
| Archivos con `supabase.from()` | ~96 (estimación pre-audit) | **62 migrados + 2 borrados (02/06)** (incl. `sitemap-static.xml` server route que faltaba en notas); quedan ~5 server (4 rotos por schema drift) + client-side (pages/components/hooks/contexts, refactor a endpoint) + 2 client-trackers + 3 falsos positivos | ≤ 5 (allowlist documentada) — el resto requiere decisión (rotos), refactor cliente→endpoint, o Fase 4 Auth |
