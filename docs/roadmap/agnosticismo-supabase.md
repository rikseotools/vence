...
| Archivos con `supabase.from()` | ~96 (estimación pre-audit) | **63 migrados + 4 borrados (02/06)** (incl. `sitemap-static.xml` + `adminConversationTracking` server puro; borrados emailTracker+notificationTracker dead-code); quedan los 4 rotos por schema drift + client-side restante (pages/components/hooks/contexts, refactor a endpoint) + 3 falsos positivos | ≤ 5 (allowlist documentada) — el resto requiere decisión (rotos), refactor cliente→endpoint, o Fase 4 Auth |
