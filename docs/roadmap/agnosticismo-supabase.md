...
- ⏳ **C — Componentes/hooks cliente por lotes** (C1 fetchers token-only → C2 modales/login+OAuth → C3 chat/notif → C4 `auth/callback` → C5 AvatarChanger).
  ...
  - 🔴 **C4 (callback) — GAP de agnosticidad pendiente**: `app/auth/callback/page.tsx` hace un **intercambio PKCE por HTTP crudo** contra `${supabaseUrl}/auth/v1/token?grant_type=pkce` leyendo `code_verifier` de localStorage (workaround porque `exchangeCodeForSession` de supabase-js deadlockea con `_acquireLock`). Eso NO está en el port y es Supabase-hardcoded. Migración: **encapsularlo dentro de `supabaseAdapter`** (o añadir método al port `completeOAuthCallback()`) para que el callback no hable directamente con el endpoint de Supabase. `onAuthStateChange`/`getSession` del callback ya están en el port. El endpoint server `/api/v2/auth/process-callback` ya usa `verifyAuth` (agnóstico).
