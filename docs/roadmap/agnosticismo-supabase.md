...
2. **Client-trackers** — ✅ `adminConversationTracking` RESUELTO 02/06 (era server-only, NO client: lo llaman 2 API routes; migrado a Drizzle + borrado factory cliente muerto → módulo 100% server, −5 from, commit `3535dfac`). Quedan `emailTracker` (4) y `notificationTracker` (3): **NO son refactor limpio a endpoint** — son `'use client'` + `supabase.auth.getUser()` (acoplados a **Fase 4 Auth**; no hay helper de sesión server-side, los endpoints actuales resuelven user por query param). Además `emailTracker` está **superado** (los email_events opened/clicked vienen de los endpoints server `/api/email-tracking/*`, device_type=null lo prueba; sus métodos no tienen callers explícitos) y `notificationTracker` es del **push deshabilitado por Manuel 01/06** (último evento cliente 2026-04-22). → Auditar para BORRAR (estilo adaptiveDifficulty) o dejar para el cleanup push-decommission + Fase 4. Ambos cableados en `AuthContext` (`setSupabaseInstance`), borrarlos toca AuthContext.
...
| Archivos con `supabase.from()` | ~96 (estimación pre-audit) | **63 migrados + 2 borrados (02/06)** (incl. `sitemap-static.xml` + `adminConversationTracking` server puro); quedan los 4 rotos por schema drift + client-side (pages/components/hooks/contexts, refactor a endpoint) + 2 trackers cliente (emailTracker/notificationTracker, acoplados Fase 4 Auth + superados/push-off) + 3 falsos positivos | ≤ 5 (allowlist documentada) — el resto requiere decisión (rotos), refactor cliente→endpoint, o Fase 4 Auth |
