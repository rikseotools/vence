// T10 Galicia Ley 1/2016 Transparencia — full pipeline
...
const LAW_SHORT = 'Ley 1/2016 Galicia';
...
function parseQuestion(raw) {
  ...
  return { question: cleanQuestion(raw), lawHint: isLaw ? inside : null, articleHint: isArt ? inside : null };
}
...
function contentHash(q, oa, ob, oc, od) {
  return crypto.createHash('sha256').update(normalize(q) + '||' + normalize(oa) + '||' + normalize(ob) + '||' + normalize(oc) + '||' + normalize(od)).digest('hex');
}
...
(async () => {
  ...
  const f67 = fs.readdirSync(d67dir).find(x => x.startsWith('Tema_10.'));
  const f72 = fs.readdirSync(d72dir).find(x => /1_2016|transparencia/i.test(x));
  ...
  for (const [lib, d] of [['67', d67], ['72', d72]]) {
    for (const q of d.questions) {
      ...
      all.push({
        original: q.question, cleaned: parsed.question,
        lawHint: parsed.lawHint, articleHint: parsed.articleHint,
  ...
  console.log('Total unificado:', all.length);
  ...
  for (const q of dbQuestions) {
    dbByShuffle.add(shuffleKey(q.question_text, [{ text: q.option_a }, { text: q.option_b }, { text: q.option_c }, { text: q.option_d }]));
  ...
  console.log('Arts Ley 1/2016 en BD:', arts.length);
  ...
  if (unmapped.length > 0) {
    unmapped.slice(0, 5).forEach((q, i) => console.log('  [' + (i+1) + '] art=' + (q._art || '—') + ' | ' + q.cleaned.slice(0, 80)));
  }
  ...
  const rows = mapped.map(q => {
    ...
    return {
      ...
      correct_option: correctIdx, explanation: q.explanation,
  ...
  for (let i = 0; i < rows.length; i += 50) {
    ...
    if (error) {
      for (const row of chunk) {
        ...
        else console.error('row err:', e2.message);
      }
    } else { inserted += data.length; insertedIds.push(...data.map(r => r.id)); }
  }
  console.log('Importadas:', inserted, '/', rows.length);
  fs.writeFileSync('t10_galicia_imported_ids.json', JSON.stringify(insertedIds, null, 2));
  fs.writeFileSync('t10_galicia_unmapped.json', JSON.stringify(unmapped, null, 2));
})();
