...
interface AuthSuccessEventDetail {
...
export default function ProgressiveRegistrationModal({
  ...
  useEffect(() => {
    const handleAuthSuccess = (event: Event) => {
      const { user } = (event as CustomEvent<AuthSuccessEventDetail>).detail
      if (user) {
    ...
    const handleAuthChange = (event: Event) => {
      const { event: authEvent, user } = (event as CustomEvent<AuthChangeEventDetail>).detail
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center p-4" style={{ zIndex: CAPAS.modal }}>
