// components/PersistentRegistrationManager.tsx
'use client'
...
interface PersistentRegistrationManagerProps {
  // Props del test
  tema: number
...
export default function PersistentRegistrationManager({
  ...
  children
}: PersistentRegistrationManagerProps) {
  // Hook de router
  const router = useRouter()

  // Estados internos
  // Si se pasa externalUser, usar ese (más fiable, viene de AuthContext con refresh token)
  const [internalUser, setInternalUser] = useState<SupabaseUser | null>(null)
  const [isCheckingAuth, setIsCheckingAuth] = useState<boolean>(externalUser === undefined)
