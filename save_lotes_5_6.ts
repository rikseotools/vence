...
async function main() {
  console.log('🚀 Cargando resultados finales - Lotes 5 y 6...\n');

  // Cargar archivos JSON
  const lote5 = JSON.parse(fs.readFileSync('verification_results_lote5.json', 'utf-8')) as VerificationResult[];
  const lote6 = JSON.parse(fs.readFileSync('verification_results_lote6.json', 'utf-8')) as VerificationResult[];

  const allResults = [...lote5, ...lote6];
