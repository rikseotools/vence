...
const LAW_SHORT = 'Ley 7/2023 Galicia';
...
(async () => {
  ...
  const f67 = fs.readdirSync(d67dir).find(x => x.startsWith('Tema_12.'));
  const d = JSON.parse(fs.readFileSync(d67dir + f67));
  console.log('libro-67 T12:', d.questions.length);

  const all = d.questions.map(q => {
    const p = parseQuestion(q.question);
    return {
      cleaned: p.question, articleHint: p.articleHint,
  ...
  if (unmapped.length > 0) {
    unmapped.forEach((q, i) => console.log('  [' + (i+1) + '] art=' + (q._art || '—') + ' | ' + q.cleaned.slice(0, 90)));
  }
  ...
  fs.writeFileSync('t12_galicia_imported_ids.json', JSON.stringify(insertedIds, null, 2));
  fs.writeFileSync('t12_galicia_unmapped.json', JSON.stringify(unmapped, null, 2));
})();
