...
(async () => {
  ...
  for (const log of logs) {
    ...
    console.log("Ley:", log.question_context_law, "| ID:", question.id);
    ...
    console.log("Correcta:", letters[question.correct_option], "| Calidad:", quality);
    console.log("Explicacion (" + exp.length + " chars):", exp.substring(0, 300));
    if (question.primary_article_id) {
      ...
      if (a) {
        ...
        console.log("\nArt:", l?.short_name, a.article_number);
        console.log((a.content || "").substring(0, 800));
      }
    }
    console.log("(Buenas:", countGood + ")");
    return;
  }
  console.log("FIN COMPLETO - " + seen.size + " unicas (" + countGood + " buenas) en " + logs.length + " logs");
})();
