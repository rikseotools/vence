...
- **Al terminar hay CUATRO salidas y el CLI las impone (30/07):** `done --outcome` solo si está verificada — **aborta si el outcome dice "pendiente/falta/queda/sin desplegar"** y te manda a `pause`; `pause --tras-deploy` (la despierta el deploy) o `pause --hasta` (la despierta el reloj), ambas con `--hecho`/`--falta` obligatorios; `release` si la sueltas sin avanzar. Y al empezar, `list` enseña ARRIBA **⏰ listas para verificar** (se cierran en minutos) y **🙋 esperando decisión de Manuel** (enséñaselas).
