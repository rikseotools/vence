...
- **Comandos:** `node scripts/backlog.cjs list | next | claim T-042 | heartbeat | mine | done T-042 --outcome "…" | release T-042 | snooze T-042 --horas 12 --motivo "…" | wake T-042 | reserve "<título>" | sync`.
- **`snooze` = la tarea espera a un RELOJ**, no a una persona (`claimed_by`) ni a otra tarea (`blocked_by`): un cron que aún no ha corrido, una cosecha, la fecha en la que toca medir. `list` la pinta `🕒 en espera hasta …` con su motivo, `next` la salta y **se despierta sola**; `claim` no lo impide, solo avisa (a veces sí quieres adelantar la preparación). Nace de que T-221 llevaba `⛔ NO COGER HASTA EL 29/07 07:00 UTC` **en el título de la ficha** y `next` la ofrecía igual: gritar en el markdown no es un mecanismo.
