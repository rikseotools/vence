...
- **Mapa (frase → qué mira):**
  ...
  - *"revisa los artículos sin preguntas"* → artículos que están en el `topic_scope` con contenido real pero **0 preguntas activas** (al usuario nunca le salen en los tests aunque el tema en conjunto sí tenga preguntas; caso M/SMS Tema 7) → `salud-contenido.md`. Detección: `article_no_coverage` en `health-sweep.cjs` (≥4 arts sin preguntas en un tema, excluye derogados). Generar preguntas ancladas al texto del artículo + doble auditoría ciega antes de activar. **PRIORIZAR ANTES DE ESCRIBIR con `npm run cobertura:huerfanos`** (`--ley "X"` orden dentro de la ley, `--simular "X"` efecto estimado): el hallazgo es por oposición pero **el trabajo es por ARTÍCULO, que es fila COMPARTIDA** — cubrirlo repara todas las oposiciones que lo escopan a la vez (medido 26/07: 6.533 pares sobre 3.075 artículos, reuso 2,1×). La rentabilidad varía ~8× entre leyes (la LPRL cerró 19 temas con 6 artículos); elegir por corazonada multiplica el coste. Núcleo puro `lib/salud/coberturaHuerfanos.js`, con guardarraíl que ata sus umbrales a los del detector real. **Distingue `temasACero` (arreglo real) de `temasBajoUmbral` (solo apaga el badge): optimizar por el segundo es maquillar.**
