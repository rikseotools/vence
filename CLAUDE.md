...
- **Mapa (frase → qué mira):**
  ...
  - *"revisa los descuadres de convocatoria"* → lo que mostramos (fecha de examen, plazas) no cuadra con el documento oficial, detectado por la reconciliación LLM contra el corpus → `verificar-convocatorias.md`. El hallazgo trae la **cita literal** y la URL. NUNCA auto-flip: el LLM alucina (caso real 16/07: inventó 130 plazas donde el BOCM decía "ciento siete (107)" — nuestro dato era el correcto).
