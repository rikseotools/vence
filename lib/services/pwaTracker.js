...
class PWATracker {
  ...
  async trackPWAEvent(eventType, additionalData = {}) {
    try {
      ...
      // Log para debug
      console.log('📱 PWA event data being inserted:', {
        eventType,
        fields: Object.keys(eventData),
        hasSessionId: 'sessionId' in eventData,
      ...
      if (error) {
        ...
        } else if (errorMsg.includes('sessionId')) {
          console.log('📱 PWA tracking: sessionId column issue - checking table schema', {
            error: errorMsg,
