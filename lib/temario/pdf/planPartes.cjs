...
/**
 * Nº de artículo, admitiendo las DOS grafías.
 *
 * El contenido que llega de `lib/api/temario/queries.ts` trae **`articleNumber`** (camelCase), pero
 * esto leía solo `article_number`: al partir una ley en tramos, el rango salía vacío y **todas las
 * partes se llamaban igual**, sin manera de distinguirlas al descargarlas. Lo destapó el test de
 * pre-generación de partes (30/07), no un usuario, porque la función está detrás de un flag apagado.
 * Se aceptan ambas en vez de elegir una: este módulo es `.cjs` compartido y lo alimentan varios
 * sitios; fallar en silencio por una mayúscula es justo lo que acaba de pasar.
 */
function numeroArticulo(a) {
  return a?.articleNumber != null ? a.articleNumber : a?.article_number
}
...
function etiqueta(nombre, articulos, partido) {
  ...
  const nums = articulos.map(numeroArticulo).filter(x => x != null)
