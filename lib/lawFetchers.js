  ...
  try {
    ...
    if (parsedSectionFilter && parsedSectionFilter.articleRange) {
      ...
      filteredQuestions = validQuestions.filter(q => {
        ...
        console.log(`📚 [FILTER] Pregunta: Art. ${articleNumber} ${isInRange ? '✅ INCLUIDA' : '❌ DESCARTADA'} (rango: ${startArticle}-${endArticle}${parsedSectionFilter.title === "PREÁMBULO Y TÍTULO PRELIMINAR" ? ' + Art. 0' : ''})`)
    ...
    if (selectedArticles && selectedArticles.length > 0) {
      ...
      filteredQuestions = filteredQuestions.filter(q => {
        ...
        console.log(`📄 [FILTER] Pregunta: Art. ${articleNumber} ${isIncluded ? '✅ INCLUIDA' : '❌ DESCARTADA'} (artículos seleccionados: ${selectedArticles.join(', ')})`)
