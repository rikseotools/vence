...
export function extractArticlesFromBOE(html) {
  ...
  // El ID puede ser: a1, a123, auno, acuao, aveinte, av, aprimero, regla1, etc.
  // Simplificado: cualquier div.bloque con id que empiece con "a" + alfanumérico, o "regla", o ordinal
  const articleBlockRegex = /<div[^>]*class="bloque"[^>]*id="(a[a-z0-9]+|regla\d+|primero|segundo|tercero|cuarto|quinto|sexto|s[eé]ptimo|octavo|noveno|d[eé]cimo)[^"]*"[^>]*>([\s\S]*?)(?=<div[^>]*class="bloque"|$)/gi

  let match
  while ((match = articleBlockRegex.exec(html)) !== null) {
    const blockContent = match[2] // El contenido está en grupo 2 ahora (grupo 1 es el ID)
    ...
    // Filtrar artículos suprimidos (contienen "(Suprimido)" en el contenido)
