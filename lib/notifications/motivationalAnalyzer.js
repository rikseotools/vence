...
export class MotivationalAnalyzer {
  ...
  async generateMotivationalNotifications() {
    try {
      ...
      // Progreso constructivo (reemplaza críticas de baja preparación)
...
.limit(1000)
  ...
  // Analizar progreso diario (racha de días estudiando)
  async analyzeDailyProgress(userData) {
    try {
      const { recentTests } = userData
      
      // Calcular días consecutivos estudiando
      const studyDates = [...new Set(recentTests.map(test => 
      ...
      const conditions = MOTIVATIONAL_NOTIFICATION_TYPES.daily_progress.conditions
      if (consecutiveDays < conditions.min_consecutive_days) return null

      // Calcular tiempo total
      const totalTimeSeconds = recentTests.reduce((sum, test) => sum + (test.total_time_seconds || 0), 0)
      const totalTimeFormatted = this.formatTime(totalTimeSeconds)

      const template = getRandomMessageTemplate('daily_progress')
      ...
      return {
        id: `motivational-daily-progress-${consecutiveDays}d`,
        type: 'daily_progress',
        ...
        consecutive_days: consecutiveDays,
        total_time: totalTimeFormatted,
        ...MOTIVATIONAL_NOTIFICATION_TYPES.daily_progress
      }
    } catch (error) {
      console.error('Error analizando progreso diario:', error)
      return null
    }
  }
