...
export async function fetchLawsList() {
  try {
    ...
    // Query más simple primero para obtener todas las leyes activas
    ...
    console.log(`📋 ${lawsData.length} leyes activas encontradas`)
    
    // Para cada ley, contar sus artículos con contenido
    const lawsWithStats = []
    ...
    for (const law of lawsData) {
      try {
        ...
        if (articlesError) {
          console.error(`❌ Error cargando artículos para ${law.short_name}:`, articlesError)
          continue
        }
        
        // Filtrar solo artículos numéricos reales
        ...
        if (validArticles.length > 0) {
          ...
          console.log(`✅ ${law.short_name}: ${validArticles.length} artículos válidos`)
        }
        
      } catch (innerError) {
        console.error(`❌ Error procesando ley ${law.short_name}:`, innerError)
      }
    }
    
    const sortedLaws = lawsWithStats
      .filter(law => law.articleCount > 0)
      .sort((a, b) => b.articleCount - a.articleCount)
    
    console.log(`✅ ${sortedLaws.length} leyes cargadas con teoría`)
    return sortedLaws
...
// ================================================================
export async function fetchLawSections(lawSlug) {
  try {
    console.log(`📚 Cargando secciones de ley: ${lawSlug}`)
    
    // Convertir slug a short_name
    const lawShortName = mapLawSlugToShortName(lawSlug)
    console.log(`🔍 Mapeo: "${lawSlug}" → "${lawShortName}"`)
    
    if (!lawShortName) {
      throw new Error(`Ley "${lawSlug}" no reconocida`)
    }
