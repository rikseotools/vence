...
export async function fetchLawsList() {
  try {
    ...
    // Procesar en JS - filtrar artículos válidos y contar
    const lawsWithStats = data
      .map(law => {
        // Filtrar solo artículos numéricos con contenido sustancial
        const validArticles = (law.articles || []).filter(article => {
          ...
          // Solo números puros (1, 2, 10, etc.)
          if (!/^\d+$/.test(articleNum.trim())) return false
          // Contenido sustancial (>50 chars)
          return article.content && article.content.trim().length > 50
        })
