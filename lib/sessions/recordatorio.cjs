...
// Orden de Manuel: «las frases no solo deben saltar en cada tarea, también cada x minutos para
// que no se le olviden». Los tres canales anteriores no alcanzan a un trabajador autónomo que
// encadena: el hook necesita PROMPTS (un `claude -p` no los tiene), el `pre-commit` solo dispara
// al estrenar ficheros, y `heartbeat` exige que se ACUERDE de lanzarlo — que es justo lo que
// falla. Éste se cuelga del reloj del servidor y lo comprueba cualquier comando del andamiaje:
// mientras encadena pasa por el CLI todo el rato, así que la cadencia sale sin pedirle nada.

/** Cada cuánto se repite. 30 min: por debajo es ruido, por encima cabe una tarea entera sin verlo. */
const CADA_MIN = 30

/**
 * ¿Toca recordárselo ya? PURA.
 *
 * @param {{ultimo:string|Date|null, ahora?:Date, cadaMin?:number}} p
 *   `ultimo` NULL significa «nunca se le ha recordado» → toca (así una sesión nueva lo ve enseguida).
 */
function tocaRecordar({ ultimo, ahora = new Date(), cadaMin = CADA_MIN }) {
  if (!ultimo) return true
  const min = (new Date(ahora).getTime() - new Date(ultimo).getTime()) / 60000
  return min >= cadaMin
}

/** El bloque tal cual se imprime, con la cabecera que dice por qué aparece otra vez. */
function bloqueMetodo({ cadaMin = CADA_MIN } = {}) {
  return [`📐 RECORDATORIO DE MÉTODO (cada ${cadaMin} min — para que no se olvide a media tarea):`,
          ...METODO.map((l) => `   ${l}`)]
}

module.exports = {
  METODO,
  CADA_MIN, tocaRecordar, bloqueMetodo,
  esHerramienta,
  palabrasDe,
