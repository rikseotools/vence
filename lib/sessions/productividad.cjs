...
// El estado de una entrega lo decide UN sitio (T-486): aquí solo se cuenta.
...
function medir({ cerradas = [], entregadas = [], ahora = new Date() } = {}) {
  ...
  // Solo las que AÚN ESPERAN REVISOR (T-486). Una ya revisada no es cola de revisión: su coste
  // pasó a ser una decisión, no un turno de trabajador. Contarlas juntas inflaba «pendientes» y
  // la mediana de espera con tareas que ya nadie tenía que mirar.
  const enCola = entregadas.filter((e) => REV.esperaRevision(e))
  const esperas = enCola
  ...
  const entregas = {
    ...
    deLaFlota: enCola.filter((e) => quienEs(e.review_requested_by) === 'trabajador').length,
    // Ya miradas y esperando DECISIÓN: es otra cola, con otro dueño (una persona) y otro remedio.
    conVeredicto: entregadas.filter((e) => REV.esperaDecision(e)).length,
  }
