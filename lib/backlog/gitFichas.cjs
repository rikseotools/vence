...
/**
 * ¿La ficha está commiteada en ALGUNA rama del repositorio, aunque no sea la mía ni `origin/main`?
 *
 * Es la pregunta que separa las dos situaciones que hasta ahora se veían idénticas y NO son la
 * misma cosa (T-445): «otra sesión la tiene commiteada en su rama, pendiente de fusionar» —el día
 * normal con 2-10 sesiones— y «se escribió y se perdió sin llegar a ningún commit» —trabajo
 * perdido que hay que reescribir a mano, como pasó con T-407—.
 *
 * Se nombra la rama porque sin ella el aviso obliga a repetir la búsqueda para poder actuar.
 *
 * @returns {{consultable: boolean, estuvo: boolean, donde: string|null}}
 *   `consultable:false` = git no contestó. Igual que en `hechosDeOrigin`, ahí se dice «no lo sé»:
 *   inventarse un «no está en ninguna parte» es acusar de una pérdida que puede no haber ocurrido.
 */
function enAlgunaRama(id, { cwd, mdRel = MD_REL_POR_DEFECTO } = {}) {
  // `--source` hace que git imprima, por cada commit, la ref por la que llegó a él. GOTCHA: con un
  // `--format` propio NO se emite sola — hay que pedirla con `%S`. Sin ese `%S` el comando
  // funciona, la clasificación sale bien y la ref viene vacía: un fallo que solo se ve si el test
  // comprueba el CONTENIDO del aviso, no solo el veredicto.
  const out = gitOut(['log', '--all', '--source', '--format=%h %S', '-S', marcaDe(id), '--', mdRel], { cwd })
  if (out === null) return { consultable: false, estuvo: false, donde: null }
  if (!out.length) return { consultable: true, estuvo: false, donde: null }
  // Formato de cada línea con --source: "<sha>\t<ref>". Se queda con la ref del commit más
  // reciente, que es el primero que imprime `git log`.
  const primera = out.split('\n')[0] || ''
  const ref = primera.split(/\s+/)[1] || null
  return { consultable: true, estuvo: true, donde: ref }
}
...
// ── VERSIÓN «UNA FICHA = UN FICHERO» (T-532) ──────────────────────────────────────────────────
//
// Las cuatro funciones de arriba buscan una MARCA (`### [T-nnn]`) dentro del historial de un
// fichero COMPARTIDO (`-S`, pickaxe). Con una ficha por fichero esa búsqueda deja de hacer falta:
// la pregunta «¿existió esta ficha?» pasa a ser «¿tiene commits este PATH?», que git contesta de
// forma nativa y sin ambigüedad de marcador — no hay pickaxe que gane el 1 MB de buffer ni marca
// que pueda aparecer citada dentro del título de otra ficha (el gotcha de `insertarFicha.cjs`).
//
// Se AÑADEN, no sustituyen: las de arriba siguen siendo la verdad para investigar historia
// ANTERIOR a la migración (todo lo que pasó mientras el contenido vivía en el monolito). El
// llamador nuevo (`sync`) usa estas; nada rompe lo que ya había.

const DIR_FICHAS_POR_DEFECTO = path.join('docs', 'roadmap', 'tareas')

/** Ruta (relativa al repo) del fichero de una ficha. */
function rutaDeFichero(id, dirFichas = DIR_FICHAS_POR_DEFECTO) {
  return path.join(dirFichas, `${id}.md`)
}

/**
 * ¿Tiene commits este PATH en mi historial LOCAL? Gemelo de `estuvoEnElHistorialLocal`, pero
 * preguntando por el fichero en vez de rastrear una marca dentro de otro. Mismo fail-open a
 * `false`: si git no contesta, no se inventa una regresión.
 */
function estuvoEnElHistorialLocalFichero(id, { cwd, dirFichas = DIR_FICHAS_POR_DEFECTO } = {}) {
  const ruta = rutaDeFichero(id, dirFichas)
  const out = gitOut(['log', '--format=%h', '--', ruta], { cwd })
  return out !== null && out.length > 0
}

/**
 * Lo que `origin/main` puede contestar sobre el FICHERO de una ficha. Misma forma de salida que
 * `hechosDeOrigin`, para que `fichaHuerfana.cjs` (la decisión, que no sabe nada de git) no tenga
 * que distinguir de dónde vinieron los hechos.
 *
 * `estaAhora` se comprueba con `cat-file -e` (existencia, sin traerse el contenido) en vez de
 * `show` + `includes(marca)`: no hay marca que buscar, el fichero ES la ficha.
 */
function hechosDeOrigenFichero(id, { cwd, dirFichas = DIR_FICHAS_POR_DEFECTO, ref = REF_POR_DEFECTO } = {}) {
  const noSe = { consultable: false, estaAhora: false, estuvo: false }
  if (!gitOut(['rev-parse', '--verify', '--quiet', `${ref}^{commit}`], { cwd })) return noSe
  const ruta = rutaDeFichero(id, dirFichas)
  // `cat-file -e` sale 0 si el blob existe en esa ref; gitOut ya traduce «no cero» a null.
  const existe = gitOut(['cat-file', '-e', `${ref}:${ruta}`], { cwd })
  const historial = gitOut(['log', '--format=%h', ref, '--', ruta], { cwd })
  if (historial === null) return noSe
  return {
    consultable: true,
    estaAhora: existe !== null,
    estuvo: historial.length > 0,
  }
}

/** Gemelo de `commitQueLaQuito`, sobre el PATH del fichero: el último commit de `ref` que lo tocó
 *  (si ya no existe, ES el commit que lo borró). */
function commitQueLaQuitoFichero(id, { cwd, dirFichas = DIR_FICHAS_POR_DEFECTO, ref = REF_POR_DEFECTO } = {}) {
  const ruta = rutaDeFichero(id, dirFichas)
  return gitOut(['log', '-1', '--format=%h %an — %s', ref, '--', ruta], { cwd }) || null
}

/** Gemelo de `enAlgunaRama`, sobre el PATH del fichero. */
function enAlgunaRamaFichero(id, { cwd, dirFichas = DIR_FICHAS_POR_DEFECTO } = {}) {
  const ruta = rutaDeFichero(id, dirFichas)
  const out = gitOut(['log', '--all', '--source', '--format=%h %S', '--', ruta], { cwd })
  if (out === null) return { consultable: false, estuvo: false, donde: null }
  if (!out.length) return { consultable: true, estuvo: false, donde: null }
  const primera = out.split('\n')[0] || ''
  const ref = primera.split(/\s+/)[1] || null
...
module.exports = {
  ...
  MD_REL_POR_DEFECTO,
  REF_POR_DEFECTO,
  // Variantes «una ficha = un fichero» (T-532)
  rutaDeFichero,
  estuvoEnElHistorialLocalFichero,
  hechosDeOrigenFichero,
  commitQueLaQuitoFichero,
  enAlgunaRamaFichero,
  DIR_FICHAS_POR_DEFECTO,
}
