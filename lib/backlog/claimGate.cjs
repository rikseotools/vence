...
function claimGate(task, sid, now = new Date(), openIds = new Set()) {
  ...
  if (task.review_requested_at) {
    const quien = task.review_requested_by ? ` (la dejó ${String(task.review_requested_by).slice(0, 14)})` : ''
    return {
      ok: false,
...
function isAwaitingVerification(task, now = new Date()) {
  ...
  if (task.review_requested_at) return false
...
function detectarTrabajoPendiente(outcome) {
  ...
  // pegada a la fecha.
  const esMedidaPasada = (texto, indice) => PASADO.test(texto.slice(Math.max(0, indice - 30), indice + 25))
