...
/**
 * Shas que cita un texto de entrega. Son los que el trabajador escribe para que otro los mire
 * («commit db2c44e2c», «rama flota/w1 (db2c44e2c)»), así que es lo que hay que comprobar.
 * Mínimo 7 caracteres: por debajo se casa cualquier palabra hexadecimal («added», «face»).
 */
function shasCitados(texto) {
  return [...new Set([...String(texto || '').matchAll(/\b([0-9a-f]{7,40})\b/g)].map((m) => m[1]))]
}

/**
 * ¿El trabajo entregado está A SALVO? (T-632)
 *
 * «A salvo» = el commit cuelga de alguna referencia REMOTA. Un commit que solo existe como objeto
 * —porque la rama que lo apuntaba se movió— se lo lleva el recolector de basura, y es exactamente
 * lo que le pasó a [T-560]: trabajo hecho, revisado con rigor, y evaporado.
 *
 * @param {string} entrega  el texto de `review_note`
 * @returns {{alcanzable:boolean|null, referencias:string[], sha:string|null}}
 *   `alcanzable: null` = no se pudo decidir (sin git, o la entrega no cita ningún sha).
 */
function trabajoASalvo(entrega) {
  const shas = shasCitados(entrega)
  if (!shas.length) return { alcanzable: null, referencias: [], sha: null }
  for (const sha of shas) {
    // ¿git lo reconoce como commit? Si no, no es un sha: es una palabra que lo parecía.
    if (!git(['rev-parse', '--verify', `${sha}^{commit}`])) continue
    const refs = git(['branch', '-r', '--contains', sha]).split('\n').map((s) => s.trim()).filter(Boolean)
    // Basta con que UNO de los citados esté a salvo: la entrega puede nombrar varios (el commit
    // y el de su padre, o el de una rama hermana) y exigir que TODOS lo estén daría falsos noes.
    if (refs.length) return { alcanzable: true, referencias: refs, sha }
  }
  // Se reconoció algún sha y ninguno cuelga de un remoto: eso SÍ es un «no».
  const reconocido = shas.find((s) => git(['rev-parse', '--verify', `${s}^{commit}`]))
  return reconocido ? { alcanzable: false, referencias: [], sha: reconocido } : { alcanzable: null, referencias: [], sha: null }
}

module.exports = { indiceDeRamas, hechosDeGit, idsDeclarados, shasCitados, trabajoASalvo }
