...
/**
 * ¿Alguna de las tareas que este push DECLARA como tuyas se va a publicar SIN ficha en el
 * markdown? (T-443, 05/08/2026)
 *
 * Nace de un hueco que ni [T-427] ni [T-428] cerraban: la ficha de [T-435] se escribió, `sync` la
 * reconcilió con la tabla… y `git log -S` no la encuentra en NINGUNA revisión — se perdió antes
 * de llegar a un commit. `lib/backlog/fichaHuerfana.cjs` ya distingue este caso como
 * `mia_sin_escribir`, pero solo se ve corriendo `sync` A MANO — nada lo asoma en el momento en
 * que de verdad importa, que es cuando el trabajo se comparte.
 *
 * AVISA, no bloquea, y a propósito: pushear trabajo en curso de una tarea reclamada ANTES de
 * terminar de escribir su ficha es el flujo normal (se sube código, se itera, la ficha se remata
 * al final) — bloquear cada push intermedio de cada tarea abierta dispararía casi siempre y
 * enseñaría a saltárselo (la misma lección de [T-375]). Lo que hacía falta no era una puerta más,
 * era que el aviso que YA EXISTE en `sync` dejara de depender de que alguien se acuerde de
 * correrlo.
 *
 * @param {string[]} referencedIds     ids que el push DECLARA (ya filtrados de citas, T-403)
 * @param {Map|Object} tasksById       id → { status, claimed_by }
 * @param {string} sid                 session-id de esta sesión
 * @param {string} mdHeadContent       contenido de `docs/roadmap/tareas-pendientes.md` en HEAD
 *                                     (lo que se va a publicar si el push sigue adelante)
 * @returns {string[]} ids mías, vivas, sin cabecera en el markdown que se va a publicar
 */
function fichaAusenteEnPush({ referencedIds, tasksById, sid, mdHeadContent }) {
  const get = (id) => (tasksById && typeof tasksById.get === 'function' ? tasksById.get(id) : tasksById && tasksById[id])
  const md = String(mdHeadContent || '')
  const ausentes = []
  for (const id of referencedIds || []) {
    const t = get(id)
    if (!t || t.claimed_by !== sid) continue          // no es mía → no me corresponde avisar
    if (!OPEN_STATUSES.includes(t.status)) continue    // cerrada → su ficha puede vivir en `## Hechas` distinto
    const tieneCabecera = new RegExp(`^###\\s+.*?\\[${id}\\]`, 'm').test(md)
    if (!tieneCabecera) ausentes.push(id)
  }
  return ausentes
}

module.exports = {
  extractTaskIds, parseGitLog, clasificarMenciones, evaluatePush, fichaAusenteEnPush,
