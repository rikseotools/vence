...
/** Fichero de UNA ficha, en el modelo «una ficha = un fichero» (T-532). */
const RE_FICHERO_FICHA = /^docs\/roadmap\/tareas\/T-\d+\.md$/

/**
 * ¿El push toca ÚNICAMENTE papeleo de fichas — el índice generado, y/o ficheros de ficha
 * individuales (T-532)?
 *
 ...
 *
 * El corte es deliberadamente estrecho: SOLO el índice y/o ficheros `docs/roadmap/tareas/T-nnn.md`.
 * Escribir un runbook en `docs/` sí es trabajo y sigue pidiendo claim. Y las ediciones
 * concurrentes de estos ficheros ya las caza git con un conflicto de fusión — no es este guard
 * quien lo protege.
 */
function esSoloFichas(changedFiles) {
  const f = (changedFiles || []).filter(Boolean)
  return f.length > 0 && f.every((p) => {
    const t = String(p).trim()
    return t === MD_BACKLOG || RE_FICHERO_FICHA.test(t)
  })
}
