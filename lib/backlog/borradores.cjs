...
/**
 * ── EL MENSAJE VA EN PLURAL, PORQUE LO FIRMA UN EQUIPO ───────────────────────────────────────
 * Norma de Manuel (05/08/2026, textual): «además he comprobado no, hemos comprobado, siempre en
 * plural». Es la firma «Equipo de Vence» vista desde dentro del texto: un «he comprobado» delata
 * a un individuo detrás y rompe justo lo que la firma construye.
 *
 * AVISA, NO BLOQUEA — a propósito. El borrador puede CITAR al usuario («dices que has probado
 * a…») y ahí el singular es correcto; un bloqueo por expresión regular impediría escribir esos.
 * Quien lo lee decide. Medido el 05/08: 4 de 26 borradores de la flota estaban en singular, y
 * ninguna capa lo miraba — ni el manual lo decía.
 *
 * @returns {string[]} las formas en singular encontradas (vacío si está bien)
 */
const SINGULAR = /(?:^|[^\wáéíóúñ])(he\s+(?:comprobado|revisado|verificado|mirado|corregido|analizado)|no\s+puedo|te\s+(?:confirmo|aseguro)|voy\s+a\s+\w+|considero\s+que)/gi

function primeraPersonaSingular(cuerpo) {
  return [...String(cuerpo || '').matchAll(SINGULAR)].map((m) => m[1].replace(/\s+/g, ' ').trim())
}

/** Lo que se le dice a quien escribió en singular. Corto: es un aviso, no una puerta. */
function avisoPlural(hallazgos) {
  return [
    `⚠️  el mensaje va en PRIMERA PERSONA DEL SINGULAR (${hallazgos.slice(0, 3).map((h) => `«${h}»`).join(', ')}).`,
    '   Lo firma «Equipo de Vence»: se escribe en plural — «hemos comprobado», «te confirmamos».',
    '   Si es una CITA de lo que dice el usuario, está bien: ignóralo.',
  ].join('\n')
}

module.exports = {
  claveDe, yaHayUno, mensajeDuplicado, sqlSinBorradorPendiente,
  primeraPersonaSingular, avisoPlural,
}
