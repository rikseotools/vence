...
const MOTIVOS = /** @type {const} */ ([
  'borrada', 'no_verificable', 'mia_sin_escribir', 'desactualizada', 'sin_pushear',
])
...
/**
 * @param {{id: string, estuvoEnElMarkdown?: boolean, esMia?: boolean, origen?: HechosDeOrigin}} huerfana
 ...
 * @returns {{id: string, motivo: 'borrada'|'no_verificable'|'mia_sin_escribir'|'desactualizada'|'sin_pushear', esRegresion: boolean, alcance: 'origin'|'local'|'ninguno'}}
 */
function clasificarHuerfana(huerfana) {
  ...
  if (origen && origen.consultable) {
    ...
    // Ver `mia_sin_escribir` abajo — el hueco que ni T-427 ni T-428 cubrían.
