 ...
 *  · `sin_pushear`    — no está en NINGUNA rama. Suele ser un id recién reservado cuya ficha aún se
 *    está escribiendo, pero es también la única forma que tiene de verse el trabajo PERDIDO (T-407).
 *  · `mia_sin_escribir` — la tarea la tengo YO reclamada y su ficha no está en ninguna parte.
 *  · `en_otra_rama`   — commiteada en la rama de otra sesión, pendiente de fusionar. El día normal
 *    con 2-10 sesiones, y lo único que hay que hacer es esperar a que la fusionen. Va el ÚLTIMO
 *    porque es el único donde no hay nada que hacer.
 *
 * `mia_sin_escribir` nace de un caso propio (31/07): se escribió la ficha de T-435, el `sync` la
 * reconcilió… y no llegó a ningún commit — `git log -S` no la encuentra en ninguna revisión. El
 * aviso la dio por `sin_pushear`, que era la clasificación CORRECTA (en el historial no estuvo
 * nunca) y por eso mismo sonó inofensiva. **Es un hueco que ni T-427 ni T-428 cubren: los dos
 * protegen contra BORRAR una ficha que ya existió, no contra que nunca llegue a existir.**
 * El dato que lo desambigua estaba a mano y no se usaba: si el claim es MÍO, «otra sesión no la ha
 * pusheado» es imposible — la sesión soy yo.
 *
 * `en_otra_rama` (T-445) cierra la otra mitad del mismo problema: «commiteada en la rama de otra
 * sesión» y «no está en ninguna parte» se veían idénticas, y una es el día normal mientras la otra
 * es trabajo perdido. Se comprueba ANTES que `mia_sin_escribir` porque una ficha que aparece en un
 * commit no puede estar «sin escribir»: existe, solo que todavía no aquí.
 */
const MOTIVOS = /** @type {const} */ ([
...
/**
 * @typedef {Object} HechosDeRamas
 * @property {boolean} consultable  ¿pudo git mirar TODAS las ramas? Si no, no se opina.
 * @property {boolean} estuvo       ¿aparece en algún commit de cualquier rama?
 * @property {string|null} donde    la ref donde se encontró, para poder decirlo en el aviso.
 */

/**
 * @param {{id: string, estuvoEnElMarkdown?: boolean, esMia?: boolean, origen?: HechosDeOrigin, ramas?: HechosDeRamas}} huerfana
 ...
 *   `ramas` = lo que dicen TODAS las ramas del repo. Solo se consulta cuando origin ya ha dicho que
 *   la ficha no está ni estuvo, y sirve para separar «en vuelo» de «perdida».
 * @returns {{id: string, motivo: 'borrada'|'no_verificable'|'mia_sin_escribir'|'desactualizada'|'sin_pushear'|'en_otra_rama', esRegresion: boolean, alcance: 'origin'|'local'|'ramas'|'ninguno', donde?: string|null}}
 */
function clasificarHuerfana(huerfana) {
  ...
  if (origen && origen.consultable) {
    ...
    // Si la ficha está commiteada en ALGUNA rama, EXISTE: no puede estar «sin escribir» ni
    // perdida, solo pendiente de fusionar. Por eso se pregunta antes que por el claim (T-445).
    const ramas = huerfana.ramas
    if (ramas && ramas.consultable && ramas.estuvo) {
      return { ...v(id, 'en_otra_rama', false, 'ramas'), donde: ramas.donde || null }
    }
    // Y si la tarea la tengo RECLAMADA yo, «otra sesión no la ha pusheado» es imposible: soy yo.
    // Ver `mia_sin_escribir` arriba — el hueco que ni T-427 ni T-428 cubrían.
    if (huerfana.esMia) return v(id, 'mia_sin_escribir', false, 'local')
    return v(id, 'sin_pushear', false, 'origin')
  }
 ...
 * `noVerificables` se mira a mano, `desactualizadas` se resuelve actualizando la rama, `enOtraRama`
 * se resuelve solo cuando esa sesión fusione (va con la ref, para poder ir a buscarla) y
 * `sinPushear` —que ya significa «en NINGUNA rama»— es el día normal salvo que sea trabajo perdido.
 * Los dos últimos van en una línea cada uno para no tapar a los primeros.
 * @param {Array<{id: string, estuvoEnElMarkdown?: boolean, origen?: HechosDeOrigin, ramas?: HechosDeRamas}>} huerfanas
 */
function clasificarHuerfanas(huerfanas) {
  ...
  return {
    todas,
    borradas: de('borrada'),
    miasSinEscribir: de('mia_sin_escribir'),
    noVerificables: de('no_verificable'),
    desactualizadas: de('desactualizada'),
    enOtraRama: todas.filter(h => h.motivo === 'en_otra_rama'),
