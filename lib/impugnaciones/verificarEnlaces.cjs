...
function verificarDocumento(html, { ancla, articulo, citas = [] } = {}) {
  ...
  if (ancla) {
    ...
    if (i === -1) {
      ...
      //
      // ⚠️ `Art.` ABREVIADO cuenta, y por no aceptarlo esto mentía (04/08, impugnación
      // `70110a29`). Las leyes viejas rotulan «Art. 20.», no «Artículo 20»: en la Ley
      // 1/1983 de Madrid el ancla #a20 es CORRECTA, pero al no reconocer la abreviatura
      // la regex saltaba al primer «artículo 23» que aparece DENTRO del texto del 20.1
      // («de conformidad con el artículo 23 del Estatuto…») y abortaba el envío
      // diciendo «lleva a Artículo 23». Un falso positivo con un diagnóstico seguro y
      // equivocado, que es peor que no comprobar: manda a corregir un enlace correcto.
      // Va SIN el flag `i` a propósito, y esto es lo que sostiene la comprobación: la
      // RÚBRICA del bloque lleva mayúscula («Art. 20.», «Artículo 53») mientras que las
      // referencias cruzadas del cuerpo van en minúscula («de conformidad con el
      // artículo 23…»). Distinguirlas por la mayúscula es lo que impide que un
      // artículo mencionado de pasada valide un ancla equivocada.
      const DESIGNACION = 'Art(?:[íi]culo|\\.)\\s+'
      const m = cabecera.match(
        new RegExp(`${DESIGNACION}(\\d+(?:\\s+(?:bis|ter|quater|quinquies))?(?:\\s+a\\s+\\d+)?)`),
      )
      tituloDelBloque = m ? `Artículo ${m[1].trim()}` : null
      if (articulo) {
        const casa = new RegExp(`${DESIGNACION}${articulo}\\b`).test(cabecera)
        if (!casa) {
          problemas.push(
