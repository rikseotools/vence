...
/**
 * El SQL que resuelve el estado de un puñado de claves — UNA consulta POR TABLA, a propósito.
 *
 ...
 *
 * ── POR QUÉ NO ES UN SOLO `UNION ALL` (era así hasta el 06/08/2026) ──────────────────────────
 * Bajo el rol de la flota (`vence_coordinacion`), `user_feedback` está sin GRANT a propósito —
 * es tabla de negocio con PII (T-581), y nunca se le concede—. Con las tres tablas en un único
 * `UNION ALL`, un permiso denegado en ESA tabla tumbaba la consulta ENTERA, aunque las otras dos
 * sí eran legibles. El fail-open de `marcarEmbudo`/`marcarCasosCerradosEnEmbudo` se comía el
 * error en silencio, así que el aviso de T-614 —el que existe para no volver a mandarle a un
 * usuario real el mismo correo dos veces— nunca llegó a dispararse para un trabajador de la
 * flota: NINGÚN caso, aunque estuviera cerrado desde `question_disputes`. Medido el 06/08/2026
 * con un `DATABASE_URL` real de trabajador: `permission denied for table user_feedback`, en la
 * consulta combinada, para cualquier clave. Separarlas es el mismo principio que ya usa
 * `tocaFeedback` en `cola.cjs` (T-581): un permiso denegado en una tabla no puede callar lo que
 * las otras SÍ pueden decir.
 */
function sqlsEstadoDeCasos() {
  const cols = 'left(id::text, 8) AS clave, status::text AS status'
  return [
    { tbl: 'question_disputes', sql: `SELECT ${cols} FROM public.question_disputes WHERE left(id::text, 8) = ANY($1)` },
    { tbl: 'psychometric_question_disputes', sql: `SELECT ${cols} FROM public.psychometric_question_disputes WHERE left(id::text, 8) = ANY($1)` },
    { tbl: 'user_feedback', sql: `SELECT ${cols} FROM public.user_feedback WHERE left(id::text, 8) = ANY($1)` },
  ]
}

/**
 * Resuelve el estado de las claves citadas contra las tres colas, cada una POR SU CUENTA: si una
 * tabla no es legible con el rol de quien pregunta, las otras dos siguen respondiendo (ver
 * `sqlsEstadoDeCasos`). Es el único sitio que hace este viaje a la BD — los dos lectores
 * (`backlog.cjs`, `flota.cjs`) solo llaman a esto y envuelven el resultado en su propio fail-open.
 *
 * @param {{unsafe: (sql: string, params: any[]) => Promise<any[]>}} sql  cliente `postgres` conectado
 * @param {string[]} claves
 * @returns {Promise<Array<{clave, status}>>}
 */
async function estadosDeCasos(sql, claves) {
  if (!claves || !claves.length) return []
  const tandas = await Promise.all(
    sqlsEstadoDeCasos().map(({ sql: q }) => sql.unsafe(q, [claves]).catch(() => [])),
  )
  return tandas.flat()
}
...
module.exports = {
  ...
  casosCitadosEn, clavesDeCasos, marcarCasosCerrados, sqlsEstadoDeCasos, estadosDeCasos,
  avisoCasoCerrado, CERRADOS,
}
