...
function puedeCerrar({ claimedBy, claimedAt, sesiones, sid, igualmente, ahora = new Date() } = {}) {
  ...
  //
  // ⚠️ SIN ESCAPE, a propósito (T-609, 06/08/2026). Antes `igualmente` se comprobaba ANTES que
  // esto, así que un `--igualmente "<motivo>"` se saltaba TAMBIÉN el claim vivo de otra sesión —
  // no solo «nadie la tiene». Medido el 06/08: cuatro cierres de la misma ráfaga usaron
  // `--igualmente` contra una reserva VIVA de `colas-06ago` y mandaron tres correos con un texto
  // que Manuel había vetado ocho minutos antes. Mismo criterio que [T-375] en el backlog: un
  // guard deja de bloquear lo que NO se puede satisfacer, pero un lease vivo SÍ se puede
  // satisfacer — hablando con la otra sesión, o esperando — así que sigue bloqueando.
