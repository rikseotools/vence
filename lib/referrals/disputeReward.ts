...
import { shouldRewardResolvedDispute } from './logic'
...
export interface DisputeRewardResult {
  granted: boolean
  reason?: 'not_resolved' | 'not_premium' | 'not_user_source' | 'duplicate' | 'monthly_cap' | 'error'
  amount?: number
}
...
export async function maybeRewardResolvedDispute(params: {
  ...
  try {
    ...
    const db = getReadDb()
    // Plan del usuario + origen de la impugnación. `source` solo existe en las legislativas; las
    // psicotécnicas son siempre de usuario (no hay pipeline `ai_auto` para ellas), de ahí el 'user'.
    ...
    if (!shouldRewardResolvedDispute({ status: params.status, source, planType, userId: params.userId })) {
      return { granted: false, reason: source !== 'user' ? 'not_user_source' : 'not_premium' }
    }
