...
export async function fetchQuickQuestions(tema, searchParams, config) {
  try {
    ...
    const numQuestions = parseInt(searchParams?.get?.('n')) || config?.numQuestions || 10
...
export async function fetchOfficialQuestions(tema, searchParams, config) {
  try {
    ...
    const numQuestions = parseInt(searchParams?.get?.('n')) || config?.numQuestions || 20
    const positionType = config?.positionType || 'auxiliar_administrativo'
...
export async function fetchAleatorioMultiTema(themes, searchParams, config) {
  try {
    ...
    const excludeRecent = searchParams.get('exclude_recent') === 'true'
    const excludeDays = parseInt(searchParams.get('exclude_days')) || 15
    const onlyOfficialQuestions = searchParams.get('official_only') === 'true'
    const focusEssentialArticles = searchParams.get('focus_essential') === 'true'
