...
export async function fetchPersonalizedQuestions(tema, searchParams, config) {
  try {
    ...
    if (configParams.excludeRecent && user) {
      ...
      const { data: recentAnswers, error: recentError } = testIds.length > 0 ? await supabase
        .from('test_questions')
    ...
    const { data: userAnswers, error: answersError } = testIds.length > 0 ? await supabase
      .from('test_questions')
      .select('question_id, created_at, test_id')
      .in('test_id', testIds)
      .order('created_at', { ascending: false })
      : { data: null, error: null }
...
export async function fetchQuestionsByTopicScope(tema, searchParams, config) {
  try {
    ...
    const focusWeakAreas = config?.focusWeakAreas ?? (searchParams.get('focus_weak') === 'true') // 🎯 ÁREAS DÉBILES (prioriza config)
...
async function fetchMantenerRachaFallback(n, user) {
  try {
    ...
    if (user) {
      ...
      const { data: userQuestionHistory, error: historyError } = testIds.length > 0 ? await supabase
        .from('test_questions')
        .select(`
          articles!inner(
            laws!inner(short_name)
...
export async function fetchAleatorioMultiTema(themes, searchParams, config) {
  try {
    ...
    // ✅ OPTIMIZACIÓN: Query en dos pasos para evitar timeout del JOIN
    // Paso 1: Obtener IDs de los tests del usuario (rápido, usa índice en user_id)
    console.log('🔄 Paso 1/2: Obteniendo tests del usuario...')
    ...
    const testIds = userTests?.map(t => t.id) || []
    console.log(`✅ Tests del usuario encontrados: ${testIds.length}`)

    if (testIds.length === 0) {
      console.log('⚠️ Usuario sin historial de tests, usando selección aleatoria')
      const selectedQuestions = filteredQuestions
        .sort(() => Math.random() - 0.5)
        .slice(0, configParams.numQuestions)
      return transformQuestions(selectedQuestions)
    ...
    // Paso 2: Obtener respuestas de esos tests (rápido, usa índice en test_id)
    console.log('🔄 Paso 2/2: Obteniendo respuestas de los tests...')
    ...
    filteredQuestions.forEach(question => {
      ...
      if (isAnswered) {
        // Pregunta ya respondida - agregar fecha para ordenamiento
        question._lastAnswered = questionLastAnswered.get(question.id)
        question._priority = 2 // Marca de prioridad para selección proporcional
        answeredQuestions.push(question)
      } else {
        // Pregunta nunca vista - máxima prioridad
        question._priority = 1 // Marca de prioridad para selección proporcional
        neverSeenQuestions.push(question)
      }
    })
    ...
    // 🎯 NUEVO: Aplicar selección proporcional para multi-tema
    // Solo aplicar si hay múltiples temas seleccionados
    if (themes.length > 1) {
      console.log('📊 APLICANDO SELECCIÓN PROPORCIONAL para multi-tema')

      // Usar la función de selección proporcional
      // Nota: La función ya maneja la priorización internamente
      selectedQuestions = selectQuestionsProportionally(
        themes,
        filteredQuestions, // Usar todas las preguntas filtradas (incluye nunca vistas y respondidas)
        configParams.numQuestions
      )

      console.log(`✅ Selección proporcional completada: ${selectedQuestions.length} preguntas`)
    } else if (neverSeenQuestions.length >= configParams.numQuestions) {
      // ✅ 1º PRIORIDAD: Si hay suficientes preguntas nunca vistas, usar solo esas
    ...
    // Solo mezclar si NO es selección proporcional (ya viene mezclada)
    if (themes.length === 1) {
      selectedQuestions = shuffleArray(selectedQuestions)
    }
...
// =================================================================
/**
 * Selecciona preguntas de forma proporcional entre los temas seleccionados
 * Prioriza preguntas nunca vistas dentro de cada tema
 * @param {Array} themes - Array de temas seleccionados
 * @param {Array} allQuestions - Todas las preguntas disponibles con source_topic
 * @param {Number} numQuestions - Número total de preguntas a seleccionar
 * @returns {Array} Preguntas seleccionadas proporcionalmente
 */
function selectQuestionsProportionally(themes, allQuestions, numQuestions) {
  try {
    console.log('📊 SELECCIÓN PROPORCIONAL ACTIVADA')
    ...
    // Calcular cuántas preguntas por tema
    const questionsPerTheme = Math.floor(numQuestions / themes.length)
    const remainder = numQuestions % themes.length

    console.log(`📐 Distribución base: ${questionsPerTheme} preguntas por tema`)
    console.log(`📐 Preguntas extra (resto): ${remainder}`)

    // Agrupar preguntas por tema, separando nunca vistas de respondidas
    const questionsByTheme = {}

    themes.forEach(theme => {
      const themeQuestions = allQuestions.filter(q => q.source_topic === theme)

      // Separar nunca vistas de respondidas (detectar por la presencia de _priority)
      const neverSeen = themeQuestions.filter(q => q._priority === 1) // Nunca vistas tienen prioridad 1
      const answered = themeQuestions.filter(q => q._priority === 2) // Respondidas tienen prioridad 2
      ...
      console.log(`📚 Tema ${theme}: ${themeQuestions.length} total (${neverSeen.length} nunca vistas, ${answered.length} respondidas)`)
    })

    // Verificar disponibilidad y ajustar si es necesario
    const availability = themes.map(theme => ({
      theme,
      available: questionsByTheme[theme].total,
      neverSeen: questionsByTheme[theme].neverSeen.length,
      answered: questionsByTheme[theme].answered.length,
      needed: questionsPerTheme
    }))

    // Ordenar temas por disponibilidad (más preguntas primero) para asignar extras
    const sortedThemes = [...availability].sort((a, b) => b.available - a.available)

    // Asignar preguntas extra a los temas con más disponibilidad
    let extrasAssigned = 0
    sortedThemes.forEach(item => {
      if (extrasAssigned < remainder && item.available > item.needed) {
        item.needed++
        extrasAssigned++
      }
    })

    // Seleccionar preguntas proporcionalmente con prioridad a nunca vistas
    const selectedQuestions = []
    let totalSelected = 0

    availability.forEach(({ theme, available, neverSeen, answered, needed }) => {
      const themeData = questionsByTheme[theme]
      let toSelect = needed

      // Si no hay suficientes, tomar todas las disponibles
      if (available < needed) {
        toSelect = available
        console.log(`⚠️ Tema ${theme}: solo ${available} disponibles de ${needed} solicitadas`)
      } else {
        console.log(`✅ Tema ${theme}: seleccionando ${toSelect} preguntas`)
      }

      // Seleccionar con prioridad: primero nunca vistas, luego respondidas
      let themeSelected = []

      if (neverSeen >= toSelect) {
        // Caso ideal: suficientes nunca vistas
        const shuffled = [...themeData.neverSeen].sort(() => Math.random() - 0.5)
        themeSelected = shuffled.slice(0, toSelect)
        console.log(`   ✅ ${toSelect} nunca vistas seleccionadas`)
      } else {
        // Combinar nunca vistas + respondidas más antiguas
        const shuffledNeverSeen = [...themeData.neverSeen].sort(() => Math.random() - 0.5)
        const answeredNeeded = toSelect - neverSeen

        // Las respondidas ya están ordenadas por antigüedad (_lastAnswered)
        const selectedAnswered = themeData.answered.slice(0, answeredNeeded)

        themeSelected = [...shuffledNeverSeen, ...selectedAnswered]
        console.log(`   ⚠️ ${neverSeen} nunca vistas + ${selectedAnswered.length} respondidas`)
      }

      selectedQuestions.push(...themeSelected)
      totalSelected += themeSelected.length
    })

    // Si faltan preguntas (porque algún tema no tenía suficientes), compensar
    if (totalSelected < numQuestions) {
      console.log(`⚠️ Faltan ${numQuestions - totalSelected} preguntas, compensando...`)

      // Recopilar preguntas no seleccionadas de todos los temas
      const remainingQuestions = allQuestions.filter(q =>
        !selectedQuestions.some(sq => sq.id === q.id)
      )

      // Mezclar y agregar las que faltan
      const shuffledRemaining = [...remainingQuestions].sort(() => Math.random() - 0.5)
      const additionalNeeded = numQuestions - totalSelected
      const additional = shuffledRemaining.slice(0, additionalNeeded)

      selectedQuestions.push(...additional)
      console.log(`✅ Agregadas ${additional.length} preguntas adicionales para completar`)
    }
    ...
    console.error('❌ Error en selección proporcional:', error)
    // Fallback: selección aleatoria normal
