...
export async function fetchQuestionsByTopicScope(tema, searchParams, config) {
  try {
    ...
    if (user) {
      ...
      if (!answersError && userAnswers && userAnswers.length > 0) {
        ...
        if (neverSeenCount >= numQuestions) {
          ...
          const cutoffDate = new Date(Date.now() - (excludeRecent ? recentDays * 24 * 60 * 60 * 1000 : 7 * 24 * 60 * 60 * 1000))
          ...
          console.log(`🔍 DEBUG REPASO: ${uniqueAnswered.length} respondidas → ${eligibleForReview.length} elegibles para repaso (${excludeRecent ? recentDays : 7} días mínimo)`)
    ...
    if (needsAdaptiveCatalog && user && userAnswers) {
      ...
      console.log('🧠 CATÁLOGO ADAPTATIVO GENERADO:')
      console.log(`   📊 Nunca vistas: easy=${catalogByDifficulty.neverSeen.easy.length}, medium=${catalogByDifficulty.neverSeen.medium.length}, hard=${catalogByDifficulty.neverSeen.hard.length}`)
