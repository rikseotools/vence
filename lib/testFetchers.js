...
export async function fetchQuestionsByTopicScope(tema, searchParams, config) {
  try {
    ...
    // 🔍 DEBUG: Mostrar configuración detallada
    console.log('🔍 DEBUG selectedLaws:', selectedLaws)
    console.log('🔍 DEBUG selectedArticlesByLaw:', selectedArticlesByLaw)
    ...
    // 🔍 DEBUG: Mostrar mappings detallados
    console.log('🔍 DEBUG mappings completos:', mappings.map(m => ({
      lawShortName: m.laws.short_name,
      lawId: m.laws.id,
      articleNumbers: m.article_numbers
    })))
    
    // 🆕 FILTRAR MAPEOS POR LEYES SELECCIONADAS
    let filteredMappings = mappings
    if (selectedLaws.length > 0) {
      console.log('🔍 DEBUG: Iniciando filtrado por leyes...')
      filteredMappings = mappings.filter(mapping => {
        const lawShortName = mapping.laws.short_name
        const isIncluded = selectedLaws.includes(lawShortName)
        console.log(`🔍 DEBUG: Ley "${lawShortName}" ${isIncluded ? '✅ INCLUIDA' : '❌ EXCLUIDA'}`)
        return isIncluded
      })
    ...
    if (Object.keys(selectedArticlesByLaw).length > 0) {
      console.log('🔍 DEBUG: Iniciando filtrado por artículos...')
      filteredMappings = filteredMappings.map(mapping => {
        const lawShortName = mapping.laws.short_name
        const selectedArticles = selectedArticlesByLaw[lawShortName]
        
        console.log(`🔍 DEBUG: Procesando ley "${lawShortName}"`)
        console.log(`🔍 DEBUG: Artículos disponibles en mapping:`, mapping.article_numbers)
        console.log(`🔍 DEBUG: Artículos seleccionados en config:`, selectedArticles)
        ...
        if (selectedArticles && selectedArticles.length > 0) {
          ...
          const filteredArticleNumbers = mapping.article_numbers.filter(articleNum => {
            const isIncluded = selectedArticlesAsStrings.includes(articleNum)
            console.log(`🔍 DEBUG: Artículo ${articleNum} ${isIncluded ? '✅ INCLUIDO' : '❌ EXCLUIDO'}`)
            return isIncluded
          })
          ...
          console.log(`🔍 DEBUG: Artículos finales para consulta:`, filteredArticleNumbers)
        ...
        console.log(`🔍 DEBUG: No hay filtro de artículos para "${lawShortName}", usando todos los artículos`)
        return mapping
    ...
    for (const mapping of filteredMappings) {
      console.log(`🔍 Consultando ${mapping.laws.short_name} con artículos:`, mapping.article_numbers)
      
      // 🔍 DEBUG: Mostrar consulta SQL que se va a ejecutar
      console.log(`🔍 DEBUG SQL: Buscando preguntas donde:`)
      console.log(`  - is_active = true`)
      console.log(`  - articles.laws.short_name = "${mapping.laws.short_name}"`)
      console.log(`  - articles.article_number IN [${mapping.article_numbers.join(', ')}]`)
      ...
      console.log(`🔍 DEBUG: Consulta para "${mapping.laws.short_name}" devolvió ${lawQuestions?.length || 0} preguntas`)
      if (lawQuestions && lawQuestions.length > 0) {
        console.log(`🔍 DEBUG: Primeras preguntas encontradas:`)
        lawQuestions.slice(0, 3).forEach((q, i) => {
          console.log(`  ${i + 1}. ID: ${q.id}, Artículo: ${q.articles?.article_number}, Ley: ${q.articles?.laws?.short_name}`)
        })
      }
