...
function buildExamPositionFilter(positionType) {
  ...
  // Construir filtro: exam_position IS NULL OR exam_position IN (valores)
  // En Supabase: 'exam_position.is.null,exam_position.in.(val1,val2,...)'
  const escaped = validPositions.map(p => p.replace(/,/g, '\\,')).join(',')
  return `exam_position.is.null,exam_position.in.(${escaped})`
}
