...
export async function fetchAleatorioMultiTema(themes, searchParams, config) {
  try {
    ...
    // Paso 2: Obtener respuestas de esos tests (usa batching para evitar URLs largas)
    console.log('🔄 Paso 2/2: Obteniendo respuestas de los tests...')
    const { data: allUserAnswers, error: allAnswersError } = await batchedTestQuestionsQuery(
      testIds,
      'question_id, created_at, test_id',
      { order: { column: 'created_at', ascending: false }, limit: 5000 }
    )
