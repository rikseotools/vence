...
/**
 * El plan de saneamiento [T-465]: de TODAS las filas de verificación de preguntas activas,
 * ¿cuáles hay que limpiar (poner `article_ok`/`answer_ok` a NULL) y de qué preguntas son?
 *
 * PURA a propósito: la usa `scripts/calidad/sanear-verificacion-cosmetica.cjs` para decidir qué
 * escribir, y así el criterio se puede fijar con tests sin tocar la BD.
 *
 * @param {Array<{id, question_id, explanation, article_ok, answer_ok}>} filas TODAS las filas de
 *   `ai_verification_results` de preguntas ACTIVAS (el caller filtra por `questions.is_active`).
 * @returns {{aLimpiar: Array<{fila, flags:string[]}>, preguntasAfectadas: Set<string>}}
 */
function calcularSaneamiento(filas) {
  ...
  const aLimpiar = []
  const preguntasAfectadas = new Set()
  for (const [qid, lista] of porPregunta) {
    if (!soloVerificadaPorPasesCosmeticos(lista)) continue
    for (const fila of lista) {
      const veredicto = clasificarFirma(fila)
      if (veredicto.infractora) {
        aLimpiar.push({ fila, flags: veredicto.flags })
        preguntasAfectadas.add(qid)
      }
    }
  }
  return { aLimpiar, preguntasAfectadas }
}

module.exports = {
  RE_COSMETICO, FLAGS_DE_FONDO, clasificarFirma, soloVerificadaPorPasesCosmeticos,
  calcularSaneamiento,
}
