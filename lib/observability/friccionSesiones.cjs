...
const CLASES = {
  ...
  // T-682. Hermano de `deploy_espera` y por la misma razón: contesta la misma pregunta —¿cuánto
  // nos cuesta trabajar en paralelo?—. Nace de medir por qué el VPS estaba al 98,7 % de presión de
  // memoria con la CPU al 0 %: cuatro `tsc --noEmit` a la vez, 3,8 GB, en una máquina de 7,7. Con
  // el candado puesto, **este contador es el que dice si sobra máquina o falta**: si las esperas
  // son raras y cortas, el candado basta; si son constantes, el cuello está antes y no se arregla
  // serializando.
  typecheck_espera: 'una sesión esperó al candado del typecheck',
  ...
  // T-568. Mismo piso que `identidad_compartida`: una entrada de `.git/config` COMÚN a los 10+
  // worktrees, así que cualquier sesión que la pise apaga `pre-commit`/`pre-push` para TODAS a la
  // vez, en silencio (git no ejecuta hooks si el valor no resuelve a un directorio real — no
  // bloquea nada, deja de proteger nada). Lo detecta y arregla `latir.cjs` en cada latido, porque
  // un hook no puede curarse a sí mismo si es precisamente lo que dejó de dispararse.
  hookspath_corrupto: 'core.hooksPath (compartido) apuntaba a un sitio sin hooks reales',
  // T-687. El latido de PRESENCIA (`worktree_sessions.last_signal_at`) es el único escritor de
  // esa tabla y por diseño nunca falla hacia fuera — así que un fallo dentro de su `main()` (BD
  // caída, `resolverSid()` roto, una consulta mala) se lo traga el `.catch()` de más afuera SIN
  // dejar rastro: se invoca sin `--verbose` y con `stdio: 'ignore'`. Con la marca local de
  // `lib/sessions/latidoFallo.cjs`, el siguiente latido que SÍ tiene éxito convierte ese silencio
  // en un número (intentos, minutos) en vez de perderlo. Es `warn`: mientras dura, el reparto
  // puede dar por muerta a una sesión que sigue trabajando (el caso real que abrió esta ficha).
  latido_fallido: 'el latido de presencia (worktree_sessions) falló y se recuperó — cuánto estuvo callado',
}
 ...
 *  · `hookspath_corrupto` (T-568): mientras dura, NINGÚN hook corre en NINGÚN worktree — cero
 *    protección, no un aviso puntual. Se auto-repara sola en el siguiente latido; el evento es lo
 *    único que deja constancia de cuánto tiempo estuvo así y con qué frecuencia se repite.
 */
const SEVERIDAD = { guard_escape: 'warn', identidad_compartida: 'warn', hookspath_corrupto: 'warn', latido_fallido: 'warn' }
...
function ratioEscape(eventos, { minimo = 4 } = {}) {
  ...
  return [...por.values()]
    .map((g) => {
      ...
      // ── SIN UN SOLO BLOQUEO, ESTE RATIO NO PUEDE DAR OTRA COSA QUE 100% (T-702) ───────────
      // El denominador es `bloqueos + escapes`. Un guardarraíl que no emite `guard_bloqueo`
      // cuando se lo saltan sale SIEMPRE al 100%, o sea en la banda «muerto», haga lo que haga:
      // el veredicto no describe la realidad, describe qué eventos emite. Pasó con la puerta de
      // temario, que llevaba 0 bloqueos y 9 escapes en 30 días y el panel prescribía quitarla —
      // cuando sus 9 escapes eran 2 decisiones razonadas y bien escritas. Antes de juzgar una
      // puerta hay que poder distinguir «nadie la respeta» de «no cuenta cuando para a alguien».
      const veredicto = total < minimo ? 'sin_datos'
        : g.bloqueos === 0 ? 'no_medible'
        : ratio >= 2 / 3 ? 'muerto'
        : ratio >= 0.25 ? 'erosion'
        : 'sano'
...
function diagnostico(g) {
  ...
  switch (g.veredicto) {
    ...
    case 'no_medible': return `⚪ ${g.guard}: ${g.escapes} escape(s) y NI UN bloqueo registrado. ` +
      'No se puede juzgar: sin bloqueos el ratio da 100% siempre. Comprueba si emite `guard_bloqueo` al pararte.'
 ...
 *
 * ── DOS DEFECTOS DE ESTA CUENTA, ENCONTRADOS AL AUDITARLA (T-702, 08/08) ────────────────────
 *
 * **(1) La resta puede ser aritméticamente forzosa.** `preventivos = escapes - bloqueos` supone
 * que un guardarraíl que bloquea EMITE ese bloqueo. La puerta de temario no podía: con el escape
 * puesto retornaba antes de evaluar nada, así que llevaba **0 bloqueos y 9 escapes en 30 días** y
 * el panel la daba por «rodeada el 100%, ya no protege, quítala» — un número que no podía dar otro
 * valor dijera lo que dijera la realidad. Por eso ahora, si el evento trae `evitoBloqueo`, MANDA
 * ese dato: es la respuesta medida a la única pregunta que importa (*¿había algo que rodear?*), no
 * una resta. `fuente` dice cuál de las dos se usó, para que nadie vuelva a borrar una puerta
 * apoyándose en una inferencia sin saber que lo es.
 *
 * **(2) Repetir una decisión no son varias decisiones.** Los 9 escapes de temario eran **2**: uno
 * el 05/08 y otro el 06/08 aplicado **siete veces en seis minutos** a impugnaciones HERMANAS del
 * mismo caso (el último decía literalmente «mismo motivo que las hermanas»). Contar siete castiga
 * a quien trabaja un lote, que es justo donde más se repite un escape legítimo. Se agrupa por
 * (guard, sesión, motivo): la misma razón escrita dos veces es UNA decisión.
 *
 * @param eventos [{ clase, guard, sid, motivo?, evitoBloqueo? }]
 * @returns Array<{ guard, escapes, decisiones, preventivos, ratioPreventivo, fuente }>
 */
function escapesSinBloqueo(eventos) {
  const norm = (m) => String(m || '').toLowerCase().replace(/\s+/g, ' ').trim().slice(0, 120)
  const medido = new Map() // guard -> { medidos, sinNadaQueRodear }
  const por = new Map()
  for (const e of eventos || []) {
    if (!e || !e.guard || !e.sid) continue
    if (e.clase !== 'guard_bloqueo' && e.clase !== 'guard_escape') continue
    const k = `${e.guard}\u0000${e.sid}`
    if (!por.has(k)) por.set(k, { guard: e.guard, bloqueos: 0, escapes: new Set(), brutos: 0 })
    const v = por.get(k)
    if (e.clase === 'guard_bloqueo') { v.bloqueos++; continue }

    v.brutos++
    // Sin motivo no se puede agrupar: cada escape cuenta solo (no se inventa un parecido).
    v.escapes.add(norm(e.motivo) || `${v.brutos}`)

    if (typeof e.evitoBloqueo === 'boolean') {
      if (!medido.has(e.guard)) medido.set(e.guard, { medidos: 0, sinNadaQueRodear: 0 })
      const m = medido.get(e.guard)
      m.medidos++
      if (!e.evitoBloqueo) m.sinNadaQueRodear++
    }
  }
  const porGuard = new Map()
  for (const v of por.values()) {
    if (!porGuard.has(v.guard)) porGuard.set(v.guard, { guard: v.guard, escapes: 0, decisiones: 0, preventivos: 0 })
    const g = porGuard.get(v.guard)
    g.escapes += v.brutos
    g.decisiones += v.escapes.size
    // Cota inferior, como siempre: los bloqueos se descuentan de las DECISIONES, no de las repeticiones.
    g.preventivos += Math.max(0, v.escapes.size - v.bloqueos)
  }
  return [...porGuard.values()]
    .filter((g) => g.escapes > 0)
    .map((g) => {
      const m = medido.get(g.guard)
      // Solo se cree la medición si cubre TODOS los escapes de ese guard: a medias mezclaría dos
      // formas de contar en la misma cifra, que es como se fabrica un número que no significa nada.
      const completa = !!m && m.medidos >= g.escapes
      const preventivos = completa ? m.sinNadaQueRodear : g.preventivos
      const base = completa ? g.escapes : g.decisiones
      return {
        ...g,
        preventivos,
        fuente: completa ? 'medida' : 'inferida',
        ratioPreventivo: base > 0 ? Math.round((preventivos / base) * 100) / 100 : 0,
      }
    })
