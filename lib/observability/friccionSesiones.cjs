...
const CLASES = {
  ...
  // T-443. Distinta familia de las dos primeras: `guard_bloqueo`/`guard_escape` son de guards que
  // PARAN y se pueden rodear; un `guard_aviso` (codigo-push-guard, backlog-push-guard/ficha
  // ausente) NUNCA bloquea, así que no tiene ratio de escape —no hay nada que rodear—. Se cuenta
  // aparte para poder responder, con datos y no a ojo, si el umbral calibrado en
  // `sim-codigo-guard.cjs` sigue siendo el que interesa dentro de unos meses.
  guard_aviso: 'un guardarraíl NO bloqueante avisó (no hay bloqueo que rodear)',
}
