...
const SONDA_GIT = (arbol) => [
  ...
  'echo ADELANTE=$(git rev-list --count origin/main..HEAD 2>/dev/null || echo -1)',
...
function evaluarClon(sonda) {
  ...
  if (s.sucio > 0 && s.adelante > 0) {
    return { estado: 'divergido', puedeEncargar: false, hayQueActualizar: false, motivo: `${s.sucio} fichero(s) sin commitear y ${s.adelante} commit(s) que no están en origin/main` }
  }
  if (s.sucio > 0) {
  ...
  if (s.adelante > 0) {
    return { estado: 'adelantado', puedeEncargar: false, hayQueActualizar: false, motivo: `${s.adelante} commit(s) suyos no están en origin/main` }
  }
  if (s.atras > 0) {
 ...
 */
const severidad = (v) => (v.estado === 'al_dia' ? 'info' : v.estado === 'atrasado' ? 'info' : 'error')
