 ...
 *
 * El árbol se pasa desde el registro de máquinas (`arbolDe`) porque **la ruta no es la misma en
 * todas**: bajo el `$HOME` del usuario en el portátil, bajo `~flota` en el VPS. En los dos casos es
 * el WORKTREE PROPIO del trabajador, nunca un clon compartido — probar el compartido (T-592) hacía
 * que esto viera SIEMPRE "al día", porque ahí nadie escribe.
 */
const SONDA_GIT = (arbol) => [
  ...
  // ¿Puede AVANZAR sin fabricar un merge? Solo si su HEAD no tiene nada que `origin/main` no
  // tenga. Sin esto se intentaba `pull --ff-only` sobre la rama propia del trabajador —que casi
  // siempre lleva un commit suyo— y fallaba SIEMPRE, reportándose como «no se sabe» (05/08, l3).
  'echo FUERA_DE_MAIN=$(git rev-list --count origin/main..HEAD 2>/dev/null || echo -1)',
...
/**
 * Devolver al código VIGENTE a un trabajador que se quedó en su rama de la tarea anterior.
 *
 * Solo se usa cuando su árbol está limpio y **todos sus commits ya están en un remoto**, así que no
 * puede perder nada: su rama sigue existiendo. Sin esto, un trabajador arrastra para siempre el
 * código del día en que hizo su primera tarea — medido el 05/08: `l1` informó de que el comando
 * `backlog.cjs borrador` «no existe en este checkout», cuando llevaba horas en `main`.
 *
 * Es el mismo daño que la puerta del clon venía a evitar, reintroducido por dejar de tocarles la
 * rama. Empezar cada tarea desde `origin/main` es además lo que el propio encargo les pide.
 *
 * ⚠️ **No se hace `checkout main`**: git no permite la MISMA rama en dos worktrees a la vez, y
 * `main` la tiene el árbol principal — el intento muere con «'main' is already used by worktree at
 * …» (05/08). Se usa una rama propia por trabajador reposicionada en `origin/main`. El `-B` la
 * reescribe, y eso solo es seguro porque aquí ya se ha comprobado que no queda nada sin empujar;
 * su rama anterior sigue existiendo, local y en el remoto.
 */
const ORDEN_A_MAIN = (arbol, trabajador) =>
  `cd ${arbol} && git checkout -q -B flota/${trabajador} origin/main && git rev-parse --short HEAD`
...
function leerSonda(salida) {
  ...
  return { fetch: campo('FETCH'), head: campo('HEAD'), atras: num('ATRAS'), adelante: num('ADELANTE'),
    fueraDeMain: num('FUERA_DE_MAIN'), sucio: num('SUCIO') }
}
...
function evaluarClon(sonda, { reanuda = false } = {}) {
  ...
  if (s.fetch !== 'ok' || s.atras === null || s.adelante === null || s.fueraDeMain === null) {
  ...
  if (s.atras > 0) {
    // Actualizar SOLO si el avance es directo. Un trabajador en su propia rama, con su commit ya
    // empujado, está perfectamente y no hay nada que moverle: lo que necesitaba —`origin/main` al
    // día— ya lo hizo el `fetch` de la sonda. Su rama la reconcilia él cuando le toque.
    if (s.fueraDeMain === 0) {
      return { estado: 'atrasado', puedeEncargar: true, hayQueActualizar: true, motivo: `${s.atras} commit(s) por detrás de origin/main` }
    }
    // Se le devuelve a `main`: su árbol está limpio y sus commits ya están en un remoto (los dos
    // casos contrarios se han descartado arriba), así que no puede perder nada — su rama sigue ahí.
    // Quedarse en ella significaba arrastrar el código del día de su primera tarea.
    return {
      estado: 'en_su_rama', puedeEncargar: true, hayQueActualizar: true, volverAMain: true,
      motivo: `venía de su rama con ${s.fueraDeMain} commit(s) propio(s) ya a salvo — vuelve a main al día`,
    }
  }
 ...
 */
// ── LA CALIBRACIÓN, CORREGIDA CON LA SERIE DELANTE ──────────────────────────────────────────
// Primera versión: todo lo que no fuera «al día» era `error`. Medido a las 24 h: **53 errores en un
// día**, casi todos por trabajadores con trabajo sin commitear — que NO es una avería, es el
// sistema comportándose bien y rehusando pisar algo que puede ser único. Una alerta que salta 53
// veces al día por operación normal se ignora, y con ella se ignoran las que sí importan.
//
// La línea es «¿hay avería, o solo hay que mirar algo?»:
//   · `info`  — se arregla solo aquí mismo (al día, actualizado, viene de su rama, retoma lo suyo)
//   · `warn`  — hay trabajo que alguien tiene que mirar, pero nada está roto
//   · `error` — no se puede saber, o no hay clon: eso sí es que algo falla
const severidad = (v) => {
  if (['al_dia', 'atrasado', 'a_medias', 'en_su_rama'].includes(v.estado)) return 'info'
  if (['sucio', 'adelantado', 'divergido'].includes(v.estado)) return 'warn'
  return 'error'
}
...
module.exports = { SONDA_GIT, ORDEN_ACTUALIZAR, ORDEN_A_MAIN, leerSonda, evaluarClon, severidad, diagnostico }
