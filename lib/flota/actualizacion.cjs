...
function evaluarClon(sonda, { reanuda = false } = {}) {
  ...
  if (s.atras > 0) {
    ...
    return {
      estado: 'en_su_rama', puedeEncargar: true, hayQueActualizar: false,
      motivo: `en su rama, con ${s.fueraDeMain} commit(s) propio(s) ya a salvo; origin/main queda al día tras el fetch`,
    }
  }
