...
/**
 * Devolver a `main` a un trabajador que se quedó en su rama de la tarea anterior.
 *
 ...
 * rama. Empezar cada tarea desde `main` es además lo que el propio encargo les pide.
 */
const ORDEN_A_MAIN = (arbol) =>
  `cd ${arbol} && git checkout -q main && git merge --ff-only -q origin/main && git rev-parse --short HEAD`
