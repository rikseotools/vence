...
const MAQUINAS = {
  ...
  'flota-1': {
    // ── LOS TRABAJADORES LOS LEVANTA SYSTEMD, SIEMPRE (T-642/T-647, 07/08) ──────────────
    // No es una preferencia de estilo: decide EN QUÉ CGROUP acaban los turnos, y de eso depende
    // que se les pueda poner un límite de memoria. Medido ese día: el supervisor arrancaba las
    // sesiones él mismo (`tmux new-session`) por estar en la misma máquina, así que el servidor
    // de tmux —y con él TODOS los `claude -p`— colgaba del cgroup del SUPERVISOR. Consecuencias:
    // un límite en `vence-flota@w1` no habría acotado nada, y cuando un turno se desbocó a 6,6 GB
    // el kernel mató al supervisor, que era quien «gastaba» esa memoria. Con systemd, cada
    // trabajador vive en su propio cgroup y responde de lo suyo.
    //
    // El portátil no la declara porque allí no hay unidades: la sesión es de Manuel, no del
    // sistema.
    systemd: true,
    ...
    // ── ESTA MÁQUINA no puede empujar, tenga quien la llame credenciales o no — HISTÓRICO ───
    // Es una propiedad DE LA MÁQUINA, no de quien pregunta — y esa distinción es la que faltaba
    // cuando esto se escribió (T-628, 06/08). `necesitaSegundaFase` (rescate.cjs) usaba `.local`
    // para decidir si el push del propio trabajador iba a funcionar, pero `.local` responde "¿el
    // que llama está en la misma máquina que el trabajador?", no "¿esta máquina tiene con qué
    // empujar?". Coinciden siempre en el portátil (quien llama TIENE sus credenciales) pero NO
    // coincidían aquí en su momento: el supervisor systemd corre con `VENCE_FLOTA_AQUI=flota-1`
    // (T-617), así que para él `maquinaDe('w1').local` daba `true` — y con la lógica vieja eso
    // bastaba para decir "no hace falta rematar, su propio push vale", cuando ese push NO
    // funcionaba entonces. Esa distinción `.local` vs `tieneCredencialesGit` sigue siendo
    // correcta y se conserva — lo que cambió es el VALOR de abajo.
    //
    // ── T-486/T-628, 08/08: REVERTIDO A `true` — MEDIDO, no la ficha ────────────────────────
    // La máquina SÍ tiene credenciales de git hoy: `~/.ssh/id_ed25519` existe (usuario `flota`,
    // uid 1000, compartido por los cuatro trabajadores — no hay aislamiento por trabajador en
    // esta VPS) y funciona. Reproducido en vivo: `ssh -T git@github.com` responde
    // "Hi rikseotools! You've successfully authenticated" y `git fetch origin` desde el clon
    // base (`~/vence`) tiene éxito (exit 0). Además, esta MISMA sesión (w1) empujó con éxito a
    // `origin/flota/w1` más de una decena de veces en este turno con ese mismo usuario/clave. No
    // se pudo verificar un PUSH real desde el clon BASE compartido específicamente (el guard de
    // "una sesión por directorio" del pre-commit y el push-guard del backlog bloquean cualquier
    // intento de prueba ahí, correctamente — protegen contra justo ese uso), pero la autorización
    // SSH es por USUARIO del sistema operativo, no por directorio, así que el mismo par
    // clave/cuenta que empuja desde el worktree de w1 empuja igual desde `~/vence`. SOSPECHO
    // (no confirmado, no hace falta más para este arreglo) que alguien añadió esta clave entre
    // el 06/08 (cuando T-628 la declaró ausente) y el 08/08 sin actualizar ni la ficha ni este
    // registro — el fichero de la clave privada tiene fecha 05/08 19:25.
    tieneCredencialesGit: true,
  },
}
 ...
 *
 * ── EL TIPO EQUIVOCADO LANZA, NO DEVUELVE `null` ([T-712], 08/08) ──────────────────────────
 * `null` significa UNA cosa: «ese trabajador no está declarado en ninguna máquina», que es un
 * estado normal (un nombre viejo, un trabajador retirado) y el llamador lo salta con un
 * `continue`. Pasarle un OBJETO era un error de programación y salía por la misma puerta, así
 * que se saltaba igual — en silencio y para siempre.
 *
 * Es exactamente lo que pasó: la sonda de salud de máquina del bucle recorría
 * `trabajadoresQueReciben()`, que devuelve `{ trabajador, maquina }`, y le pasaba **el objeto
 * entero**. `maquinaDe` no lo reconocía, devolvía `null`, el bucle hacía `continue` y **la
 * máquina no se medía NUNCA**. El panel llamaba bien (`f.trabajador`), así que el mismo código
 * medía perfectamente cuando una persona ejecutaba `npm run flota` y no medía nada sola: un
 * falso verde en la herramienta que vigila a las demás. Medido: 439 pasadas del bucle y UN solo
 * evento `flota_maquina_salud`, el que emitió una persona a mano.
 *
 * Con esto ese error es imposible de cometer callando, y se arregla en el punto por el que pasan
 * los once llamadores en vez de en cada uno.
 */
function maquinaDe(trabajador, reg = MAQUINAS) {
  if (typeof trabajador !== 'string') {
    throw new TypeError(
      `maquinaDe() espera el NOMBRE del trabajador ("w1"), no ${
        trabajador && typeof trabajador === 'object' ? JSON.stringify(trabajador) : String(trabajador)
      }. Si vienes de trabajadoresEsperados()/trabajadoresQueReciben(), desestructura: ({ trabajador }).`,
    )
  }
 ...
 */
/**
 * De todas las filas de `worktree_sessions` de un mismo trabajador, LA VIGENTE: la que ha dado
 * señal más recientemente. Devuelve un Map `slug → fila`.
 *
 * ── POR QUÉ HACE FALTA, y no es defensivo (T-667, 07/08/2026) ────────────────────────────────
 * Un trabajador acumula una fila por cada identidad que ha tenido: al reinstalarlo, al recrear su
 * worktree, al cambiarle el `.session-id`. Medido en el VPS: **`w1` y `w2` tenían DOS filas cada
 * uno**, la vieja del 05/08 y la viva de hoy. Quien resolvía el sid hacía
 * `new Map(sesiones.map((s) => [s.slug, s.sid]))` — se queda con **la ÚLTIMA que llegue**, y la
 * consulta **no lleva `ORDER BY`**, así que cuál gana es arbitrario. Salió la del 05/08.
 *
 * Lo que rompe no es cosmético: con el sid equivocado, `conTarea.has(sid)` da **false** para un
 * trabajador que SÍ tiene tarea cogida, así que el supervisor lo cuenta como libre, no reconoce su
 * turno muerto y no le devuelve SU tarea — que es el fallo que más caro sale (`w1` estuvo seis
 * horas con T-168 cogida sin que nadie la retomara). Y al revés: con `estado`, una fila rancia
 * puede hacer pasar por «callado» a quien acaba de latir.
 *
 * Es la regla que el sistema ya tiene escrita —**la señal de vida manda sobre la antigüedad**—
 * aplicada donde se estaba incumpliendo. Sin señal pierde siempre contra cualquiera que la tenga.
 */
function sesionVigente(sesiones) {
  const m = new Map()
  for (const s of sesiones || []) {
    if (!s || !s.slug) continue
    const previa = m.get(s.slug)
    if (!previa) { m.set(s.slug, s); continue }
    const t = s.last_signal_at ? new Date(s.last_signal_at).getTime() : -Infinity
    const p = previa.last_signal_at ? new Date(previa.last_signal_at).getTime() : -Infinity
    if (t > p) m.set(s.slug, s)
  }
  return m
}

function comparar(sesiones, { ahora = new Date(), calladoMin = 15, reg = MAQUINAS } = {}) {
  const vivas = sesionVigente(sesiones)
...
module.exports = {
  ...
  puedeDesplegar, tieneCredencialesGit, comparar, sesionVigente, esLocal, inalcanzable,
}
