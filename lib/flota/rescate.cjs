...
function ordenRescate({ arbol, trabajador, conGuardas = true }) {
  ...
  return [
    ...
    'for R in $(git for-each-ref --format="%(refname:short)" refs/heads); do ' +
      ...
      'echo RAMA=$RAMA; ' +
      // ── Y DE QUÉ RAMA VIENE, para poder empujarla desde OTRA máquina (T-628) ─────────────
      // En el VPS este `git push` falla siempre: los trabajadores no tienen credenciales de
      // git, así que el rescate encontraba el trabajo y lo dejaba donde estaba (medido el
      // 06/08: 11 ramas atrapadas, una de ellas con un bug de producción). El portátil SÍ
      // puede empujar, pero necesita saber QUÉ ref traerse y CON QUÉ NOMBRE dejarla — y ese
      // nombre tiene que ser el MISMO, no uno recalculado allí: dos generadores del mismo
      // nombre divergen (T-130). Por eso se emite el par, y el namer sigue siendo este único.
      'echo ORIGEN=$R\\|$RAMA; ' +
...
/**
 * ¿Hay que rematar el rescate DESDE EL PORTÁTIL? (T-628)
 *
 ...
 *
 * **No se hace nunca sobre una máquina local**: allí el push del propio rescate es el bueno, y
 * repetirlo desde fuera sería un segundo camino al mismo sitio.
 *
 * @param {{local?:boolean}} maquina
 * @param {{salvado:number|null, pares:Array}} parsed
 */
function necesitaSegundaFase(maquina, parsed) {
  if (!maquina || maquina.local) return { hace_falta: false, motivo: 'máquina local: su propio push es el bueno' }
