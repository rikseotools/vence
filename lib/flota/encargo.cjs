...
// El criterio de «qué es una devolución» NO se reescribe aquí: ya vive en `lib/backlog/revision.cjs`
// y lo comparten `claim`, `list` y el `parte`. Un cuarto criterio para lo mismo es exactamente
// cómo nacieron las dos puertas que se contradicen de [T-665].
const REV = require('../backlog/revision.cjs')
...
/**
 * ¿Esta tarea es una devolución que se le puede dar YA a un trabajador? (T-700, 08/08)
 *
 * Es el criterio de `devueltaConProblemas` MÁS la única condición que el reparto añade: que no la
 * tenga nadie cogida. No se reimplementa el veredicto — se delega, para que el día que cambie qué
 * cuenta como devolución, cambie en un sitio.
 */
function esCorreccionPendiente(task) {
  if (!task) return false
  if (task.claimed_by) return false
  return REV.devueltaConProblemas(task)
}

/**
 * Encargo para RETOMAR una tarea que volvió con veredicto `problemas`. (T-700, 08/08)
 *
 * ── POR QUÉ NO VALE EL ENCARGO NORMAL ────────────────────────────────────────────────────────
 * Es la misma lección que ya costó cinco borradores del mismo caso en las impugnaciones y que hizo
 * nacer `encargoRevision`: con el encargo de siempre («tu tarea es T-nnn») el trabajador **la
 * rehace desde cero**, porque nada en el texto le dice que ya está hecha y que lo que falta es
 * arreglar tres cosas concretas. El veredicto —que es el trabajo de OTRO trabajador— se perdería
 * justo en el momento en que sirve para algo.
 *
 * Los hallazgos van ENTEROS y no recortados, por lo mismo que `retomarTrasProblemas` dejó de
 * recortarlos ([T-518]): los veredictos reales miden 2.400-4.600 caracteres y **terminan** por los
 * problemas, así que cortar por el final es cortar precisamente lo que hay que arreglar.
 */
function encargoCorreccion({ trabajador, tarea, hallazgos = '', revisor = null, puedeDesplegar = false }) {
  return [
    `Eres ${trabajador}, un TRABAJADOR de la flota de Vence. Trabajas solo, sin nadie mirando.`,
    '',
    'Tu encargo NO es una tarea nueva: es **ARREGLAR una entrega que la revisión devolvió**.',
    '',
    `   tarea:  ${tarea.id} — ${tarea.title}`,
    revisor ? `   la revisó: ${revisor}` : '',
    '',
    '⚠️ YA ESTÁ HECHA EN SU MAYOR PARTE. No empieces de cero: si la rehaces, tiras el trabajo que',
    '   ya está bien Y el de quien la revisó. Lo que se te pide es cerrar los huecos de abajo.',
    '',
    'ESTO ES LO QUE LA REVISIÓN ENCONTRÓ — es tu lista de trabajo, punto por punto:',
    ...String(hallazgos || '(sin hallazgos registrados: míralo con `claim` y pregunta en el embudo)')
      .split('\n').map((l) => `   │ ${l}`),
    '',
    'CÓMO SE HACE:',
    `  1. node scripts/backlog.cjs claim ${tarea.id}      # te imprime la ficha entera: léela`,
    '  2. Busca su rama (`git branch -r | grep ' + tarea.id + '`) y ponte encima de lo que ya hay.',
    '  3. Arregla CADA hallazgo. Si crees que alguno NO es cierto, dilo por escrito con la prueba',
    '     (el comando, la cifra, la consulta) — un desacuerdo demostrado es una respuesta válida;',
    '     ignorarlo en silencio no, porque el siguiente revisor lo volverá a encontrar.',
    '  4. Vuelve a entregarla:',
    `       node scripts/backlog.cjs revision ${tarea.id} --entrega "qué has arreglado de cada hallazgo"`,
    '',
    'Y SIGUE EL MÉTODO DE LA CASA, que es contra lo que se te va a volver a juzgar:',
    ...METODO.map((l) => `  ${l}`),
    '',
    ...COLA_COMUN({ puedeDesplegar }),
  ].filter((l) => l !== '').join('\n')
...
/**
 * La SONDA de que la orden de arranque podrá ejecutarse de verdad — y no es paranoia (T-663, 07/08).
 *
 * `ordenDeArranque` produce `systemctl restart vence-flota@wN`, y el supervisor corre como un
 * usuario NORMAL. Un usuario normal no puede gestionar una unidad del sistema salvo que polkit se
 * lo permita: sin la regla, la orden falla SIEMPRE con «Interactive authentication required», y el
 * supervisor cree que lo intentó. Medido en el VPS: `w1` estuvo **6 h muerto con su tarea cogida**
 * —contado como ocupado, así que tampoco recibía trabajo nuevo— y el diario tenía el rastro exacto.
 *
 * La sonda es **una unidad INEXISTENTE con el prefijo de la flota**, y eso es lo que la hace
 * utilizable: polkit autoriza ANTES de que systemd busque la unidad, así que la respuesta separa
 * las dos causas sin reiniciar nada de nadie —«not found» = el permiso está; «authentication
 * required» = no está—. Comprobar leyendo el fichero de reglas no vale: puede existir, estar mal
 * escrito y no conceder nada.
 *
 * @param {string} salida  lo que escupió `systemctl restart vence-flota-sonda-inexistente.service`
 * @returns {{puede:boolean, motivo:string}}
 */
function permisoDeResurreccion(salida) {
  const s = String(salida || '')
  if (/interactive authentication required|not authorized|access denied/i.test(s)) {
    return {
      puede: false,
      motivo:
        'el supervisor NO puede relanzar a un trabajador muerto: systemctl le pide autenticación ' +
        'interactiva. Instala la regla de polkit (scripts/flota/arrancar-trabajador.sh la pone).',
    }
  }
  if (/not found|no such file/i.test(s)) return { puede: true, motivo: 'polkit autoriza; la unidad de sonda no existe, que es lo esperado' }
  return { puede: true, motivo: 'sin señal de bloqueo en la respuesta' }
}

/** La orden de la sonda. Unidad inexistente A PROPÓSITO: no reinicia el turno de nadie. */
const SONDA_RESURRECCION = 'systemctl restart vence-flota-sonda-inexistente.service'
...
module.exports = { NO_APTAS, flotaCogeImpugnaciones, encargoRevision, encargoCorreccion, esCorreccionPendiente, esApta, elegir, encargo, encargoImpugnacion, COLA_COMUN, puedeRecibir, arrancoDeVerdad, presenciaDelPanel, ordenDeArranque, permisoDeResurreccion, SONDA_RESURRECCION, SHELLS, avisoTrabajoAMedias }
