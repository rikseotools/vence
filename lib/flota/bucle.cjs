 ...
 *
 * @param {{repartidos:number, cada?:number, anterior?:number}} p
 * @returns segundos
 */
function siguientePausa({ repartidos, cada = CADA_S, anterior = null }) {
  if (repartidos > 0) return cada                       // hay movimiento: ritmo normal
  const base = anterior && anterior > 0 ? anterior : cada
  return Math.min(CADA_MAX_S, Math.round(base * 1.5))   // flota llena o sin trabajo: espaciar
}
...
module.exports = {
  ...
  siguientePausa, puedeRepartir, turnosAtascados, resumenPasada,
}
