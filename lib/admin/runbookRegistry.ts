...
export const RUNBOOK_BY_KIND: Record<string, RunbookEntry> = {
  ...
  seguimiento_fuente_error: {
    title: 'Seguimiento de convocatoria en error: el cron ni siquiera obtiene un HTTP 2xx',
    triggerPhrase: 'revisa los seguimientos en error',
    runbook: 'docs/maintenance/oeps-convocatorias-seguimiento.md',
    claudeHace:
      'para cada oposición señalada, `oposiciones.seguimiento_change_status = \'error\'`: el cron FALLA al comprobar la seguimiento_url (DNS roto, timeout, TLS caído, conexión rechazada, HTTP 4xx/5xx). A diferencia de `seguimiento_url_stale` (URL al ciclo equivocado) y `seguimiento_fuente_ciega` (responde 200 pero sin contenido), aquí ni siquiera llega la respuesta — y ningún otro detector lo miraba: `seguimiento_fuente_ciega` descarta el caso `error` a propósito asumiendo "ya visible", y no lo era. Primer paso: `curl -sL` con las cabeceras EXACTAS del cron (`CABECERAS_CRON` en `lib/convocatoria/seguimientoVigilable.cjs`) para reproducir el fallo y clasificarlo — 403/WAF pide fetcher headless (`oposiciones.fetcher_type`), 404/DNS pide una URL nueva. Repuntar SIEMPRE con `node scripts/seguimiento/repuntar-url.cjs <slug> <url> --anclas "…"`, que rehúsa escribir una URL no vigilable. NUNCA repuntar sin abrir la página. `error` = la oposición tiene convocatoria con ficha viva (estado con proceso concreto en marcha) y el cron la deja ciega a sus cambios; `warn` = sin ficha viva todavía, el resto del radar puede seguir cazando la convocatoria nueva por otra vía pero conviene arreglarlo antes de que salga.',
  },
  texto_examen_pasado: {
  ...
  pregunta_duplicada: {
    title: 'La MISMA pregunta repetida en el banco (mismo artículo, enunciado y las 4 opciones)',
    triggerPhrase: 'revisa las preguntas duplicadas',
    runbook: 'docs/runbooks/salud-contenido.md',
    claudeHace:
      'distingue las DOS bandas antes de tocar nada, con el MISMO criterio que `opciones_duplicadas` (compara el TEXTO de la respuesta correcta, nunca `correct_option`: las copias vienen barajadas entre sí). `error` = las gemelas dan respuestas DISTINTAS — irresoluble para el opositor, se repara hoy verificando la clave contra la FUENTE oficial (no a ojo: el caso real que motivó esto eran clones a los que se les cambió un detalle del enunciado y se les dejó la clave de la variante original — la explicación de la gemela mala describe la OTRA versión). `warn` = misma respuesta, solo repetición: se jubila el clon peor con `scripts/calidad/duplicados-exactos.cjs` (criterio de superviviente: oficial > explicación estructurada > más servida > más antigua), NUNCA a mano ni las dos a la vez. Jubilar es SIEMPRE `admin_duplicate_of` → `retired_duplicate` vía `transition_question_state` — es TERMINAL, así que simula primero (sin `--aplicar`) y lee la salida. El corte de este kind es ESTRICTO a propósito (enunciado normalizado + las 4 opciones idénticas): el corte BORROSO (variantes con un detalle distinto, tipo «Access»/«Access 365») sigue siendo bajo demanda (`--parafraseadas`, `--misma-clave`), no automatizable sin criterio humano. NUNCA jubilar una pregunta de EXAMEN OFICIAL ni dejar un artículo por debajo de 4 preguntas activas — el script ya lo protege, pero verifícalo si tocas algo a mano.',
  },
  ...
  scope_title_boundary_overflow: {
    ...
    runbook: 'docs/runbooks/verificar-epigrafes-scope.md',
    claudeHace: 'corre el runner on-demand `npx tsx scripts/scope/sim-title-boundary.ts <position_type> [topic]` (núcleo puro lib/laws/scopeTitleBoundary.js sobre la estructura título→rango del índice del BOE): lista los artículos escopados cuyo TÍTULO no aparece en el epígrafe (caso raíz LOSU/Mario 24/07: art.1 = Título Preliminar y art.6 = Título III en un epígrafe que solo pide Título I, II y IX Cap I). Desde T-333 (06/08/2026) también prueba SECCIÓN y SUBSECCIÓN (lib/laws/parseBoeSections.js#parseSeccionesSubsecciones, deliberadamente separado del poblador de law_sections — ver comentario en el fichero) cuando el epígrafe nombra bloques que el nivel externo título/capítulo no puede discriminar (caso real reproducido contra la API BOE en vivo: Ley 9/2017 Tema 22, cuyo Título Preliminar entero —arts.1-27— queda "sin overflow" porque a nivel título no hay nada que partir, y el problema real —la Sección 2.ª "Negocios y contratos excluidos" no está en el epígrafe— solo se ve un peldaño más abajo). El runner imprime TODOS los niveles con overflow, no solo uno: MEDIDO que el nivel más fino no siempre acierta (TRLGSS T15: una sección que el epígrafe PARAFRASEA en corto —"Afiliación y cotización" vs. la rúbrica real "Afiliación al sistema y altas, bajas..."— sale como candidato sin serlo). OJO: es alta-recall/baja-precisión (no pinga badge) — muchos epígrafes nombran el bloque por su MATERIA o en prosa corta, no por su número ni citando la rúbrica literal, así que hay falsos positivos en cualquier nivel; ADJUDICA cada candidato: verifica contra el BOE si el epígrafe nombra ese título/sección por número, por rúbrica, O por una paráfrasis razonable del capítulo que lo contiene. Solo si el epígrafe no lo pide de NINGUNA forma, recorta esos artículos del article_numbers (las preguntas quedan en BD, dejan de servirse en ese tema) + revalida cache temario/test-counts. Distinto de la sobre-inclusión (aquí el scope es AJUSTADO, solo se cuela 1-2 arts en la frontera); lo introduce verify:scope al razonar por rango contiguo en vez de por pertenencia real al título. NUNCA recortes un artículo que el epígrafe sí pide, ni por parecido de vocabulario.',
  },
