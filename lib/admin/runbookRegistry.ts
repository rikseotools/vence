...
export const RUNBOOK_BY_KIND: Record<string, RunbookEntry> = {
  ...
  orphan_inactive_article: {
    title: 'Artículo inactivo Y sin escopar, pero con preguntas activas — invisible por partida doble',
    triggerPhrase: 'revisa los artículos huérfanos inactivos',
    runbook: 'docs/runbooks/verificar-epigrafes-scope.md',
    claudeHace: 'para cada artículo señalado, decide primero si entra en el temario de ALGUNA oposición: lee el epígrafe oficial correspondiente y comprueba si su materia lo pide. Si SÍ entra: añádelo al `topic_scope` del tema que corresponda (criterio de `verificar-epigrafes-scope.md`) y SOLO ENTONCES reactívalo con `node scripts/reactivar-articulo-boe.cjs "<ley>" "<art>"` (da el veredicto tipado — identico/erratas/reordenado/incompleto/contaminado — contra el bloque VIGENTE del BOE; el script bloquea a propósito reactivar sin scope, porque seguiría sin servirse). Si NO entra en ningún temario: jubila las preguntas (no reactives el artículo solo para que deje de sonar la alarma). NUNCA reactives un artículo sin haberlo escopado antes: la reactivación sin scope no cambia nada de lo que ve el usuario.',
  },
