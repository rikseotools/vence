...
const { norm, words, recall, RE_NEGATIVA, RE_META } = require('./vinculoArticuloVecino.cjs')
...
const RE_INSTRUMENTO =
  /\b(plan(es)? (estrategico|estrategicos|de igualdad|nacional|director|de accion)|estrategia (nacional|andaluza|estatal|espanola)|informe de evaluacion de impacto)/
...
function clasificarInstrumentoDerivado({
  ...
  // Exclusiones heredadas: en la negación el desajuste es POR DISEÑO, y las meta-opciones
  // («todas son correctas») tienen recall cero contra cualquier texto.
  if (RE_NEGATIVA.test(norm(enunciado))) return no('negacion')
  ...
  if (medible && recallPropio >= UMBRAL_RESPONDE) {
