 ...
 */
const RE_NEGATIVA =
  /\b(incorrect|falsa|excepto|salvo\b|se[ñn]ale\s+la\s+que\s+no|cu[áa]l\s+de\s+.{0,90}?\bno\b|entre\s+(los|las)\s+que\s+no|no\s+(es\s|ser[áa]|procede|corresponde|tendr[áa]|tienen?\s|podr[áa]|deber[áa]|incluye|se\s+aplica|forma\s+parte|figura|cabe|constituye|requiere|exige|est[áa]|atribuye|se\s+encuentra|aparece|contempla|recoge|comprende))/i
