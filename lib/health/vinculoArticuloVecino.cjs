 ...
 *
 * La lista de verbos tras «no» se amplió el 29/07 tras un piloto: la primera versión solo cubría
 * «no es / será / procede / corresponde» y se le coló «el escenario de ingresos **no tendrá en
 * cuenta**:» (LGP art. 28), donde la opción correcta cita el art. 29 precisamente porque es lo
 * EXCLUIDO. Al añadir un verbo aquí, acuérdate de que el coste de un falso negativo (no marcar algo
 * que estaba mal) es MUCHO menor que el de un falso positivo: esta cola se adjudica a mano.
 ...
 *
 * ⚠️ SEGUNDA ampliación (01/08/2026, T-465). Se escapaban dos enunciados con MUCHAS exposiciones:
 *   · «¿Cuál de las siguientes funciones la Constitución Española NO ATRIBUYE al Gobierno?» (87 exp)
 *     — entre «cuál de» y «no» hay 48 caracteres y la ventana era de 30. Ampliada a 90 (perezosa).
 *   · «…órganos y centros, ENTRE LOS QUE NO SE ENCUENTRA:» (70 exp) — ni el verbo ni el giro estaban.
 * Es la segunda vez que la lista cerrada de verbos se queda corta (la primera fue el 29/07). Al
 * ampliarla, mirar también la VENTANA: una lista completa no sirve si el patrón no llega a leerla.
 * Y el `(se\s+)?` opcional cubre la familia REFLEXIVA entera («no se encuentra», «no se requiere»,
 * «no se aplica») en vez de ir añadiéndolas de una en una, que es como se llegó hasta aquí.
 */
const RE_NEGATIVA =
  /\b(incorrect|falsa|excepto|salvo\b|se[ñn]ale\s+la\s+que\s+no|cu[áa]l\s+de\s+.{0,90}?\bno\b|entre\s+(los|las)\s+que\s+no|no\s+(se\s+)?(es\s|ser[áa]|procede|corresponde|tendr[áa]|tienen?\s|podr[áa]|deber[áa]|incluye|aplica|forma\s+parte|figura|cabe|constituye|requiere|exige|est[áa]|atribuye|encuentra|aparece|contempla|recoge|comprende|considera|constituyen|contiene|contienen|figuran?\b))/i
