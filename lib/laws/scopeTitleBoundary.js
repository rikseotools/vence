...
function epigrafeTitles(epigrafe) {
  ...
  const reRomano = /(?:[Tt][íi]tulo|T[ÍI]TULO)\s+([IVXLC]+(?:\.bis)?)/g
  let m
  while ((m = reRomano.exec(texto)) !== null) {
...
const NORMAS_SIN_NUMERO = /constituci[óo]n\b|estatuto\s+de\s+autonom[íi]a|texto\s+refundido|carta\s+de\s+derechos|\btratado\b/i
...
function titlesForLaw(epigrafe, law) {
  ...
  let descartados = 0
  const re = /(?:[Tt][íi]tulo|T[ÍI]TULO)\s+([IVXLC]+(?:\.bis)?|[Pp]reliminar|PRELIMINAR)/g
  let m
  while ((m = re.exec(texto)) !== null) {
    const v = /prelim/i.test(m[1]) ? 0 : romanToInt(m[1])
    if (v == null) continue
    // Última norma mencionada ANTES de este título.
    let dueno = null
    for (const n of normas) { if (n.idx < m.index) dueno = n; else break }
    if (!dueno) { propios.add(v); continue }            // sin norma previa → genérico
    const esEsta = dueno.ref ? refsLey.has(dueno.ref) : normLey.includes(norm(dueno.etiqueta))
    if (esEsta) propios.add(v)
    else descartados++
  }
...
module.exports = { classifyTitleBoundary, seccionNumToInt, epigrafeTitles, titlesForLaw, romanToInt, epigrafeNamesRubrica, resumenBarrida }
