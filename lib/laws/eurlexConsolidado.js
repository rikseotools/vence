...
function articuloDeEurLex(html, numero, rubrica) {
  ...
  // EUR-Lex ancla cada artículo en `id="art_N"`; los sub-anclajes llevan punto (`art_N.tit_1`),
  // así que se exige que la comilla siga al número.
  const marca = `id="art_${n}"`
  const i = src.indexOf(marca)
  if (i < 0) return null

  // Fin del bloque: el siguiente `id="art_M"` con M distinto (los repetidos son sub-anclajes).
  let fin = src.length
  const re = /id="art_(\d+)"/g
  re.lastIndex = i + marca.length
  let m
  while ((m = re.exec(src))) {
    if (m[1] !== n) { fin = m.index; break }
  }

  let texto = limpiar(src.slice(i, fin))
    .replace(/^id="art_\d+">?\s*/, '')
...
function recorteArticulo(html, numero) {
  ...
  re.lastIndex = i + marca.length
  let m
  while ((m = re.exec(src))) { if (m[1] !== n) { fin = m.index; break } }
