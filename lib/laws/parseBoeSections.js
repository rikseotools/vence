...
const ROMANO = {
  preliminar: 'Preliminar', primero: 'I', segundo: 'II', tercero: 'III', cuarto: 'IV', quinto: 'V', sexto: 'VI', septimo: 'VII', octavo: 'VIII', noveno: 'IX', decimo: 'X', undecimo: 'XI', duodecimo: 'XII', decimotercero: 'XIII', decimocuarto: 'XIV', decimoquinto: 'XV',
  // ── CARDINALES, no solo ordinales (T-140, 07/08/2026) ─────────────────────────────────────
  // El BOE no siempre numera "TÍTULO SEGUNDO": el Estatuto de Cantabria (BOE-A-1982-635) tiene
  // un bloque real con id `tdos` y rúbrica «TÍTULO DOS» — cardinal, no ordinal, y mezclado en
  // el MISMO índice con `tprimero` y con romanos (`tii`, `tiii`…). Sin esto sus artículos se
  // agregaban en silencio al título anterior: la sección no desaparecía con un error, se leía
  // como si nunca hubiera existido. Mismo rango que los ordinales de arriba (hasta 15) por la
  // misma razón: es lo que cubre el corpus real, no una cota arbitraria.
  uno: 'I', dos: 'II', tres: 'III', cuatro: 'IV', cinco: 'V', seis: 'VI', siete: 'VII', ocho: 'VIII', nueve: 'IX', diez: 'X', once: 'XI', doce: 'XII', trece: 'XIII', catorce: 'XIV', quince: 'XV',
}
...
const RE_TIT = /^t(p|u|preliminar|primero|segundo|tercero|cuarto|quinto|sexto|septimo|octavo|noveno|decimo|undecimo|duodecimo|decimotercero|decimocuarto|decimoquinto|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce|trece|catorce|quince|[ivxlcdm]+)(-\d+)?$/i
const RE_CAP = /^c(p|u|preliminar|primero|segundo|tercero|cuarto|quinto|sexto|septimo|octavo|noveno|decimo|unico|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez|once|doce|trece|catorce|quince|[ivxlcdm]+)(-\d+)?$/i
...
//
// GOTCHA (07/08/2026, T-140): algunas normas NO numeran por "Artículo" en absoluto — la
// Orden de 1/2/1996 de operatoria contable (BOE-A-1996-2627) numera sus 115 unidades como
// "Regla 1", "Regla 2"… "Regla 5 bis" (id `regla1`, `regla2`…). `articles.article_number`
// ya las tiene importadas como número puro ("1", "5 bis", sin el prefijo "Regla"), así que
// el hueco estaba SOLO aquí: sin reconocerlas como artículo, sus 3 capítulos (`ci`/`cii`/…)
// quedaban con 0 artículos cada uno → `sin_secciones`. Mismo patrón que el de arriba: el
// BOE usa una palabra distinta de "Artículo" y el parser solo conocía una.
const esArticulo = (id, label) =>
  (/^(?:a|art)/i.test(id) && /^Art(?:[íi]culo)?\.?\s/i.test(String(label || '')))
  || (/^regla/i.test(id) && /^Regla\s/i.test(String(label || '')))
...
const numDeLabel = (label) => {
  ...
  // (y ahora también "Regla 10 bis" → 10, mismo formato de sufijo).
  const d = L.match(/^(?:Art(?:[íi]culo)?\.?|Regla)\s+(\d+)/i)
  if (d) return +d[1]
  // En letra: se reutiliza `spanishTextToNumber`, el conversor que ya usa el otro parser.
  const t = L.match(/^(?:Art(?:[íi]culo)?\.?|Regla)\s+(.+?)\s*\.?$/i)
  if (!t) return null
...
// ── SECCIÓN / SUBSECCIÓN (T-333, 06/08/2026) ──────────────────────────────────────────────
//
// A PROPÓSITO NO viven dentro de `parseBoeSectionsMultinivel`: esa función la usa también
// `scripts/poblar-law-sections-boe.cjs` para ESCRIBIR `law_sections`, y su CHECK de BD
// (`check_section_type`) no admite el valor 'subseccion' — meterlas ahí habría roto el
// poblador con un valor que la tabla rechaza, sin que nadie lo hubiera pedido. Este bloque es
// SOLO para el detector de frontera de scope (`scopeTitleBoundary.js`), que consume la
// estructura EN VIVO desde el índice del BOE y nunca escribe en `law_sections`.
//
// GOTCHA medido contra la API real (BOE-A-2017-12902 y BOE-A-2015-11724, 06/08/2026): el
// `id` del bloque NO distingue sección de subsección de forma fiable — en la Ley 9/2017
// ambas comparten el prefijo "s" (`s1`="Sección 1", pero también hay subsecciones con "s4");
// en el TRLGSS (RDL 8/2015) sección usa "s" y subsección usa "ss". La única fuente fiable es
// el LABEL ("Sección N" / "Subsección N", con número ARÁBIGO, no romano — a diferencia de
// título/capítulo). Por eso esta función lee el label, igual que `numSeccionDeLabel`.
const RE_SECC_SUBSECC_LABEL = /^\s*(Sub)?secci[oó]n\s+([ivxlcdm]+|\d+|preliminar)\b/i

/** "Sección 3"/"Subsección 1" → {tipo, num}. null si el label no es de este tipo. */
function numSeccSubseccDeLabel(label) {
  const m = RE_SECC_SUBSECC_LABEL.exec(String(label || ''))
  if (!m) return null
  return { tipo: m[1] ? 'subseccion' : 'seccion', num: m[2] }
}

/**
 * Sección o subsección → rango de artículos, leyendo el LABEL de cada bloque (no el id).
 *
 * Un Título/Capítulo nuevo, o una sección/subsección del OTRO tipo, CIERRA la que estuviera
 * abierta — evita que los artículos de un capítulo SIN secciones (o de una sección sin
 * subsecciones) se cuelen en la última abierta de un bloque HERMANO anterior. Ejemplo real
 * (TRLGSS): Capítulo I (arts. 1-6) no tiene secciones; sin este cierre, esos 6 artículos se
 * habrían colado dentro de la última sección del Capítulo... si lo hubiera antes. Con el
 * cierre, simplemente quedan sin sección (correcto: no la tienen).
 *
 * @param {{id:string,label:string}[]} bloques  índice del BOE en orden
 * @param {'seccion'|'subseccion'} tipoDeseado
 * @returns {{num:string,from:number,to:number,blockId:string}[]}
 */
function parseNivelSeccSubsecc(bloques, tipoDeseado) {
  const secs = []
  let cur = null
  for (const b of bloques) {
    if (RE_TIT.test(b.id) || RE_CAP.test(b.id)) { cur = null; continue }
    const info = numSeccSubseccDeLabel(b.label)
    if (info) {
      cur = info.tipo === tipoDeseado ? { blockId: b.id, num: info.num, arts: [] } : null
      if (cur) secs.push(cur)
      continue
    }
    if (esArticulo(b.id, b.label) && cur) { const n = numDeLabel(b.label); if (n != null) cur.arts.push(n) }
  }
  return secs.filter((s) => s.arts.length).map((s) => ({ num: s.num, blockId: s.blockId, from: Math.min(...s.arts), to: Math.max(...s.arts) }))
}

/**
 * TODOS los niveles de Sección/Subsección de una ley (paralelo a `parseBoeSectionsMultinivel`,
 * pero deliberadamente SEPARADO de él — ver el comentario de arriba). De más fino a más
 * grueso: subsección antes que sección. Sin ninguno de los dos → array vacío.
 *
 * @param {{id:string,label:string}[]} bloques
 * @returns {{niveles:{tipo:'subseccion'|'seccion', secciones:{num,from,to,blockId}[]}[]}}
 */
function parseSeccionesSubsecciones(bloques) {
  const niveles = []
  for (const tipo of ['subseccion', 'seccion']) {
    const secciones = parseNivelSeccSubsecc(bloques, tipo)
    if (secciones.length) niveles.push({ tipo, secciones })
  }
  return { niveles }
}
...
function rubricaVigente(xml, hoy) {
  ...
  for (let i = cand.length - 1; i >= 0; i--) {
    // T-333 (06/08/2026): la clase real de una Sección/Subsección en el XML del BOE es
    // "seccion"/"subseccion" A SECAS, SIN el sufijo "_tit" que sí llevan título/capítulo/
    // libro/parte. Confirmado contra la API real (BOE-A-2017-12902, bloque s1: `<p
    // class="seccion">Sección 1.ª Objeto y ámbito de aplicación</p>`; BOE-A-2015-11724,
    // bloque ss1: `<p class="subseccion">Subsección 1.ª Disposiciones generales</p>`). Sin
    // esto `rubricaVigente` devolvía SIEMPRE null para una sección o subsección, así que
    // nada que dependiera de su materia podía funcionar por mucho que se le pasara el bloque.
    const m = /<p class="(?:titulo_tit|capitulo_tit|libro_tit|parte_tit|seccion_tit|seccion|subseccion)">([\s\S]*?)<\/p>/.exec(cand[i].cuerpo)
    if (!m) continue
...
module.exports = {
  ...
  parseSeccionesSubsecciones, parseNivelSeccSubsecc, numSeccSubseccDeLabel }
