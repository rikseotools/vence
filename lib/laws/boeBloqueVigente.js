...
function mapaBloquesPorArticulo(indiceXml) {
  ...
  for (const m of bloques) {
    ...
    // Los ordinales altos y la variante ACENTUADA ("quáter") también existen: la
    // LECrim rotula "Artículo 367 quáter" y el CP llega hasta "127 octies". Sin
    // ellos en la lista, esos artículos no entraban en el mapa y quedaban fuera de
    // toda comprobación contra el BOE (T-146, 26/07/2026).
    const conSufijo = crudo.match(/^(\d+)\s+(bis|ter|qu[aá]ter|quinquies|sexies|septies|octies|nonies|decies)(\s+[a-z]\)?)?$/i)
