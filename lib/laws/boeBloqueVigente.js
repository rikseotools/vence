...
function mapaBloquesPorArticulo(indiceXml) {
  ...
  for (const m of bloques) {
    ...
    const conSufijo = crudo.match(/^(\d+)\s+(bis|ter|quater|quinquies|sexies|septies)(\s+[a-z]\)?)?$/i)
...
module.exports = { bloqueVigente, comparaConBd, mapaBloquesPorArticulo, articuloDeDocumento, esNotaEditorial, NOTAS_EDITORIALES, decodificar, normalizar }
