...
function bloqueVigente(xml) {
  ...
  // ⚠️ `<blockquote\b[^>]*>`, CON atributos — no `<blockquote>` a secas (T-376, 31/07/2026).
  // El BOE envuelve la REDACCIÓN DEROGADA en `<blockquote class="noDesde20140101">` (la clase
  // lleva la fecha desde la que ya no rige), mientras que la nota de pie va en un `<blockquote>`
  // pelado. La regex vieja solo podaba el segundo, así que la redacción antigua ENTERA se colaba
  // en el texto «oficial».
  //
  // Dos daños, y el segundo es el grave:
  //  · FALSO DIVERGE en el Paso 1 del manual de generación: el art. 69 del RDL 2/2004 salía como
  //    «BD 2.450 ch / oficial 4.805», y nuestro texto estaba PERFECTO — los 2.355 de diferencia
  //    eran la redacción vigente hasta 2013 más una nota editorial. Con eso se excluyó del lote un
  //    artículo sano y se abrió una ficha diciendo que servíamos texto truncado.
  //  · Y en cuanto existió una herramienta que ESCRIBE `content` desde el BOE, ese mismo texto se
  //    habría guardado: habríamos metido la redacción derogada en el temario que se sirve. Lo paró
  //    la previsualización `--ver`, no un test.
  //
  // El marcador léxico de más abajo («Redacción anterior:» + bloque entrecomillado) sigue siendo
  // necesario: cubre la variante que viaja SUELTA dentro del cuerpo, sin blockquote que la envuelva.
  const sinNotas = v.cuerpo.replace(/<blockquote\b[^>]*>[\s\S]*?<\/blockquote>/g, '')
