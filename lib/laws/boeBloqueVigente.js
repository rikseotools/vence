...
function bloqueVigente(xml) {
  ...
  // LA REDACCIÓN ANTERIOR VIENE DENTRO DEL BLOQUE VIGENTE (26/07/2026). El BOE remata
  // algunos bloques con un párrafo marcador «Redacción anterior:» y, detrás, el texto
  // DEROGADO entre comillas. No es una nota suelta que se pueda filtrar por párrafo: hay que
  // cortar ahí y tirar todo lo que sigue.
  //
  // Incluirlo es precisamente el fallo que este módulo existe para evitar —servir Derecho
  // derogado— y encima produce un falso DIVERGE contra un `content` correcto: el art. 177
  // quinquies de la LGT salía "BD 2.877 ch / BOE 3.492 ch", y los 615 de diferencia eran la
  // redacción anterior a la Ley 13/2023.
  const corte = parrafos.findIndex((p, i) => i > 0 && /^Redacci[óo]n(es)? anterior(es)?\s*:?\s*$/i.test(p))
  const cuerpo = (corte > 0 ? parrafos.slice(1, corte) : parrafos.slice(1))
