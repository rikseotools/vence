...
function spanishTextToNumber(text) {
  ...
  // Millares: "mil ochocientos veinte", "dos mil setecientos cuatro" (T-142). Aditivo — para
  // ≤999 el resultado no cambia. Hace falta porque los boletines escriben las cifras grandes en
  // letra ("Se convocan dos mil setecientas cuatro plazas") y sin esto la verificación de lo que
  // la landing AFIRMA contra el documento oficial daría un falso "no respaldado" justo en el dato
  // más visible de la página: las plazas.
  const mil = normalized.match(/^(?:(.+?)\s+)?mil(?:\s+(.+))?$/i)
  if (mil) {
    const multiplicador = mil[1] ? convertPart(mil[1].trim()) : 1
    if (multiplicador !== null && multiplicador > 0) {
      let total = multiplicador * 1000
      if (mil[2]) {
        const restoTxt = mil[2].trim()
        const resto = convertPart(restoTxt)
        if (resto !== null) total += resto
        else {
          const rh = restoTxt.match(RE_HUNDREDS)
          if (!rh) return null // "mil y pico" no es un número: mejor null que inventar
          const base = HUNDREDS[rh[1].toLowerCase()] || 0
          if (rh[2]) {
            const sub = convertPart(rh[2])
