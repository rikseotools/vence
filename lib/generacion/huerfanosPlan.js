...
function rankingHuerfanos(filas, { soloQueDisparan = true, cubiertosExtra = new Set(), demanda, oposicion = null, incluirNoNumerados = false, tipos = null } = {}) {
  ...
  for (const f of filas) {
    ...
    const k = claveArt(f)
    if (!porArt.has(k)) {
      porArt.set(k, { leySlug: f.leySlug, ley: f.ley, articulo: f.articulo, tipo: nat.tipo, oposiciones: new Set(), temas: new Set() })
    }
...
function proponeLote(filas, { maxArticulos = 6, excluirLeyes = [], demanda, oposicion = null } = {}) {
  const ranking = rankingHuerfanos(filas, { demanda, oposicion }).filter((a) => !excluirLeyes.includes(a.leySlug))
