...
/**
 * Margen EXPLOTABLE (30%) por el que la correcta debe superar al mayor distractor
 * para que el tell de longitud sea el (T) — el mismo número que usa el check
 * (T) de abajo. Exportado para que `simularBatch.analizarLote` (T-150, tell de
 * FORMA a nivel de LOTE) reutilice el MISMO umbral en vez de declarar el suyo:
 * dos copias del número son cómo nacieron los cinco escritores de [T-130].
 */
const MARGEN_EXPLOTABLE = 0.3

/** Margen relativo (0 si la correcta no es la más larga) de la correcta sobre el mayor distractor. */
function margenLarga(options, correctIdx) {
  ...
  return lc > maxDist ? (lc - maxDist) / maxDist : 0
}
...
function analizarLongitud(options, correctIdx) {
  ...
  if (margenLarga(options, correctIdx) > MARGEN_EXPLOTABLE) {
  ...
  if (lc < minDist && minDist > lc * (1 + MARGEN_EXPLOTABLE)) {
...
module.exports = { analizarLongitud, margenLarga, MARGEN_EXPLOTABLE }
