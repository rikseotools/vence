 ...
 *
 * `TIC` y `RD` entran el 08/08/2026, al auditar el lote de Guardia Civil T17. Salían como
 * `candidata` en casi todas sus preguntas —ni catalogadas ni permitidas—, y un aviso que se repite
 * en cada batch acaba en la papelera junto con los avisos que sí importan (es la lección literal
 * del gate de la cabecera, que fallaba en el 100% de los lotes buenos).
 *
 * El corte es el de la regla, no la comodidad: §2.2-quater existe por una impugnación de «LBRL a
 * pelo», que es OPACA. Estas dos no lo son. `RD` va siempre seguido de número y año («RD 1125/2024»),
 * que identifica la norma sin ambigüedad. Y `TIC` la usa la propia norma en el título de sus
 * artículos («Estrategia en materia de TIC») y en el nombre del órgano («Comisión de Estrategia
 * TIC»): desarrollarla dentro de un nombre propio lo estropea más que lo aclara.
 *
 * Ojo al añadir aquí: esta lista APAGA la comprobación para esa sigla en todo el banco. Una sigla
 * que un opositor no reconocería a la primera va al diccionario `DESARROLLO`, no aquí.
 */
const ALLOWLIST = new Set(['CE', 'UE', 'TREBEP', 'LOPJ', 'TUE', 'TFUE', 'TIC', 'RD'])
...
const DESARROLLO = {
  ...
  // Añadida el 08/08/2026 al auditar el lote de Policía Nacional T11: 16 de sus 17 preguntas
  // decían «el artículo 220 del REx 2024» sin desarrollarlo en ningún punto de esa pregunta. Es
  // el caso opuesto a `RD`/`TIC` de la ALLOWLIST y por eso va aquí: «REx» es una abreviatura
  // NUESTRA, no una convención que el opositor reconozca — exactamente el «LBRL a pelo» que dio
  // origen a §2.2-quater. Se escribe con la x minúscula, que es como la usan los lotes.
  REx: /Reglamento de (la Ley Org[áa]nica 4\/2000|Extranjer[íi]a)|(Real Decreto|RD) 1155\/2024/i,
}

// El cierre NO puede ser `\b`: en JavaScript solo conoce [A-Za-z0-9_], así que una vocal
// acentuada abre una frontera falsa y una palabra normal se lee como sigla. Medido el 08/08/2026
// auditando el lote de Policía Nacional T11: «el plazo de prescripción de la SANCIÓN» salía como
// «posible sigla no catalogada: SANCI», porque la Ó parte la palabra. Es el MISMO defecto que se
// arregló ese día en `lib/health/explicacionEcoClave.cjs` (T-557) — el `\b` de JS y el español no
// se llevan bien, y donde aparece uno conviene mirar si hay más.
// Un aviso que salta en falso cada lote es un aviso que se deja de leer.
const RE_CANDIDATA =
  /\b(?:el|la|los|las|del|al|de la|de los|de las)\s+([A-Z][A-Z0-9]{1,7})(?![A-ZÁÉÍÓÚÜÑa-záéíóúüñ0-9])/g
