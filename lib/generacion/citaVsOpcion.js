 ...
 * La versión inicial NO recibía el enunciado y lo dejaba anotado como limitación de
 * diseño, con la medida hecha: **7 avisos de 67 preguntas y 5 eran falsos positivos**
 * de ese tipo. Y el enunciado es justo lo que convierte una opción "parcial" en
 * correcta: si pregunta *"¿con quién se elaboran los mapas?"*, la opción responde
 * exactamente lo pedido aunque el artículo siga hablando.
 *
 * Ahora se le puede pasar (parámetro opcional, por compatibilidad) y se reutiliza
 * `clausulaEnEnunciado` de `citaTruncada.js` — el helper que el repo ya tenía para
 * esta misma pregunta en el otro check, comparando por PALABRAS al 80% porque el
 * enunciado reformula. Si la cola omitida ya está en el enunciado, el lector no
 * queda engañado y el aviso se marca `enElEnunciado` en vez de dispararse.
 *
 * Lo que motivó el cambio: el lote `gen_atc_t225_2026-07-26_s26c` sacó **6 avisos de
 * 16 preguntas**, y los seis eran el mismo patrón — la opción completa la frase legal
 * y la continuación es el predicado que YA está en el enunciado (art. 21: *"…se
 * valorarán con arreglo a los criterios señalados en el Impuesto…"*). Ese nivel de
 * ruido es el que enseña a ignorar el gate, que es el fallo caro (§5.bis del manual:
 * *"un gate que siempre falla no se lee"*).
 *
 * Medido sobre el banco vivo (32.343 explicaciones activas con cita): el aviso baja de
 * **1.415 (4,4%) a 1.242 (3,8%)** — 173 silenciadas — y **ninguna pregunta pasa a
 * avisar** que antes no avisara: el cambio solo estrecha. Los dos defectos REALES que
 * este check cazó (art. 46.3 y art. 2.2 del RDL 1/1993) siguen disparándose con el
 * enunciado delante, y hay un test que lo fija como invariante.
 *
 * Sigue siendo un AVISO, no un error duro: hay condensaciones válidas (§2.2). Lo que
 * pide es mirar: **o se acota el enunciado, o se completa la opción.**
 */

const { clausulaEnEnunciado } = require('./citaTruncada.js')
 ...
 * @param {string} [enunciado] `question_text`. Si se pasa y la cola omitida ya está
 *   en el enunciado, NO se avisa (condensación válida §2.2) y se devuelve
 *   `enElEnunciado:true` para poder auditar la decisión. Omitirlo mantiene el
 *   comportamiento anterior.
 * @returns {{aviso:boolean, cola?:string, enElEnunciado?:boolean}} `cola` = lo que la
 *   cita dice de más, dentro de la misma frase.
 */
function analizarCitaVsOpcion(explanation, correcta, enunciado) {
  ...
  // La cola ya está en el enunciado → condensación válida, no hay omisión que engañe.
  // Se reutiliza el helper de `citaTruncada.js`, que resuelve esta misma pregunta para
  // el otro check: comparar por palabras (80%) porque el enunciado reformula.
  if (enunciado && clausulaEnEnunciado(cola, enunciado)) {
    return { aviso: false, cola, enElEnunciado: true }
  }
