...
/**
 * Ventanas de la cola (ms).
 *
 * ⚠️ AQUÍ HABÍA UN AGUJERO GRAVE (arreglado 30/07/2026). El «sin responder» tenía una ventana
 * de **6 horas**: pasadas esas, un feedback que nadie había contestado **desaparecía del
 * vigía**. Es exactamente lo contrario de para lo que existe. Medido el día que se cazó: el
 * vigía decía «cola limpia» y había **9 sin responder**, uno de ellos un BUG y otro un premium
 * preguntando por el cambio de método de suscripción; el más viejo llevaba dos días.
 *
 * Lo detectó Manuel mirando el panel, no el vigía. Ahora **pendiente es pendiente, tenga la
 * edad que tenga**: la ventana amplia solo evita arrastrar restos históricos, y lo de no
 * repetir avisos ya lo resuelve el registro de vistos del bucle.
 */
const VENTANA_NUEVO_MS = 30 * 24 * 60 * 60 * 1000;
const VENTANA_REPLICA_MS = 7 * 24 * 60 * 60 * 1000;
