...
const PESO = { bug: 1, preventa: 2, premium: 3, construir: 4, baja: 5 }

/**
 * Señales de que una sugerencia **exige construir una herramienta**.
 *
 * Van las penúltimas (Manuel, 30/07/2026): son lo más caro de la cola y, sobre todo, **se
 * responden DESPUÉS de construirlas**. Un «buena idea, lo apuntamos» gasta el turno de esa
 * persona sin darle nada; responder con la función ya viva convierte su sugerencia en algo
 * que puede usar ese mismo día.
 *
 * Se detectan por lo que PIDEN («se podría añadir», «sería posible que…», «me gustaría poder
 * ver/filtrar/crear»), no por el tipo del feedback: casi todas llegan como `suggestion`, y
 * también lo hacen las que se contestan con una frase.
 */
const SENALES_CONSTRUIR = [
  /\bse podr[íi]a (a[ñn]adir|incluir|poner|enlazar|mostrar|crear|implementar|hacer)\b/i,
  /\b(ser[íi]a|estar[íi]a) (posible|bien|genial|[úu]til)\b.{0,40}\b(que|poder|a[ñn]adir|incluir)\b/i,
  /\bme gustar[íi]a (poder|que se pueda|que hubiera|que hubiese)\b/i,
  /\b(pod[ée]is|podr[íi]ais) (a[ñn]adir|incluir|implementar|habilitar|poner)\b/i,
  /\bcomo sugerencia de mejora\b/i,
  /\b(existe|hay) la posibilidad de\b/i,
  /\bfuncionalidad\b|\bopci[óo]n de (filtrar|crear|seleccionar)\b/i,
]
...
function clasificar(f) {
  ...
  const texto = String(f && f.message || '')

  if (type === 'account_deletion') return 'baja'
  if (type === 'bug') return 'bug'
  // Ojo al orden: una PRE-VENTA que además propone algo sigue siendo pre-venta. Quien está
  // decidiendo si paga no puede esperar a que construyamos su idea.
  if (plan !== 'premium') return 'preventa'
  if (SENALES_CONSTRUIR.some((re) => re.test(texto))) return 'construir'
  return 'premium'
}
...
const ETIQUETA = { bug: '🐞 BUG', preventa: '💰 PREVENTA', premium: '⭐ PREMIUM', construir: '🛠️ CONSTRUIR', baja: '👋 BAJA' }

module.exports = { clasificar, ordenarCola, tieneSenalDeCompra, ETIQUETA, PESO, SENALES_PREVENTA, SENALES_CONSTRUIR }
