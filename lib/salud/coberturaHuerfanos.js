 ...
 * @returns {Array<{ley:string, articulos:number, temasACero:number, temasBajoUmbral:number, findingsCerrados:number, ratio:number, usuarios:number}>}
 *   ordenado por `ratio` (temas cerrados por artículo escrito) descendente.
 */
function rankearLeyes(pares, demandaPorOposicion) {
  const dem =
