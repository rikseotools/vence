...
export const getSupabaseClient = () => {
  ...
  if (!supabaseInstance) {
    ...
    if (typeof window !== 'undefined') {
      ...
      // 🐛 FIX iOS Safari: Debounce + verificación robusta para evitar falsos logouts
      ...
      const VISIBILITY_DEBOUNCE_MS = isIOSSafari ? 2000 : 500 // Más conservador en iOS
      const VISIBILITY_MIN_INTERVAL_MS = isIOSSafari ? 5000 : 1000 // Más conservador en iOS
        ...
        if (document.visibilityState === 'visible') {
          ...
          visibilityDebounceTimer = setTimeout(async () => {
            ...
            try {
              ...
              // 🐛 FIX iOS Safari ONLY: Si hay error o no hay sesión, verificar con getUser()
              if (isIOSSafari && (error || !session)) {
                console.log('⚠️ [iOS Safari] getSession() devolvió vacío, verificando con getUser()...')
                ...
                if (user && !userError) {
                  ...
                  console.log('✅ [iOS Safari] Usuario válido encontrado, manteniendo sesión')
              ...
              if (!error) {
                window.dispatchEvent(new CustomEvent('supabaseAuthSync', {
                  detail: { session, source: 'visibility_change', isIOSSafari }
                }))
              }
            } catch (error) {
              console.error('❌ Error verificando sesión:', error)
              // 🐛 FIX iOS Safari: No propagar errores como logout en iOS
              if (!isIOSSafari) {
                throw error
              }
            }
