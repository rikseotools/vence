 ...
 * desde el rol de lectura de la flota; alguien con `DATABASE_URL` de escritura (o superusuario)
 * debería confirmar con `\dp observable_events` que no hay ningún otro grant antes de aplicar el
 * `swap` de verdad, por si existe alguno que este rol no puede ver.
 */
