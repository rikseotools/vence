...
function verificarAfirmaciones(afirmaciones, textoDocumento) {
  ...
  if (doc.length < 600) return { sinDocumento: true, respaldadas: [], sinRespaldo: [], sinBase: [] }
  ...
  for (const a of (afirmaciones || []).filter((x) => TIPOS_VERIFICABLES.has(x.tipo))) {
    ...
    const encontrada = ventanas.some((v) => patron.test(v))
    ;(encontrada ? respaldadas : sinRespaldo).push(a)
  }
  return { sinDocumento: false, respaldadas, sinRespaldo, sinBase }
}
...
function detectarContradicciones(afirmaciones) {
  ...
  for (const [tipo, lista] of porTipo) {
    ...
    out.push({
      ...
      detalle: `${concepto ? concepto.etiqueta : tipo}: la página dice ${porValor.join(' y ')}`,
    })
  }
  return out
}
