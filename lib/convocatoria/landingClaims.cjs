...
// `re` captura el número: grupo 1 si va DELANTE de la palabra («N plazas», con hasta 3 caracteres
// de puntuación de por medio — «(1704) plazas», «1704.- plazas»); grupo 2 si va DETRÁS, en la forma
// invertida que usan tablas de reparto («Plazas del cupo general: 1.747»), acotada a una ventana
// corta antes de los dos puntos para no tragarse cualquier número de la frase [T-224]. Los dos
// grupos son mutuamente excluyentes (alternancia), así que solo uno de los dos participa por match.
const CONCEPTOS = [
  {
    tipo: 'plazas',
    etiqueta: 'plazas convocadas',
    re: /\b([\d][\d.\s]{0,8}\d|\d)\s*[).:-]{0,3}\s*plazas\b|\bplazas\b[^\d\n:]{0,30}:\s*([\d][\d.\s]{0,8}\d|\d)\b/gi,
    contexto: /plazas?/gi,
  },
  {
    tipo: 'preguntas',
  ...
  for (const s of superficies || []) {
    ...
    for (const c of CONCEPTOS) {
      ...
      while ((m = re.exec(texto)) !== null) {
        // Grupo 1 (forma «N plazas») o grupo 2 (forma invertida «plazas … : N») — la alternancia
        // deja indefinido el que no participó en el match [T-224].
        const valor = aEntero(m[1] !== undefined ? m[1] : m[2])
...
function numerosDelConcepto(docNormalizado, conceptoRe) {
  ...
  while ((m = re.exec(docNormalizado)) !== null) {
    const n = aEntero(m[1] !== undefined ? m[1] : m[2])
