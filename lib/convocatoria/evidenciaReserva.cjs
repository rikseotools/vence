...
/** Prosa que solo puede significar que el cupo sale DEL total (no se suma). */
const PROSA_DENTRO = [
  /del total de (?:las |estas )?plazas(?: convocadas)?[^.]{0,80}?se reserv\w+/i,
  /de las cuales[^.]{0,60}?se reserv\w+/i,
  /de estas plazas[^.]{0,60}?se reserv\w+/i,
]
...
function proponerRelacion(corpus, { plazasLibres, plazasDiscapacidad } = {}) {
  ...
  for (const [patrones, incluidas, via] of [[PROSA_DENTRO, true, 'prosa: el cupo sale DEL total'], [PROSA_APARTE, false, 'prosa: el boletín enumera dos cupos']]) {
    for (const re of patrones) {
      const g = new RegExp(re.source, re.flags.includes('g') ? re.flags : re.flags + 'g')
      let m
      while ((m = g.exec(t)) !== null) {
        if (cerca(m.index)) añade(incluidas, via, frag(t, m.index))
      }
    }
  }
