...
const PROSA_DEL_TOTAL = [
  ...
  // «De DICHAS plazas, se reserva una» — BOJA de subalterno-parlamento-andalucia (28/07). Igual de
  // clara que «del total» y no la reconocía: cada boletín tiene su muletilla.
  /de dichas plazas(?:[^.]|\.(?=\d)){0,140}?se reserv\w+/i,
  // Ficha de bases sin prosa: «Número: 46 … Plazas turno libre: 46 · Plazas reservadas a personas
  // con discapacidad: 5» (Ayto. Sevilla). No dice «del total», pero la aritmética la resuelve la
  // MISMA regla: si en la ventana está nuestra cifra y NO está la suma, guardamos el total.
  /plazas reservadas a personas con discapacidad/i,
]

/**
 * Prosa que REPARTE un total entre turnos. No concluye sola: hay que ver si la cifra que guardamos
 * es el total o una de las partes (ver dónde se usa). Sale del BOP de Badajoz, 28/07.
 */
const PROSA_REPARTO = [
  /turno de (?:acceso )?libre(?:[^.]|\.(?=\d)){0,160}?turno de discapacidad/i,
  /se cubrir[áa]n por el turno(?:[^.]|\.(?=\d)){0,160}?discapacidad/i,
]
...
function proponerRelacion(corpus, { plazasLibres, plazasDiscapacidad } = {}) {
  ...
  // ENUMERACIÓN DE TURNOS: «9 plazas, de las cuales 8 por el turno de acceso libre y 1 por el turno
  // de discapacidad» (BOP Badajoz). Aquí el boletín nombra un TOTAL y sus PARTES; si nuestra cifra
  // aparece junto a la suma, es que guardamos una parte ⇒ el cupo se suma. Si solo apareciera la
  // nuestra, no se concluye: podríamos estar guardando el total.
  for (const re of PROSA_REPARTO) {
    const g = new RegExp(re.source, re.flags.includes('g') ? re.flags : re.flags + 'g')
    let m
    while ((m = g.exec(t)) !== null) {
      const v = frag(t, m.index)
      if (cifraEnTexto(plazasLibres, v) && cifraEnTexto(total, v)) {
        añade(false, 'prosa: el boletín reparte el total entre turno libre y turno de discapacidad', v)
      }
    }
  }
