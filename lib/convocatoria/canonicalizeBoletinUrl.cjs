...
const PATTERNS = [
  ...
  {
    boletin: 'BOC',
    // Variante de la SEDE: sede.gobiernodecanarias.org/boc/boc-a-2026-150-2685.pdf → BOC-2026-150-2685.
    // Es el MISMO anuncio que /boc/2026/150/2685.html (los dos salen del mismo sumario), así que
    // deben converger al mismo docKey o el hub guardaría el documento dos veces. [T-221]
    re: /gobiernodecanarias\.org\/boc\/boc-[a-z]-(\d{4})-(\d+)-(\d+)/i,
    key: (m) => `BOC-${m[1]}-${m[2]}-${m[3]}`,
    canonical: (k) => {
      const [, y, b, a] = k.match(/^BOC-(\d{4})-(\d+)-(\d+)$/);
      return `https://www.gobiernodecanarias.org/boc/${y}/${b}/${a}.html`;
  ...
  // ── Añadidos el 28/07/2026 (T-221) ─────────────────────────────────────────────────
  // El sensor de boletines ya emite el enlace del ANUNCIO (antes solo el sumario del día),
  // pero un enlace que este núcleo no reconoce NO deja provenance: el `apply` no registra
  // documento. Estos 4 son los que se verificaron uno a uno contra la URL real —que sirve el
  // documento y no el caparazón del portal— en la simulación `sim-enlace-anuncio.ts`.
  // Deliberadamente NO se añaden DOE ni BOPV: la URL que publican en su sumario es un
  // ENVOLTORIO (el DOE devuelve una página de título+analítica sin la disposición; el BOPV
  // mete el texto en un iframe). Darles docKey crearía provenance apuntando a un caparazón.
  {
    boletin: 'BOPA',
    // Asturias (Liferay): …/bopa/disposiciones?…p_r_p_dispositionReference=2026-06220 → BOPA-2026-06220.
    // La referencia es la identidad; el resto de parámetros del portlet son ruido de sesión.
    re: /miprincipado\.asturias\.es\/bopa\/.*dispositionReference=(\d{4}-\d+)/i,
    key: (m) => `BOPA-${m[1]}`,
  },
  {
    boletin: 'BON',
    // Navarra: bon.navarra.es/es/anuncio/-/texto/2026/146/1 → BON-2026-146-1
    // (año / nº boletín / orden dentro del boletín). La variante /eu/ converge al mismo docKey.
    re: /bon\.navarra\.es\/[a-z]{2}\/anuncio\/-\/texto\/(\d{4})\/(\d+)\/(\d+)/i,
    key: (m) => `BON-${m[1]}-${m[2]}-${m[3]}`,
  },
  {
    boletin: 'BOME',
    // Melilla: bomemelilla.es/bome/BOME-B-2026-6400/articulo/872 → BOME-A-2026-872.
    // OJO: en la URL, `BOME-B-…` es el BOLETÍN (el número del día) y `/articulo/N` el anuncio.
    // Quedarse con el código del boletín colapsaría TODOS los anuncios de ese día en un mismo
    // docKey. El código propio del artículo (BOME-A-año-N) es el que el portal pone como título
    // de la página — verificado con /articulo/870, /871 y /872.
    re: /bomemelilla\.es\/bome\/BOME-[A-Z]-(\d{4})-\d+\/articulo\/(\d+)/i,
    key: (m) => `BOME-A-${m[1]}-${m[2]}`,
  },
  {
    boletin: 'DOCM',
    // Castilla-La Mancha: docm.jccm.es/docm/descargarArchivo.do?ruta=2026/07/23/pdf/2026_5573.pdf
    // → DOCM-2026-5573 (año_número de la disposición). Sirve el PDF real (verificado, 341 KB).
    re: /docm\.jccm\.es\/.*ruta=\d{4}\/\d{2}\/\d{2}\/pdf\/(\d{4})_(\d+)/i,
    key: (m) => `DOCM-${m[1]}-${m[2]}`,
  },
  {
    boletin: 'MIA',
