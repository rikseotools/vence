...
const { canonicalizeBoletinUrl } = require('./canonicalizeBoletinUrl.cjs')
...
function checkConvocatoriaLinks(c) {
  ...
  if (etiqueta && c.programaUrl) {
    const { boletin, recognized } = canonicalizeBoletinUrl(c.programaUrl)
    if (recognized && boletin && boletin !== 'unknown' && boletin !== etiqueta) {
      issues.push({
...
module.exports = { extraerIdBoe, extraerAño, checkConvocatoriaLinks, normalizarEtiquetaBoletin }
