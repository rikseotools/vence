...
// Estados de `convocatorias.estado_proceso` en los que EXISTE una convocatoria publicada con su
// ficha concreta viva → vigilar un índice genérico es CEGUERA (procesoEnJuego=true). Fuera de este
// conjunto (p.ej. `oep_aprobada` esperando bases, `sin_oep`, `examen_realizado`/`nombramientos` ya
// pasados) NO hay ficha concreta que apuntar: el índice del portal de empleo es la vigilancia
// LEGÍTIMA —el cron de hash caza el cambio cuando la convocatoria aparezca— y no debe pingar.
// Antes se usaba `oposiciones.is_convocatoria_activa`, que es `true` también en `oep_aprobada`
// (OEP viva pero sin ficha) → sobre-marcaba genéricas legítimas (T-112, 25/07: salamanca/ávila/huelva).
...
/**
 * ¿El estado del proceso implica una convocatoria PUBLICADA con ficha concreta viva?
 * Solo entonces una `seguimiento_url` genérica es ceguera accionable (procesoEnJuego).
 * @param {string|null|undefined} estadoProceso  `convocatorias.estado_proceso`
 * @returns {boolean}
 */
function procesoConFichaViva(estadoProceso) {
  return ESTADOS_FICHA_VIVA.has(estadoProceso)
}
...
module.exports = { diagnosticarSeguimientoUrl, procesoConFichaViva }
