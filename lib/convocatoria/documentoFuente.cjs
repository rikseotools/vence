...
function paredDelPortal(texto) {
  ...
  // El muro de cookies del DOGC: 4.188 caracteres de aviso legal y ni una línea del documento.
  // Pasaba los dos filtros anteriores (200 y >600 chars) y se colaba como "documento legible".
  if (/sortir r[àa]pid|per poder navegar amb m[ée]s tranquil/.test(t)) return 'muro de cookies del DOGC (hace falta fetcher headless)'
...
/**
 * ¿Esto es el BOLETÍN COMPLETO del día en vez del anuncio concreto?
 *
 * Caso real (27/07/2026): el `programa_url` de `administrativa-universidad-de-murcia` apunta a
 * `borm.es/services/boletin/ano/2026/numero/146/pdf` — **739.029 caracteres, 179 anuncios y un
 * sumario**. Clonarlo al hub como "la convocatoria" sería peor que no tenerlo: el detector de
 * cifras da por respaldada cualquier afirmación cuyo número aparezca en el documento, y en 739 KB
 * de boletín aparece CUALQUIER número. Un falso verde es peor que un aviso.
 *
 * Dos señales, ambas necesarias para no marcar unas bases largas de verdad: tamaño desmedido Y
 * pinta de recopilatorio (sumario al principio o muchísimas rúbricas de anuncio distintas).
 *
 * @returns {string|null} motivo, o null si parece un documento concreto
 */
function pareceBoletinCompleto(texto) {
  const t = String(texto || '')
  if (t.length < 250000) return null
  const cab = t.slice(0, 1200).toLowerCase().replace(/\s+/g, ' ')
  if (/s\s?u\s?m\s?a\s?r\s?i\s?o|sumario/.test(cab)) return `boletín completo (${Math.round(t.length / 1000)}k chars, empieza por el sumario)`
  const rubricas = (t.match(/\b(anuncio|resoluci[óo]n|orden|decreto)\b/gi) || []).length
  if (rubricas > 100) return `boletín completo (${Math.round(t.length / 1000)}k chars, ${rubricas} rúbricas de anuncio)`
  return null
}
...
module.exports = { aTexto, plano, buscarAnclas, paredDelPortal, pareceBoletinCompleto, dictaminar, descargar, verificar, CABECERAS, MIN_CHARS, MARCA_PDF_ILEGIBLE }
