...
/**
 * Todas las formas en que un boletín puede escribir la cifra `n`: dígitos, dígitos con separador de
 * millar, y el numeral en letra en sus DOS géneros («ocho plazas» / «ochocientas plazas»).
 *
 * Existe para que quien necesite las formas —no el veredicto— no se copie la tabla. Era el caso de
 * `scripts/proponer-plazas-boe.cjs`, que llevaba su propia `enLetra` con `U[1]='un'` en vez de
 * `'uno'` (T-209): una tercera copia sin paridad con nada, y la que PROPONE cifras al operador.
 *
 * **La divergencia no era inocua, y no en la dirección que parecía.** «un» es también el artículo
 * indeterminado, así que para una convocatoria de 1 plaza esa copia daba por evidencia cualquier
 * «**un** plazo de veinte días» que tuviera la palabra «plaza» a menos de 90 caracteres — ruido con
 * pinta de prueba, que es lo peor en una herramienta de verificación. Y a la vez se perdía «**una**
 * plaza», que es como se escribe de verdad. Fallaba en los dos sentidos.
 *
 * Medido antes de unificar (30/07, sobre las 177 cifras de plazas del catálogo): **13 se escriben
 * distinto** con una tabla y con la otra (1, 31, 41, 131, 161, 201, 261, 551, 561, 1131, 1191, 1361,
 * 1371), y solo **3 convocatorias vigentes de oposición activa** llevan una de ellas. La apócope de
 * los compuestos («treinta y **un** plazas») **no aparece en ningún documento** donde el núcleo no vea
 * ya la cifra: 0 de las vigentes con documento. Por eso NO se añade como forma — en un detector que
 * afirma «este documento prueba la cifra», una forma más sin ganancia medida es solo riesgo de verde
 * falso.
 */
function formasDeCifra(n) {
  if (!Number.isInteger(n) || n < 0) return []
  const letra = n <= 9999 ? enLetra(n) : null
  return [
    String(n),
    String(n).replace(/\B(?=(\d{3})+(?!\d))/g, '.'),
    ...(letra ? formasDeGenero(letra) : []),
  ]
}

module.exports = { enLetra, cifraEnTexto, esPlazaHuerfana, formasDeCifra }
