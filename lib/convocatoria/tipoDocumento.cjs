...
const REGLAS = [
  ...
  // Las dos reglas de temario van DESPUÉS de `convocatoria` por lo mismo que `oep_decreto`: la
  // convocatoria MENCIONA su temario ("el temario para la provisión figura en el Anexo I") y un
  // temario de verdad nunca dice "por la que se convoca". El orden lo resuelve sin más guardas.
  {
    tipo: 'temario',
    // Señal FUERTE: la rúbrica propia de un programa oficial. No se le exige estructura porque
    // muchos temarios enumeran «1.», «2.»… sin escribir la palabra "Tema" — el de Granada
    // («ANEXO I: PROGRAMA DE MATERIAS para el turno libre», 15 plazas) tiene CERO "Tema N" y se
    // quedaba sin clasificar por culpa de la guarda. Falso negativo cazado el 27/07 al clonar.
    // El plural es tan válido como el singular y costaba un documento real: el SAS publica
    // «…por la que se aprueban y publican los nuevos PROGRAMAS DE MATERIAS que habrán de regir
    // las pruebas» y se quedaba sin clasificar por una `s` (27/07).
    re: /programas? de materias|temarios? (para|de) (la|el|las|los) (provision|ingreso|acceso|escalas)/,
    // …salvo que el propio documento se presente como otra cosa: la nota informativa del órgano
    // de selección de Aragón MENCIONA los "programas de materias" en su cuerpo sin ser el
    // programa. Lo que manda es cómo se identifica el documento, no lo que cita.
    excluir: /^(nota informativa|acuerdo del? [óo]rgano|acuerdo de la comision)/,
  },
  {
    tipo: 'temario',
    // Un temario puede no tener rúbrica ninguna: el PDF de la Diputación de Huelva **empieza
    // directamente por «Tema 22. El órgano de contratación…»** porque es el anexo suelto. Se
    // reconoce por lo que ES: una enumeración densa que ARRANCA el documento.
    //
    // El "arranca" no es un adorno: sin él, la simulación coló como temario una **nota
    // informativa del órgano de selección de Aragón** que enumeraba los temas del ejercicio
    // (27/07). Enumerar temas lo hacen muchos documentos; EMPEZAR por la enumeración, solo el
    // temario.
    // `^` literal: el documento ARRANCA con la enumeración. "En los primeros 300 caracteres" no
    // bastaba — la nota informativa de Aragón decía "…Se informa del reparto: Tema 1…" y entraba.
    re: /^\s*tema \d+\b/,
    extra: (cab, doc) => (String(doc.texto || '').match(/\btema \d+/gi) || []).length >= 8,
  },
  {
    tipo: 'temario',
    // Señal DÉBIL: "Anexo I … temario" lo dice también la convocatoria que solo lo MENCIONA. A
    // esta sí se le exige la estructura de un temario de verdad.
