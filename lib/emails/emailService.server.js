...
export async function getEmailPreferences(userId) {
  try {
    ...
    // Manejar columnas que podrían no existir aún
    const email_soporte_disabled = preferences.email_soporte_disabled ?? false
    const email_newsletter_disabled = preferences.email_newsletter_disabled ?? false
    ...
    console.log(`✅ Preferencias obtenidas:`, {
      ...
      soporte_disabled: email_soporte_disabled,
      newsletter_disabled: email_newsletter_disabled
    })
...
export async function canSendEmailType(userId, emailType) {
  ...
  // Si desactivó todos los emails, no enviar
  ...
  switch (emailType) {
    ...
    // Emails de soporte (respuestas a impugnaciones y consultas)
    case 'soporte_respuesta':
    case 'impugnacion_respuesta':
      // Si la columna no existe (es undefined/null), permitir por defecto
      if (preferences.email_soporte_disabled === undefined || preferences.email_soporte_disabled === null) {
        return true
      }
      return !preferences.email_soporte_disabled
    // Emails de newsletter (información de la oposición)
    case 'newsletter':
    case 'newsletter_oposicion':
      // Si la columna no existe, permitir por defecto
      if (preferences.email_newsletter_disabled === undefined || preferences.email_newsletter_disabled === null) {
        return true
      }
      return !preferences.email_newsletter_disabled
    default:
...
export async function processUnsubscribeByToken(token, specificTypes = null, unsubscribeAll = false) {
  try {
    ...
    console.log(`📝 Actualizando preferencias para usuario ${userId}:`, updateData)
    
    // Usar UPDATE directo ya que las preferencias se crean automáticamente
...
export async function sendEmail(userId, emailType, customData = {}) {
  try {
    ...
    // URL del test según tipo de email - Siempre usar URL de producción en emails
    const baseUrl = 'https://www.vence.es'
    let testUrl
    testUrl = `${baseUrl}/auxiliar-administrativo-estado/test?utm_source=email&utm_campaign=${emailType}`
    ...
    let unsubscribeUrl
    if (unsubscribeToken) {
      unsubscribeUrl = `${baseUrl}/unsubscribe?token=${unsubscribeToken}&email=${encodeURIComponent(user.email)}`
    } else {
      // Fallback
      console.warn('⚠️ No se pudo generar token, usando URL de fallback')
      unsubscribeUrl = `${baseUrl}/perfil?tab=emails&utm_source=email_unsubscribe`
    }
    
    let subject, html
    
    // Manejar diferentes tipos de templates con parámetros específicos
    ...
    // Verificar si hubo error en Resend
    if (emailResponse.error) {
      console.error('❌ Error de Resend:', emailResponse.error)
      throw new Error(`Error de Resend: ${emailResponse.error.message || emailResponse.error}`)
    }
    
    if (!emailResponse.data) {
      console.error('❌ Resend no devolvió datos de email')
      throw new Error('Resend no devolvió datos de email')
    }
    ...
    const campaignId = `${emailType}_${Date.now()}_${userId.slice(-4)}`
...
export async function sendWeeklyReportEmail(userId, articlesData = []) {
  try {
    ...
    // Generar URL de test dirigido con los artículos problemáticos - Siempre usar URL de producción
    const baseUrl = 'https://www.vence.es'
