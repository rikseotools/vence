...
export async function getEmailPreferences(userId) {
  try {
    ...
    console.log(`✅ Preferencias obtenidas:`, {
      ...
      inmediato: preferences.email_welcome,
...
export async function canSendEmailType(userId, emailType) {
  ...
  switch (emailType) {
    ...
    case 'welcome':
      return preferences.email_welcome
...
export async function processUnsubscribeByToken(token, specificTypes = null) {
  try {
    ...
    if (specificTypes) {
      ...
      // Desactivar según el tipo del token
      if (emailType === 'all') {
        updateData = {
          email_reactivacion: false,
          email_urgente: false,
...
export async function sendEmail(userId, emailType, customData = {}) {
  try {
    ...
    let testUrl
    testUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/es/auxiliar-administrativo-estado/test?utm_source=email&utm_campaign=${emailType}`
    ...
    let unsubscribeUrl
    if (unsubscribeToken) {
      unsubscribeUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/es/unsubscribe?token=${unsubscribeToken}&email=${encodeURIComponent(user.email)}`
    } else {
      // Fallback
      console.warn('⚠️ No se pudo generar token, usando URL de fallback')
      unsubscribeUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/es/perfil?tab=emails&utm_source=email_unsubscribe`
    }
...
export async function sendWeeklyReportEmail(userId, articlesData = []) {
  try {
    ...
    const testUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/es/auxiliar-administrativo-estado/test/articulos-dirigido?articles=${encodeURIComponent(articleIds)}&utm_source=email&utm_campaign=resumen_semanal`
