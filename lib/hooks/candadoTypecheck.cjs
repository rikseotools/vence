...
/**
 * Marca que viaja a los hijos: «el candado ya lo tiene un antepasado, no lo pidas otra vez». (T-708)
 *
 * Hace falta porque el candado se puso primero en el `pre-push`, que envuelve `npm run typecheck`.
 * Al candar TAMBIÉN el script (que es por donde entra el resto del tráfico: quien lo corre a mano
 * antes de pushear, y quien revisa una entrega en un clon aparte), los dos `flock` caerían sobre
 * el MISMO fichero — y `flock` no es reentrante entre procesos, así que el hijo esperaría los 900 s
 * a un candado que tiene su propio padre. Sería un cuelgue de 15 min en cada push.
 */
const MARCA_ENTORNO = 'VENCE_TYPECHECK_CON_CANDADO'

/** ¿Ya estoy corriendo dentro de un candado que puso alguien por encima? */
function yaEstaBajoCandado(env = process.env) {
  return String((env || {})[MARCA_ENTORNO] || '') === '1'
}
...
module.exports = { conCandado, interpretarSalida, yaEstaBajoCandado, RUTA_CANDADO, SALIDA_SIN_CANDADO, MARCA_ENTORNO }
