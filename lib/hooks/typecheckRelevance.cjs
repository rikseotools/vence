...
/**
 * El BACKEND tiene su propio `tsconfig` y su propio `npm run typecheck`, y el de la raíz lo
 * **excluye**. Consecuencia medida el 28/07: dos sesiones portaron el mismo detector al
 * servicio del `@Cron` en sitios distintos, el rebase no dio conflicto y `main` acabó con una
 * variable declarada dos veces → **el backend no compilaba y nadie podía desplegarlo**. Ni el
 * CI ni este mismo hook lo vieron: los dos miran solo la raíz. Se descubrió al desplegar,
 * que es cuando ya bloquea a todo el mundo.
 *
 * Cuesta ~9 s (proyecto mucho más pequeño), así que se paga solo cuando el push toca
 * `backend/`.
 */
const PREFIJO_BACKEND = 'backend/'
/** Lo que el compilador del backend no mira (artefactos y dependencias). */
const BACKEND_IGNORADO = ['backend/node_modules/', 'backend/dist/']

/**
 * ¿Hay que correr el typecheck DEL BACKEND para este conjunto de ficheros?
 * Mismo criterio conservador que su hermano de la raíz: ante la duda, se corre.
 */
function needsBackendTypecheck(archivos) {
  const lista = Array.isArray(archivos) ? archivos.filter((f) => typeof f === 'string' && f) : []
  if (lista.length === 0) {
    return { correr: true, motivo: 'no se pudo determinar qué ficheros cambian', relevantes: [] }
  }
  const relevantes = lista.filter((f) => {
    const ruta = f.replace(/^\.\//, '')
    if (!ruta.startsWith(PREFIJO_BACKEND)) return false
    if (BACKEND_IGNORADO.some((p) => ruta.startsWith(p))) return false
    return EXT_CODIGO.test(ruta) || /^backend\/(tsconfig[\w.-]*\.json|package(-lock)?\.json)$/.test(ruta)
  })
  if (relevantes.length === 0) {
    return { correr: false, motivo: 'el push no toca el backend', relevantes: [] }
  }
  return { correr: true, motivo: `${relevantes.length} fichero(s) del backend`, relevantes }
}

module.exports = {
  needsTypecheck,
  needsBackendTypecheck,
  PREFIJOS_IGNORADOS,
  BACKEND_IGNORADO,
  EXT_CODIGO,
  FICHEROS_DE_CONFIG,
