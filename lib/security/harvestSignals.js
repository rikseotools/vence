...
const DEFAULTS = {
  ...
  maxAnswerRatio: 0.2,
  /** Servidas a partir de las cuales el volumen por sí solo es anómalo,
   *  aunque responda (firma de barrido con auto-respuesta). */
...
function classifyHarvest(input, opts) {
  ...
  // Responde, pero el volumen es de otra liga.
  if (served >= o.egregiousServed) {
    reasons.push(`volumen_${served}_servidas`)
    if (sinNavegador) reasons.push('sin_dispositivo_ni_navegador')
    return { kind: 'harvest_volume', severity: 'high', ratio, reasons }
  }
