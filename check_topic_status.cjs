...
(async () => {
  ...
  console.log('Estado de las 32 preguntas en tabla questions:\n');
  ...
  // Ver cuáles NO son perfect o tech_perfect
  ...
  if (withErrors.length > 0) {
    console.log('\nEstados con errores:');
    withErrors.forEach(q => {
      console.log(`  ${q.id}: ${q.topic_review_status || 'null'}`);
    });
  }
})();
