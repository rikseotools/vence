...
(async () => {
  ...
  const fixed = new Set([
    ...
    "41e06bd3-d0dd-43b9-82f5-3efe83122249"
  ]);
  const seen = new Set();
  let countGood = 0;
  ...
  for (const log of logs) {
    ...
    console.log("Explicacion (" + exp.length + " chars):", exp.substring(0, 400));
    ...
    console.log("(Buenas saltadas:", countGood + ")");
    return;
  }
  console.log("FIN - Revisadas " + seen.size + " unicas (" + countGood + " buenas) en " + logs.length + " logs restantes");
})();
