    ...
    try {
      ...
      const insertPromise = this.db.execute(sql`
        INSERT INTO public.observable_events (
          ...
          -- Clamp defensivo del ts del EVENTO: en eventos frontend viene del reloj
          -- del cliente (un navegador con reloj roto metió ts=2067 el 04/06). Fuera
          -- de [NOW()-7d, NOW()+1h] → NOW(). created_at (hora de inserción) es
          -- siempre fiable; esto solo sanea la hora del evento para queries por rango.
