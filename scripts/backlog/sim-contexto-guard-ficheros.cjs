...
;(function main() {
  ...
  // [T-704] El escape pide MOTIVO desde el 08/08 (con un `=1` se había vuelto un prefijo: 25
  // escapes en 7 días sin una sola razón escrita). Esta simulación nació en la rama de [T-532]
  // contra el contrato anterior, así que al fusionarlas el caso 4 salía rojo — un choque que
  // ninguno de los dos lados podía ver por separado.
  caso(4, 'El borrado es a propósito y se usa el escape CON MOTIVO', 0,
    correrGuard({ CONTEXTO_GUARD_SKIP: 'la entrada no era una tarea, la renumero' }))
  caso(4.5, 'El escape con un «1» ya NO abre la puerta', 1, correrGuard({ CONTEXTO_GUARD_SKIP: '1' }))
