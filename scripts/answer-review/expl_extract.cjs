// Extractor Cubo C (sin explicación): activas legal-real con explanation<40 chars.
// WAVE=<id> [SIZE=160]. Produce wave{W}_ledger.json + wave{W}_blind_{1..8}.json.
// El blind incluye correct_option_letter (el agente la necesita para redactar
// "por qué X es correcta") + el artículo vinculado (fuente literal para la cita).
const fs = require('fs');
...
const SIZE = parseInt(process.env.SIZE || '160', 10);
const PER = Math.ceil(SIZE / 8);
...
(async () => {
  const [{ n: poolN }] = await sql`
    ...
    WHERE q.is_active=true AND COALESCE(l.is_virtual,false)=false
      AND COALESCE(length(trim(q.explanation)),0)<40`;
  console.log(`Pool sin explicación (legal real activo): ${poolN}`);

  const rows = await sql`
    SELECT q.id, q.question_text, q.option_a,q.option_b,q.option_c,q.option_d,q.option_e,
      q.correct_option, q.is_official_exam, q.lifecycle_state,
  ...
  const blind = rows.map((q, i) => ({
    n: i + 1, id: q.id, question_text: q.question_text,
    option_a: q.option_a, option_b: q.option_b, option_c: q.option_c, option_d: q.option_d, option_e: q.option_e,
    correct_option_letter: ['A','B','C','D','E'][q.correct_option],
    is_official_exam: q.is_official_exam, law_short_name: q.law,
    linked_article_number: q.article_number, linked_article_title: q.art_title, linked_article_content: q.art_content
  }));
  let nf = 0;
  for (let i = 0; i < blind.length; i += PER) { nf++; fs.writeFileSync(`/tmp/wave${W}_blind_${nf}.json`, JSON.stringify(blind.slice(i, i + PER), null, 2)); }
