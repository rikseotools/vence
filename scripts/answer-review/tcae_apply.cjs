// Apply mecánico de una tanda de auditoría TCAE.
// Uso: node scripts/answer-review/tcae_apply.cjs <ruta_output_workflow.json>
...
const QUEUE = '/tmp/claude-1000/-home-manuel-Documentos-github-vence/61c02061-b6d4-4b33-90e2-403f855d4c26/scratchpad/tcae_clavemala_queue.json'
...
;(async () => {
  ...
  const res = (j.result && j.result.resultados) || []
  const valid = res.filter(r => r && UUID.test(r.id))

  // 1) Explicaciones (clave OK)
  const rw = valid.filter(r => r.clave_ok && !r.explicacion_didactica && r.nueva_explicacion && r.nueva_explicacion.length > 60)
  const ids = rw.map(r => r.id)
  const map = {}
  for (let i = 0; i < ids.length; i += 100) { const { data } = await q(() => s.from('questions').select('id,correct_option,lifecycle_state').in('id', ids.slice(i, i + 100))); (data || []).forEach(x => map[x.id] = x) }
  ...
  const cm = valid.filter(r => !r.clave_ok && r.confirm && r.confirm.confirmado_clave_mala === true)
  let deact = 0
  ...
  console.log(JSON.stringify({ file: file.split('/').pop(), total: valid.length, expl_aplicadas: applied, expl_saltadas: skip, clavemala_desactivadas: deact, articulo_no_soporta: artNo, cola_total: queue.length }))
})()
