-- Primero eliminar la función rota
DROP FUNCTION IF EXISTS get_user_public_stats(uuid);

-- Crear versión FUNCIONAL de la RPC con questions_this_week
...
BEGIN
  ...
  -- NUEVA: Preguntas de esta semana (últimos 7 días)
  week_questions AS (
    SELECT
      ...
      AND created_at >= (
        ...
        FROM (
          ...
          WHERE NOT EXISTS (
            SELECT 1 FROM tests
            WHERE user_id = p_user_id
            AND DATE(created_at) = date_check
          )
        ) as missing_days
      )
  )
  SELECT
    ...
    'auxiliar_administrativo_estado'::TEXT as target_oposicion,  -- Valor por defecto
    COALESCE(sc.current_streak, 0)::INTEGER,
    COALESCE(sc.current_streak, 0)::INTEGER as longest_streak,  -- Temporal, igual que current
    COALESCE(ts.today_tests, 0)::INTEGER,
    COALESCE(ts.today_questions, 0)::INTEGER,
    COALESCE(ts.today_correct, 0)::INTEGER,
    COALESCE(wq.questions_count, 0)::INTEGER as questions_this_week,  -- NUEVO CAMPO
    0::INTEGER as mastered_topics,  -- Temporal
...
-- Verificar que funciona para Manuel
