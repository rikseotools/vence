...
const LAWS = { 'RDL 8/2015': 'eabb640e-fa9b-47a8-8d76-2a580115cfb0' };
const LETTER_TO_INDEX = { 'A': 0, 'B': 1, 'C': 2, 'D': 3 };

(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_9,_El_régimen_de_la_Seguridad_Social_del_personal_laboral';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T409: SS Personal Laboral ===\n');
  ...
  for (const fileName of files) {
    ...
    for (const q of data.questions) {
      ...
      const { data: article } = await supabase.from('articles').select('id').eq('law_id', LAWS['RDL 8/2015']).eq('article_number', artMatch[1]).single();
  ...
  console.log('\nRESUMEN T409: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T409']).eq('is_active', true);
  console.log('Total T409 en BD:', count);
})();
