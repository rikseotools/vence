...
const DIR=path.join(__dirname,'cola-larga-shards');

function norm(s){
  return (s||'')
    .replace(/\*\*/g,'')                    // negritas markdown (en cita Y en artículo)

    // Encabezados: GLOBAL. En el artículo van a principio de línea; en la cita quedan
    // detrás de la comilla de apertura. Anclarlos a ^ era ASIMÉTRICO (5º falso positivo).
    .replace(/(^|\s|«)#{1,6}\s+/g,'$1')
    .replace(/^[\t ]*\|/gm,'')               // bordes de tabla markdown
    .replace(/[«»""''"]/g,'')            // comillas (envolventes o anidadas)
    // Viñetas: GLOBAL, no ancladas a ^. En el artículo van a principio de línea; en la cita,
    // al unirse en un solo renglón, quedan a mitad de línea. Anclarlas a ^ era ASIMÉTRICO.
    .replace(/[•·▪‣]\s*/g,'')
    .replace(/(^|\s)[*\-–—]\s+/g,'$1')
...
fs.writeFileSync(path.join(__dirname,'validacion-citas-cl.json'),JSON.stringify({ok,malas,flags,sinBQ},null,1));
