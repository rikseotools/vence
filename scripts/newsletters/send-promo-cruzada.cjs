...
// Cross-sell: variables de la plantilla oposicion-cruzada. oposicionActual va FIJO
// (un segmento = una oposición homogénea → versión limpia, sin el "(examen ...)" del nombre completo).
const BASE_VARS = {
  nuevaOposicion: cfg.nuevaOposicion,
  nuevaOposicionCorta: cfg.nuevaOposicionCorta,
  oposicionActual: cfg.oposicionActual,
  temasComunesHtml: (cfg.temasComunes || []).map((t) => `<li>${t}</li>`).join(''),
  ctaUrl: `https://www.vence.es/${cfg.slug}?utm_source=email&utm_campaign=cross_ujieres`,
  slug: cfg.slug,
};
...
(async () => {
  ...
  const exclSql = excl.length
    ...
    : '';
  // Envío incremental / make-up: no re-enviar a quien ya recibió campañas previas.
  // Acepta un id (string) o varios (array) → excluye a quien recibió CUALQUIERA de ellas.
  const skipCampaigns = cfg.excludeSentCampaignId
    ? (Array.isArray(cfg.excludeSentCampaignId) ? cfg.excludeSentCampaignId : [cfg.excludeSentCampaignId])
    : [];
