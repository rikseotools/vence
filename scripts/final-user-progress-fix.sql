-- scripts/final-user-progress-fix.sql
-- MIGRACIÓN FINAL: user_progress con estructura correcta

-- =====================================================
-- PASO 1: ANÁLISIS DE DATOS
-- =====================================================

-- Ver qué temas tenemos y obtener sus topic_id
SELECT 
  'TEMAS DISPONIBLES:' as info,
  id as topic_id,
  number as tema_number,
  name as tema_name
FROM topics 
WHERE is_active = true
ORDER BY number;

-- Ver tests completados por tema
WITH test_stats AS (
  SELECT 
    t.user_id,
    tq.tema_number,
...
SELECT 
  'STATS POR USUARIO-TEMA:' as info,
  user_id,
  tema_number,
  tests_completed,
...
-- =====================================================
-- PASO 2: INSERCIÓN CON ESTRUCTURA CORRECTA
-- =====================================================

-- Insertar en user_progress usando la estructura real
...
SELECT 
  ts.user_id,
  tp.id as topic_id,  -- Usar el UUID real del topic
  ts.total_answers,
  ts.correct_answers,
  ...
  NOW()
FROM test_stats ts
INNER JOIN topics tp ON tp.number = ts.tema_number  -- JOIN con topics para obtener UUID
...
SELECT 
  ...
  STRING_AGG(tp.number::text, ',' ORDER BY tp.number) as temas_numeros
...
SELECT 
  ...
  up.user_id,
  tp.number as tema_numero,
  tp.name as tema_nombre,
  up.total_attempts,
  up.correct_attempts,
