-- FIX: Corregir cálculo de preguntas de hoy en la RPC
-- El problema es que estaba filtrando por test.created_at en lugar de test_questions.created_at
...
BEGIN
  ...
  -- OBTENER RACHAS DE LA TABLA user_streaks
  streak_data AS (
    SELECT
      COALESCE(current_streak, 0) as current_streak,
      COALESCE(longest_streak, COALESCE(current_streak, 0)) as longest_streak
    FROM user_streaks
    WHERE user_id = p_user_id
  ),
  -- FIX: Usar test_questions.created_at para contar preguntas de hoy
  today_stats AS (
    SELECT
  ...
  month_stats AS (
    ...
    INNER JOIN tests t ON t.id = tq.test_id
    WHERE t.user_id = p_user_id
      AND tq.created_at >= CURRENT_DATE - INTERVAL '30 days'
  ),
  three_month_stats AS (
    ...
    INNER JOIN tests t ON t.id = tq.test_id
    WHERE t.user_id = p_user_id
      AND tq.created_at >= CURRENT_DATE - INTERVAL '90 days'
  )
  SELECT
...
-- Verificar que funciona para Carmen
SELECT
  today_tests,
  today_questions,
  today_correct
FROM get_user_public_stats('414fae63-1e52-4943-9c1d-68f065ec7fba'::uuid);