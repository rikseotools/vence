-- Agregar evolución de precisión a la función RPC
...
BEGIN
  ...
  streak_info AS (
    SELECT
      us.user_id,
      us.current_streak::BIGINT,
      us.longest_streak::BIGINT
  ...
  -- Nueva: Precisión del mes pasado
  last_month_stats AS (
    SELECT
      CASE
        WHEN COUNT(tq.id) > 0 THEN
  ...
  three_months_stats AS (
    ...
    LEFT JOIN test_questions tq ON tq.test_id = t.id
    WHERE t.user_id = p_user_id
      AND t.created_at >= CURRENT_DATE - INTERVAL '120 days'
  ...
  SELECT
    ...
    lms.accuracy as accuracy_last_month,
  ...
  CROSS JOIN three_months_stats tms;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

GRANT EXECUTE ON FUNCTION get_user_public_stats(UUID) TO authenticated;

COMMENT ON FUNCTION get_user_public_stats IS 'Estadísticas públicas con evolución de precisión (semana actual, mes pasado, hace 3 meses)';