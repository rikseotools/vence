...
(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_4,_Adquisición_y_pérdida_de_la_condición_de_funcionario';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T404: Adquisición y Pérdida ===\n');
  ...
  console.log('\nRESUMEN T404: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T404']).eq('is_active', true);
  console.log('Total T404 en BD:', count);
})();
