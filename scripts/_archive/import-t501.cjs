...
(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_1,_Concepto_y_aspectos_generales_del_presupuesto';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T501: El Presupuesto ===\n');
  ...
  console.log('\nRESUMEN T501: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T501']).eq('is_active', true);
  console.log('Total T501 en BD:', count);
})();
