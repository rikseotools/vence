...
(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_5,_Provisión_de_puestos_de_trabajo_en_la_función_pública';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T405: Provisión de Puestos ===\n');
  ...
  for (const fileName of files) {
    ...
    for (const q of data.questions) {
      ...
      const { data: article } = await supabase.from('articles').select('id').eq('law_id', LAWS['RDL 5/2015']).eq('article_number', artMatch[1]).single();
  ...
  console.log('\nRESUMEN T405: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T405']).eq('is_active', true);
  console.log('Total T405 en BD:', count);
})();
