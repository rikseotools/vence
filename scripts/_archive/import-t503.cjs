...
(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_3,_El_procedimiento_administrativo_de_ejecución_del_presupuesto_de_gasto';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T503: Ejecución Presupuesto Gasto ===\n');
  ...
  console.log('\nRESUMEN T503: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T503']).eq('is_active', true);
