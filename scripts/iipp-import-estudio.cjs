...
const DIR = 'preguntas-para-subir/instituciones-penitenciarias/estudio-random';
...
const LAW = {
  ...
  RP_1981: '378e81b5-b521-4364-8b66-2b848bfe17d4', // RP 1981 vigente por DT3ª RP 1996 (T219)
};
...
function canonLaw(rest) {
  const map = [
    [/RP\s*1981|RP\s*81|RD\s*1201\/1981|1201\/1981/i, 'RP_1981'], // antes que RP_190_1996 (1981 explícito)
...
(async () => {
  ...
  // dedup: cargar question_text normalizados de TODA la BD (no solo activas) por keyset
  // (OFFSET sin ORDER BY saltaba ~49k filas en silencio — keyset pagination)
  console.error('Cargando preguntas BD para dedup...');
  const dbNorm = new Set();
  let last = '00000000-0000-0000-0000-000000000000';
  while (true) {
    const r = await c.query('SELECT id, question_text FROM questions WHERE id>$1 ORDER BY id LIMIT 10000', [last]);
    if (!r.rows.length) break;
    r.rows.forEach(x => dbNorm.add(normalize(x.question_text)));
    last = r.rows[r.rows.length - 1].id;
    if (r.rows.length < 10000) break;
  }
  console.error('  BD cargadas (todas):', dbNorm.size);
  ...
  for (const f of fs.readdirSync(DIR).filter(f => f.endsWith('.json'))) {
    ...
    for (const q of d.questions) {
      ...
      toInsert.push({
        ...
        tags: [PT, 'T' + topicNum, 'B' + bb.b, 'opositatest-estudio'],
      });
      stats.ready++;
    }
  }
