...
async function main() {
  console.log('\n══════════════════════════════════════════════════════════════');
  console.log('  VERIFICACIÓN DE RESULTADOS - BATCH 4 (Word 365)');
  console.log('══════════════════════════════════════════════════════════════\n');

  // 1. Verificar topic_review_status
  ...
  if (statusError) {
    console.error('❌ Error al verificar topic_review_status:', statusError);
    process.exit(1);
  }

  const counts = {};
  statusCounts.forEach(item => {
    const status = item.topic_review_status || 'null';
    counts[status] = (counts[status] || 0) + 1;
  });

  console.log('📊 Estado de topic_review_status en questions:');
  console.log(`   ✅ verified_correct:    ${counts.verified_correct || 0}`);
  console.log(`   ⚠️  needs_review:        ${counts.needs_review || 0}`);
  console.log(`   ❌ verified_incorrect:  ${counts.verified_incorrect || 0}`);
  console.log(`   📝 null/otros:          ${counts.null || 0}`);
  console.log(`   📊 TOTAL:               ${statusCounts.length}`);

  // 2. Verificar ai_verification_results
  ...
  if (verError) {
    console.error('❌ Error al verificar ai_verification_results:', verError);
    process.exit(1);
  }
  ...
  verificationResults.forEach(r => {
    ...
    else if (r.confidence === 'needs_review') needsReview++;
  });

  console.log('\n📊 Resultados en ai_verification_results:');
  console.log(`   ✅ Correctas:           ${correct} (${(correct/verificationResults.length*100).toFixed(1)}%)`);
  console.log(`   ⚠️  Incorrectas/Revisión: ${incorrect} (${(incorrect/verificationResults.length*100).toFixed(1)}%)`);
  console.log(`   🔒 Alta confianza:      ${highConfidence}`);
  console.log(`   🔍 Necesitan revisión:  ${needsReview}`);
  console.log(`   📝 TOTAL registros:     ${verificationResults.length}/50`);

  console.log('\n══════════════════════════════════════════════════════════════');
  console.log('  RESUMEN FINAL - BATCH 4');
  console.log('══════════════════════════════════════════════════════════════\n');
  console.log(`  Total preguntas:        50`);
  console.log(`  Verificadas correctas:  ${correct} (${(correct/50*100).toFixed(1)}%)`);
  console.log(`  Necesitan revisión:     ${needsReview} (${(needsReview/50*100).toFixed(1)}%)`);
  console.log('\n  FUENTES UTILIZADAS:');
  console.log('  - support.microsoft.com/es-es (combinación de correspondencia)');
  console.log('  - support.microsoft.com/en-us (keyboard shortcuts, mail merge)');
  console.log('  - learn.microsoft.com/es-es (documentación oficial)');
  console.log('\n══════════════════════════════════════════════════════════════\n');
}
