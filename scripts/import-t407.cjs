...
const LAWS = {
  'RDL 4/2000': '3735e11a-5400-469f-881f-8c9695d50f96',
  'RD 375/2003': '6a6710e7-9853-4a3a-a9ff-b0d6bc60d5f1'
};
const LETTER_TO_INDEX = { 'A': 0, 'B': 1, 'C': 2, 'D': 3 };

(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_7,_El_régimen_de_la_Seguridad_Social_de_los_funcionarios';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T407: Seguridad Social Funcionarios ===\n');
  ...
  for (const fileName of files) {
    ...
    for (const q of data.questions) {
      ...
      // Determinar ley
      let lawKey = 'RD 375/2003'; // MUFACE por defecto
      if (fileName.includes('Pasivas') || text.includes('4/2000') || text.includes('clases pasivas')) {
        lawKey = 'RDL 4/2000';
      }

      const { data: article } = await supabase.from('articles').select('id').eq('law_id', LAWS[lawKey]).eq('article_number', artMatch[1]).single();
  ...
  console.log('\nRESUMEN T407: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T407']).eq('is_active', true);
  console.log('Total T407 en BD:', count);
})();
