...
-- Versión 2: Usa los nombres correctos de columnas según la estructura actual de la BD
...
BEGIN
  RETURN QUERY
  WITH user_stats AS (
    -- Calculamos estadísticas desde test_questions ya que tests no tiene correct_answers
    SELECT
      tq.user_id,
      COUNT(DISTINCT t.id) as total_tests,
      COUNT(*) as total_questions,
      COUNT(*) FILTER (WHERE tq.is_correct = true) as correct_answers,
...
COMMENT ON FUNCTION get_user_public_stats IS 'Obtiene estadísticas públicas de cualquier usuario para mostrar en su perfil (v2 - usa test_questions para correct_answers)';
