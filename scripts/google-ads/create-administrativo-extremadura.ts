// scripts/google-ads/create-administrativo-extremadura.ts
//
// Campaña Search (Maximizar clics · CPC máx 0,05€ · 3€/día) para Administrativo (C1)
// de la Junta de Extremadura. Modelo "de las exitosas" del runbook google-ads-analisis.md.
// CONTEXTO (verificado 13/07/2026): 11 plazas turno libre C1 (DOE nº244, 19/12/2025),
//   convocatoria publicada (única de su grupo). Examen sin fecha oficial → PREVISIÓN de
//   consenso ~Q4 2026 (patrón histórico + calendario del C2 hermano). Geo Extremadura.
//   Landing sin ranking orgánico + ya vende 79€ sin anunciarse → ads da visibilidad.
//
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-extremadura.ts          # dry-run
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-extremadura.ts --apply  # CREA (ENABLED)
...
const GEO = 'geoTargetConstants/20279' // Extremadura
const LANG_ES = 'languageConstants/1003'
const FINAL_URL = 'https://www.vence.es/administrativo-extremadura'

const HEADLINES = [
  'Administrativo Extremadura',   // 26
  'Oposición Junta Extremadura',  // 27
...
const DESCRIPTIONS = [
  'Prepara las 11 plazas de Administrativo de la Junta de Extremadura. Tests gratis hoy.', // 85
  'Examen tipo test del temario oficial. Practica por temas y mide tu progreso. Gratis.',  // 84
  'Tests reales de Administrativo de la Junta de Extremadura. Empieza gratis, sin tarjeta.',// 87
]
const KEYWORDS = [
  'administrativo junta de extremadura',
  'oposiciones administrativo extremadura',
  'oposicion administrativo extremadura',
...
async function main() {
  ...
  const operations: any[] = [
    ...
    { entity: 'ad_group', operation: 'create', resource: { resource_name: R('adGroups', -3), name: 'C1 Admin Extremadura - grupo', campaign: R('campaigns', -2), type: 'SEARCH_STANDARD', status: 'ENABLED' } },
  ...
  console.log(`Modo: ${APPLY ? '🔴 APPLY (ENABLED)' : '🔍 DRY-RUN'} · geo Extremadura (20279) · 3€/día · CPC máx 0,05€ · ${FINAL_URL}`)
  try {
