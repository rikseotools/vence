// scripts/google-ads/create-tcae-galicia.ts
//
// Crea la campaña "C2 TCAE Sergas Galicia" siguiendo el runbook
// google-ads-analisis.md §"Crear campaña nueva" y la plantilla create-tcae-sas.ts.
//
// CONTEXTO: TCAE SERGAS (Galicia), 558 plazas. Producto YA completo (31/31 temas).
//   Sin campaña previa. Examen previsto 2º semestre 2026 (sept-nov, sin fecha oficial
//   en DOG aún) → captación forward de pago + orgánico. Presupuesto autorregulado
//   (Maximizar clics + 3€/día + CPC máx 0,05€). --apply crea ENABLED.
//
//   npx tsx --env-file=.env.local scripts/google-ads/create-tcae-galicia.ts          # dry-run
//   npx tsx --env-file=.env.local scripts/google-ads/create-tcae-galicia.ts --apply  # CREA (ENABLED)
...
const GEO_GALICIA = 'geoTargetConstants/20280' // Galicia (Autonomous Community)
const LANG_ES = 'languageConstants/1003'
const FINAL_URL = 'https://www.vence.es/tcae-galicia'

const HEADLINES = [
  'TCAE Sergas Galicia',          // 20
  '558 plazas TCAE Galicia',      // 24
  'Temario oficial y tests',      // 23
  'Oposición TCAE Sergas',        // 22
  'Tests tipo examen TCAE',       // 22
  'Auxiliar Enfermería Sergas',   // 26
  'Empieza gratis',               // 14
]
const DESCRIPTIONS = [
  '558 plazas de TCAE del Servicio Gallego de Salud. Practica con el temario oficial.', // 82
  'Tests por tema y simulacros del examen. Mide tu progreso y empieza gratis.',         // 73
  'Temario oficial del Sergas con preguntas comentadas. Practica desde hoy.',           // 72
]
const KEYWORDS = [
  'tcae sergas',
  'tcae galicia',
  'auxiliar enfermeria sergas',
  'oposicion tcae servicio gallego de salud',
  'tecnico cuidados auxiliares enfermeria galicia',
]
...
async function main() {
  ...
  const operations: any[] = [
    { entity: 'campaign_budget', operation: 'create', resource: { resource_name: R('campaignBudgets', -1), name: 'C2 TCAE Sergas Galicia - budget', amount_micros: 3_000_000, delivery_method: 'STANDARD', explicitly_shared: false } },
    { entity: 'campaign', operation: 'create', resource: { resource_name: R('campaigns', -2), name: 'C2 TCAE Sergas Galicia', status: campaignStatus, advertising_channel_type: 'SEARCH', campaign_budget: R('campaignBudgets', -1), target_spend: { cpc_bid_ceiling_micros: 50_000 }, network_settings: { target_google_search: true, target_search_network: false, target_content_network: false, target_partner_search_network: false }, contains_eu_political_advertising: 'DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING' } },
    { entity: 'campaign_criterion', operation: 'create', resource: { campaign: R('campaigns', -2), location: { geo_target_constant: GEO_GALICIA } } },
    { entity: 'campaign_criterion', operation: 'create', resource: { campaign: R('campaigns', -2), language: { language_constant: LANG_ES } } },
    { entity: 'ad_group', operation: 'create', resource: { resource_name: R('adGroups', -3), name: 'C2 TCAE Sergas - grupo', campaign: R('campaigns', -2), type: 'SEARCH_STANDARD', status: 'ENABLED' } },
  ...
  console.log(`Operaciones: ${operations.length} | geo: Galicia (20280) | 3€/día | CPC máx 0,05€`)
  try {
