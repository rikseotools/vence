// scripts/google-ads/create-administrativo-andalucia.ts
//
// Campaña Search (Maximizar clics · CPC máx 0,05€ · 3€/día) para Administrativo (C1,
// Cuerpo General Administrativo) de la Junta de Andalucía. Modelo del runbook google-ads-analisis.md.
// CONTEXTO (13/07/2026): 216 plazas turno libre (OEP 2025-2026, Decreto 214/2025), convocatoria
//   del turno libre NO publicada aún en BOJA. Examen sin fecha → PREVISIÓN de consenso ~Q1 2027
//   (patrón histórico convocatoria→examen ~5 meses). ⚠️ NO confundir con el examen de 15/03/2025
//   (turno libre anterior, ya pasado) ni el de 30/05/2026 (promoción interna). Geo Andalucía.
//
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-andalucia.ts          # dry-run
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-andalucia.ts --apply  # CREA (ENABLED)
...
const GEO = 'geoTargetConstants/20269' // Andalusia
const LANG_ES = 'languageConstants/1003'
const FINAL_URL = 'https://www.vence.es/administrativo-andalucia'

const HEADLINES = [
  'Administrativo Andalucía',     // 25
  'Oposición Junta Andalucía',    // 26
  '216 plazas turno libre',       // 23
...
const DESCRIPTIONS = [
  'Prepara las 216 plazas de Administrativo de la Junta de Andalucía. Tests gratis hoy.', // 85
  'Examen tipo test del temario oficial. Practica por temas y mide tu progreso. Gratis.', // 84
  'Tests reales del Cuerpo Administrativo de Andalucía. Empieza gratis, sin tarjeta.',     // 81
]
const KEYWORDS = [
  'administrativo junta de andalucia',
  'oposiciones administrativo andalucia',
  'cuerpo administrativo andalucia',
  'tests administrativo andalucia',
  'temario administrativo andalucia',
]
...
async function main() {
  ...
  const operations: any[] = [
    ...
    { entity: 'ad_group', operation: 'create', resource: { resource_name: R('adGroups', -3), name: 'C1 Admin Andalucia - grupo', campaign: R('campaigns', -2), type: 'SEARCH_STANDARD', status: 'ENABLED' } },
  ...
  console.log(`Modo: ${APPLY ? '🔴 APPLY (ENABLED)' : '🔍 DRY-RUN'} · geo Andalucía (20269) · 3€/día · CPC máx 0,05€ · ${FINAL_URL}`)
  try {
