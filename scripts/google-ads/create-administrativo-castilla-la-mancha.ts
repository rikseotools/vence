// scripts/google-ads/create-administrativo-castilla-la-mancha.ts
//
// Campaña Search (Maximizar clics · CPC máx 0,05€ · 3€/día) para Administrativo (C1) de la
// Junta de Comunidades de Castilla-La Mancha (JCCM). Modelo del runbook google-ads-analisis.md.
// CONTEXTO (13/07/2026): OEP 2025 aprobada (827 plazas admón. general, DOCM dic-2025); el C1 sin bases.
//   Plazas ~120 ⚠️ NO oficial (no se comunica cifra en copy). Examen sin fecha → PREVISIÓN ~H2 2027.
//   Slow-burn (Manuel opción 2). Geo Castilla-La Mancha.
//
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-castilla-la-mancha.ts          # dry-run
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-castilla-la-mancha.ts --apply  # CREA (ENABLED)
...
const GEO = 'geoTargetConstants/20276' // Castile-La Mancha
const LANG_ES = 'languageConstants/1003'
const FINAL_URL = 'https://www.vence.es/administrativo-castilla-la-mancha'

const HEADLINES = [
  'Administrativo JCCM',          // 19
  'Oposición Castilla-La Mancha', // 28
...
const DESCRIPTIONS = [
  'Prepara el Cuerpo Administrativo de Castilla-La Mancha (JCCM). Tests gratis hoy.',      // 79
  'Examen tipo test del temario oficial. Practica por temas y mide tu progreso. Gratis.',  // 84
  'Tests reales de Administrativo de Castilla-La Mancha. Empieza gratis y sin tarjeta.',    // 82
]
const KEYWORDS = [
  'administrativo castilla la mancha',
  'oposiciones administrativo castilla la mancha',
  'cuerpo administrativo jccm',
  'tests administrativo castilla la mancha',
...
async function main() {
  ...
  const operations: any[] = [
    ...
    { entity: 'ad_group', operation: 'create', resource: { resource_name: R('adGroups', -3), name: 'C1 Admin CLM - grupo', campaign: R('campaigns', -2), type: 'SEARCH_STANDARD', status: 'ENABLED' } },
  ...
  console.log(`Modo: ${APPLY ? '🔴 APPLY (ENABLED)' : '🔍 DRY-RUN'} · geo Castilla-La Mancha (20276) · 3€/día · CPC máx 0,05€ · ${FINAL_URL}`)
  try {
