// scripts/google-ads/create-administrativo-madrid.ts
//
// Campaña Search (Maximizar clics · CPC máx 0,05€ · 3€/día) para Administrativo (C1) de la
// COMUNIDAD de Madrid (NO ayuntamiento). Modelo del runbook google-ads-analisis.md.
// CONTEXTO (13/07/2026): 129 plazas (120 libre + 9 discap., OEP 2026 Decreto 54/2026, BOCM 28/05/2026).
//   Convocatoria NO publicada. Examen sin fecha → PREVISIÓN de consenso ~Q4 2027/2028 (lag largo).
//   ⚠️ NO confundir con el examen del 7/06/2026 (23 plazas, OEP anterior, YA pasado) ni con el Ayuntamiento.
//   Slow-burn / captación temprana a CPC bajo (Manuel opción 2; premisa "examen lejano no convierte" EN DUDA).
//
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-madrid.ts          # dry-run
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-madrid.ts --apply  # CREA (ENABLED)
...
const FINAL_URL = 'https://www.vence.es/administrativo-madrid'

const HEADLINES = [
  'Administrativo Madrid',        // 21
  'Oposición Comunidad Madrid',   // 26
  '129 plazas turno libre',       // 23
...
const DESCRIPTIONS = [
  'Prepara las 129 plazas de Administrativo de la Comunidad de Madrid. Tests gratis hoy.', // 85
  'Examen tipo test del temario oficial. Practica por temas y mide tu progreso. Gratis.',  // 84
  'Tests reales del Cuerpo Administrativo de Madrid. Empieza gratis, sin tarjeta.',         // 78
]
const KEYWORDS = [
  'administrativo comunidad de madrid',
  'oposiciones administrativo madrid',
  'cuerpo administrativo madrid',
  'tests administrativo comunidad madrid',
  'temario administrativo comunidad madrid',
]
...
async function main() {
  ...
  const operations: any[] = [
    ...
    { entity: 'ad_group', operation: 'create', resource: { resource_name: R('adGroups', -3), name: 'C1 Admin Madrid CM - grupo', campaign: R('campaigns', -2), type: 'SEARCH_STANDARD', status: 'ENABLED' } },
  ...
  console.log(`Modo: ${APPLY ? '🔴 APPLY (ENABLED)' : '🔍 DRY-RUN'} · geo Comunidad de Madrid (20282) · 3€/día · CPC máx 0,05€ · ${FINAL_URL}`)
  try {
