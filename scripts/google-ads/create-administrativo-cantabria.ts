// scripts/google-ads/create-administrativo-cantabria.ts
//
// Campaña Search (Maximizar clics · CPC máx 0,05€ · 3€/día) para Administrativo (C1)
// del Gobierno de Cantabria. Modelo "de las exitosas" del runbook google-ads-analisis.md.
// CONTEXTO (13/07/2026): OEP 2026 aprobada (Decreto 23/2026), convocatoria del C1 NO publicada.
//   Plazas ~30 ⚠️ SIN confirmar (no se comunica cifra en el copy). Examen sin fecha oficial →
//   PREVISIÓN de consenso ~Q4 2026 / Q1 2027 (calendario oficial por bloques + patrón). Geo Cantabria.
//
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-cantabria.ts          # dry-run
//   npx tsx --env-file=.env.local scripts/google-ads/create-administrativo-cantabria.ts --apply  # CREA (ENABLED)
...
const GEO = 'geoTargetConstants/20290' // Cantabria
const LANG_ES = 'languageConstants/1003'
const FINAL_URL = 'https://www.vence.es/administrativo-cantabria'

const HEADLINES = [
  'Administrativo Cantabria',     // 25
  'Oposición Cantabria 2026',     // 24
  'Cuerpo Administrativo C1',     // 24
...
const DESCRIPTIONS = [
  'Prepara el Cuerpo Administrativo del Gobierno de Cantabria. Tests gratis hoy.',        // 77
  'Examen tipo test del temario oficial. Practica por temas y mide tu progreso. Gratis.', // 84
  'Tests reales de Administrativo de Cantabria. Empieza gratis y sin tarjeta.',           // 74
]
const KEYWORDS = [
  'administrativo cantabria',
  'oposiciones administrativo cantabria',
  'oposicion cuerpo administrativo cantabria',
  'tests administrativo cantabria',
  'temario administrativo cantabria',
]
...
async function main() {
  ...
  const operations: any[] = [
    ...
    { entity: 'ad_group', operation: 'create', resource: { resource_name: R('adGroups', -3), name: 'C1 Admin Cantabria - grupo', campaign: R('campaigns', -2), type: 'SEARCH_STANDARD', status: 'ENABLED' } },
  ...
  console.log(`Modo: ${APPLY ? '🔴 APPLY (ENABLED)' : '🔍 DRY-RUN'} · geo Cantabria (20290) · 3€/día · CPC máx 0,05€ · ${FINAL_URL}`)
  try {
