...
const LAWS = {
  ...
  'Ley 53/1984': 'f6f4da4d-845f-45d0-b69f-3554524e23e7'
};
const LETTER_TO_INDEX = { 'A': 0, 'B': 1, 'C': 2, 'D': 3 };

(async () => {
  const basePath = '/home/manuel/Documentos/github/vence/preguntas-para-subir/Tema_6,_Las_incompatibilidades_y_régimen_disciplinario';
  const files = fs.readdirSync(basePath).filter(f => f.endsWith('.json'));

  console.log('=== IMPORTANDO T406: Incompatibilidades y Régimen Disciplinario ===\n');
  ...
  for (const fileName of files) {
    ...
    const isIncompat = fileName.includes('incompatib');
    ...
    for (const q of data.questions) {
      ...
      // Determinar ley según archivo o mención
      let lawId = LAWS['RDL 5/2015'];
      if (isIncompat || text.includes('53/1984') || text.includes('incompatibilidad')) {
        lawId = LAWS['Ley 53/1984'];
      }

      const { data: article } = await supabase.from('articles').select('id').eq('law_id', lawId).eq('article_number', artMatch[1]).single();
  ...
  console.log('\nRESUMEN T406: ✅', totalImported, '⏭️', totalSkipped, '⚠️', totalNoArticle);
  const { count } = await supabase.from('questions').select('id', { count: 'exact', head: true }).contains('tags', ['T406']).eq('is_active', true);
  console.log('Total T406 en BD:', count);
})();
