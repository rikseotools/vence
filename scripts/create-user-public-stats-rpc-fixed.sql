...
-- Usa los nombres correctos de las tablas en tu BD
...
BEGIN
  RETURN QUERY
  WITH user_stats AS (
    -- Usar tabla 'tests' en lugar de 'test_sessions'
    SELECT
      t.user_id,
      COUNT(DISTINCT t.id) as total_tests,
      SUM(t.total_questions) as total_questions,
      SUM(t.correct_answers) as correct_answers,
...
COMMENT ON FUNCTION get_user_public_stats IS 'Obtiene estadísticas públicas de cualquier usuario para mostrar en su perfil (versión corregida)';
