// T9 Galicia DL 1/1999 Régimen financiero — full pipeline
...
const LAW_SHORT = 'DL 1/1999';
const LIBRO67_TEMA = 'Tema_9.';
const LIBRO72_TEMA_PREFIX = /^Tema_14.*1_1999/;
...
function contentHash(q, oa, ob, oc, od) {
  return crypto.createHash('sha256').update(
    normalize(q) + '||' + normalize(oa) + '||' + normalize(ob) + '||' + normalize(oc) + '||' + normalize(od)
  ).digest('hex');
}
...
(async () => {
  ...
  const f67 = fs.readdirSync(d67dir).find(x => x.startsWith(LIBRO67_TEMA));
  const f72 = fs.readdirSync(d72dir).find(x => LIBRO72_TEMA_PREFIX.test(x));
  console.log('libro-67:', f67);
  console.log('libro-72:', f72);
  const d67 = JSON.parse(fs.readFileSync(d67dir + f67));
  const d72 = JSON.parse(fs.readFileSync(d72dir + f72));
  console.log('libro-67 Q:', d67.questions.length);
  console.log('libro-72 Q:', d72.questions.length);
  ...
  for (const [lib, d] of [['67', d67], ['72', d72]]) {
    for (const q of d.questions) {
      ...
      all.push({
        ...
        explanation: q.explanation || null, source_book: 'libro-' + lib,
      });
    }
  }
  ...
  console.log('Nuevas vs BD:', newQs.length, '(dbQ=' + dbQuestions.length + ')');
  ...
  if (unmapped.length > 0) {
    console.log('\nUnmapped samples:');
    unmapped.slice(0, 5).forEach((q, i) => console.log('  [' + (i+1) + '] art=' + (q._art || '—') + ' | ' + q.cleaned.slice(0, 90)));
  }
  ...
  fs.writeFileSync('t9_galicia_imported_ids.json', JSON.stringify(insertedIds, null, 2));
  fs.writeFileSync('t9_galicia_unmapped.json', JSON.stringify(unmapped, null, 2));
})();
