// app/administrativo-cantabria/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Administrativo del Gobierno de Cantabria, C1, 40 temas / 4 bloques)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 21) {
    return { block: 'Derecho Administrativo y Constitucional', displayNum: topicNumber }
  }
  if (topicNumber >= 22 && topicNumber <= 28) {
    return { block: 'Gestión de Personal', displayNum: topicNumber - 21 }
  }
  if (topicNumber >= 29 && topicNumber <= 34) {
    return { block: 'Gestión Financiera', displayNum: topicNumber - 28 }
  }
  if (topicNumber >= 35 && topicNumber <= 40) {
    return { block: 'Organización de las Oficinas Públicas', displayNum: topicNumber - 34 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-cantabria', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_cantabria&return_to=/administrativo-cantabria/temario"
