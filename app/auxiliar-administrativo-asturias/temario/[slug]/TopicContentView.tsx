// app/auxiliar-administrativo-asturias/temario/[slug]/TopicContentView.tsx
...
// Determinar el bloque segun el numero de tema (Auxiliar Administrativo Asturias)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 6) {
    return { block: 'Derecho Constitucional y Organizacion Administrativa', displayNum: topicNumber }
  } else if (topicNumber >= 7 && topicNumber <= 20) {
    return { block: 'Derecho Administrativo y Comunitario', displayNum: topicNumber }
  } else if (topicNumber >= 21 && topicNumber <= 25) {
    return { block: 'Ofimatica', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-asturias', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_administrativo_asturias&return_to=/auxiliar-administrativo-asturias/temario"
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <header className="print-header mb-8">
          {blockInfo.block && (
            <span className="inline-block px-3 py-1 mb-3 text-sm font-medium text-indigo-700 dark:text-indigo-300 bg-indigo-100 dark:bg-indigo-900/30 rounded-full">
              {blockInfo.block}
            </span>
          )}
