// app/auxiliar-administrativo-aragon/test/tema/[numero]/page.tsx
'use client'
...
export default function TemaAragonPage({ params }: PageProps) {
  ...
  useEffect(() => {
    async function resolveParams() {
      ...
      // Aragon: 20 temas (1-20)
      if (isNaN(tema) || tema < 1 || tema > 20) {
  ...
  async function handleStartCustomTest(config: any) {
    ...
    try {
      ...
      const testUrl = `/auxiliar-administrativo-aragon/test/tema/${temaNumber}/${testPath}?${testParams.toString()}`
  ...
  const getBloque = (num: number) => {
    if (num >= 1 && num <= 15) return 'Materias Comunes'
    if (num >= 16 && num <= 20) return 'Materias Especificas'
    return ''
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md mx-auto text-center p-6">
          ...
          <p className="text-gray-600 mb-6">
            El Tema {temaNumber || ''} no existe o no esta disponible para Auxiliar Administrativo de Aragon.
          </p>
          <Link
      ...
      <div className="max-w-4xl mx-auto px-3 py-6">
        ...
        <div className="text-center mb-8">
          <div className="relative inline-block" ref={dropdownRef}>
            <div className="inline-flex items-center px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-medium mb-3">
              <span className="mr-1">🏔️</span>
              <Link href="/auxiliar-administrativo-aragon/test" className="hover:text-yellow-900 transition-colors">
                Aux. Admin. Aragon (C2)
              </Link>
        ...
        <section className="mb-6">
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className={`p-4 rounded-lg ${testMode === 'practica' ? 'bg-yellow-50 border border-yellow-200' : 'bg-yellow-50 border border-yellow-200'}`}>
              <div className="text-sm">
                {testMode === 'practica' ? (
                  <>
                    <span className="font-semibold text-yellow-800">Modo Practica:</span>
                    <span className="text-yellow-700"> Veras una pregunta a la vez con explicacion inmediata.</span>
                  </>
                ) : (
                  <>
                    <span className="font-semibold text-yellow-800">Modo Examen:</span>
                    <span className="text-yellow-700"> Veras todas las preguntas en scroll. Correccion al final.</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
            ...
            <div className="mt-4 pt-3 border-t border-gray-200">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-yellow-600 text-lg">{totalQuestions} preguntas</span>
              </div>
            </div>
          </div>
        )}
