// app/tcae-aragon/temario/[slug]/page.tsx
...
// TCAE Aragón: 30 temas secuenciales (1-30)
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('tcae-aragon', topicNumber)
  ...
  return {
    title: `${content.title} | TCAE Aragón`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para TCAE Aragón`,
    openGraph: {
      title: `${content.title} | TCAE Aragón`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/tcae-aragon/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/tcae-aragon/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="tcae-aragon" />
    </div>
  )
}
