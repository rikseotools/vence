// app/mecanico-conductor-estado/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('mecanico-conductor-estado', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Conducción de Vehículos de Transporte por Carretera del Parque Móvil del Estado`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Conducción de Vehículos de Transporte por Carretera del Parque Móvil del Estado`,
    openGraph: {
      title: `${content.title} | Cuerpo Conducción de Vehículos de Transporte por Carretera del Parque Móvil del Estado`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/mecanico-conductor-estado/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/mecanico-conductor-estado/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="mecanico-conductor-estado" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
