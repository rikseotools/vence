// app/mecanico-conductor-estado/temario/[slug]/TopicContentView.tsx
'use client'
...
// Bloques de Conductor Parque Móvil (generado por el scaffolder desde el spec)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 5) {
    return { block: 'Parte común: organización y funcionamiento de la Administración General del Estado', displayNum: topicNumber }
  } else if (topicNumber >= 6 && topicNumber <= 15) {
    return { block: 'Parte específica: conducción y seguridad vial', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'mecanico-conductor-estado', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=mecanico_conductor_estado&return_to=/mecanico-conductor-estado/temario"
