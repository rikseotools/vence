...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('celador-sas', topicNumber)
  ...
  return {
    title: `${content.title} | Celador SAS Andalucía`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Celador SAS Andalucía`,
    openGraph: {
      title: `${content.title} | Celador SAS Andalucía`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/celador-sas/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/celador-sas/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="celador-sas" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
