// app/auxiliar-biblioteca-estado/temario/[slug]/TopicContentView.tsx
'use client'
...
// Bloques de Auxiliar de Biblioteca (Estado) (generado por el scaffolder desde el spec)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 11) {
    return { block: 'Legislación', displayNum: topicNumber }
  } else if (topicNumber >= 101 && topicNumber <= 104) {
    return { block: 'Historia del libro y las bibliotecas', displayNum: topicNumber - 100 }
  } else if (topicNumber >= 201 && topicNumber <= 230) {
    return { block: 'Biblioteconomía (parte específica)', displayNum: topicNumber - 200 }
  } else if (topicNumber >= 301 && topicNumber <= 303) {
    return { block: 'Parte práctica', displayNum: topicNumber - 300 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-biblioteca-estado', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_biblioteca_estado&return_to=/auxiliar-biblioteca-estado/temario"
