// app/auxiliar-biblioteca-estado/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('auxiliar-biblioteca-estado', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado (Sección Bibliotecas)`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado (Sección Bibliotecas)`,
    openGraph: {
      title: `${content.title} | Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado (Sección Bibliotecas)`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/auxiliar-biblioteca-estado/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/auxiliar-biblioteca-estado/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="auxiliar-biblioteca-estado" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
