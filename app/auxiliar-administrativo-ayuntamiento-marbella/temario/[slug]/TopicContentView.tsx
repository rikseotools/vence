// app/auxiliar-administrativo-ayuntamiento-marbella/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Auxiliar Administrativo C2 Marbella)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  // Numeración secuencial 1-27 en 5 bloques (6 comunes + 21 específicas)
  if (topicNumber >= 1 && topicNumber <= 6) {
    return { block: 'Materias Comunes', displayNum: topicNumber }
  } else if (topicNumber >= 7 && topicNumber <= 11) {
    return { block: 'Administración Local', displayNum: topicNumber }
  } else if (topicNumber >= 12 && topicNumber <= 18) {
    return { block: 'Régimen Jurídico y Derechos', displayNum: topicNumber }
  } else if (topicNumber >= 19 && topicNumber <= 25) {
    return { block: 'Gestión y Función Pública Local', displayNum: topicNumber }
  } else if (topicNumber >= 26 && topicNumber <= 27) {
    return { block: 'Transparencia e Informática', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-ayuntamiento-marbella', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_administrativo_ayuntamiento_marbella&return_to=/auxiliar-administrativo-ayuntamiento-marbella/temario"
