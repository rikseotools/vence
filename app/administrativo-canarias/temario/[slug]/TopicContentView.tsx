// app/administrativo-canarias/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Administrativo C1 Canarias)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  // Numeración secuencial 1-30 en 5 bloques
  if (topicNumber >= 1 && topicNumber <= 8) {
    return { block: 'Organización del Estado', displayNum: topicNumber }
  } else if (topicNumber >= 9 && topicNumber <= 14) {
    return { block: 'Organización de Canarias', displayNum: topicNumber }
  } else if (topicNumber >= 15 && topicNumber <= 20) {
    return { block: 'La Unión Europea', displayNum: topicNumber }
  } else if (topicNumber >= 21 && topicNumber <= 24) {
    return { block: 'Régimen Jurídico, Transparencia y Datos', displayNum: topicNumber }
  } else if (topicNumber >= 25 && topicNumber <= 30) {
    return { block: 'Derechos, Igualdad, Empleo Público y Prevención', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-canarias', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_canarias&return_to=/administrativo-canarias/temario"
