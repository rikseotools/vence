// app/auxilio-judicial/temario/[slug]/page.tsx
...
export async function generateStaticParams() {
  // Auxilio Judicial tiene 26 temas
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('auxilio-judicial', topicNumber)
  ...
  return {
    title: `${content.title} | Auxilio Judicial`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Auxilio Judicial`,
    openGraph: {
      title: `${content.title} | Auxilio Judicial`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/auxilio-judicial/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/auxilio-judicial/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="auxilio-judicial" />
    </div>
  )
}
