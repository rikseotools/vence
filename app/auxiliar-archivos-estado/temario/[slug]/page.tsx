// app/auxiliar-archivos-estado/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('auxiliar-archivos-estado', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado (Sección Archivos)`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado (Sección Archivos)`,
    openGraph: {
      title: `${content.title} | Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado (Sección Archivos)`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/auxiliar-archivos-estado/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/auxiliar-archivos-estado/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="auxiliar-archivos-estado" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
