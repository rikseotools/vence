// app/enfermero-scs-canarias/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('enfermero-scs-canarias', topicNumber)
  ...
  return {
    title: `${content.title} | Enfermero SCS Canarias`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Enfermero SCS Canarias`,
    openGraph: {
      title: `${content.title} | Enfermero SCS Canarias`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/enfermero-scs-canarias/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/enfermero-scs-canarias/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="enfermero-scs-canarias" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
