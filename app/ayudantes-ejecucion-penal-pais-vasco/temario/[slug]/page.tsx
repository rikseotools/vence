// app/ayudantes-ejecucion-penal-pais-vasco/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('ayudantes-ejecucion-penal-pais-vasco', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Ayudantes en Ejecución Penal (Euskadi)`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Ayudantes en Ejecución Penal (Euskadi)`,
    openGraph: {
      title: `${content.title} | Cuerpo Ayudantes en Ejecución Penal (Euskadi)`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/ayudantes-ejecucion-penal-pais-vasco/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/ayudantes-ejecucion-penal-pais-vasco/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="ayudantes-ejecucion-penal-pais-vasco" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
