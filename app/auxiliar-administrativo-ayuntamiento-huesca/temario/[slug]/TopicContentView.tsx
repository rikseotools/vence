// app/auxiliar-administrativo-ayuntamiento-huesca/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Auxiliar Administrativo del Ayuntamiento de Huesca)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 11) {
    return { block: 'Bloque I: Derecho Constitucional y Administrativo', displayNum: topicNumber }
  }
  if (topicNumber >= 12 && topicNumber <= 19) {
    return { block: 'Bloque II: Régimen Local y Hacienda Local', displayNum: topicNumber }
  }
  if (topicNumber >= 20 && topicNumber <= 24) {
    return { block: 'Bloque III: Prevención y Gestión Documental', displayNum: topicNumber }
  }
  if (topicNumber === 25) {
    return { block: 'Bloque IV: Ofimática e Informática', displayNum: topicNumber }
  }
  if (topicNumber >= 26 && topicNumber <= 28) {
    return { block: 'Bloque V: Protección de Datos e Igualdad', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-ayuntamiento-huesca', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_administrativo_ayuntamiento_huesca&return_to=/auxiliar-administrativo-ayuntamiento-huesca/temario"
