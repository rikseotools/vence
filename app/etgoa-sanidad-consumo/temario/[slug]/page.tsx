// app/etgoa-sanidad-consumo/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('etgoa-sanidad-consumo', topicNumber)
  ...
  return {
    title: `${content.title} | ETGOA Sanidad y Consumo`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para ETGOA Sanidad y Consumo`,
    openGraph: {
      title: `${content.title} | ETGOA Sanidad y Consumo`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/etgoa-sanidad-consumo/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/etgoa-sanidad-consumo/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="etgoa-sanidad-consumo" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
