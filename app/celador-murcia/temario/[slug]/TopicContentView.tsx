// app/celador-murcia/temario/[slug]/TopicContentView.tsx
'use client'
...
// Bloques de Celador SMS Murcia (generado por el scaffolder desde el spec)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 7) {
    return { block: 'Parte general (materias comunes)', displayNum: topicNumber }
  } else if (topicNumber >= 8 && topicNumber <= 14) {
    return { block: 'Parte específica (Celador-Subalterno)', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'celador-murcia', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=celador_murcia&return_to=/celador-murcia/temario"
