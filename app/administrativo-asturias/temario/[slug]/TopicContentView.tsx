// app/administrativo-asturias/temario/[slug]/TopicContentView.tsx
...
// Determinar la parte segun el numero de tema (Administrativo Principado de Asturias C1)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 7) {
    return { block: 'Bloque I: Derecho Constitucional y Organización Administrativa', displayNum: topicNumber }
  } else if (topicNumber >= 201 && topicNumber <= 216) {
    return { block: 'Bloque II: Derecho Administrativo y Comunitario', displayNum: topicNumber - 193 }
  } else if (topicNumber >= 301 && topicNumber <= 306) {
    return { block: 'Bloque III: Gestión de Recursos Humanos', displayNum: topicNumber - 277 }
  } else if (topicNumber >= 401 && topicNumber <= 405) {
    return { block: 'Bloque IV: Gestión Financiera', displayNum: topicNumber - 371 }
  } else if (topicNumber >= 501 && topicNumber <= 504) {
    return { block: 'Bloque V: Ofimática', displayNum: topicNumber - 466 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-asturias', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_asturias&return_to=/administrativo-asturias/temario"
