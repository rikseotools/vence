// app/auxiliar-administrativo-sermas/temario/[slug]/TopicContentView.tsx
// Auxiliar Administrativo SERMAS: 5 bloques, temas 1-31
'use client'
...
// Determinar el bloque segun el numero de tema (Auxiliar Administrativo SERMAS)
// Bloque I (Organizacion Politica y Sanitaria): temas 1-8
// Bloque II (Derecho Administrativo): temas 9-11
// Bloque III (Personal y Legislacion Laboral): temas 12-19
// Bloque IV (Gestion Administrativa y Sanitaria): temas 20-26
// Bloque V (Informatica y Administracion Electronica): temas 27-31
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 8) {
    return { block: 'Bloque I — Organización Política y Sanitaria', displayNum: topicNumber }
  } else if (topicNumber >= 9 && topicNumber <= 11) {
    return { block: 'Bloque II — Derecho Administrativo', displayNum: topicNumber }
  } else if (topicNumber >= 12 && topicNumber <= 19) {
    return { block: 'Bloque III — Personal y Legislación Laboral', displayNum: topicNumber }
  } else if (topicNumber >= 20 && topicNumber <= 26) {
    return { block: 'Bloque IV — Gestión Administrativa y Sanitaria', displayNum: topicNumber }
  } else if (topicNumber >= 27 && topicNumber <= 31) {
    return { block: 'Bloque V — Informática y Administración Electrónica', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-sermas', updatedAt }: TopicContentViewProps) {
