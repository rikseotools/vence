// app/ayudante-instituciones-penitenciarias/temario/[slug]/TopicContentView.tsx
'use client'
...
// Mapping de temas a cursos de video (ayudante de instituciones penitenciarias comparte informática con tramitación)
...
// Determinar el bloque según el número de tema (ayudante de instituciones penitenciarias)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 17) {
    return { block: 'Bloque I', displayNum: topicNumber }
  } else if (topicNumber >= 101 && topicNumber <= 110) {
    return { block: 'Bloque II', displayNum: topicNumber - 100 }
  } else if (topicNumber >= 201 && topicNumber <= 220) {
    return { block: 'Bloque III', displayNum: topicNumber - 200 }
  } else if (topicNumber >= 301 && topicNumber <= 303) {
    return { block: 'Bloque IV', displayNum: topicNumber - 300 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'ayudante-instituciones-penitenciarias', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=ayudante_instituciones_penitenciarias&return_to=/ayudante-instituciones-penitenciarias/temario"
