...
export default function TemaCylPage({ params }: PageProps) {
  ...
  async function handleStartCustomTest(config: any) {
    ...
    try {
      ...
      const testUrl = `/auxiliar-administrativo-cyl/test/tema/${temaNumber}/${testPath}?${testParams.toString()}`
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md mx-auto text-center p-6">
          ...
          <p className="text-gray-600 mb-6">
            El Tema {temaNumber || ''} no existe o no está disponible para Auxiliar Administrativo Castilla y León.
          </p>
          <Link
      ...
      <div className="max-w-4xl mx-auto px-3 py-6">
        ...
        <div className="text-center mb-8">
          <div className="relative inline-block" ref={dropdownRef}>
            <div className="inline-flex items-center px-3 py-1 bg-rose-100 text-rose-800 rounded-full text-xs font-medium mb-3">
              <span className="mr-1">🏛️</span>
              <Link href="/auxiliar-administrativo-cyl/test" className="hover:text-rose-900 transition-colors">
                Aux. Admin. CyL (C2)
              </Link>
              <span className="mx-2 text-rose-600">›</span>
              <span className="font-semibold">{getGrupo(temaNumber || 0)}</span>
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {officialQuestionsCount > 0 && (
              <p className="text-rose-600 font-medium text-sm md:text-base">
        ...
        <section className="mb-6">
          <div className="bg-white rounded-lg shadow-sm p-6">
            ...
            <div className={`p-4 rounded-lg ${testMode === 'practica' ? 'bg-rose-50 border border-rose-200' : 'bg-pink-50 border border-pink-200'}`}>
              <div className="text-sm">
                {testMode === 'practica' ? (
                  <>
                    <span className="font-semibold text-rose-800">Modo Práctica:</span>
                    <span className="text-rose-700"> Verás una pregunta a la vez con explicación inmediata.</span>
                  </>
                ) : (
                  <>
                    <span className="font-semibold text-pink-800">Modo Examen:</span>
                    <span className="text-pink-700"> Verás todas las preguntas en scroll. Corrección al final.</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-lg shadow-sm p-6 mb-8">
            ...
            <div className="mt-4 pt-3 border-t border-gray-200">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-rose-600 text-lg">{totalQuestions} preguntas</span>
              </div>
            </div>
          </div>
        )}
