// app/administrativo-universidad-leon/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('administrativo-universidad-leon', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Escala Administrativa Universidad de León`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Escala Administrativa Universidad de León`,
    openGraph: {
      title: `${content.title} | Cuerpo Escala Administrativa Universidad de León`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/administrativo-universidad-leon/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/administrativo-universidad-leon/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="administrativo-universidad-leon" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
