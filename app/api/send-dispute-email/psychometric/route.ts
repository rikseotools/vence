...
interface PsychometricDisputeInfo {
  id: string
  user_id: string
  question_id: string
  status: string
  ...
  questions: { question_text: string; question_subtype: string } | null
}

async function getDisputeInfo(disputeId: string): Promise<PsychometricDisputeInfo | null> {
  try {
...
export async function POST(request: Request) {
  try {
    ...
    console.error('❌ API: Error enviando email psicotécnico:', 'error' in result ? result.error : 'Unknown')
    return NextResponse.json({
      success: false,
