...
interface DebugResult {
  ...
  token_generation: { status: string; token?: string; secret_length?: number; error?: string } | null
  supabase_connection: { status: string; status_code?: number; url?: string; error?: string; missing?: string } | null
  user_lookup: {
    status: string
    status_code?: number
    user_found?: string
      ...
      try {
        ...
        if (response.ok) {
          const userResponse = await fetch(
            `${SUPABASE_URL}/rest/v1/user_profiles?email=eq.${encodeURIComponent(email)}&select=id,email`,
            {
              headers: {
                apikey: SUPABASE_KEY,
                Authorization: `Bearer ${SUPABASE_KEY}`,
