// app/administrativo-estado/temario/[slug]/TopicContentView.tsx
'use client'
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_estado&return_to=/administrativo-estado/temario"
      ...
      {(isExpanded || true) && (
        <div className={`mt-2 space-y-4 ${!isExpanded ? 'hidden print:block' : ''}`}>
          {articles.map((article) => (
            <ArticleCard key={article.id} article={article} lawShortName={law.shortName} lawName={law.name} />
          ))}
        </div>
      )}
    </section>
...
function ArticleCard({ article, lawShortName, lawName }: { article: Article; lawShortName: string; lawName: string }) {
  const { getSlug } = useLawSlugs()
