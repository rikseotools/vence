// app/administrativo-navarra/temario/[slug]/TopicContentView.tsx
// Reuses the shared TopicContentView with Navarra-specific block ranges
'use client'
...
const topicVideoCourses: Record<number, {
  ...
  // No video courses mapped yet for Navarra
}
...
// Determinar el bloque segun el numero de tema (Administrativo Navarra)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 5) {
    return { block: 'Parte I: Actividad Administrativa', displayNum: topicNumber }
  } else if (topicNumber >= 6 && topicNumber <= 23) {
    return { block: 'Parte II: Normativa', displayNum: topicNumber }
  } else if (topicNumber >= 24 && topicNumber <= 27) {
    return { block: 'Parte III: Informatica', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-navarra', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <header className="print-header mb-8">
          {blockInfo.block && (
            <span className="inline-block px-3 py-1 mb-3 text-sm font-medium text-yellow-700 dark:text-yellow-300 bg-yellow-100 dark:bg-yellow-900/30 rounded-full">
              {blockInfo.block}
            </span>
          )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6 relative">
            ...
            <div className="text-center">
              ...
              <div className="space-y-3">
                <Link
                  href="/login?oposicion=administrativo_navarra&return_to=/administrativo-navarra/temario"
