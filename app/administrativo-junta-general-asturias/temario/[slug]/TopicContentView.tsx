// app/administrativo-junta-general-asturias/temario/[slug]/TopicContentView.tsx
// Reuses the CyL TopicContentView with Asturias-specific block ranges
'use client'
...
// Determinar la parte segun el numero de tema (Administrativo de la Junta General del Principado de Asturias C1)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 7) {
    return { block: 'Bloque I: Derecho Constitucional y Organización', displayNum: topicNumber }
  } else if (topicNumber >= 8 && topicNumber <= 19) {
    return { block: 'Bloque II: La Junta General del Principado', displayNum: topicNumber }
  } else if (topicNumber >= 20 && topicNumber <= 26) {
    return { block: 'Bloque III: Lengua Española y Documentación', displayNum: topicNumber }
  } else if (topicNumber >= 27 && topicNumber <= 36) {
    return { block: 'Bloque IV: Derecho Administrativo y Laboral', displayNum: topicNumber }
  } else if (topicNumber >= 37 && topicNumber <= 40) {
    return { block: 'Bloque V: Ofimática', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-junta-general-asturias', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_junta_general_asturias&return_to=/administrativo-junta-general-asturias/temario"
