// app/tcae-sescam/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('tcae-sescam', topicNumber)
  if (!content) return { title: 'Tema no encontrado' }

  return {
    title: `${content.title} | TCAE SESCAM`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para TCAE SESCAM`,
    openGraph: {
      title: `${content.title} | TCAE SESCAM`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/tcae-sescam/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: { canonical: `https://www.vence.es/tcae-sescam/temario/${slug}` },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="tcae-sescam" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
