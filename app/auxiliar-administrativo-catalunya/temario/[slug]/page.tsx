// app/auxiliar-administrativo-catalunya/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('auxiliar-administrativo-catalunya', topicNumber)
  ...
  return {
    title: `${content.title} | Auxiliar Administrativo Generalitat de Catalunya`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Auxiliar Administrativo del Generalitat de Catalunya`,
    openGraph: {
      title: `${content.title} | Auxiliar Administrativo Generalitat de Catalunya`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/auxiliar-administrativo-catalunya/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/auxiliar-administrativo-catalunya/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="auxiliar-administrativo-catalunya" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
