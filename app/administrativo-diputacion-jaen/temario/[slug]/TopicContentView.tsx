// app/administrativo-diputacion-jaen/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Administrativo C1 Jaén)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  // Numeración secuencial 1-40 en 6 bloques (8 comunes + 32 específicas)
  if (topicNumber >= 1 && topicNumber <= 8) {
    return { block: 'Materias Comunes', displayNum: topicNumber }
  } else if (topicNumber >= 9 && topicNumber <= 21) {
    return { block: 'Derecho Administrativo General', displayNum: topicNumber }
  } else if (topicNumber >= 22 && topicNumber <= 28) {
    return { block: 'Régimen Local y Protección de Datos', displayNum: topicNumber }
  } else if (topicNumber >= 29 && topicNumber <= 33) {
    return { block: 'Función Pública Local', displayNum: topicNumber }
  } else if (topicNumber >= 34 && topicNumber <= 38) {
    return { block: 'Haciendas Locales', displayNum: topicNumber }
  } else if (topicNumber >= 39 && topicNumber <= 40) {
    return { block: 'Documentación y Administración Electrónica', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-diputacion-jaen', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_diputacion_jaen&return_to=/administrativo-diputacion-jaen/temario"
