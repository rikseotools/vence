...
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 19) {
    return { block: 'Bloque I — Temario Común', displayNum: topicNumber }
  } else if (topicNumber >= 101 && topicNumber <= 130) {
    return { block: 'Bloque II — Temario Específico', displayNum: topicNumber - 100 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-enfermeria-gva', updatedAt }: TopicContentViewProps) {
