// app/auxiliar-administrativo-baleares/temario/[slug]/TopicContentView.tsx
// Reuses the CyL TopicContentView with Baleares-specific block ranges
'use client'
...
// Determinar el bloque segun el numero de tema (Auxiliar Administrativo Baleares)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  ...
  } else if (topicNumber >= 21 && topicNumber <= 36) {
    return { block: 'Ofimatica', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-baleares' }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <button
            onClick={handlePrint}
            className="flex items-center gap-2 px-4 py-1.5 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 transition-colors"
          >
      ...
      <main className="max-w-4xl mx-auto px-4 py-8">
        ...
        <header className="print-header mb-8">
          {blockInfo.block && (
            <span className="inline-block px-3 py-1 mb-3 text-sm font-medium text-cyan-700 dark:text-cyan-300 bg-cyan-100 dark:bg-cyan-900/30 rounded-full">
              {blockInfo.block}
            </span>
          )}
            ...
            return (
              <div className="mt-4 p-4 bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-cyan-900/20 dark:to-blue-900/20 border border-cyan-200 dark:border-cyan-700 rounded-xl">
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 w-10 h-10 bg-cyan-100 dark:bg-cyan-800 rounded-full flex items-center justify-center overflow-hidden">
                    {avatarUrl ? (
                  ...
                  <div className="flex-1">
                    <p className="text-sm text-cyan-900 dark:text-cyan-100">
                      ...
                      <span className="font-semibold text-cyan-700 dark:text-cyan-300">personalizado para ti</span>.{' '}
            ...
            <div className="flex gap-3">
              {content.topicNumber > 1 && (
                <Link
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-gray-600 dark:text-gray-300 hover:text-cyan-600 dark:hover:text-cyan-400 border border-gray-300 dark:border-gray-600 rounded-md transition-colors"
                >
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full p-6 relative">
            ...
            <div className="text-center">
              ...
              <div className="space-y-3">
                <Link
                  href="/login?oposicion=auxiliar_administrativo_baleares&return_to=/auxiliar-administrativo-baleares/temario"
                  className="block w-full py-3 px-4 bg-cyan-600 text-white font-medium rounded-lg hover:bg-cyan-700 transition-colors"
                >
                  Registrarse gratis
                </Link>
                <button
      ...
      <div className={`px-4 py-3 border-b ${hasOfficialQuestions ? 'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-700' : 'bg-gray-50 dark:bg-gray-750 border-gray-200 dark:border-gray-700'}`}>
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-baseline gap-2 flex-1 min-w-0">
            <span className="font-mono text-sm font-semibold text-cyan-600 dark:text-cyan-400 flex-shrink-0">Art. {article.articleNumber}</span>
