// app/administrativo-carm/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('administrativo-carm', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Administrativo CARM`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Administrativo CARM`,
    openGraph: {
      title: `${content.title} | Cuerpo Administrativo CARM`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/administrativo-carm/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/administrativo-carm/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="administrativo-carm" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
