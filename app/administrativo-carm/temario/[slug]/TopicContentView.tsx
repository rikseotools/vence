// app/administrativo-carm/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Administrativo C1 Murcia)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  // Numeración secuencial 1-28 en 3 bloques
  if (topicNumber >= 1 && topicNumber <= 14) {
    return { block: 'Organización del Estado y Gestión Administrativa', displayNum: topicNumber }
  } else if (topicNumber >= 15 && topicNumber <= 21) {
    return { block: 'Gestión de Recursos Humanos', displayNum: topicNumber }
  } else if (topicNumber >= 22 && topicNumber <= 28) {
    return { block: 'Gestión Económico-Presupuestaria y Tributaria', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-carm', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_carm&return_to=/administrativo-carm/temario"
