// app/tramitacion-procesal/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('tramitacion-procesal', topicNumber)
  ...
  return {
    title: `${content.title} | Tramitación Procesal`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Tramitación Procesal`,
    openGraph: {
      title: `${content.title} | Tramitación Procesal`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/tramitacion-procesal/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/tramitacion-procesal/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="tramitacion-procesal" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
