// app/administrativo-castilla-la-mancha/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('administrativo-castilla-la-mancha', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Administrativo Castilla-La Mancha`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Administrativo Castilla-La Mancha`,
    openGraph: {
      title: `${content.title} | Cuerpo Administrativo Castilla-La Mancha`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/administrativo-castilla-la-mancha/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/administrativo-castilla-la-mancha/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="administrativo-castilla-la-mancha" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
