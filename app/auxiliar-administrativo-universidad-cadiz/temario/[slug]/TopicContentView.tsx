// app/auxiliar-administrativo-universidad-cadiz/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Auxiliar Administrativo de la Universidad de Cádiz)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 7) {
    return { block: 'Bloque I: Organización de la Administración', displayNum: topicNumber }
  } else if (topicNumber >= 8 && topicNumber <= 14) {
    return { block: 'Bloque II: Derecho Administrativo', displayNum: topicNumber }
  } else if (topicNumber >= 15 && topicNumber <= 19) {
    return { block: 'Bloque III: Gestión de personal', displayNum: topicNumber }
  } else if (topicNumber >= 20 && topicNumber <= 27) {
    return { block: 'Bloque IV: Gestión universitaria', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-universidad-cadiz', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_administrativo_universidad_cadiz&return_to=/auxiliar-administrativo-universidad-cadiz/temario"
