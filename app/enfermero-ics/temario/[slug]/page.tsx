// app/enfermero-ics/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('enfermero-ics', topicNumber)
  ...
  return {
    title: `${content.title} | Enfermero ICS`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Enfermero ICS`,
    openGraph: {
      title: `${content.title} | Enfermero ICS`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/enfermero-ics/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/enfermero-ics/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="enfermero-ics" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
