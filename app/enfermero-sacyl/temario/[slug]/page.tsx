// app/enfermero-sacyl/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('enfermero-sacyl', topicNumber)
  ...
  return {
    title: `${content.title} | Enfermero SACYL`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Enfermero SACYL`,
    openGraph: {
      title: `${content.title} | Enfermero SACYL`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/enfermero-sacyl/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/enfermero-sacyl/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="enfermero-sacyl" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
