...
export default function TemaCarmPage({ params }: PageProps) {
  ...
  async function handleStartCustomTest(config: any) {
    ...
    try {
      ...
      const testUrl = `/auxiliar-administrativo-carm/test/tema/${temaNumber}/${testPath}?${testParams.toString()}`
  ...
  const getBloque = (num: number) => {
    if (num >= 1 && num <= 9) return 'Bloque I'
    if (num >= 10 && num <= 16) return 'Bloque II'
    return ''
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="max-w-md mx-auto text-center p-6">
          ...
          <p className="text-gray-600 mb-6">
            El Tema {temaNumber || ''} no existe o no está disponible para Auxiliar Administrativo CARM.
          </p>
          <Link
        ...
        <div className="text-center mb-8">
          <div className="relative inline-block" ref={dropdownRef}>
            <div className="inline-flex items-center px-3 py-1 bg-amber-100 text-amber-800 rounded-full text-xs font-medium mb-3">
              ...
              <Link href="/auxiliar-administrativo-carm/test" className="hover:text-amber-900 transition-colors">
                Aux. Admin. CARM (C2)
              </Link>
