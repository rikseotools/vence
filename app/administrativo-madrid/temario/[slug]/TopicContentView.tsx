// app/administrativo-madrid/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Administrativo de la Comunidad de Madrid, C1, 47 temas / 5 bloques)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 8) {
    return { block: 'Organización del Estado y de la Comunidad de Madrid', displayNum: topicNumber }
  }
  if (topicNumber >= 9 && topicNumber <= 24) {
    return { block: 'Derecho Administrativo General', displayNum: topicNumber - 8 }
  }
  if (topicNumber >= 25 && topicNumber <= 32) {
    return { block: 'Gestión de Recursos Humanos', displayNum: topicNumber - 24 }
  }
  if (topicNumber >= 33 && topicNumber <= 39) {
    return { block: 'Gestión Financiera', displayNum: topicNumber - 32 }
  }
  if (topicNumber >= 40 && topicNumber <= 47) {
    return { block: 'Informática y Ofimática', displayNum: topicNumber - 39 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-madrid', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_madrid&return_to=/administrativo-madrid/temario"
