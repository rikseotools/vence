// app/administrativo-castilla-leon/temario/[slug]/TopicContentView.tsx
'use client'
...
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  ...
  if (topicNumber >= 1 && topicNumber <= 10) {
    return { block: 'Grupo I', displayNum: topicNumber }
  } else if (topicNumber >= 201 && topicNumber <= 209) {
    return { block: 'Grupo II', displayNum: topicNumber - 190 }
  } else if (topicNumber >= 301 && topicNumber <= 305) {
    return { block: 'Grupo III', displayNum: topicNumber - 281 }
  } else if (topicNumber >= 401 && topicNumber <= 406) {
    return { block: 'Grupo IV', displayNum: topicNumber - 376 }
  } else if (topicNumber >= 501 && topicNumber <= 511) {
    return { block: 'Grupo V', displayNum: topicNumber - 470 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-castilla-leon', updatedAt }: TopicContentViewProps) {
