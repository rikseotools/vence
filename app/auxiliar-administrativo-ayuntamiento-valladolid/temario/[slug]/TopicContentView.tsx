// app/auxiliar-administrativo-ayuntamiento-valladolid/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Auxiliar Administrativo C2 Valladolid)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  // Numeración oficial 1-31 sin tema 8, en 5 bloques (30 temas)
  if (topicNumber >= 1 && topicNumber <= 6) {
    return { block: 'Organización del Estado y de Castilla y León', displayNum: topicNumber }
  } else if (topicNumber >= 7 && topicNumber <= 10) {
    return { block: 'El Ayuntamiento de Valladolid', displayNum: topicNumber }
  } else if (topicNumber >= 11 && topicNumber <= 19) {
    return { block: 'Derecho Administrativo y Contratación', displayNum: topicNumber }
  } else if (topicNumber >= 20 && topicNumber <= 22) {
    return { block: 'Personal, Igualdad y Prevención de Riesgos', displayNum: topicNumber }
  } else if (topicNumber >= 23 && topicNumber <= 31) {
    return { block: 'Información, Archivo, Transparencia e Informática', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-ayuntamiento-valladolid', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_administrativo_ayuntamiento_valladolid&return_to=/auxiliar-administrativo-ayuntamiento-valladolid/temario"
