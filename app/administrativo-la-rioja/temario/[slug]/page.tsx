// app/administrativo-la-rioja/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('administrativo-la-rioja', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Administrativo La Rioja`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Administrativo La Rioja`,
    openGraph: {
      title: `${content.title} | Cuerpo Administrativo La Rioja`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/administrativo-la-rioja/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/administrativo-la-rioja/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="administrativo-la-rioja" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
