...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('administrativo-aragon', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Administrativo de la Comunidad Autónoma de Aragón`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Administrativo de la Comunidad Autónoma de Aragón`,
    openGraph: {
      title: `${content.title} | Cuerpo Administrativo de la Comunidad Autónoma de Aragón`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/administrativo-aragon/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/administrativo-aragon/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="administrativo-aragon" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
