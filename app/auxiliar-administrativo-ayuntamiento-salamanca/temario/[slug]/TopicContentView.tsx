// app/auxiliar-administrativo-ayuntamiento-salamanca/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Auxiliar Administrativo C2 Salamanca)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  // Numeración oficial 1-41 en 5 bloques (BOP Salamanca nº 93/2024)
  if (topicNumber >= 1 && topicNumber <= 10) {
    return { block: 'Derecho Constitucional y Administrativo', displayNum: topicNumber }
  } else if (topicNumber >= 11 && topicNumber <= 22) {
    return { block: 'Administración Local', displayNum: topicNumber }
  } else if (topicNumber >= 23 && topicNumber <= 29) {
    return { block: 'Atención, Transparencia, Protección de Datos y Archivo', displayNum: topicNumber }
  } else if (topicNumber >= 30 && topicNumber <= 34) {
    return { block: 'Calidad, Igualdad y Violencia de Género', displayNum: topicNumber }
  } else if (topicNumber >= 35 && topicNumber <= 41) {
    return { block: 'Ofimática e Informática', displayNum: topicNumber }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'auxiliar-administrativo-ayuntamiento-salamanca', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=auxiliar_administrativo_ayuntamiento_salamanca&return_to=/auxiliar-administrativo-ayuntamiento-salamanca/temario"
