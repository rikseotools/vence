// app/celador-ibsalut/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('celador-ibsalut', topicNumber)
  ...
  return {
    title: `${content.title} | Celador IB-Salut Balears`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Celador IB-Salut Balears`,
    openGraph: {
      title: `${content.title} | Celador IB-Salut Balears`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/celador-ibsalut/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/celador-ibsalut/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="celador-ibsalut" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
