// app/enfermero-sms/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('enfermero-sms', topicNumber)
  ...
  return {
    title: `${content.title} | Enfermero SMS Murcia`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Enfermero SMS Murcia`,
    openGraph: {
      title: `${content.title} | Enfermero SMS Murcia`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/enfermero-sms/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/enfermero-sms/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="enfermero-sms" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
