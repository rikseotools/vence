// app/auxiliar-administrativo-diputacion-huelva/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('auxiliar-administrativo-diputacion-huelva', topicNumber)
  ...
  return {
    title: `${content.title} | Auxiliar Administrativo de la Diputación de Huelva`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Auxiliar Administrativo de la Diputación de Huelva`,
    openGraph: {
      title: `${content.title} | Auxiliar Administrativo de la Diputación de Huelva`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/auxiliar-administrativo-diputacion-huelva/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/auxiliar-administrativo-diputacion-huelva/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="auxiliar-administrativo-diputacion-huelva" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
