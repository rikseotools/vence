// app/administrativo-andalucia/temario/[slug]/TopicContentView.tsx
'use client'
...
// Determinar el bloque según el número de tema (Administrativo de la Junta de Andalucía, C1, 42 temas / 5 bloques)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 1 && topicNumber <= 16) {
    return { block: 'Área Jurídico-Administrativa General', displayNum: topicNumber }
  }
  if (topicNumber >= 17 && topicNumber <= 24) {
    return { block: 'Gestión Financiera', displayNum: topicNumber - 16 }
  }
  if (topicNumber >= 25 && topicNumber <= 29) {
    return { block: 'Gestión de Personal', displayNum: topicNumber - 24 }
  }
  if (topicNumber >= 30 && topicNumber <= 37) {
    return { block: 'Organización y Gestión Administrativa', displayNum: topicNumber - 29 }
  }
  if (topicNumber >= 38 && topicNumber <= 42) {
    return { block: 'Tecnología', displayNum: topicNumber - 37 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'administrativo-andalucia', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=administrativo_andalucia&return_to=/administrativo-andalucia/temario"
