// app/auxiliar-museos-estado/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('auxiliar-museos-estado', topicNumber)
  ...
  return {
    title: `${content.title} | Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado — Sección Museos`,
    description: content.description || `Contenido teórico del Tema ${topicNumber} para Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado — Sección Museos`,
    openGraph: {
      title: `${content.title} | Cuerpo Auxiliar de Archivos, Bibliotecas y Museos del Estado — Sección Museos`,
      description: content.description || `Contenido teórico del Tema ${topicNumber}`,
      url: `https://www.vence.es/auxiliar-museos-estado/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/auxiliar-museos-estado/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="auxiliar-museos-estado" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
