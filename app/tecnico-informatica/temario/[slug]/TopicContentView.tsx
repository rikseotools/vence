// app/tecnico-informatica/temario/[slug]/TopicContentView.tsx
// Reuses the shared TopicContentView with TAI (Estado) block ranges
'use client'
...
// Determinar el bloque segun el numero de tema (TAI del Estado: 101-109, 201-205, 301-309, 401-410)
function getBlockInfo(topicNumber: number): { block: string; displayNum: number } {
  if (topicNumber >= 101 && topicNumber <= 109) {
    return { block: 'Bloque I: Organizacion del Estado y Administracion electronica', displayNum: topicNumber - 100 }
  } else if (topicNumber >= 201 && topicNumber <= 205) {
    return { block: 'Bloque II: Tecnologia basica', displayNum: topicNumber - 200 }
  } else if (topicNumber >= 301 && topicNumber <= 309) {
    return { block: 'Bloque III: Desarrollo de sistemas', displayNum: topicNumber - 300 }
  } else if (topicNumber >= 401 && topicNumber <= 410) {
    return { block: 'Bloque IV: Sistemas y comunicaciones', displayNum: topicNumber - 400 }
  }
  return { block: '', displayNum: topicNumber }
}

export default function TopicContentView({ content, oposicion = 'tecnico-informatica', updatedAt }: TopicContentViewProps) {
    ...
    <>
      ...
      <div className="no-print sticky top-0 z-10 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <TopicPrintButton
            loginHref="/login?oposicion=tecnico_informatica&return_to=/tecnico-informatica/temario"
