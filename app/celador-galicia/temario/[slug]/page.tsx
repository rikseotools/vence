// app/celador-galicia/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('celador-galicia', topicNumber)
  ...
  return {
    title: `${content.title} | Celador SERGAS Galicia`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Celador SERGAS Galicia`,
    openGraph: {
      title: `${content.title} | Celador SERGAS Galicia`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/celador-galicia/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/celador-galicia/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="celador-galicia" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
