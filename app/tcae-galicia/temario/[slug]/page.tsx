// app/tcae-galicia/temario/[slug]/page.tsx
...
// TCAE Galicia: 31 temas secuenciales (1-30)
export async function generateStaticParams() {
  return Array.from({ length: 31 }, (_, i) => ({ slug: `tema-${i + 1}` }))
}
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('tcae-galicia', topicNumber)
  ...
  return {
    title: `${content.title} | TCAE Galicia`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para TCAE Galicia`,
    openGraph: {
      title: `${content.title} | TCAE Galicia`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/tcae-galicia/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/tcae-galicia/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="tcae-galicia" />
    </div>
  )
}
