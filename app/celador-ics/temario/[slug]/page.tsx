// app/celador-ics/temario/[slug]/page.tsx
...
export async function generateMetadata({ params }: PageProps) {
  ...
  const content = await getTopicContent('celador-ics', topicNumber)
  ...
  return {
    title: `${content.title} | Zelador ICS Catalunya`,
    description: content.description || `Contenido teorico del Tema ${topicNumber} para Zelador ICS Catalunya`,
    openGraph: {
      title: `${content.title} | Zelador ICS Catalunya`,
      description: content.description || `Contenido teorico del Tema ${topicNumber}`,
      url: `https://www.vence.es/celador-ics/temario/${slug}`,
      siteName: 'Vence.es',
      locale: 'es_ES',
      type: 'article',
    },
    alternates: {
      canonical: `https://www.vence.es/celador-ics/temario/${slug}`,
    },
  }
}
...
export default async function TemarioTemaPage({ params }: PageProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <TopicContentView content={content} oposicion="celador-ics" updatedAt={formatUpdatedAt()} />
    </div>
  )
}
