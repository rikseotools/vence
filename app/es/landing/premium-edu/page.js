// app/es/premium-edu/page.js - LANDING EDUCATIVA ORIENTADA A PAGO
'use client'
  ...
  const handleStartTrial = async () => {
    ...
    try {
      ...
      const { error: authError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?return_to=${encodeURIComponent(`/es/premium-edu?start_checkout=true&campaign=${campaignId}`)}`,
          queryParams: {
            access_type: 'offline',
