// app/es/Header.js - VERSIÓN CON ENLACE DE ADMIN Y LOGO SEPARADO EN MÓVIL
'use client'
...
export default function HeaderES() {
  ...
  const getLoggedInNavLinks = () => {
    if (!hasOposicion || loading) {
      return [
        { href: '/es/auxiliar-administrativo-estado/test', label: 'Test', icon: '🎯' },
        { href: '/es/auxiliar-administrativo-estado/temario', label: 'Temario', icon: '📚' },
        { href: '/es/oposiciones', label: 'Oposiciones', icon: '🔄' }
      ]
    }

    try {
      const featuredLink = oposicionMenu?.navLinks?.find(link => link?.featured)
      const basePath = featuredLink?.href || '/es/auxiliar-administrativo-estado'
  ...
  const getMobileNavLinks = () => {
    ...
    if (user && isAdmin && !adminLoading) {
      return [
        ...baseLinks,
        { href: '/es/admin', label: 'Panel Admin', icon: '👨‍💼', isAdmin: true }
      ]
    }
  ...
  const getTestsLink = () => {
    if (!user || !hasOposicion || loading) return '/es/auxiliar-administrativo-estado/test'
    ...
    try {
      ...
      return '/es/auxiliar-administrativo-estado/test'
    }
  }
