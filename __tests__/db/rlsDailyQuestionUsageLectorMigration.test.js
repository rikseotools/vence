 ...
 */
// T-639 (spinoff de T-450/T-573/T-574). `daily_question_usage` tiene RLS activo y CERO políticas
// para `vence_lector`: el GRANT SELECT de tabla que ya tiene (20260805_rol_lector_flota.sql) no
// basta — el motor filtra en silencio y devuelve 0 filas siempre, mismo mecanismo que
...
// alcance mayor del decidido.
const fs = require('fs')
const path = require('path')

const MIGRATION_PATH = path.join(
  __dirname,
  '../../supabase/migrations/20260807_rls_daily_question_usage_lector.sql'
)
...
describe('migración RLS daily_question_usage para vence_lector', () => {
  ...
  it('crea la política de SELECT en daily_question_usage', () => {
    expect(sql).toMatch(/CREATE POLICY flota_lector_lee ON public\.daily_question_usage/)
  ...
  it('no toca ninguna otra tabla fuera de daily_question_usage (no ampliar sin medir)', () => {
    ...
    expect(matches.map((m) => m[1])).toEqual(['daily_question_usage'])
  })
