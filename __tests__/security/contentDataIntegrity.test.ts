...
describeIf('Content Data Integrity', () => {
  ...
  test('NO debe haber preguntas legislativas activas que referencien imágenes no disponibles', async () => {
    ...
    for (const pattern of patterns) {
      const rows = await query(
        'questions',
        `select=id&is_active=eq.true&question_text=ilike.*${encodeURIComponent(pattern)}*`
      ) as Array<{ id: string }>
      total += rows.length
    }

    if (total > 0) {
      console.error(`${total} preguntas legislativas activas referencian imágenes no disponibles`)
    }
