  ...
  for (let i = 0; i < logs.length; i++) {
    ...
    if (log.question_context_id) {
      const { data } = await supabase
        .from("questions")
        .select("id, question_text, option_a, option_b, option_c, option_d, correct_option, explanation, primary_article_id, is_official_exam, difficulty")
    ...
    if (quality !== "BUENA") {
      ...
      console.log(exp.substring(0, 500));
    }
  }
})();
